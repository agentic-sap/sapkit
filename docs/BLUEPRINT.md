...
|---|---|---|---|
| ① | **몸통은 전부 자작** | ~~엔진·검사기·지식 상당량이 차용 포크~~ → **검사기 ✅(2026-08-16) · 엔진 ✅(2026-08-19) · 지식 갈래 소진 ✅** · 남은 것은 `interactive/server/` 결부 **copy 36**(판8이 **동결 30 / 활성 6**으로 갈랐다 · D-103)에 더해 **`transform` 42**다 — 후자는 2026-08-23 리뷰가 드러낸 **소관 없던 범주**이고 38파일이 상류 문장을 공유한다(D-104 · 판3.7) | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
...
때문이다. 그 범주는 지금 `interactive/` 아래 **42건** 살아 있고 **38파일이 상류 원본과
60자 초과 동일 행 1,326행을 공유**한다(페르소나 **26이 전부 이 범주**이며 25가
54~95%). `MIGRATION-MANIFEST.md`의 정의대로 `transform`은 **「형태 변환 이식」**이지
...
| `transform` — 페르소나 26 전량 · 절차 5 · 훅 2 · 정책 2 · `tools/extract` 2 · 그 밖 | **42** | **미판정** — 38파일이 상류 문장 잔존(1,326행) · 재저작은 **판3.7** |
...
- **㉯ 활성 차용분 소진 — 대기 (판10).** `server/tool-catalog/` 4는 **엔진 등록점에서 생성**하는
  ...
  처분은 **판10이 정한다.**
- **㉰ `transform` 42 재저작 — 대기 (판3.7).** 2026-08-23에 드러난 **소관 없던 범주**다
  (D-104 ⓒ). 페르소나 **26 전량**이 여기 있고 25파일이 상류 원본과 실질 행의 **54~95%**를
  공유한다. 명부는 [`reference/copy-baseline.md`](reference/copy-baseline.md) **§3.4**이고
  계수·체크 규칙은 §2(재저작만 · 독립 리뷰 통과 후)를 그대로 따른다.

**검증 기준**: ㉮ = 고지 4곳이 **같은 내역**을 말하고, 그것이 명부 §4 레시피의 도출 결과와
일치한다(**기계 재현 가능** — 인벤토리 487 · 미매칭 0 · `copy` 원본 282 · `server/` 결부 60 =
keyring 21 + 번들 3 + **잔존 36** · `transform` 목적지 유니크 현존 **42**). ㉯ = 활성 6이 자체
저작으로 치환됐음을 git 이력으로 확인 · `tool-catalog` 대조 게이트가 서 있을 것.
㉰ = §3.4 명부 **42/42 체크**(각 파일 독립 리뷰 통과).
...
| ⑷ 고지 **㉮ 범위 정합 ⚠ 미완**(2026-08-22 시도 · **08-23 철회** · D-104) · **㉯ 활성 6 소진 = 대기(판10)** · **㉰ `transform` 42 재저작 = 대기(판3.7)** | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + 그 36의 git 이력(**이식 커밋 포함 총계** — 동결 30 = **1**(이식뿐) / 활성 6 = **6·3·3**) | 고지 **4곳**이 **같은 내역**을 말하고 그것이 실제 잔존물과 일치 — ⚠ **아직 아니다**: `transform` 42가 미판정이라 고지는 서브트리 전체에 걸려 있다 | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). 갚을 것은 **활성 6**(판10)과 **`transform` 42**(판3.7) |
