...
| # | 조건 | 지금 (2026-08-22 갱신) | 끝그림 |
|---|---|---|---|
| ① | **몸통은 전부 자작** | ~~엔진·검사기·지식 상당량이 차용 포크~~ → **검사기 ✅(2026-08-16) · 엔진 ✅(2026-08-19) · 지식 갈래 소진 ✅** · 남은 것은 `interactive/server/` 결부 36파일이고, 판8이 그것을 **동결 30 / 활성 6**으로 갈랐다(D-103) | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
...
| ③ | **계보성 고지 은퇴** | 잔존 고지 **1건**(`interactive/LICENSE` — `vsp/`는 2026-08-16, `engine/`은 2026-08-22 판7.5 은퇴). **2026-08-22 판8이 그 고지의 범위를 실제 잔존물 36파일로 좁혔다**(D-103 ⓒ) | ~~대체가 끝난 부품의 고지부터 차례로 은퇴, **최종 0건**~~ → **고지가 덮는 범위가 실제 잔존물과 일치 + 활성 차용분 0.** ⚠ **0건은 도달 불가**다 — `server/sap-assets/` 30이 D-079 ⑥의 존치 결정으로 영구히 남는다(D-103 ⓓ) |
...
`038085c2` 이후 **변경 커밋이 0**이라 ②의 정의(「레포 안에서 개작·유지보수하지 않는다」)를
...
**⚠ 남은 1건은 은퇴하지 않는다 — 2026-08-22 판8이 대신 범위를 좁혔다 (D-103 ⓒ).** 그
고지가 루트 표에서 걸려 있던 대상은 `interactive/` **서브트리 전체**였는데, 지식 갈래
170이 재저작으로 소진된 뒤로 그 범위는 실제보다 넓었다. 지금은 **실제 잔존 36파일**을
가리킨다 — `server/sap-assets/`(30) · `server/tool-catalog/`(4) ·
`server/verify-engine.mjs` · `server/bundle-keyring.mjs`. 그 36 중 **30은 영구 존치**
(D-079 ⑥ · SAP 오브젝트 무접촉)이고 **6이 은퇴 대상**이다.
...
| [`interactive/LICENSE`](../interactive/LICENSE) | ~~`interactive/` 서브트리 전체~~ → **`interactive/server/` 아래 36파일** (2026-08-22 좁힘 · D-103 ⓒ · 파일 자신의 「Scope of this notice」 절이 목록을 갖는다) | **은퇴하지 않는다.** 30(`sap-assets/`)은 영구 존치이므로 이 고지도 영구히 남는다. 남은 일은 은퇴가 아니라 **활성 6의 소진**이고, 그것이 끝나면 이 고지는 「부채의 표시」가 아니라 **「SAP 측 자산의 정직한 귀속」**만 남긴다 | ⑶·⑷ |
...
- **㉮ 고지 범위 정합 — ✅ 완료 (2026-08-22 · 판8).** [`interactive/LICENSE`](../interactive/LICENSE)를
  **은퇴시키지 않고**, 그것이 덮는 범위를 `interactive/` 서브트리 전체에서 **실제 잔존 36파일**로
  좁혔다. 루트 [`LICENSE`](../LICENSE) 표의 Scope 칸 · `interactive/LICENSE`의 「Scope of this
  notice」 절 · [`THIRD_PARTY_NOTICES.md`](../interactive/THIRD_PARTY_NOTICES.md)의 행 분할
  (동결 30 / 활성 6) · [`interactive/README.md`](../interactive/README.md)의 라이선스 줄이 **같은
  범위를 말한다.** 번들 런타임 의존(외부 npm 패키지) 귀속은 **남는다** — 정식 의존성이고 끝그림
  ②가 허용하는 형태다.
...
**검증 기준**: ㉮ = 고지 4곳이 같은 범위를 말하고, 그 범위가 명부 §4 레시피의 도출 결과와
일치한다(**기계 재현 가능** — 인벤토리 487 · 미매칭 0 · copy 원본 282 · `server/` 결부 60 =
keyring 21 + 번들 3 + **잔존 36**). ㉯ = 활성 6이 자체 저작으로 치환됐음을 git 이력으로 확인 ·
`tool-catalog` 대조 게이트가 서 있을 것.
...
| ⑷ 고지 **㉮ 범위 정합 ✅ 2026-08-22(판8)** · **㉯ 활성 6 소진 = 대기(판10)** | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + 그 36의 git 이력(동결 30 = 변경 커밋 **0** / 활성 6 = 6·3·3) | 고지 **4곳**(루트 `LICENSE` 표 · `interactive/LICENSE` Scope 절 · `THIRD_PARTY_NOTICES.md` · `interactive/README.md`)이 **같은 범위**를 말하고 그것이 실제 잔존물과 일치 | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). 은퇴 대상은 **활성 6**이고 판10 소관 |
