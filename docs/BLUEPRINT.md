...
|---|---|---|---|
| ⑴ 엔진 | tool-catalog diff 0 · `smoke-mcp` · `conformance-server-gates` · 실데이터 게이트 음성시험 | 대표 절차 1건 구·신 동일 결과 | **교체 판(판7)에서는 은퇴 없음** — `engine/` 소스 + 고지의 은퇴는 **교체 이후 별도 판**(D-093 ⓐ · 되돌아갈 자리를 남긴다) |
