...
| 잔존 — 산재 **계약행** (`server/` 밖 서브트리 전체) | **~2,200행 / ~130파일** | **의도된 보존** (D-112 ⓓ) — 참조표·펜스 템플릿·print-verbatim 블록·**체크박스 규범항**(이것만은 보존 판정이 아니라 재저작 **보류** — D-111·D-113 ⓒ⑤)·사용자 노출 문구·승인 키워드·공유 라벨. 나머지는 재저작하면 제품 동작·데이터 정합이 깨진다(배치3 리뷰가 파일별로 근거를 세웠다). ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다**(2026-08-23 · **D-113 ⓐ**) — 구성 **3갈래** = §3.4 갈래 **~600/40** + `copy` 재저작 체크분 **~950/64** + 무변경 검증 참조표 **~660/25** |
