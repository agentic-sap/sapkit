...
- **「증거 급」 — 전 M 공통.** 완료 판정의 "전량 재생 diff 0"은 **재생이 가능한 도구에 대한
  것**이다. **`Create*` 계열은 재생 대상이 아니다** — 재생은 신 엔진이 같은 요청을 SAP에 다시
  던지는 일이라 생성 시퀀스는 두 번째 실행에서 "이미 있다"로 실패한다(신 엔진이 옳아도
  실패한다). 그래서 그 계열의 증거 급은 **재생이 아니라 attended 실기 기록**이며, 규칙과
  실측 근거의 정본은 [`sapkit-engine/fixtures/README.md`](../sapkit-engine/fixtures/README.md)다.
  **M1·M4·M5의 "전량"은 이 급 구분을 포함한 전량**이다 — 재생 가능한 것은 정규화 diff 0,
  재생 불가한 것은 실기 기록, 의도적 차이 목록 등재분은 대체 기대 시험. 셋 중 하나도 없는
  도구가 있으면 그 M은 닫히지 않는다.
