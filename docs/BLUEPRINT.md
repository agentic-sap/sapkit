...
| # | 조건 | 지금 (2026-08-23 · 판11 마감) | 끝그림 |
|---|---|---|---|
| ① | **몸통은 전부 자작** | ✅ **도달** (2026-08-23 · D-112 ⓒ) — **활성 차용분 0**이다. 검사기 ✅(2026-08-16 · 판1) · 엔진 ✅(2026-08-19 · 판7-b) · `copy` 지식 170 ✅(2026-08-18) · 활성 6 ✅(판10 · D-107) · 상류 잔존 56 ✅(판3.7 · D-109~D-111). **남은 파일 단위 차용은 `interactive/server/sap-assets/` 30뿐**이고 그것은 개작·유지보수하는 것이 아니라 **무접촉 동봉 자산**이다(영구 · D-079 ⑥ · 이식 이후 변경 커밋 0). ⚠ **「도달」은 파일 단위 기준이다** — 행 단위로는 **계약행 ~2,200행/~130파일**(`server/` 밖 서브트리 전체 · 구성 3갈래는 §1.3)이 남고, 그것은 갚을 부채가 아니라 **의도된 보존**이다(D-112 ⓓ — 재저작하면 제품 동작·데이터 정합이 깨진다). **이 구분을 흐리면 「전부 자작」이 과대 주장이 된다**(D-112 유보 ⓒ). ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다** — 독립 리뷰가 같은 스캔을 `copy` 갈래에도 돌려 총계를 정정했다(2026-08-23 · **D-113 ⓐ**) ⚠ **2026-08-23 판S가 이 행의 마지막 예외를 지웠다**(D-130·D-131·D-132): `server/sap-assets/` 29파일 **7,087행**이 무접촉 상류 자산이었는데 **자작으로 재저작하고 이름까지 옮겼다**(`ZMCP_ADT_*` → `ZSAPKIT_ADT_*` · `ZCL_S4SAP_CM_*` → `ZCL_SAPKIT_CM_*`). 축자 겹침은 판3.x와 같은 기준(60자 초과)으로 **17행**이고 전부 계약이다. 그중 **9종은 실제 S/4HANA에 설치·활성화**까지 확인됐다. ⇒ **파일 단위 차용은 이제 0이다** — 행 단위 계약행 ~2,200행만 남고 그것은 여전히 의도된 보존이다. | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
| ② | **외부는 정식 의존성만** | ✅ **도달** (2026-08-23 · D-112 ⓒ) — **레포 안에서 개작·유지보수하는 상류 코드가 0**이다. 활성 6이 판10에서 소진됐고(`tool-catalog/` 4 = 등록점 생성물 · 유틸 2 = 골격 재저작 · 역방향 스캔 잔존 0 · D-107), 외부 코드는 전부 패키지 매니저로 당겨 쓰는 의존성이다. 예외인 `sap-assets/` 30은 이식 커밋 `038085c2` 뒤로 **변경 커밋 0**이라 ②의 정의를 이미 만족한다. ⚠ **각주 — 「전부 패키지 매니저로」의 예외가 하나 더 있다**(2026-08-23 · **D-113 ⓒ④**): `server/runtime-deps/keyring/` **21파일**은 npm으로 당겨 쓰는 것이 아니라 **vendored로 레포에 커밋된 채 배포된다**. 레포 안에서 **개작 0**이라 ②의 **운용 정의**(「개작·유지보수하지 않는다」)는 만족하지만 **형태는 미달**이고, **정식 패키지 귀속은 별건**이다. D-112 ⓒ가 이것을 ④(브랜드 · 「이름의 문제」)로 분류한 것은 **오분류였다** — 귀속·의존성 **형태**의 문제라 **② 소관**이다 | 외부 코드는 패키지 매니저로 당겨 쓰는 의존성 형태로만 존재. **예외 = SAP 시스템 측 자산** — 패키지 매니저가 없는 영역이라 ③의 예외와 같은 자리다 |
| ③ | ~~**계보성 고지 은퇴**~~ → **계보성 고지 정합** | ✅ **도달** (2026-08-23 · 판11 · D-112 ⓐⓒ) — **D-103이 재정의한 기준**(「고지가 덮는 범위가 실제 잔존물과 일치」)으로 판정한다. 잔존 고지는 **1건**(`interactive/LICENSE` — `vsp/`는 2026-08-16, `engine/`은 2026-08-22 판7.5 은퇴)이고 **`interactive/` 서브트리 전체**에 걸린 채 **영구히 남는다**. ⚠ **답은 「좁히기」가 아니라 「내역의 정확성」이었다** — 파일 목록으로 좁히려던 두 번의 시도가 리뷰에서 물러났고(D-104 ⓐ · D-106 ⓐ), 상류로 남는 것이 **파일이 아니라 산재 계약행**이라 목록이라는 **형태 자체가 틀린 답**이었다. 지금 고지 4곳이 **같은 내역**을 말한다 ⚠ **「0건은 도달 불가」가 뒤집혔다**(2026-08-23 · **D-130 ⓑ**): D-103 ⓓ가 그렇게 판정한 **유일한 근거가 `sap-assets/` 30의 영구 존치**였는데 판S가 그것을 재저작했다. **다만 고지는 아직 내리지 않는다** — 이제 그 존속 이유가 **산재 계약행 ~2,200행**으로 바뀌었을 뿐 사라진 것은 아니다(D-112 ⓓ의 의도된 보존). `interactive/LICENSE`의 내역 표와 루트 `README.md`를 그 사실로 갱신했다. **「고지 0건」이 가능해지려면 그 계약행까지 재저작해야 하고, 그것은 새 판단이다.** | ~~대체가 끝난 부품의 고지부터 차례로 은퇴, **최종 0건**~~ → **고지가 덮는 범위가 실제 잔존물과 일치 + 활성 차용분 0.** ⚠ **0건은 도달 불가**다 — `server/sap-assets/` 30이 D-079 ⑥의 존치 결정으로 영구히 남는다(D-103 ⓓ) |
| ④ | **브랜드 단일** | ✅ **도달** (2026-08-23 · 판Z · **D-123**) — 두 갈래가 다 닫혔다: ⓐ `server/tool-catalog/` 4파일이 **`sapkit-mcp-tools*.md`로 개명**됐다(**생성기가 정본**이라 `render-tool-catalog.mjs` 8곳을 고쳐 재생성 · 등록점이 입력이라 **도구 목록 diff 0** · 게이트 상수·`usage-census.mjs`·참조 문서 동반). ⚠ **참조자는 「12파일」이 아니라 20파일이었다** — 이 판의 실사가 정정했다 ⓑ **훅 마커 개명** — 태그 3건(`[SAPKIT …]`)에 더해 실사가 찾은 **메시지 접두어 11줄**까지 옮겼다(태그가 아닐 뿐 **모델이 읽는 출력**이라 성격이 같다 · 3건만 고치면 반쪽 단일이 된다). **D-111이 「별건」으로 기각한 자리를 판Z가 연 것**이고, 그것은 영구 보존이 아니라 미룬 자리였다. 여기에 제품 자기지칭 **20줄 / 14파일**을 더했다(초기 보고 「16곳」은 **과소계수**였다 — D-124 ⓓ). ⚠ **접두어 11 중 「모델이 읽는 출력」은 9**이고 나머지 2는 JSDoc 파일 헤더다 ~~ⓒ **keyring 21의 정식 패키지 귀속**~~ → **② 소관**(D-113 ⓒ④). ⚠ **정직하게 — 두 가지가 남는다**: ⑴ **고지·계보·이력의 `sc4sap`은 영구 존치**다(프론트매터 `source:` 80줄 · `LICENSE` · `THIRD_PARTY_NOTICES` · `provenance/` · `keychain:sc4sap/` 서비스명 · 실재 ABAP 오브젝트명) — 바꾸면 **없던 사실**이 된다 ⑵ **훅 출력의 대소문자는 통일되지 않았다**(태그는 대문자 · 접두어는 소문자) — 「이름만 바꾼다」를 지킨 결과이고 **브랜드가 아니라 스타일 문제**라 별건이다 ⚠ **후속 회수 있음**(2026-08-23 · **D-126**): 판Z 직후 시점에는 **미완이었다** — 루트 README 첫 화면의 「상류 고지 승계」 한 줄 · 살아 있는 환경변수 **2종**(`SC4SAP_LITE_NS`·`SC4SAP_AGENTS_DIR`) · SAP 패키지 이름 예시 1건이 남아 있었고 **사용자가 GitHub 화면에서 찾았다.** 원인은 세 겹 — ⑴ 작업자 소유 범위 밖(`interactive/scripts/`가 어느 목록에도 없었다) ⑵ 확인 grep의 검색어 누락(「상류 고지」라는 한글 표현) ⑶ **게이트가 `SC4SAP_HOME_DIR` 한 이름만 감시**. 셋 다 닫았고 **게이트를 접두어 전체(`SC4SAP_[A-Z0-9_]+`)로 넓힌 것이 핵심**이다 — 넓히자마자 세 자리가 더 드러났다. | 아래 1.2 |
...
`bundle-keyring.mjs`)이었다. **그 6이 ⑷의 실제 과녁이었고, 2026-08-23 판10이 소진했다**
(D-107 — 생성물 전환 4 + 재저작 2 · 마감 역방향 스캔에서 6파일 전부 겹침 3행 미만).
...
**✅ 2026-08-23 · 사다리가 완주됐다 (D-112 ⓑⓒ).** 위 두 ⚠ 문단이 남긴 「56 · 미판정」은
**판3.7이 갚았고**(§3.4 명부 56/56 · D-109~D-111), ㉮의 답은 **판11**이 냈다 — **범위를
좁히는 것이 아니라 내역을 정확히 적는 것**이었다. 그래서 **2026-08-10 D-079가 착수한
교체 사다리가 2026-08-23 ⑷ 완료로 닫힌다**: ⑴ 엔진(판7-b 교체 · 판7.5 소스 은퇴) ·
⑵ 검사기(판1) · ⑶ 지식·템플릿(판3.x · 판4 · 판3.7) · ⑷ 고지 범위 정합 + 활성 차용분
소진(판10 · 판3.7 · 판11).

**「포크 0」의 도달 형태는 ①~③이다.** ④(브랜드)는 저작권이 아니라 **이름**의 문제라
⑷와 무관한 **별건**으로 남고, ⑤(무게)는 완료가 없는 **상시 원칙**이다. ⚠ **「전부 자작」을
행 단위로 읽지 말 것** — ①의 도달은 **파일 단위** 기준이고, 행 단위로는 계약행 **~2,200행**
(D-113 ⓐ로 정정된 총계 — §1.3)이 **의도된 보존**으로 남아 있으며 그 구분을 고지 4곳이
명시한다(D-112 유보 ⓒ).
독립 리뷰 이력: **R-FAIL → 회수(D-113 · 총계·문장 정정) → 경량 재리뷰 → 닫는 조건 반영(`54c10a37`)** — 두 리뷰 모두 ⑷ 완료 판정은 유지했고, **판11은 종결됐다.**
...
| 도구 카탈로그 파일명 | `server/tool-catalog/sapkit-mcp-tools*.md` | ✅ **개명 완료** (2026-08-23 · 판Z · **D-123**). 판10의 명시 이월(D-107 기각 (a))이 여기서 닫혔다. 내용은 판10부터 **자체 저작 생성물**이었고 이름만 상류에 남아 있었다 — **생성기(`render-tool-catalog.mjs`) 8곳 + 게이트 상수 + `usage-census.mjs` + 물리 rename 4 + 참조 문서**가 한 판에서 움직였고 **도구 목록 diff 0**이다. 실사 정정: 참조자는 **20파일**이었다(종전 서술 12) |
...
**⚠ 남은 1건은 은퇴하지 않는다 (D-103 ⓓ) — 그리고 범위도 좁히지 않는다 (D-112 ⓐ).**
파일 목록으로 좁히려던 두 번의 시도가 독립 리뷰에서 물러났고(D-104 ⓐ — `class: transform`
42건 누락 · D-106 ⓐ — 그 대체 수치마저 미달), **재저작 완주가 그 이유를 확정했다**: 상류
저작물로 남는 것은 **파일이 아니라 산재한 계약행**이라 **파일 목록이라는 형태 자체가 틀린
답**이었다. 지금 알려진 내역은 이렇다:

| 갈래 | 수 | 상태 |
|---|---:|---|
| ~~`copy` — `server/sap-assets/`~~ | 30 | ~~**영구 존치** (D-079 ⑥ · 이식 이후 변경 커밋 0)~~ → ✅ **재저작·개명 완료** (2026-08-23~24 · 판S1~S5 · **D-130·D-131**) — 자산 **29파일 7,087행** 자작 재저작 + `ZMCP`/`S4SAP` → `ZSAPKIT` 개명(30번째는 이 디렉터리 자신의 `README.md`). SAP 실기 9종 설치·활성화(**D-132**) · 실기 결함 3건 수리 + 런타임 왕복 실증(**D-134**) · 오배치 수리 되돌림(**D-136**). **파일 단위 차용은 이제 0이다** |
| ~~`copy` — `tool-catalog/` 4 · `verify-engine.mjs` · `bundle-keyring.mjs`~~ | 6 | ✅ **소진 (2026-08-23 · 판10 · D-107)** — 생성물 전환 4 + 골격 재저작 2 · 역방향 스캔 잔존 0 |
| ~~상류 잔존 — 페르소나 25 · 절차 16 · 어댑터 5 · 지식 5 · 정책 2 · 도구 3~~ | 56 | ✅ **재저작 완료 (2026-08-23 · 판3.7 · D-109~D-111)** — 배치 3회 · 명부 `reference/copy-baseline.md` **§3.4 56/56 · 잔량 0** · 독립 리뷰 누계 4회 |
| `copy` — 지식·정책 갈래 | 170 | ✅ 완료 (2026-08-18 · 각 파일 독립 리뷰 통과) — 다만 **「전부 재저작」이 아니다**: **26파일은 D-088 ②의 「검증 체크(무변경)」**(순수 참조표 · 바이트 동일 보존)이고 재저작이 아니다(2026-08-23 · **D-113 ⓑ**) |
| 잔존 — 산재 **계약행** (`server/` 밖 서브트리 전체) | **~2,200행 / ~130파일** | **의도된 보존** (D-112 ⓓ) — 참조표·펜스 템플릿·print-verbatim 블록·사용자 노출 문구·승인 키워드·공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다(배치3 리뷰가 파일별로 근거를 세웠다). ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다**(2026-08-23 · **D-113 ⓐ**) — 구성 **3갈래** = §3.4 갈래 **~570/39** + `copy` 재저작 체크분 **~950/64** + 무변경 검증 참조표 **~660/25**. ⚠ **~~체크박스 규범항(보존 판정이 아니라 재저작 보류 — D-111·D-113 ⓒ⑤)~~은 2026-08-24에 소진됐다** — `review-checklist.md` 24행(§7~§12) · `troubleshooting.md` 11행을 재저작했고 포기한 줄은 없다. §3.4 갈래가 ~600/40 → **~570/39**로 내려간 것이 그 몫이다 |

**그러므로 고지는 서브트리 전체에 걸린 채 남는다** — 잔존이 **행 단위로 산재하는 계약행**이라
경로 목록으로는 표현할 수 없고, 저작물성이 있을 수 있으므로 고지가 그것을 **명시적으로
커버하는 것**이 정확하다(D-112 ⓓ). 바뀐 것은 범위가 아니라 **알려진 내역**이고,
**그 내역이 2026-08-23에 완주 상태가 됐다**(D-112 ⓐ).

⚠ **그리고 그 내역은 같은 날 독립 리뷰에서 한 번 정정됐다** (2026-08-23 · **D-113 ⓐⓑ**).
판11이 공표한 잔존 총계는 **§3.4 갈래만** 센 값이었고(도출 4단계가 설계상 `copy` 명부
170의 목적지를 뺀다), 「전부 재저작」이라는 서술은 **무변경 검증 26파일에 대해 거짓**이었다.
**㉮의 방향(서브트리 유지 + 내역의 정확성)과 성격 판정(계약행)은 리뷰가 옹호했고 ⑷ 완료
판정도 유지된다** — 고쳐진 것은 **총계와 두 문장**이다. **여기서 남는 규칙: 총계를 공표할
때는 「무엇을 빼고 셌는가」를 문장에 같이 적는다**(D-113 유보 ⓑ).

| 고지 | 대상 서브트리 | 은퇴 조건 | 대응 사다리 |
|---|---|---|---|
| ~~`engine/LICENSE`~~ | ~~`engine/`~~ (~~+ `interactive/server/` 번들~~ — **2026-08-19 교체로 번들은 빠졌다**) | **은퇴 완료 (2026-08-22 · 판7.5)** — SAPKIT Engine이 번들 자리를 대체한 뒤(D-095) `engine/` 소스가 레포를 떠났다 | ⑴ |
| ~~`vsp/LICENSE`~~ | ~~`vsp/`~~ | **은퇴 완료 (2026-08-16 · 판1 · D-085)** — `sapkit-cli/`가 로컬 검사를 대체하고 `vsp/` 서브트리가 레포를 떠났다 | ⑵ |
| [`interactive/LICENSE`](../interactive/LICENSE) | **`interactive/` 서브트리 전체** (2026-08-22 좁혔다가 **08-23 복원** · D-103 ⓒ → D-104 ⓐ → **D-112 ⓐ로 확정** · 파일 자신의 「What this notice covers」 절이 알려진 내역을 갖는다) | **영구 — 내역 정합 완료 (2026-08-23 · 판11 · D-112).** ~~30(`sap-assets/`)은 영구 존치이므로 이 고지도 영구히 남는다.~~ **정정 (2026-09-09)**: `sap-assets/`는 2026-08-23~24에 **재저작·개명**됐다(**D-130~D-136**) — 이 고지가 남는 이유는 이제 그것이 아니라 **산재 계약행**이다(D-112 ⓐⓓ). **남은 일은 없다** — 소진해야 할 것(활성 6 · 상류 잔존 56)이 전부 갚였고, 이 고지는 이제 「부채의 표시」가 아니라 ~~「SAP 측 자산의 정직한 귀속」~~ **「산재 계약행의 정직한 커버」**이다. ⚠ **브랜드(파일명 · 훅 태그)는 별건**이다 — 저작권이 아니라 **이름**의 문제이고 끝그림 ④ 소관이다 | ⑶·⑷ **완료** |
...
⑶ 지식 재저작                   ← 가장 김: 집필 노동이 실사용 검증 속도에 묶인다
   │                              (⑴·⑵와 병행 가능 — 코드 부품과 독립)
   ▼
⑷ interactive/ 고지 범위 정합    ← ✅ 완료 (2026-08-23 · 판11 · D-112 · 사다리 완주)
...
> `sapkit-engine`의 번들이다(교체 시점 **1.0.0** · 3.81MB · 46%. **현행 버전·바이트의
> 정본은 `HANDOFF.md` 📌표다** — 여기 복제하지 않는다). 검증 기준 1~4가 선 뒤에 갈아끼웠고,
...
1. **표면 186종 전량**이 **동일 이름·동일 호출 계약**으로 응답한다 — 기존
   [tool-catalog](../interactive/server/tool-catalog/sapkit-mcp-tools.md) 대조 diff 0.
...
| **M2** | **접속 방식 확장** — destination · service-key · broker 인증. `--mcp=<destination>` · `--env=<name>` 통로 구현. **오프라인 배선은 2026-08-23 판M2-a에서 끝났다**(D-114 — 기동 토큰 취득 · 진단 5종). 판M2-b(D-115)가 실접속을 재어 **`client_credentials`로는 ABAP ADT가 열리지 않음**을 실측했고(토큰은 200이나 신원이 없어 전 경로 401), **판M2-c(D-117)가 `authorization_code` 기동 배선을 명시 옵트인 `--auth-interactive`로 세웠으며, 그 attended 확인(D-118)이 섰다** — `GetSystemInfo` → `{"supported":true,"system_id":"TRL",…}`. | ⓐ destination 프로파일로 **실접속 성공**(inspection-only 강등 아님). ⓑ `--mcp` 진단 `MCP_DESTINATION_UNSUPPORTED`가 더 이상 나오지 않는다. ⓒ 기존 Basic 경로 회귀 0. **→ D15 해소** | **✅ (2026-08-23 · 판M2-c · D-118)** ¹ | ⬜ **`증거 완료` 미달** · 🔒 **완주 판정으로는 닫힘** (2026-08-23 · **D-122** · 표기 정정 **D-124 ⓑ**) — 실접속은 섰으나(D-118) **그 표면의 도구 증거는 0**이다. 증거 판이 **접속 3곳 전부 막혀** 못 섰다. ⚠ **검증됨이 아니다** |
| **M3** | **전송 + RFC 완성** — HTTP · SSE 전송 2종. RFC `soap`·`native`·`gateway`·`zrfc` 4경로. | **완료 = 전송 3종**(**D-093 ⓑ**, 2026-08-19): ⓐ stdio·HTTP·SSE 각각으로 기동 → 도구 호출 성공 + 전 M 공통 요건. ⓑ RFC 5경로 각각 통로 생성이 계약대로(이미 ✅). — **이월**: `native`·`gateway`·`zrfc` **3경로의 실동작 확인**(옛 ⓒ 포함)은 M3에서 떼어 **교체 판정 시점의 재평가**로 옮겼다. 셋 다 장비·비용·SAP 측 설치가 선행돼야 밟히므로(유료 RFC SDK · 중계 미들웨어 호스트 · SAP 측 핸들러+SICF+FM 설치=P3/P4) M3에 묶어 두면 **M3가 어느 판에서도 닫히지 않는다**. **교체를 막는 조건은 아니다** — 관문은 검증 기준 1~4이고(D-093 ⓐ) 그 넷은 RFC 경로를 요구하지 않는다. 등재는 `sapkit-engine/harness/DIVERGENCES.md` D21·D22 + **`gateway` 신규 등재**. **뼈대는 여기서 끝난다.** | ✅ (2026-08-14) | ⬜ **`증거 완료` 미달** · 🔒 **완주 판정으로는 닫힘** (2026-08-23 · **D-122** · 문면 정정 **D-124 ⓒ**) — ⚠ **D-093 ⓑ 기준(완료 = 전송 3종)으로는 섰다**: 엔진 게이트가 stdio·HTTP 7·SSE 11을 **매 실행 확인**한다. RFC 3경로는 **M3 기준 밖의 이월분**이고(D-093 ⓑ가 그렇게 뗐다) 통과 조건이 아니다. **그럼에도 올리지 않는 것은 전 M 공통 요건 ⑥ — 「마일스톤별 실 SAP attended 확인 1건」 — 이 미충족이기 때문**이다(2026-08-23 · **D-125**). ⚠ **CI green은 확인됐다**(`782e3e3c` · `completed | success`) — 그것이 받치는 것은 공통 요건 ①②③이고, **⑥은 접속이 막혀 못 했다.** ~~push 후 green이면 올릴 수 있다~~ → **그 조건은 오독이었다**: 공통 요건은 CI green 하나가 아니라 **여섯 항목**이고 M1 각주의 「CI green도 확인했다」는 그중 하나를 언급한 것이지 정의가 아니었다. D-081 순서 고정도 같은 결론을 가리킨다. ⚠ **검증됨이 아니다** |
| **M4** | **도구 — 실호출 116종** (§4.7 실사 정본 기준). M1이 이미 지은 것 포함. | 116종 **전량**이 증거 급을 갖는다(아래 「증거 급」 참조) — 원칙은 녹화-재생 정규화 diff 0, `Create*`·`Delete*`는 attended 실기 기록, 의도적 차이 목록 등재분은 **대체 기대 시험** 통과. | ✅ (2026-08-14) | ⬜ **`증거 완료` 미달** · 🔒 **완주 판정으로는 닫힘** (2026-08-23 · **D-122** · 표기 정정 **D-124 ⓑ**) — 증거 대기분이 **재생 대조의 수집·배정 단위 불일치**(재생기는 최상위만 모으는데 요구 급은 재귀로 매긴다)와 **접속 차단**으로 못 섰다. ⚠ **검증됨이 아니다** |
| **M5** | **도구 — 나머지 70종** (자산참조만 21 + 꼬리 49 — 앞은 문서가 부르지만 호출 이력이 0인 것, 뒤는 양쪽 증거가 모두 0인 것. §4.7). | ⓐ 70종 전량 **계약 시험 신규 저작** 통과 + 대표 건 attended 실기(증거 급은 아래 「증거 급」 참조 — 이 구간의 주 증거는 재생이 아니라 계약 시험이다). ⓑ **tools/list 186종 전량**이 tool-catalog 대조 **diff 0**. | ✅ (2026-08-14) | ⬜ **`증거 완료` 미달** · 🔒 **완주 판정으로는 닫힘** (2026-08-23 · **D-122** · 표기 정정 **D-124 ⓑ**) — 계약 시험은 통과하나 **대표 건 attended 실기가 접속 차단으로 못 섰다**. ⚠ **검증됨이 아니다** |
...
`증거 완료`가 서기 전에는 그 M이 닫히지 않는다. **`증거 완료`가 선 것은 여전히 M1 하나뿐이다**
(판6.1 · 2026-08-18).

> ⚠ **M2~M5의 증거 칸을 「`증거 완료`」로 읽지 말 것 — 두 눈금이 다르다**
> (2026-08-23 · **D-124 ⓑ**). 이 칸이 재는 것은 **`증거 완료`**이고, 거기 적힌
> 「완주 판정으로는 닫힘」은 **D-120 ⓓ의 완주 판정**, 즉 **「판이 닫혔다」**는 다른 눈금이다.
> 2026-08-23 세션은 **증거를 한 건도 만들지 않았다** — 대장의 증거 대기는 **30 그대로**이고
> D-122가 그 사실을 명시했다. 한 칸에 두 눈금을 적은 것이 혼동의 원인이었고, **표기를 갈라**
> 이 경고를 둔다.
>
> **D-081의 순서 고정(M1→M6 불변)은 존속한다.** 그 규칙은 **증거를 태우는 순서**에 걸리고
> 그것이 지키는 눈금이 바로 **`증거 완료`**다 — 완주 판정이 그 눈금을 요구하지 않았을 뿐이므로
> **순서 고정은 깨지지 않았다.** 증거 작업을 재개하면 **여전히 M2부터**다.
>
> **넷의 미충족에는 공통분모가 하나 있다**(2026-08-23 · **D-125**): 아래 **「전 M 공통 완료
> 요건」의 ⑥ — 「마일스톤별 실 SAP attended 확인 1건」**이 **M2·M3·M4·M5 전부에 대해
> 미충족**이고 이유가 **동일**하다(접속 차단). 즉 「이유와 함께 못 섰다」는 각 M의 개별
> 사정에 더해 **공통 요건 하나로도 성립**한다. ⚠ **CI green은 확인됐다**(`782e3e3c`) —
> 그것이 받치는 것은 ①②③이지 ⑥이 아니다.
>
> ~~M2가 BTP 부재로 봉쇄돼 있다~~ → **그 봉쇄는 끝났다** — 계정은 D-115에서 생겼고
> 실왕복은 D-118이 세웠다. 지금 M2를 막는 것은 **계정이 아니라 증거**다.
...
>
> **⚠ 갱신 (판M2-a · 2026-08-23 · D-114)**: 판5가 남긴 마지막 자리 — **기동이 토큰
> 취득을 돌리지 않는 것**(의도된 정지선) — 도 이어졌다. `client_credentials`는 기동이
> 자동으로 취득하고 `authorization_code`는 정지선을 유지하며, 진단이 5종으로 갈렸다.
> **그럼에도 이 행은 여전히 `일부`다** — 이 판의 증거는 **전부 오프라인 목이고 실 UAA
> 왕복이 0회**라 `짓기 완료` 기준 ⓐ(실접속)를 채우지 않는다(D-114 정직 유보 ⓐ).
> **남은 것은 코드가 아니라 계정이다** — `판M2-b`.
>
> **⚠ 갱신 (판M2-b · 2026-08-23 · D-115)**: **계정 보류는 끝났고, `일부`는 그대로다.**
> BTP trial(`TRL`)에 실제로 붙어 잰 결과 — 판M2-a가 자동화한 `client_credentials`
> 통로는 **토큰까지만 선다**: UAA는 200으로 내주지만 그 토큰에 `user_name`이 없어
> ADT 4경로가 **전부 401**이었다(Bearer 유무와 무관하게 동일 응답). 반면
> `authorization_code` 사용자 토큰은 ADT 3경로 **전부 200**이다. 그러므로 **`짓기 완료`
> 기준 ⓐ(실접속)에 도달하는 경로는 그 그랜트 하나**이고, **막고 있는 것은 계정도 배선
> 결함도 아니라 정지선**(D-091 ⓑ → D-114 ⓑ)이다 — 그것을 여는 것이 **`판M2-c`**다.
> ⚠ **이 판이 세운 실접속은 엔진 기동이 아니라 엔진 밖 프로브가 세웠다** — 그래서 M2는
> 승격하지 않는다. 기구(`src/auth/callback.ts`·`uaa.ts`의 `exchangeCode`·`refresh`·
> `authorizeUrl`)는 **이미 지어져 있고 시험도 있다**; 없는 것은 **기동이 그것을 부르는
...
- **오프라인 증거**: jest **4,001 통과 / 1 skip**(당시 수 — 판6.1이 시험을 더해 **지금은
  ...
  당시 8종. 이후 판1의 `verify-checker`·판B의 `check-doc-size`가 더해졌고 **현행 계수의
  정본은 `CLAUDE.md` 게이트 절**이다).
...
### ⑷ ~~interactive/ 상류 고지 은퇴~~ → 고지 범위 정합 + 활성 차용분 소진  ✅ **완료 (2026-08-23 · 판11 · D-112)**
...
- **㉮ 고지 범위 정합 — ✅ 완료 (2026-08-23 · 판11 · D-112 ⓐ).** 두 번 데인 자리다 — 판8이
  [`interactive/LICENSE`](../interactive/LICENSE)를 **잔존 36파일**로 좁혔다가 독립 리뷰에
  철회됐고(D-104 ⓐ — `copy`만 세었다), 그 대체 수치마저 미달이었다(D-106 ⓐ). **판11이 낸
  답은 좁히기가 아니라 「서브트리 유지 + 내역의 정확성」이다** — 상류 저작물로 남는 것은
  **파일이 아니라 산재한 계약행**이라 **파일 목록이라는 형태 자체가 틀린 답**이었다. ㉮의
  완료 정의는 범위 축소를 요구한 적이 없고 **내역의 정확성**을 요구했으며, 그것이 이제
  성립한다: 파일 단위 차용 = `server/sap-assets/` **30**(영구 · D-079 ⑥)뿐 · 명부 3개 완주
  (copy 170/170 · `server/` 결부 판정 · §3.4 56/56) · 잔존 = 산재 **계약행**. ⚠ **판11이
  공표한 「~600행/40파일」은 §3.4 갈래만의 값이었고, `server/` 밖 실제 총계는
  ~2,200행/~130파일이다**(2026-08-23 · **D-113 ⓐ** · 구성 3갈래는 §1.3).
  고지 4곳(루트 [`LICENSE`](../LICENSE) 표 · `interactive/LICENSE`의 「What this notice covers」 절 ·
  [`THIRD_PARTY_NOTICES.md`](../interactive/THIRD_PARTY_NOTICES.md) ·
  [`interactive/README.md`](../interactive/README.md))을 **그 내역으로 갱신했다** — MIT 전문·상류
  저작권 문구는 불변이고, 번들 런타임 의존(외부 npm 패키지) 귀속도 **남는다**(정식 의존성이고
  끝그림 ②가 허용하는 형태다).
  ⚠ **여기서 얻은 규칙**: 고지를 **넓게 두는 것**은 부정확할 뿐이지만 **좁히면서 「나머지는
  우리 것」이라 적는 것**은 방향이 다르다 — 판정이 설 때까지 **넓은 쪽**에 둔다. **판11은
  판정이 선 뒤에도 넓은 쪽에 두었다** — 계약행은 저작물성이 있을 수 있고 행 단위로
  산재하므로, 고지가 그것을 명시적으로 커버하는 것이 정확하기 때문이다(D-112 ⓓ).
- **㉯ 활성 차용분 소진 — ✅ 완료 (2026-08-23 · 판10 · D-107).** `server/tool-catalog/` 4는
  **엔진 등록점에서 만드는 생성물**이 됐다 — 생성기 `sapkit-engine/harness/render-tool-catalog.mjs`가
  `TOOL_REGISTRY` 186종에서 4파일을 만들고, 대조 게이트 `gates/catalog.mjs`가 「커밋된 것 ==
  생성 결과」를 재며(**엔진 게이트 5 → 6종** · 음성시험 43 → 52), 손으로 고치면 거부된다.
  그래서 **판6.1부터 이월돼 온 「tool-catalog 자동 대조 게이트 부재」가 함께 닫혔다** —
  입력을 `tools/list`(프로파일에 따라 155/186으로 갈린다)가 아니라 **정적 등록점**으로 바꾼
  것이 그 게이트가 여태 못 서던 실제 원인을 없앴다. `server/verify-engine.mjs`는 84 → **90줄**
  골격 재저작(계약 불변 + 강화 1건 — `VERSION`에 commit이 없으면 FAIL) · `server/bundle-keyring.mjs`는
  처분이 **재저작**으로 정해져 277 → **226줄**이 됐다(4모드 exit 계약·핀 형식 보존).
  **검증은 역방향 스캔**(D-106 ⓒ의 첫 이행) — 6파일 전부 겹침 3행 미만이고 잔여 2행은
  Node import 관용구와 핀 형식 계약 문자열이다. ⚠ ~~**파일명 개명은 하지 않았다 — 명시 이월**~~ → **2026-08-23 판Z에서 개명 완료**(D-123 · 참조자는 12가 아니라 **20파일**이었다 — 판Z 실사 정정). 아래는 판10 시점의 기록이다: **파일명 개명(`sapkit-mcp-tools*.md`)은
  하지 않았다 — 명시 이월**(끝그림 ④ 소관 · 참조자 12파일).
- **㉰ 상류 잔존 56 재저작 — ✅ 완료 (2026-08-23 · 판3.7 · D-109~D-111).** 2026-08-23에 리뷰
  둘이 연달아 드러낸 **소관 없던 자리**였다(D-104 ⓒ · **D-106 ⓐ**). 페르소나 **25** · 절차
  **16** · 어댑터 5 · 지식 5 · 정책 2 · 도구 3을 **배치 3회**로 갚았고, 명부
  [`reference/copy-baseline.md`](reference/copy-baseline.md) **§3.4**가 **56/56 · 잔량 0**이다
  (독립 리뷰 누계 4회 · 차단은 전부 **한 단어** — `explicitly` · `and`→`or`). 계수·체크 규칙은
  §2(재저작만 · 독립 리뷰 통과 후)를 그대로 따랐다. ⚠ **명부를 만드는 방법이 바뀌었다** —
  장부에서 목적지를 도출하지 않고 **레포 파일에서 출발하는 역방향 스캔**으로 만든다(D-106 ⓑ).
  장부 방향은 도출 규칙의 결함을 그대로 물려받는다.

**검증 기준**: ㉮ = 고지 4곳이 **같은 내역**을 말하고, 그것이 명부 §4 레시피의 도출 결과와
일치한다(**기계 재현 가능** — 인벤토리 487 · 미매칭 0 · `copy` 원본 282 · `server/` 결부 60 =
keyring 21 + 번들 3 + **잔존 36**) — ✅ **섰다** (2026-08-23 · D-112 ⓐ): 착수 **역방향 스캔**이
겹침 **40파일 · ~598행**으로 마감 실측을 냈고, 고지 4곳이 그 내역(파일 단위 차용 = `sap-assets/`
30뿐 · 명부 3개 완주 · 잔존 = 산재 계약행)으로 갱신됐다. ⚠ **그 갱신의 잔존 총계는 회수에서
정정됐다** — 스캔이 §3.4 갈래만 겨눴기 때문이다(2026-08-23 · **D-113 ⓐ** — `server/` 밖
**~2,200행/~130파일**). **방향과 성격 판정은 그대로 서고, 고쳐진 것은 총계와 두 문장이다.** ㉯ = ✅ **섰다**
(2026-08-23 · D-107) — 활성 6이 자체 저작으로 치환됐음을 **역방향 스캔**(겹침 3행 미만)으로
확인했고 `tool-catalog` 대조 게이트가 `sapkit-engine` 잡에서 돈다.
㉰ = ✅ **섰다** (2026-08-23 · D-109~D-111) — §3.4 명부 **56/56 체크**(각 파일 독립 리뷰 통과) +
**§3.5 이월분 표본 판정**(체크 정당 · D-109 ⓐ).

**⚠ 이 단계는 「사다리의 끝」이지만 「고지의 끝」은 아니다.** ㉮·㉯·㉰가 다 서도
`interactive/LICENSE`는 남는다 — 그때 그 고지는 **부채의 표시가 아니라 SAP 측 자산의 정직한
귀속**이다.

**✅ ⑷ 완료 — 2026-08-23 · 판11 · D-112 ⓑ.** ㉮(범위 정합) · ㉯(활성 6 소진) · ㉰(상류 잔존 56
재저작) 셋이 다 섰다. **⑷는 「고지 은퇴」가 아니라 「고지가 정직해지는 것」으로 끝난다** —
D-103 ⓓ가 재정의한 그대로다. 그리고 이것이 **사다리의 마지막 단계**이므로, 2026-08-10
**D-079**가 착수한 교체 사다리가 여기서 닫힌다(§1.1 아래 완주 문단).
독립 리뷰 이력: **R-FAIL → 회수(D-113) → 경량 재리뷰 → 닫는 조건 반영(`54c10a37`)** —
㉮는 두 번 데인 자리였고 세 번의 검증 끝에 **종결됐다.**
...
| ⑷ 고지 **✅ 완료 (2026-08-23 · 판11 · D-112)** — **㉮ 범위 정합 ✅**(판11 · D-112 ⓐ) · **㉯ 활성 6 소진 ✅**(판10 · D-107) · **㉰ 상류 잔존 56 재저작 ✅**(판3.7 · D-109~D-111) | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + **착수 역방향 스캔**(겹침 **40파일 · ~598행** — 재저작 전 56파일에서 내려온 값) · ㉰의 증거 = §3.4 명부 **56/56** | 고지 **4곳**이 **같은 내역**을 말하고 그것이 실제 잔존물과 일치 — ✅ **성립한다**(2026-08-23): 파일 단위 차용 = `sap-assets/` **30**뿐 · 명부 3개 완주 · 잔존 = 산재 **계약행 ~2,200행/~130파일**(**D-113 ⓐ** 정정 — 판11 공표치 ~600/40은 §3.4 갈래만의 값이었다) | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). **갚을 것이 남지 않았다** — 활성 6(D-107)과 상류 잔존 56(D-109~D-111)이 전부 소진됐고, 남는 고지 1건은 **SAP 측 자산의 정직한 귀속**이다(D-112 ⓑ) |
...
**대조 정본 2종**
- [`interactive/server/tool-catalog/`](../interactive/server/tool-catalog/sapkit-mcp-tools.md) —
...
1. 표면 정본 집합을 만든다: `interactive/server/tool-catalog/sapkit-mcp-tools-{read,write,runtime}.md`의
