...
|---|---|---|---|
| ① | **몸통은 전부 자작** | ✅ **도달** (2026-08-23 · D-112 ⓒ) — **활성 차용분 0**이다. 검사기 ✅(2026-08-16 · 판1) · 엔진 ✅(2026-08-19 · 판7-b) · `copy` 지식 170 ✅(2026-08-18) · 활성 6 ✅(판10 · D-107) · 상류 잔존 56 ✅(판3.7 · D-109~D-111). **남은 파일 단위 차용은 `interactive/server/sap-assets/` 30뿐**이고 그것은 개작·유지보수하는 것이 아니라 **무접촉 동봉 자산**이다(영구 · D-079 ⑥ · 이식 이후 변경 커밋 0). ⚠ **「도달」은 파일 단위 기준이다** — 행 단위로는 **계약행 ~600행/40파일**이 남고, 그것은 갚을 부채가 아니라 **의도된 보존**이다(D-112 ⓓ — 재저작하면 제품 동작·데이터 정합이 깨진다). **이 구분을 흐리면 「전부 자작」이 과대 주장이 된다**(D-112 유보 ⓒ) | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
| ② | **외부는 정식 의존성만** | ✅ **도달** (2026-08-23 · D-112 ⓒ) — **레포 안에서 개작·유지보수하는 상류 코드가 0**이다. 활성 6이 판10에서 소진됐고(`tool-catalog/` 4 = 등록점 생성물 · 유틸 2 = 골격 재저작 · 역방향 스캔 잔존 0 · D-107), 외부 코드는 전부 패키지 매니저로 당겨 쓰는 의존성이다. 예외인 `sap-assets/` 30은 이식 커밋 `038085c2` 뒤로 **변경 커밋 0**이라 ②의 정의를 이미 만족한다 | 외부 코드는 패키지 매니저로 당겨 쓰는 의존성 형태로만 존재. **예외 = SAP 시스템 측 자산** — 패키지 매니저가 없는 영역이라 ③의 예외와 같은 자리다 |
...
| ④ | **브랜드 단일** | ⏳ **미완 — 잔존 3갈래를 명시한다** (2026-08-23 · D-112 ⓒ): ⓐ `server/tool-catalog/sc4sap-mcp-tools*.md` **파일명**(참조자 **12파일** · 내용은 이미 자체 저작 생성물 · 판10 명시 이월) ⓑ `[SC4SAP …]` **훅 태그 3건**(모델이 보는 마커라 개명이 곧 제품 동작 변경 — D-111 기각 (c)) ⓒ **keyring 21의 정식 패키지 귀속**(②의 각주). ⚠ **이것은 저작권이 아니라 「이름」의 문제라 ⑷와 무관한 별건이다** — ⑷ 완료가 ④를 닫지 않는다 | 아래 1.2 |
...
행 단위로 읽지 말 것** — ①의 도달은 **파일 단위** 기준이고, 행 단위로는 계약행 ~600행이
**의도된 보존**으로 남아 있으며 그 구분을 고지 4곳이 명시한다(D-112 유보 ⓒ).
...
| 잔존 — 산재 **계약행** | ~600행 / 40파일 | **의도된 보존** (D-112 ⓓ) — 참조표·펜스 템플릿·print-verbatim 블록·사용자 노출 문구·승인 키워드·공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다(배치3 리뷰가 파일별로 근거를 세웠다) |
...
- **㉮ 고지 범위 정합 — ✅ 완료 (2026-08-23 · 판11 · D-112 ⓐ).** 두 번 데인 자리다 — 판8이
  ...
  **파일이 아니라 산재한 계약행**이라 **파일 목록이라는 형태 자체가 틀린 답**이었다. ㉮의
  ...
  (copy 170/170 · `server/` 결부 판정 · §3.4 56/56) · 잔존 = 산재 **계약행 ~600행/40파일**.
...
30뿐 · 명부 3개 완주 · 잔존 = 산재 계약행)으로 갱신됐다. ㉯ = ✅ **섰다**
...
| ⑷ 고지 **✅ 완료 (2026-08-23 · 판11 · D-112)** — **㉮ 범위 정합 ✅**(판11 · D-112 ⓐ) · **㉯ 활성 6 소진 ✅**(판10 · D-107) · **㉰ 상류 잔존 56 재저작 ✅**(판3.7 · D-109~D-111) | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + **착수 역방향 스캔**(겹침 **40파일 · ~598행** — 재저작 전 56파일에서 내려온 값) · ㉰의 증거 = §3.4 명부 **56/56** | 고지 **4곳**이 **같은 내역**을 말하고 그것이 실제 잔존물과 일치 — ✅ **성립한다**(2026-08-23): 파일 단위 차용 = `sap-assets/` **30**뿐 · 명부 3개 완주 · 잔존 = 산재 **계약행 ~600행/40파일** | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). **갚을 것이 남지 않았다** — 활성 6(D-107)과 상류 잔존 56(D-109~D-111)이 전부 소진됐고, 남는 고지 1건은 **SAP 측 자산의 정직한 귀속**이다(D-112 ⓑ) |
