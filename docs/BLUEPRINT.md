...
> **D-082 ⑦이 배정한 「M-표에 두 칸 반영」은 아직 수행할 수 없다.** 사다리 ⑴의 M-표가
> **이 문서에 없기 때문이다** — 2026-08-14 실측으로 이 파일에 `M1` 문자열이 0건이다. D-081과
> M-표는 미머지 가지 `dryforge/tmp-practice-scenarios`(head `c5088d3` · 7커밋)에만 있고,
> D-082 정직 유보 ⓓ가 경고한 상황 그대로다. **없는 표를 새로 창작하지 않는다** — 그 가지가
> 합류하면 위 두 단계 정의를 M-표의 두 칸으로 옮긴다. 그때까지의 기록이 아래다.
