...
| [`interactive/LICENSE`](../interactive/LICENSE) | **`interactive/` 서브트리 전체** (2026-08-22 좁혔다가 **08-23 복원** · D-103 ⓒ → D-104 ⓐ → **D-112 ⓐ로 확정** · 파일 자신의 「What this notice covers」 절이 알려진 내역을 갖는다) | **영구 — 내역 정합 완료 (2026-08-23 · 판11 · D-112).** 30(`sap-assets/`)은 영구 존치이므로 이 고지도 영구히 남는다. **남은 일은 없다** — 소진해야 할 것(활성 6 · 상류 잔존 56)이 전부 갚였고, 이 고지는 이제 「부채의 표시」가 아니라 **「SAP 측 자산의 정직한 귀속」**이다. ⚠ **브랜드(파일명 · 훅 태그)는 별건**이다 — 저작권이 아니라 **이름**의 문제이고 끝그림 ④ 소관이다 | ⑶·⑷ **완료** |
