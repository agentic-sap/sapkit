...
명시한다(D-112 유보 ⓒ).
⚠ **이 판정의 독립 리뷰는 기록 커밋 뒤에 돈다**(D-112 유보 ⓐ) — 리뷰 통과 전까지 **잠정**이다.
...
| `copy` — 지식·정책 갈래 | 170 | ✅ 완료 (2026-08-18 · 각 파일 독립 리뷰 통과) — 다만 **「전부 재저작」이 아니다**: **27파일은 D-088 ②의 「검증 체크(무변경)」**(순수 참조표 · 바이트 동일 보존)이고 재저작이 아니다(2026-08-23 · **D-113 ⓑ**) |
| 잔존 — 산재 **계약행** (`server/` 밖 서브트리 전체) | **~2,200행 / ~130파일** | **의도된 보존** (D-112 ⓓ) — 참조표·펜스 템플릿·print-verbatim 블록·**체크박스 규범항**·사용자 노출 문구·승인 키워드·공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다(배치3 리뷰가 파일별로 근거를 세웠다). ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다**(2026-08-23 · **D-113 ⓐ**) — 구성 **3갈래** = §3.4 갈래 **~600/40** + `copy` 재저작 체크분 **~950/64** + 무변경 검증 참조표 **~660/25** |
...
170의 목적지를 뺀다), 「전부 재저작」이라는 서술은 **무변경 검증 27파일에 대해 거짓**이었다.
...
⚠ **단, 이 판정의 독립 리뷰는 기록 커밋 뒤에 돈다**(D-112 정직 유보 ⓐ) — ㉮는 두 번 데인
자리이므로 **리뷰 통과 전까지 잠정**이다.
