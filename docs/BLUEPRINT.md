...
| # | 조건 | 지금 (2026-08-23 갱신) | 끝그림 |
|---|---|---|---|
| ① | **몸통은 전부 자작** | ~~엔진·검사기·지식 상당량이 차용 포크~~ → **검사기 ✅(2026-08-16) · 엔진 ✅(2026-08-19) · 지식 갈래 소진 ✅** · 남은 것은 `interactive/server/` 결부 **copy 36**(판8이 **동결 30 / 활성 6**으로 갈랐다 · D-103)에 더해 **상류 잔존 56**이다 — 2026-08-23 리뷰 둘이 연달아 드러낸 **소관 없던 자리**이고(D-104 · **D-106**) 페르소나 25 · 절차 16을 포함한다(판3.7) | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
| ② | **외부는 정식 의존성만** | ~~상류 코드를 레포에 품고 개작~~ → 개작 중인 상류 코드는 **활성 6**(`tool-catalog/` 4 + 유틸 2)뿐 | 외부 코드는 패키지 매니저로 당겨 쓰는 의존성 형태로만 존재. **예외 = SAP 시스템 측 자산** — 패키지 매니저가 없는 영역이라 ③의 예외와 같은 자리다 |
| ③ | **계보성 고지 은퇴** | 잔존 고지 **1건**(`interactive/LICENSE` — `vsp/`는 2026-08-16, `engine/`은 2026-08-22 판7.5 은퇴). ~~2026-08-22 판8이 범위를 36파일로 좁혔다~~ → **2026-08-23 철회**(D-104 ⓐ) — 좁힘이 `transform` 42를 빠뜨렸다. 고지는 **`interactive/` 서브트리 전체**에 걸린 채다 | ~~대체가 끝난 부품의 고지부터 차례로 은퇴, **최종 0건**~~ → **고지가 덮는 범위가 실제 잔존물과 일치 + 활성 차용분 0.** ⚠ **0건은 도달 불가**다 — `server/sap-assets/` 30이 D-079 ⑥의 존치 결정으로 영구히 남는다(D-103 ⓓ) |
...
우리 코드만 얹는다. 옛 `engine/`은 그 반대 형태(레포 내 개작 소스)였다(`vsp/`도 그랬으나
판1에서 은퇴했고, `engine/`은 판7.5에서 은퇴했다) —
그래서 유지보수 부채와 고지 의무를 동시에 지고 있었다.

**⚠ 2026-08-22 · 판8이 ③의 「최종 0건」을 내렸다 (D-103).** 그 0은 애초에 **D-079 ⑥**과
양립할 수 없었다 — SAP에 이미 설치된 `ZMCP_ADT_*` 오브젝트를 그대로 호출하기로 정한 이상
그 **설치 소스 30파일**은 제품에 동봉된 채 남고, 남는 한 출처 표시도 남아야 한다. 판7-b가
2026-08-19에 그 모순을 발견했고 판8이 계획서를 사실 쪽으로 옮겼다. **목표의 하향이 맞다 —
그렇지 않다고 강변하지 않는다.** 대신 척도를 바꿔 적었다: 차용의 무게는 **파일 수**가 아니라
**「우리가 그것을 떠안고 있는가」**로 잰다. 그 척도로 재면 `sap-assets/` 30은 이식 커밋
`038085c2` **말고는 커밋이 없어**(총 1)  ②의 정의(「레포 안에서 개작·유지보수하지 않는다」)를
이미 만족하고, 미달인 것은 **활성 6**(`tool-catalog/` 4 + `verify-engine.mjs` +
`bundle-keyring.mjs`)이다. **그 6이 ⑷의 실제 과녁이고, 판10이 그것을 겨눈다.**

**⚠ 2026-08-23 · 독립 리뷰가 그 판의 집행을 되돌렸다 (D-104).** 판정(동결/활성 갈림 ·
① 기각 · 완료 정의 변경)은 재현으로 확인돼 그대로 서지만, **고지 범위를 36파일로 좁힌
것은 철회**됐다 — 판8이 `class: copy`만 세고 **`class: transform`을 통째로 빠뜨렸기**
때문이다. ⚠ **그 대체 수치마저 미달이었다** — 2차 리뷰가 **장부의 경로 매핑이 절차 12파일을
아예 만들어 내지 못했음**을 잡았고(D-106 ⓐ), **역방향 스캔**으로 다시 세니
**56건**이다(페르소나 25 · 절차 16 · 어댑터 5 · 지식 5 · 정책 2 · 도구 3 · 절차 넷은
**54~65%**). `MIGRATION-MANIFEST.md`의 정의대로 `transform`은 **「형태 변환 이식」**이지
재저작이 아니다. **그러므로 ①의 「남은 것」은 36이 아니라 36 + 42이고**, 후자의 재저작이
**판3.7**이다. 명부는 `reference/copy-baseline.md` §3.4.
...
| 엔진 | **SAPKIT Engine** / 패키지 **`sapkit-engine`** | ✅ **2026-08-19 제품 번들 자리 인수**(D-095). 구 `abap-mcp-adt-powerup` 포크 번들이 물러났고 소스 `engine/`도 **2026-08-22 판7.5에 은퇴 완료** |
...
루트 [`LICENSE`](../LICENSE)는 MIT(소유자 명의)이고, 그 아래 상류 고지 **1건**을 승계 보존
중이다(판1에서 3→2, 판7.5에서 2→1). **각 고지는 해당 부품의 차용분이 소진된 시점에 은퇴한다** — 은퇴는
사다리 단계의 완료 판정과 같은 사건이다.

**⚠ 남은 1건은 은퇴하지 않는다 (D-103 ⓓ) — 그리고 범위도 좁히지 못했다 (D-104 ⓐ).**
판8이 그 대상을 `interactive/` 서브트리 전체에서 **잔존 36파일**로 좁혔으나, 독립 리뷰가
**`class: transform` 42건**을 짚어 **2026-08-23 철회**됐다. 지금 알려진 것은 이렇다:

| 갈래 | 수 | 상태 |
|---|---:|---|
| `copy` — `server/sap-assets/` | 30 | **영구 존치** (D-079 ⑥ · 이식 이후 변경 커밋 0) |
| `copy` — `tool-catalog/` 4 · `verify-engine.mjs` · `bundle-keyring.mjs` | 6 | **은퇴 대상** — 판10 |
| 상류 잔존 — 페르소나 25 · 절차 16 · 어댑터 5 · 지식 5 · 정책 2 · 도구 3 | **56** | **미판정** — 전부 상류 문장 잔존 · 명부 `reference/copy-baseline.md` §3.4 · 재저작은 **판3.7** |
| `copy` — 지식·정책 갈래 | 170 | ✅ 재저작 완료 (2026-08-18 · 각 파일 독립 리뷰 통과) |

**42가 판정될 때까지 고지는 서브트리 전체에 걸린다** — 좁히면서 「나머지는 우리 것」이라
적는 것은 부정확을 넘어 **주장을 하는 것**이기 때문이다.

| 고지 | 대상 서브트리 | 은퇴 조건 | 대응 사다리 |
|---|---|---|---|
| ~~[`engine/LICENSE`](../engine/LICENSE)~~ | ~~`engine/`~~ (~~+ `interactive/server/` 번들~~ — **2026-08-19 교체로 번들은 빠졌다**) | **은퇴 완료 (2026-08-22 · 판7.5)** — SAPKIT Engine이 번들 자리를 대체한 뒤(D-095) `engine/` 소스가 레포를 떠났다 | ⑴ |
| ~~`vsp/LICENSE`~~ | ~~`vsp/`~~ | **은퇴 완료 (2026-08-16 · 판1 · D-085)** — `sapkit-cli/`가 로컬 검사를 대체하고 `vsp/` 서브트리가 레포를 떠났다 | ⑵ |
| [`interactive/LICENSE`](../interactive/LICENSE) | **`interactive/` 서브트리 전체** (2026-08-22 좁혔다가 **08-23 복원** · D-103 ⓒ → D-104 ⓐ · 파일 자신의 「What this notice covers」 절이 알려진 내역을 갖는다) | **은퇴하지 않는다.** 30(`sap-assets/`)은 영구 존치이므로 이 고지도 영구히 남는다. 남은 일은 은퇴가 아니라 **활성 6(판10) + `transform` 42(판3.7)의 소진**이고, 그것이 끝나면 이 고지는 「부채의 표시」가 아니라 **「SAP 측 자산의 정직한 귀속」**만 남긴다 | ⑶·⑷ |
...
⑶ 지식 재저작                   ← 가장 김: 집필 노동이 실사용 검증 속도에 묶인다
   │                              (⑴·⑵와 병행 가능 — 코드 부품과 독립)
   ▼
⑷ interactive/ 고지 범위 정합    ← 마지막: ⑶의 차용분이 소진돼야 성립
   + 활성 차용분 소진              (2026-08-22 판8이 「은퇴」에서 이 이름으로 고쳤다 · D-103)
```
...
> 되돌릴 자리로 남아 있다가 **판7.5(2026-08-22)에 은퇴했다**. 아래 기준표는 그 판정의 기록으로 보존한다.
...
> ⚠ **그 「교체 직전」이 2026-08-19에 지나갔다** — 판7-b가 교체했다. 자리는 여전히 남지만
> **묻는 무게가 달라졌다**: 교체 전이라면 아직 아무에게도 안 나간 표면을 줄이는 것이었고,
> 지금은 **이미 배포된 제품의 도구를 빼는 것**이라 사용자에게 나가는 변경이다. 그래서
> 검증 기준 1(표면 186 동일 이름·호출 계약)과도 정면으로 부딪힌다. 재배치된 자리는
> **`판9`**이고 전제는 **판6.3의 attended 실기가 끝나 실사용 분포가 갱신된 뒤**다(D-097).
...
5. 위 4가 통과할 때까지 **구 번들이 현역**이다 (§3). — **D-093 ⓐ로 유지 기간이 교체
   이후까지 연장됐다**: 현역 자리는 넘겼고, 소스 `engine/`은 롤백 자산으로 남아 있다가
   **판7.5(2026-08-22)에 은퇴했다**.
   ⚠ **되돌리는 법이 은퇴로 바뀌었다.** 판7-b 시점의 「**교체 커밋 revert** 한 번」은
   더 이상 성립하지 않는다 — 그 revert는 이제 **소스 없는 번들**을 되살린다. 지금의
   롤백은 두 걸음이다: ① `engine/`이 온전한 마지막 커밋 **`2264f89d`**에서 그 트리를
   되뜨고 ② 그 위에서 교체분을 되돌린다. CI가 「롤백 번들이 여전히 지어진다」를
   확인하던 `engine-tests` 잡도 은퇴와 함께 제거됐으므로, **롤백 가능성은 이제
   상시로 기계 확인되지 않는다.** 그것이 은퇴의 대가이고, 이 판이 그 대가를 알고
   치렀다(D-101).
...
| **M6** | ~~**교체 직전 관문**~~ → **교체 뒤로 재배치됐다 (2026-08-19 · D-097).** 교체가 판7-b에서 끝나 이 관문의 **트리거가 지나갔다**. 남아 있던 둘을 각각 실제 판정이 가능한 자리로 옮겼다 — **ⓐ D8 관찰 판정 → `판6.3`**(D8 자신이 「C1 녹화에서 발동이 관찰되는가」를 판정 조건으로 적어 두었고 그 녹화가 거기서 대량으로 난다) · **ⓒ 꼬리 49종 재평가 → `판9`**(표면 재평가 · 새 판) — ✅ **2026-08-23 소진 · 전량 유지**(D-105). ⓑ **D18은 판5에서 이미 해소**됐고 ⓓ **검증 기준 1~4는 판7-b에서 성립**했다. **판정과 재평가의 자리이며, 원칙적으로 새 기능을 짓지 않는다**(아래 예외 참조). | ⓐ **D8 관찰 판정**. ⓑ **D18 해소** — `conformance-server-gates.mjs`의 무접속 거부 분류 어휘를 **구·신 두 문구를 모두 인식하도록 넓힌다.** 신 문구로 *교체*하면 안 된다 — M6 시점엔 아직 구 번들이 현역이라(위 「검증 기준」 5) 교체하는 순간 그쪽 판정이 깨진다. 안 넓히면 교체 후 신 엔진의 정상 거부가 `OTHER`로 떨어져 게이트 판정이 어긋난다. ⓒ **꼬리 49종 재평가 1회 → 새 D-항목**(D-079 ⑧). ⓓ 위 「검증 기준」 1~4 전건 + 실 SAP 대표 절차 구·신 동일 결과. | — ² | — ² |
...
### ⑷ ~~interactive/ 상류 고지 은퇴~~ → 고지 범위 정합 + 활성 차용분 소진

> **2026-08-22 · 판8이 이 단계의 이름과 완료 정의를 고쳤다 (D-103).** 옛 정의(「고지 은퇴 =
> 고지 0건」)는 **D-079 ⑥과 양립할 수 없었다** — 아래 「왜 0이 아닌가」. 아래는 고쳐진 정본이고,
> 옛 정의는 §1.1 ③ 행에 병기로 남아 있다.

**착수 조건**: ⑶-a·⑶-b·⑶-c의 차용분이 전부 소진될 때 — **2026-08-19 성립**(D-093 ⓓ + D-095 ⓔ).

**왜 0이 아닌가**: `interactive/server/sap-assets/`의 **30파일**은 SAP에 이미 설치된
`ZMCP_ADT_*`·`Z*_S4SAP_*` 오브젝트의 **설치 소스**다. **D-079 ⑥**이 「신 엔진도 기설치
오브젝트를 그대로 호출한다 · SAP 오브젝트 무접촉 · 추가 SAP 작업 0」으로 정했고,
[`install-sap-assets.md`](../interactive/core/procedures/install-sap-assets.md)가 그 파일들을
설치 소스로 직접 링크한다 — 재저작하면 SAP 측 재설치가 되고, 빼면 **완전 오프라인 설치가
깨진다.** 그러므로 그 30은 제품에 남고, 남는 한 출처 표시도 남는다. **0은 도달 불가다.**

**행위 — 둘로 갈린다.**

- **㉮ 고지 범위 정합 — ⚠ 미완 (2026-08-22 시도 · **08-23 철회** · D-104 ⓐ).** 판8이
  [`interactive/LICENSE`](../interactive/LICENSE)를 **은퇴시키지 않고** 그 범위를 **잔존
  36파일**로 좁혔으나, 독립 리뷰가 **`class: transform` 42건**을 짚어 되돌렸다 — 좁힘이
  `copy`만 세었다. **지금 고지는 `interactive/` 서브트리 전체에 걸린다.** 고지 4곳(루트
  [`LICENSE`](../LICENSE) 표 · `interactive/LICENSE`의 「What this notice covers」 절 ·
  [`THIRD_PARTY_NOTICES.md`](../interactive/THIRD_PARTY_NOTICES.md) ·
  [`interactive/README.md`](../interactive/README.md))은 **같은 것을 말하되, 이제 「범위」가
  아니라 「알려진 내역」을 적는다** — 무엇이 재저작으로 해소됐고(170) 무엇이 영구 존치이며
  (30) 무엇이 갚을 것인지(6 + 42). 번들 런타임 의존(외부 npm 패키지) 귀속은 **남는다** —
  정식 의존성이고 끝그림 ②가 허용하는 형태다.
  ⚠ **여기서 얻은 규칙**: 고지를 **넓게 두는 것**은 부정확할 뿐이지만 **좁히면서 「나머지는
  우리 것」이라 적는 것**은 방향이 다르다 — 판정이 설 때까지 **넓은 쪽**에 둔다.
- **㉯ 활성 차용분 소진 — 대기 (판10).** `server/tool-catalog/` 4는 **엔진 등록점에서 생성**하는
  것으로 대체한다(그러면 판6.1부터 이월돼 온 「tool-catalog 자동 대조 게이트 부재」도 함께
  닫힌다). `server/verify-engine.mjs`(81줄)는 골격 재저작. `server/bundle-keyring.mjs`(277줄)의
  처분은 **판10이 정한다.**
- **㉰ 상류 잔존 56 재저작 — 대기 (판3.7).** 2026-08-23에 리뷰 둘이 연달아 드러낸
  **소관 없던 자리**다(D-104 ⓒ · **D-106 ⓐ**). 페르소나 **25** · 절차 **16**(넷이
  54~65%) · 어댑터 5 · 지식 5 · 정책 2 · 도구 3. 명부는
  [`reference/copy-baseline.md`](reference/copy-baseline.md) **§3.4**이고 계수·체크
  규칙은 §2(재저작만 · 독립 리뷰 통과 후)를 그대로 따른다. ⚠ **명부를 만드는 방법이
  바뀌었다** — 장부에서 목적지를 도출하지 않고 **레포 파일에서 출발하는 역방향
  스캔**으로 만든다(D-106 ⓑ). 장부 방향은 도출 규칙의 결함을 그대로 물려받는다.

**검증 기준**: ㉮ = 고지 4곳이 **같은 내역**을 말하고, 그것이 명부 §4 레시피의 도출 결과와
일치한다(**기계 재현 가능** — 인벤토리 487 · 미매칭 0 · `copy` 원본 282 · `server/` 결부 60 =
keyring 21 + 번들 3 + **잔존 36** · **역방향 스캔** 잔존 **56**). ㉯ = 활성 6이 자체
저작으로 치환됐음을 git 이력으로 확인 · `tool-catalog` 대조 게이트가 서 있을 것.
㉰ = §3.4 명부 **56/56 체크**(각 파일 독립 리뷰 통과) + **§3.5 이월분 표본 판정**.

**⚠ 이 단계는 「사다리의 끝」이지만 「고지의 끝」은 아니다.** ㉯가 끝나도
`interactive/LICENSE`는 남는다 — 그때 그 고지는 **부채의 표시가 아니라 SAP 측 자산의 정직한
귀속**이다.
...
|---|---|---|---|
| ⑴ 엔진 **✅ 교체 완료 2026-08-19 · 소스 은퇴 완료 2026-08-22(판7.5)** | tool-catalog diff 0 · `smoke-mcp` · `conformance-server-gates` · 실데이터 게이트 음성시험 | 대표 절차 1건 구·신 동일 결과 | **교체 판(판7-b)에서는 은퇴가 없었다** — `engine/` 소스 + 고지의 은퇴는 **교체 이후 별도 판**(D-093 ⓐ · 되돌아갈 자리를 남겼다)으로 예정됐고, **판7.5가 그 판이다**. 판7-b의 소관분 판정 결과: 서버 결부 copy 39 중 **3만 대체 · 36 잔존**(D-095 ⓔ) — 그 36은 `engine/` 서브트리가 아니라 `interactive/server/`에 있으므로 이번 은퇴 대상 밖이다 |
...
| ⑷ 고지 **㉮ 범위 정합 ⚠ 미완**(2026-08-22 시도 · **08-23 철회** · D-104) · **㉯ 활성 6 소진 = 대기(판10)** · **㉰ 상류 잔존 56 재저작 = 대기(판3.7)** | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + 그 36의 git 이력(**이식 커밋 포함 총계** — 동결 30 = **1**(이식뿐) / 활성 6 = **6·3·3**) | 고지 **4곳**이 **같은 내역**을 말하고 그것이 실제 잔존물과 일치 — ⚠ **아직 아니다**: 상류 잔존 **56**이 미판정이라 고지는 서브트리 전체에 걸려 있다 | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). 갚을 것은 **활성 6**(판10)과 **상류 잔존 56**(판3.7) |
...
> **✅ 판정 완료 (2026-08-23 · 판9 · D-105) — 전량 유지, 표면에서 빼지 않는다.**
> D-079 ⑧이 예약한 「재평가 1회」가 여기서 소진됐다. 근거는 ① 묻는 무게가 달라졌다
> (교체 전 = 안 나간 표면 축소 → 지금 = **배포된 제품에서 도구 제거**) ② **검증 기준 1**
> (표면 186 동일 이름·호출 계약)과 정면 충돌 ③ 표본이 1인·1머신·1개월 ④ 「짓는 공수」가
> 이미 0이라 저울이 뒤집혔다(제거 비용 = 제품 7파일 + 엔진 135파일 + 새 D-결정) ⑤ 아래 실측.
>
> **세션 무게 실측** (번들 stdio `tools/list` 바이트 · 이 머신은 **프로파일 활성 = connected**):
>
...
>
> **`Delete*` 25종(16,228 B)은 기본 프리셋에 한 종도 없다.** 그러므로 일상 세션에서 꼬리를
...
> (`launch.cjs`가 `readonly,high`로 번역해 넘긴다 — 그대로 주면 도구 **0종**이 온다).
