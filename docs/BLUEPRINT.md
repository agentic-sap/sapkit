...
| ④ | **브랜드 단일** | ⏳ **미완 — 잔존 3갈래를 명시한다** (2026-08-23 · D-112 ⓒ): ⓐ `server/tool-catalog/sc4sap-mcp-tools*.md` **파일명**(참조자 **12파일** · 내용은 이미 자체 저작 생성물 · 판10 명시 이월) ⓑ `[SC4SAP …]` **훅 태그 3건**(모델이 보는 마커라 개명이 곧 제품 동작 변경 — D-111 기각 (c)) ~~ⓒ **keyring 21의 정식 패키지 귀속**(②의 각주)~~ → **② 소관으로 이동** (2026-08-23 · **D-113 ⓒ④**) — 이름이 아니라 **귀속·의존성 형태**의 문제였다. ②의 각주가 정본이다. ⚠ **남은 ⓐⓑ는 저작권이 아니라 「이름」의 문제라 ⑷와 무관한 별건이다** — ⑷ 완료가 ④를 닫지 않는다 | 아래 1.2 |
...
| 도구 카탈로그 파일명 | `server/tool-catalog/sc4sap-mcp-tools*.md` | ⏳ **미개명 — 판10의 명시 이월**(D-107 기각 (a)). 내용은 이제 **자체 저작 생성물**이지만 파일명에 상류 이름이 남았다. 참조자 **12파일**(역사 설계 문서 포함)이고 `check-runtime-path-rename.mjs`는 점 없는 `sc4sap` 토큰을 **범위 밖**(D-041 소관)에 둔다 — 개명은 이 ④의 몫이다 |
...
- **㉯ 활성 차용분 소진 — ✅ 완료 (2026-08-23 · 판10 · D-107).** `server/tool-catalog/` 4는
  ...
  **검증은 역방향 스캔**(D-106 ⓒ의 첫 이행) — 6파일 전부 겹침 3행 미만이고 잔여 2행은
  Node import 관용구와 핀 형식 계약 문자열이다. ⚠ **파일명 개명(`sc4sap-mcp-tools*.md`)은
