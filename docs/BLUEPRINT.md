...
|---|---|---|---|
| ① | **몸통은 전부 자작** | ~~엔진·검사기·지식 상당량이 차용 포크~~ → **검사기 ✅(2026-08-16) · 엔진 ✅(2026-08-19) · 지식 갈래 소진 ✅ · 활성 6 소진 ✅(2026-08-23 · 판10 · D-107)** · 남은 것은 `interactive/server/` 결부 **동결 30**(영구 존치 · D-103)과 **상류 잔존 56**이다 — 2026-08-23 리뷰 둘이 연달아 드러낸 **소관 없던 자리**이고(D-104 · **D-106**) 페르소나 25 · 절차 16을 포함한다(판3.7) | 제품을 이루는 부품 중 **우리가 개작·유지보수하는 것**이 전부 소유자 저작 — 즉 **활성 차용분 0** |
| ② | **외부는 정식 의존성만** | ~~상류 코드를 레포에 품고 개작~~ → ~~개작 중인 상류 코드는 **활성 6**(`tool-catalog/` 4 + 유틸 2)뿐~~ → **그 6이 0이 됐다** (2026-08-23 · 판10 · D-107 — `tool-catalog/` 4는 등록점 **생성물**이 됐고 유틸 2는 **골격 재저작**됐다 · 역방향 스캔 잔존 0). ⚠ **「0」은 판8이 센 그 갈래에 대한 것이다** — `transform` 갈래 **56**(어댑터 5 · 도구 3 등 코드 파일을 포함한다)은 여전히 미판정이고 **판3.7**이 갚는다 | 외부 코드는 패키지 매니저로 당겨 쓰는 의존성 형태로만 존재. **예외 = SAP 시스템 측 자산** — 패키지 매니저가 없는 영역이라 ③의 예외와 같은 자리다 |
...
**56이 판정될 때까지 고지는 서브트리 전체에 걸린다** — 좁히면서 「나머지는 우리 것」이라
적는 것은 부정확을 넘어 **주장을 하는 것**이기 때문이다. **판10이 활성 6을 소진한 뒤에도
이 규칙은 그대로다**(D-107 ⓕ) — 바뀐 것은 범위가 아니라 **알려진 내역**이고, 갚을 것이
「활성 6 + 56」에서 **「56」**으로 줄었다.
...
| [`interactive/LICENSE`](../interactive/LICENSE) | **`interactive/` 서브트리 전체** (2026-08-22 좁혔다가 **08-23 복원** · D-103 ⓒ → D-104 ⓐ · 파일 자신의 「What this notice covers」 절이 알려진 내역을 갖는다) | **은퇴하지 않는다.** 30(`sap-assets/`)은 영구 존치이므로 이 고지도 영구히 남는다. 남은 일은 은퇴가 아니라 소진이었고 ~~활성 6(판10)~~은 **2026-08-23 끝났다**(D-107) — 이제 **상류 잔존 56(판3.7)** 하나이며, 그것이 끝나면 이 고지는 「부채의 표시」가 아니라 **「SAP 측 자산의 정직한 귀속」**만 남긴다 | ⑶·⑷ |
...
- **㉮ 고지 범위 정합 — ⚠ 미완 (2026-08-22 시도 · **08-23 철회** · D-104 ⓐ).** 판8이
  ...
  아니라 「알려진 내역」을 적는다** — 무엇이 재저작으로 해소됐고(170 + 판10의 **6**) 무엇이
  영구 존치이며(30) 무엇이 갚을 것인지(~~6 + 42~~ → **56**). 번들 런타임 의존(외부 npm 패키지) 귀속은 **남는다** —
...
keyring 21 + 번들 3 + **잔존 36** · **역방향 스캔** 잔존 **56**). ㉯ = ✅ **섰다**
...
**⚠ 이 단계는 「사다리의 끝」이지만 「고지의 끝」은 아니다.** ㉯가 끝났어도
...
| ⑷ 고지 **㉮ 범위 정합 ⚠ 미완**(2026-08-22 시도 · **08-23 철회** · D-104) · **㉯ 활성 6 소진 ✅ 완료(2026-08-23 · 판10 · D-107)** · **㉰ 상류 잔존 56 재저작 = 대기(판3.7)** | 명부 잔량 0(증명은 git 이력) **+ 판4(oop 20) + 판7-b 소관분 판정** — 명부는 그 셋을 세지 않는다(**D-087 ③**). **셋 다 섰다(2026-08-19).** ㉮의 기계 증거 = 명부 §4 레시피 재현(인벤토리 487 · 미매칭 0 · copy 282 · `server/` 결부 60 = keyring 21 + 번들 3 + **잔존 36**) + 그 36의 git 이력(**이식 커밋 포함 총계** — 동결 30 = **1**(이식뿐) / 활성 6 = **6·3·3**) | 고지 **4곳**이 **같은 내역**을 말하고 그것이 실제 잔존물과 일치 — ⚠ **아직 아니다**: 상류 잔존 **56**이 미판정이라 고지는 서브트리 전체에 걸려 있다 | ~~`interactive/LICENSE`~~ → **은퇴 대상이 아니다**(D-103 ⓓ). 갚을 것은 ~~활성 6(판10)~~ **소진 완료**(D-107)를 빼고 **상류 잔존 56**(판3.7)뿐 |
