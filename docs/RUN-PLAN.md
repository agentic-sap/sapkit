...
| **판7-b** | ⑴ | **교체(swap)만** — 제품 번들을 sapkit-engine 산출물로 (구 `판7`) | 불필요 (**전제 = 검증 기준 1~4** · D-093 ⓐ — **넷 다 섰다**) | 대기 — **다음** |
| 판7.5 | ⑴ | **`engine/` 은퇴** — 고지 2→1건 (교체가 안정된 뒤 · 되돌아갈 자리를 먼저 없애지 않는다) | 불필요 | 대기 |
| 판8 | ⑷ | `interactive/` 고지 은퇴 — 고지 0건 | 불필요 (전제 = ⑶ 소진 — **판6.2가 마지막 조각을 닫았다**) | 대기 |
...
### 판7-b — ⑴ 교체(swap) **만** (은퇴는 분리 — D-093 ⓐ · 구 `판7`)

- **전제**: BLUEPRINT 사다리 ⑴ **「검증 기준」 1~4**(§3.2 — 검증 없는 교체 금지).
  ~~판6 완료~~ **아니다** — M-표는 제작 진도표이고 교체 조건이 아니다(D-093 ⓐ).
  **넷 다 섰다** — 1·3·4는 판6.2 실사, **2는 판7-a가 세웠고 CI가 매 푸시 갱신한다**.
- **착수 전에 볼 것**: 판7-a 브리프의 **「🔴 최종 독립 리뷰가 새로 짚은 판7-b 이월 둘」** —
  하나는 **교체가 제품 문서를 거짓으로 만드는 자리**(안전 노브의 설정 주체가 바뀐다)이고
  다른 하나는 **게이트 기대값 구조**다. 둘 다 교체와 같은 판에서 판정해야 한다.
- **목표**: 제품 번들(`interactive/server/server.bundle.cjs`)을 sapkit-engine
  ...
  ⑶-b 「서버 결부 copy 39건」 소관분 판정.
- **하지 않는 것**: `engine/` 은퇴. **되돌아갈 자리를 남긴다** — 검증 기준 5(구 번들
  현역)의 유지 기간이 교체 이후로 연장됐다(D-093 ⓐ).
...
- **전제**: ⑶ 세 갈래 소진(판2 · 판3.x 전부 · 판4 + 판6의 템플릿 확인) — **성립했다**
  ...
  차용 파일 20개는 실제로 레포를 떠났다). 남은 실질 전제는 **판7-b 소관분 판정**이다.
