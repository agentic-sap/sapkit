...
- **교체·고지 은퇴는 검증 뒤에만**(BLUEPRINT §3.2). **다만 「검증」의 정의가 바뀌었다**
  ...
  「검증 기준」 1~4**이고 M-표 완주가 아니다. 판8의 전제는 종전대로 **⑶ 세 갈래 소진**이며
...
| **판7** | ⑴ | **교체(swap)만** — 제품 번들을 sapkit-engine 산출물로 | 불필요 (**전제 = 검증 기준 1~4** · D-093 ⓐ) | 대기 |
...
- **갈래 종결 (2026-08-18 · 판3.6)**: 명부 [`reference/copy-baseline.md`](reference/copy-baseline.md)
  **170행 전량 체크 · 잔량 0.** ⑶-b에 남은 미처리 파일은 없다. 셈 밖 소관은 그대로
  ...
  핵심 10종 SAP 확인 + 옛 20파일 제거). **남은 실질 전제는 판7 소관분 판정 하나**다 —
  현행 정본은 아래 **판8 브리프**다.
...
- **셈 밖 소관**: 서버 결부 copy 39건 → **판7**(⑴ 교체가 은퇴·대체 범위에서 판정) ·
  ...
  **판8(⑷) 착수의 실질 전제 = 명부 170 소진 + 판4(oop 20) + 판7 소관분 판정.**
  ...
  판6.2의 SAP 확인 + 제거). **남은 것은 판7 소관분 판정 하나**이고 현행 정본은 판8 브리프다.
...
- **판5 신규 등재 3건**:
  - **C1 evidence 문자열이 구 문구만 명시한다** — `conformance-server-gates.mjs:612`.
    **지금은 정확하다**(러너가 구 번들만 검사한다). 정리는 **구 번들이 은퇴하는 판7** 몫
...
- **완료 기준** (**D-093 ⓐ로 갈렸다** — 2026-08-19): 이 줄은 원래 「대장 증거 대기 0 ·
  ...
  **「대장 증거 대기 0」과 「M1~M6 전부 `증거 완료`」는 교체(판7)의 전제가 아니다** —
...
- **⚠ 못 선 것 — 화면 2종 · GUI 상태 2종(0/4)**. 원인은 템플릿이 아니라
  **`ZMCP_ADT_SRV` OData 서비스의 Dispatch 핸들러 고장**이다(등록·라우팅은 살아 있다 —
  ...
  적으면 안 된다.
- **판7에 남은 것** (실사 산출 — 이 목록이 다음 판의 작업 목록이다):
  1. **기준 2 배선** — `smoke-mcp.mjs:76`·`conformance-server-gates.mjs:73`이 구 번들
     경로를 하드코딩하고 대상 override 인자가 없다. `conformance-server-gates.mjs:480`은
     번들을 **단일 파일 텍스트**로 읽는다(단일번들 전제 재설계 필요).
  2. `smoke-mcp.mjs:74` NODE_PATH 분기 · 기준 파일 이원화(provenance ↔ old-surface) 결정.
  3. `GetTableContents`의 **MCP 왕복 음성시험**(현재 단위 시험만 — 1·에서 자동 해소).
  4. (선택) tool-catalog 문서 3종 ↔ `tools/list` **자동 대조 게이트**.
...
- **전제**: 판7의 교체가 실사용에서 안정. 되돌림 필요가 없다는 판단.
...
- **전제**: ⑶ 세 갈래 소진(판2 · 판3.x 전부 · 판4 + 판6의 템플릿 확인) — **성립했다**
  ...
  차용 파일 20개는 실제로 레포를 떠났다). 남은 실질 전제는 **판7 소관분 판정**이다.
