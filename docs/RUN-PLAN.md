...
| 판6.3 | ⑴ | **attended 실기 46종 전용 판** — `$TMP`에서 `Create*`→`Delete*` 왕복을 배치로 | **필요 (attended)** | 대기 — **교체의 전제가 아니다**(D-093 ⓐ·ⓒ) |
...
| **판7.5** | ⑴ | **`engine/` 은퇴** — 고지 2→1건 (교체가 안정된 뒤 · 되돌아갈 자리를 먼저 없애지 않는다) | 불필요 | 대기 — **다음** (전제 = 교체가 실사용에서 안정) |
...
- **판7-a 이월 둘을 판정했다**:
  1. **안전 문서 정정 + 파급 판정** — 신 엔진은 blocklist 노브를 프로세스 env로도 받는다
     (옛 GAP-2의 **수리**). 존치하되 세 겹으로 막았다: 잠긴 기본값 · **프로파일 우선**
     (새 단언 B2p3) · **제품 MCP 배선에 노브 0개**(`smoke-mcp` 검사 ⑦ — 매니페스트 드리프트
     검사로는 못 막는 자리다). `adapters/claude/hooks/README.md` 층별 한계 표 정정.
...
- **검증**: 제품 게이트 9 + 음성시험 9(`test-smoke-mcp` **24 → 29**) + `sapkit-engine`
  전종(+번들 스모크) + `sapkit-cli` 전종 + 신 엔진 dist 대상 2종 — **전부 exit 0**.
