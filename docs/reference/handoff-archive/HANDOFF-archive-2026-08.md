# HANDOFF 과거 기록 보관소 — 2026-08

> **무엇** — `HANDOFF.md`에서 살아있는 재개점만 남기고 내린 전량. **4,607줄** — 재개점
> 아래의 **완료 판 기록** 전량(판M2-c ~ S0 시대 · 롤백 좌표 · 사다리 진척표 포함) ·
> `## 보조 머신 줄기` 절 · 구 본문 **§1~§7 · §9**.
> **언제·어디서** — 2026-08-24, **판B**(프로젝트 두뇌 처방 집행)가 `HANDOFF.md`에서
> 잘라 옮겼다. 옮기기 직전 원본은 **4,808줄**이었다.
> **왜** — 살아있는 재개 지침이 과거 기록에 묻혔다. 재개점과 §8 자리의 포인터만
> 원본에 남기고 나머지는 여기로 내렸다.
>
> ⚠ **순수 이동이다.** 원래 **§ 번호·순서·구조**를 그대로 보존하며, 아래 본문은
> 블록인용(`> `) 접두어까지 포함해 **한 글자도 바뀌지 않았다.** 번호를 보존하는 이유가
> 있다 — `docs/reference/DECISIONS.md`가 HANDOFF를 **157곳**에서 인용하고 그중 **30곳**이
> § 번호를 직접 가리키는데, 그 파일은 **append-only라 고칠 수 없다.** 번호가 여기
> 남아야 그 인용들이 계속 해석된다.
>
> ⚠ **§8만은 여기 없다.** 구 `## 8. 불변 규칙`(7개 항)은 이사가 아니라 **`CLAUDE.md`의
> 「안전 규칙」 절로 병합**됐다 — 같은 규칙이 두 곳에 있으면 갈리기 때문이다(단권화).
> 항별 병합 내역(채택 문안 · 버린 문구와 사유)은 그 수술 커밋 메시지에 있다. 그래서
> 아래 본문은 **§7 다음이 §9**다. `HANDOFF.md`는 §8 자리에 포인터만 남겼으므로
> 「HANDOFF §8」 인용은 거기서 `CLAUDE.md`로 이어진다.
>
> ⚠ **여기는 기록이므로 갱신 의무가 없다.** 살아있는 정본은 `HANDOFF.md`(재개점) ·
> `CLAUDE.md`(안전 규칙·게이트·문서 계약) · `docs/RUN-PLAN.md`(판 큐)다. 아래 서술은
> **그것이 쓰이던 시점의 것**이라 현행이 아닐 수 있다 — 특히 §3(설치 상태)·§6(엔진
> 백로그)·§9(검증 명령)의 수치와 경로는 낡았다. 본문의 상대 링크·파일 경로도 고치지
> 않았다. 그것도 「한 글자도 바꾸지 않는다」에 든다.
...
## 판D 절 — `HANDOFF.md` 재개점에서 내린 전량 (내린 날 2026-09-03 · D-145)

> **왜 내렸나** — 재개점이 줄 수 상한(500)을 넘겼고, 게이트가 지시하는 처방은 삭제가
> 아니라 이사다. 판D는 2026-08-26 완료분이라 이 8월 보관소가 자리다.
> ⚠ **순수 이동이다** — 블록인용(`> `) 접두어까지 한 글자도 바꾸지 않았다.
> ## ✅ 방금 닫힌 것 — 판D: abapGit 오프라인 ZIP 인도 통로 (2026-08-26 · 착수 D-141 · 완료 D-142)
>
> **제품에 두 번째 인도 통로가 섰다.** 지금까지 sapkit이 ABAP을 SAP에 넣는 길은
> **MCP(실접속 ADT) 하나**뿐이었다. 판D가 두 번째를 열었다 — 에이전트가 **로컬 abapGit
> 미러**에 소스를 쓰고 동봉 검사기로 검사한 뒤 **전 패키지 ZIP**을 만들어 주면,
> **사용자가 abapGit UI에서 직접 반입**한다. 이 통로에서 **에이전트는 SAP에 닿지 않는다.**
>
> **왜 열었나 — 셋인데 ③이 범위를 지배했다.** ① `AGENTS.md` P3가 user-operated abapGit을
> **이미 허용**하는데 절차 배선이 0이었다 ② `source-repair-protocol.md`가 다수 FM 일괄
> 수리에 이 경로를 **이미 권하는데** 받치는 절차가 없었다 ③ **MCP를 쓸 수 없는 환경에서도
> 제품이 완주해야 한다**(소유자 명시 — 이 경로가 유일한 길인 사용자가 있다). 그래서
> **화면(DYNPRO)·CUA를 안전을 이유로 빼지 않았다** — 신규 화면 XML이 최고 반복 위험
> 구간인 것은 맞지만, 빼면 무MCP 사용자가 갇힌다. 위험은 **씨앗 모사 + 착수 시 왕복
> 고지**로 다룬다.
>
> **선 것 넷.** ⑴ 공용 절차 `interactive/core/procedures/develop-abapgit.md`
...
> 메타데이터 **0.10.0**.
>
> **설계의 중심 셋.** ⓐ **씨앗 fail-closed** — 대상 패키지의 abapGit **전체 export**가
> 없으면 **진행 자체가 없다**(부분 재구성·백지 저작 금지). 직렬화 세부(BOM 유무 포함)는
> 규칙 문서가 아니라 **씨앗이 정본**이다. ⓑ **전 패키지 ZIP만** — 오프라인 반입은 ZIP을
> 원격 전체 상태로 읽어서 빠진 객체가 삭제 후보가 되고, FUGR pull은
> delete-and-recreate다. 전 패키지면 그 목록이 **구조적으로 빈다.** ⓒ **확약이 기계
> PASS로 둔갑하지 않는다** — 오프라인 기록을 `verification.json` **밖**의 신설
> `verification-offline.json`에 두고, `import_confirmed` enum에서 **`PASS`를 뺐다**
> (`USER_AFFIRMED` / `USER_REPORTED_FAILURE` / `NOT_YET`).
>
> **중간 리뷰가 셋을 잡았고 전부 회수했다**(`c960f13`) — 이 판에서 가장 값이 나간 자리다.
> ① **검사 관문이 아예 작동하지 않을 뻔했다**: 절차가 존재하지 않는 `critical` 등급을
> 방아쇠로 삼았는데 `analyze`의 enum은 `high`/`medium`/`info`이고 소견이 있어도 exit 0이라,
> 그 단어가 유일한 방아쇠였다 → `severity: "high"`로 결박. ② 진입 2종의 **강도 미기재**
> → 둘 다 **Full**로 확정. ③ `check <dir>`가 FUGR 미러에서 **거짓 청정(exit 0)과 거짓
> 결함(exit 1)을 둘 다 낸다**는 실측 → 실제 한계를 적고 씨앗 대조로 가르게 했다.
>
> **마감 리뷰 — 차단 3 · 권고 8, 회수 완료**(**D-143**). 차단 3이 **전부 한 자리**였다 —
> 오프라인 지선의 **리뷰 관문**. 그 지선에서 리뷰는 유일한 독립 관문이라 여기서 새면 남는
> 방어가 없다. ⓐ 미러 경로를 나르는 `environment_context.notes`를 **바로 다음 줄의 「없으면
> 생략」이 지웠다** ⓑ 체크리스트 §12에 오프라인 처분이 없어, **연결이 있으면
> `GetInactiveObjects`가 존재한 적 없는 객체에 0을 돌려줘 빈 집합 위에 초록을 찍는다**
> (이 설계가 막으려던 바로 그 사고) ⓒ `verification-policy.md`가 지선과 정면으로 어긋난 채였다.
> 권고는 6건 회수했고 — 그중 하나가 **`analyze`가 500KB 위에서 조용히 열리는 것**
> (`rulesApplied: 0` · `score: good` · exit 0 → 우리 규칙으로는 깨끗한 통과로 읽힌다.
> 그 지선에서 소스가 받는 **유일한 기계 검사**다) — **1건은 소유자 결정으로 올렸다**(아래).
> **재검토는 R-PASS · 차단 0 · 권고 4**(3건 회수 — 정책 포인터가 다른 규칙을 가리키던 것 ·
> **증거 보존 규칙이 오프라인 기록에 미적용**이라 `USER_REPORTED_FAILURE`의 불명 상태 기록이
> 다음 사이클에 덮여 사라지던 것 · 500KB 처방의 두 갈래 중 하나가 위임 진입에서는
> 성립하지 않던 것 · 1건은 사유와 함께 수용).
>
> **실측.** 게이트 **제품 10종 + 공방 `check-wiki` 전부 exit 0**(`doctor`는 설치본이
> 0.9.0인 동안 red가 정상 — 재설치 후 green). **병합·푸시까지 닫혔다** — `main` 병합
> (`--no-ff` · `081143e`) 후 게이트 11종을 `main`에서 재실행해 전부 exit 0 · 푸시 ·
> **CI 4잡 전부 초록**(run `32949825364` — `node-gates`·`win-gates`·`sapkit-cli`·`sapkit-engine`).
> ⚠ **SAP 실기 0이다.** 절차·문서 제품이므로 **첫 실사용이 시험대**이고, XML 대조에는
> **기계 검증기가 없다**(에이전트 자체 검토 — 체인의 가장 약한 고리). DEV-only도
> 접속이 없어 **절차 규범**이지 기계 게이트가 아니다. 검사기 결함 둘(`check <dir>`의 FUGR
> 미색인 · `analyze`의 500KB 천장)은 **고치지 않았다** — 이 판이 코드 변경 0이라 문서로
> 막았다. 수리는 별판이고 새 D-결정이 필요하다.
>
> ~~▶ **소유자에게 올린 결정 하나 — 다음 세션이 물어야 할 것.**~~ — **닫혔다**
> (2026-08-31 · **D-144** · 소유자 선택 = **좁히기**). 집행 내역은 최상단 「✅ 방금 닫힌 것」.
>
