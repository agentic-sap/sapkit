...
> ~~**갚을 것으로 남은 것은 §3.4의 상류 잔존 56뿐이다**(판3.7).~~ → **그 56은 2026-08-23
> 판3.7이 완주했다**(D-111 · §3.4 **56/56 · 잔량 0**). 그리고 **「남은 것 = 56」이라는 셈
> 자체가 좁았다** — 판11 독립 리뷰(**D-113 ⓐ**)가 같은 스캔을 `copy` 명부 170의 목적지에도
> 돌려, `server/` 밖 잔존 총계를 **~130파일 / ~2,200행**(리뷰어 실측 129/2,216의 어림 표기)으로 확정했다(구성 3갈래 — §3.4
> 갈래 **40/~600** · `copy` 재저작 체크분 **64/~950** · 무변경 검증 참조표 **25/~660**).
...
| `LICENSE` (MIT 고지 승계) | 1 | **⑷ (판8)** 자체 소관 | **판정 완료 · 집행은 회수됐다** (2026-08-22 D-103 → 08-23 **D-104**). ~~은퇴~~ · ~~범위 36으로 좁힘~~ — 좁힘은 transform 누락으로 **철회**됐고 `interactive/LICENSE`는 **서브트리 전체**에 걸린 채다. ~~남은 갚을 것 = §3.4의 **56**(판3.7)~~ → **56은 판3.7이 완주했고**(D-111) **범위 정합은 판11이 「서브트리 유지 + 내역의 정확성」으로 성립시켰다**(D-112 ⓐ). 잔존 총계는 §3.4 갈래만이 아니라 **`server/` 밖 ~130파일 / ~2,200행**이다(D-113 ⓐ — §3.4 아래 ⚠ 주석) |
...
→ **56은 판3.7이 완주했고**(D-111), 이 문장의 **「뿐이다」는 틀렸다** — §3.4 갈래만 세고
`copy` 갈래(§3.5)를 뺀 셈이었기 때문이다(2026-08-23 · **D-113 ⓐ**). **파일 단위** 차용이
동결 30뿐인 것은 맞지만, **행 단위 잔존은 `server/` 밖 ~130파일 / ~2,200행**이다
(§3.4 갈래 40/~600 + `copy` 재저작 체크분 64/~950 + 무변경 검증 참조표 25/~660).
...
⚠ **비고의 「→ N」 합과 마감 스캔이 어긋난다 — 계통 오차이지 어느 한쪽의 오류가 아니다**
(2026-08-23 · **D-113 ⓒ③**). 위 표 파일별 비고의 잔존 합은 **~670행**인데 판11의 마감
역방향 스캔은 **~600행**(598~603)을 낸다. 원인은 **측정 시점**이다 — 비고는 각 배치가
**그 배치 시점**에 잰 값이고 스캔은 **판11 마감**에 전량을 한 번에 잰 값이라, 그 사이의
회수 재저작과 정규화 처리(공백 축약·distinct) 차이가 그대로 벌어진다. 계수 방식이
