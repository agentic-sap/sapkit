...
- **결정** (2026-08-16~17 사용자 확정):
  ...
  ⑤ **체크 인정 조건 = 재저작만.** 「그 파일을 **자체 문장·자체 구현으로 다시 썼다**」만 체크한다. **부분 편집은 체크가 아니다.** **파일을 지워서 갚는 길은 이 규칙에 없다** — 실제로 버릴 파일이 나오는 판에서 그 판의 별도 결정으로 연다.
...
- **영향**: **신설 1건** = `docs/reference/copy-baseline.md`(명부 · 170행 · 이번 판 체크 5건). **수정** = `docs/reference/DECISIONS.md`(이 항목 append) · `docs/BLUEPRINT.md`(§⑶-b 주석 블록에 계수 방식 확정 포인터 — 본문·검증 기준 무수정) · 규약 5편 in-place 재저작(`interactive/core/knowledge/abap/conventions/` — `naming-conventions` · `sap-version-reference` · `cloud-abap-constraints` · `ecc-ddic-fallback` · `abap-release-reference`) · `docs/RUN-PLAN.md`·`HANDOFF.md`(판 마감·재개점). **코드 0줄.** **불변** = 지식 `.md` **148**(신설·삭제 0) · `interactive/plugin-metadata.json`·생성물 7종·플러그인 버전 · 게이트 스크립트·훅·서버 번들·검사기 번들·`engine/`·`sapkit-engine/`·`sapkit-cli/` · 소비자 문서(`sap-standards.md`·절차·페르소나) · 역사 문서(DECISIONS 기존 항목 · `interactive/MIGRATION-MANIFEST.md` · `interactive/provenance/` · `docs/superpowers/`) · `interactive/THIRD_PARTY_NOTICES.md`(고지는 ⑷·판8까지 유지) · D-074·D-085·D-086 전부.
