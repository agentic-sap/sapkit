...
> 정본과 갈리면 **정본이 이긴다**. 합성 일자 **2026-08-26**.
...
- [트랙 A의 일생](track-a-lifecycle.md) — 하네스 트랙이 vsp CLI 백엔드 확정 → 무인 실행
  ...
  거쳐 오늘 「라우팅 규약 한 장」만 남기까지의 경위. 끝에 §10이 붙어 있다 — 그 규약이
  이미 허용하던 **user-operated abapGit** 통로에 2026-08-26 판D가 절차를 놓은 일과,
  그때 「SAP 접점을 이원화하지 않는다」를 어떻게 통과했는가, 그리고 그 통로의
  `COMPLETE`를 2026-08-31 D-144가 MCP 지선과 같은 급으로 좁힌 일. 그 뒤에 §11이 하나 더
  붙어 있다 — 2026-09-18 판I가 **사람이 개입하는 자리의 배치**를 다시 그린 일(파내기 단계
  신설 · 승인이 목록 단어에서 「뜻이 분명한 긍정」으로 · 승인 뒤 멈추는 자리를 **닫힌
  일곱**으로)과, 「멈추는 자리에 이송 생성이 없는 것이 왜 P4를 푼 것이 아닌가」.
...
- [문서 기관의 역사](docs-organ-history.md) — 이 프로젝트가 결정 로그·상태 문서·판 큐를
  ...
  이 위키 신설까지. 끝에 §11이 붙어 있다 — **반대 방향의 통로**(사용 현장의 교훈이
  제품으로 돌아오는 길 · D-058~D-147)와, 그 통로가 4차에서 「등재」를 넘어 수리로 간 일.
...
- [배포·설치의 변천사](distribution-install-history.md) — 완전 오프라인 설치 · 안전 훅
  ...
  결정이 왜 지금 모양이 됐는가. D-148은 Codex 배선 수리와 설치 전 배포 복사본 시험을 더했다.
...
- [engine-backlog.md](../engine-backlog.md) — 엔진 결함 대장(**정본**). 실사용 프로젝트
  제보로 선 「엔진 백로그 13」의 15항 처분과 2026-09-09 신규 등재·소유자 결정 대기를 든다.
  ⚠ **이 층에서 드물게 정본인 파일**이다 — 콜드 층이지만 원본이 불변이 아니라, 처분이
  바뀔 때 갱신된다(신설 2026-09-09 · D-147).
