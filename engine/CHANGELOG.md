...
### Fixed
- **`UpdateTable` pre-check validated the wrong version AND never blocked a bad update (HANDOFF §6 engine backlog 11-⑪ — same root class as the structure §7 / 4.13.11 fix, but with a second gap the backlog's "one-line" estimate missed).** `handleUpdateTable`'s "check new DDL before update" step calls `client.getTable().check({ tableName, ddlCode }, 'inactive')`, but the vendored `AdtTable.check(config, status)` **(a)** silently dropped `config.ddlCode` — `runTableCheckRun(conn, 'abapCheckRun', name, undefined, version)` — so the pre-check validated the object's *stored* version, not the new DDL; and **(b)** returned the raw check response **unparsed** (unlike the structure path, whose `checkStructure` parses + throws), so `AdtTable.check` always returned `errors:[]` and the handler's `checkNewCodePassed` was **always true** — even a with-source check that found errors could not block the write. Together, a real syntax error in the new DDL slipped past the pre-check and only surfaced at the write PUT, opaquely (`ExceptionResourceAlreadyExists` / *"Kein Sichern wegen Fehler in Quelle. Details erhalten Sie mit Prüfung."* — the promised details never shown). The fix is one vendored `patch-package` edit to `table/AdtTable.js` `check()`: forward `config.ddlCode` as the source to validate (check-**with-source**, so callers that pass no `ddlCode` keep the saved-version behavior — the low-level `CheckTable` `ddl_code` path is revived by the same edit), then parse the result with the shared `parseCheckRunResponse` and throw `Table check failed: <detail>` on a real error — skipping the DDIC-benign warnings (absent inactive version / "importing from database") and never a bare message with no detail, mirroring `checkStructure`. The shared `runTableCheckRun` (used by `AdtTable.update`'s own chain) is left untouched — the parse+throw lives in `AdtTable.check`, the only method the handler's pre-check calls, so no other caller's contract changes. A regression test drives the real `handleUpdateTable` over a fake connection that answers `/checkruns` differently for source vs no-source requests, pinning: valid code → the pre-check carries the new DDL as base64 source and the update completes; a real check error → the exact SAP detail is surfaced and **no** write PUT is issued; a `notProcessed` result → an honest status-carrying error (never bare). **Reverse-verified independently for both edits**: reverting the `ddlCode`-forward makes the valid-code case send a source-less check (fails); reverting the parse+throw makes the real-error case issue the write PUT (fails). Verified live on KR-DEV (S/4HANA, logon language KO).
- **Create-payload logon-language mismatch across the eight REMAINING create paths (HANDOFF §6 engine backlog 11-⑫) — mechanical extension of the 4.13.10 (§5 / 11-⑧) language infrastructure.** 4.13.10 resolved the logon language and injected it into the DDLS-view / domain / data-element creates, but deliberately left the other vendored create payloads on their hardcoded `adtcore:language="EN"` / `adtcore:masterLanguage="EN"` (those creates *succeed* on a non-EN system — SAP tolerates the EN→logon-language normalization — so no hard failure was observed). The residual defect: the description is stored in the **EN** language row and shows **empty** under a non-EN logon (KO/CS/…). Real demand surfaced this session — a fork clone's bundle had been hand-edited in **19 places** to force KO (an ad-hoc fix lost on every rebuild). The eight create handlers that route to a vendored EN-hardcoded builder — **`CreateClass`, `CreateInterface`, `CreateProgram`, `CreatePackage`, `CreateTable`, `CreateStructure`, `CreateServiceDefinition` (SRVD), `CreateMetadataExtension` (DDLX)** — now each call `resolveLogonLanguage(connection, logger)` (`src/lib/adtLogonLanguage.ts`, the same dynamic resolver introduced in 4.13.10; EN fallback when systeminformation is unavailable) and pass it as `masterLanguage` into the create config. The vendored builders/wrappers/typings accept it via `patch-package`: each `<obj>/create.js` reads `master_language` (default EN, uppercased) and emits it into both `adtcore:language` and `adtcore:masterLanguage`; each `Adt<Obj>` wrapper's `create()` forwards `master_language: config.masterLanguage`; each `<obj>/types.d.ts` adds `master_language?` (create params) + `masterLanguage?` (config). DDLX already forwarded `masterLanguage` (only its `language="EN"` was hardcoded — fixed the same way). A family regression test drives all eight real handlers over a fake connection advertising KO and pins `adtcore:language="KO"` / `masterLanguage="KO"` (not EN) on the create POST body, plus the EN-fallback case when systeminformation 404s (reverse-verified: removing the handler inject makes the POST carry EN again). Verified live on KR-DEV (logon language KO).

### Notes
- **Capability surface unchanged** (155 tools at the default `readonly,high` exposition). Both fixes are internal validation/payload changes — no tool added, removed, or renamed, and no MCP input-schema change (the `master_language` additions are vendored-client-internal typings, not tool schemas).
- **DCL / `accessControl` create deliberately excluded from 11-⑫.** Its vendored `accessControl/create.js` also hardcodes EN, but **no handler routes to it** — there is no `CreateAccessControl` / DCL create tool — so it is unreachable dead code from the engine's side (same dead-code precedent as `tabletype` / `enhancement` in 4.13.9 §3). `functionGroup/create.js` is likewise outside the 11-⑫ named scope (FUGR-create verified green in 4.13.1). Both stay on EN, no patch made.
- **Pre-existing, out of scope (report-only):** `table/create.js` derives the create-time description from `params.table_name` (ignoring the caller's description) — a separate defect from the language row; only the language attributes were touched here. `handleCreateTable` immediately follows the create with a skeleton-replacing `UpdateTable`, so the effective description still flows through the update path.
- **Low-tool parity (report-only):** because the parse+throw now lives in `AdtTable.check`, the low-level `CheckTableLow` / `CheckObjectLow` report a **hard** table check error as a thrown message string (`… Table check failed: <detail>`) rather than the previously structured `check_result.errors[]` with line/href. Warnings-only and clean checks are unchanged, and the detail is still surfaced — this mirrors the already-accepted 4.13.11 `CheckStructureLow` behavior (intended parity, flagged by the fresh-context review).
