 ...
 *
 * Environment variables (populated from .sapkit/sap.env):
...
function required(key) {
    ...
    if (!v) {
        throw new Error(`${key} is required for SAP_RFC_BACKEND=zrfc but not set in sap.env`);
    }
    return v;
}
...
function ttlMs() {
    const sec = Number(process.env.SAP_RFC_ZRFC_CSRF_TTL_SEC ?? DEFAULT_CSRF_TTL_SEC);
...
async function fetchCsrf() {
    ...
    try {
        ...
        throw new Error(`ZRFC CSRF fetch failed (GET ${url}): ${e.message}. ` +
    ...
    if (!res.ok) {
        ...
        throw new Error(`ZRFC CSRF fetch returned HTTP ${res.status} (GET ${url}): ${text.substring(0, 256)}`);
    }
    const token = res.headers.get('x-csrf-token');
    if (!token || token.toLowerCase() === 'required') {
        throw new Error(`ZRFC CSRF fetch: server did not return an X-CSRF-Token header. ` +
...
async function postEndpoint(endpoint, body) {
    ...
    try {
        ...
        throw new Error(`ZRFC endpoint call failed (POST ${url}): ${e.message}`);
    }
    finally {
        clearTimeout(timer);
    }
    if (res.status === 403) {
        ...
        const text = await res.text().catch(() => '');
        throw new Error(`ZRFC endpoint returned HTTP 403 without CSRF refresh signal: ${text.substring(0, 256)}`);
    }
    if (!res.ok) {
        const text = await res.text().catch(() => '');
        throw new Error(`ZRFC endpoint returned HTTP ${res.status} (POST ${url}): ${text.substring(0, 256)}`);
    }
    try {
        return await res.json();
    }
    catch (e) {
        throw new Error(`ZRFC endpoint returned non-JSON body (POST ${url}): ${e.message}`);
    }
}
/**
...
//# sourceMappingURL=zrfcProxy.js.map