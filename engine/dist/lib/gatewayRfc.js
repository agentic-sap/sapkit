...
function required(key) {
    ...
    if (!v) {
        throw new Error(`${key} is required for SAP_RFC_BACKEND=gateway but not set in sap.env`);
    }
    return v;
}
...
function buildSapHeaders() {
    ...
    if (process.env.SAP_CLIENT)
        headers['X-SAP-Client'] = process.env.SAP_CLIENT;
...
async function postJson(path, body) {
    ...
    try {
        ...
        const msg = e.message;
        throw new Error(`RFC Gateway request failed (POST ${url}): ${msg}. ` +
    ...
    if (!res.ok) {
        ...
        throw new Error(`RFC Gateway returned HTTP ${res.status} (POST ${url}): ${text.substring(0, 256)}`);
    }
    try {
        return await res.json();
    }
    catch (e) {
        throw new Error(`RFC Gateway returned non-JSON response (POST ${url}): ${e.message}`);
    }
}
/**
...
//# sourceMappingURL=gatewayRfc.js.map