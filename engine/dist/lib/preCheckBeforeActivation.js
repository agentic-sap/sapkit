...
exports.downgradeReportMissingNoise = downgradeReportMissingNoise;
            ...
            case 'program': {
                ...
                try {
                    ...
                    if (isReportMissingNoiseText(checkErr?.message ?? String(checkErr))) {
                        logger?.debug?.(`program '${name}': no inactive version (REPORT-missing noise) → active source check`);
                        return await runActiveProgramSourceCheck(connection, client, name, logger);
                    }
                    throw checkErr;
                }
                break;
 ...
 */
async function runActiveProgramSourceCheck(connection, client, name, logger) {
    const programUri = `/sap/bc/adt/programs/programs/${(0, utils_1.encodeSapObjectName)(name).toLowerCase()}`;
    let activeSource = '';
    try {
        ...
        logger?.warn?.(`runActiveProgramSourceCheck: could not read active source for '${name}': ${readErr?.message || readErr}`);
    }
    if (!activeSource.trim()) {
        logger?.debug?.(`program '${name}': no inactive version and active source unavailable → treating REPORT-missing noise as clean`);
        return { ...EMPTY_RESULT };
    }
    const result = await runInlineArtifactCheck(connection, programUri, `${programUri}/source/main`, activeSource, logger);
 ...
 */
function downgradeReportMissingNoise(result) {
