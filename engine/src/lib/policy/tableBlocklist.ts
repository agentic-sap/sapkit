 ...
 */
const SQL_NON_TABLE_WORDS: ReadonlySet<string> = new Set([
  'SELECT',
  'FROM',
  'WHERE',
  'JOIN',
    ...
    for (;;) {
      ...
      if (nextTok) {
        ...
        if (nextUpper === 'AS') {
          ...
          if (aliasTok) look += aliasTok[0].length;
          idx = look;
