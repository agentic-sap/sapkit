...
import { callDdicBadi } from '../../../lib/rfcBackend';
import {
  type AxiosResponse,
  return_error,
  return_response,
  ...
  inputSchema: {
    ...
    properties: {
      badi_definition: {
        type: 'string',
        description:
          'BAdI definition name (e.g., ME_PROCESS_PO_CUST). Will be uppercased.',
      },
      customer_only: {
        type: 'boolean',
        description:
...
export async function handleGetBadiImplementations(
  ...
  try {
    if (!args?.badi_definition) {
      return return_error(new Error('badi_definition is required'));
    }
    ...
    logger?.info(
      `GetBadiImplementations: badi=${badi_definition}, customer_only=${customer_only}, active_only=${active_only}, include_methods=${include_methods}`,
    );

    const fmResult = await callDdicBadi(connection, {
      badi_definition,
      customer_only,
      active_only,
      include_methods,
    ...
    if (fmResult.subrc !== 0) {
      return return_error(
        new Error(
          `ZMCP_ADT_DDIC_BADI returned subrc=${fmResult.subrc}: ${fmResult.message}`,
        ),
      );
    }

    const parsed = (fmResult.result ?? {}) as {
      badi_definition?: string;
      kind?: string;
      interface?: string;
      multi_use?: boolean;
