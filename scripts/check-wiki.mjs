...
//     ③(D-번호)은 **인용 구간 안까지 전수**로 본다(예외를 두면 그것이 회피 통로가
//     된다). 반면 ②가 「가리켜졌다」로 인정하는 집합은 **인용 구간 3종 밖**만이다 —
//     ⓐ 코드펜스 · ⓑ HTML 주석 · ⓒ **인라인 코드 스팬**(`` `[x](../Y.md)` ``).
//     예시로 적힌 링크나 주석 처리된 링크가 **발견성을 대신 채워 주면 안 되기**
//     때문이다(리뷰 실측: 펜스 안 예시 링크 하나로 고아가 가려졌다). ⓒ를 함께 막는
//     이유는 **한 줄 예시가 펜스보다 인라인 스팬으로 쓰이는 쪽이 흔해서**다 — 형제
//     둘만 막으면 같은 구멍 계열에서 가장 밟힐 자리가 남는다. 좁아지는 방향이
//     「발견성의 정의를 엄격하게」이므로 spec §1.2 ②의 취지에 부합한다.
//     ⚠ 스팬은 **줄 안에서만** 본다(여러 줄에 걸친 스팬은 마스킹되지 않는다 — 이
//     레포의 위키에 그 형태가 없고, 있으면 ②가 느슨해지는 쪽이라 안전 방향이다).
...
/**
 * 인라인 코드 스팬(`` `…` ``)을 지운다 — 여는 백틱 N개는 **같은 개수**의 백틱으로
 * 닫힌다(CommonMark). 닫히지 않은 백틱 런은 문자 그대로 남긴다(줄 나머지를 삼키지
 * 않게). 줄 안에서만 작동하므로 줄 수는 변하지 않는다.
 */
function maskCodeSpans(line) {
  ...
  while (i < line.length) {
    ...
    if (close < 0) {
      out += line.slice(i, i + n); // 닫히지 않았다 — 스팬이 아니다
      i += n;
      continue;
    }
    i = close + n; // 스팬 전체(구분자 포함)를 버린다
  }
  return out;
}

/**
 * ②의 「가리켜졌다」 집합을 만들 때 쓰는 마스킹 — **인용 구간 3종**(코드펜스 ·
 * HTML 주석 · 인라인 코드 스팬) 안을 비운다. 줄 수는 보존한다.
 * **①·③은 이걸 쓰지 않는다**(의도된 비대칭 — 위 「측정 정의」).
 */
function maskForDiscovery(lines) {
  ...
  for (const line of lines) {
    ...
    out.push(maskCodeSpans(res)); // ⓒ 인라인 스팬까지 비운다
  }
  return out;
}
...
if (scanWiki) {
  for (const abs of wikiFiles) {
    ...
    // ②용 「가리켜졌다」 집합 — **인용 구간 3종**(펜스·HTML 주석·인라인 스팬) 밖만.
    // 예시·주석 링크가 발견성을 대신 채워 주면 안 된다(위 「측정 정의」의 비대칭).
...
const LABELS = {
  ...
  2: ['② 고아 검출', () => (scanWiki ? `분모 ${stats.denom}개 (아카이브 2종 내부·INDEX 자신 제외) · 발견 링크 ${stats.discovered}개(인용 구간 밖)` : '건너뜀 (위키 구역 부재)')],
