FUNCTION zsapkit_adt_ddic_dtel
  IMPORTING
...
* ZSAPKIT_ADT_DDIC_DTEL - data element maintenance bridge for ECC.
*
* Same shape as the table and domain bridges of this group: ECC has no
* ADT REST endpoint for DDIC writes, so create, update and delete run
* through this RFC-enabled wrapper over DDIF_DTEL_PUT and
* RS_DD_DELETE_OBJ. Not installed on S/4HANA.
*
* Import
...
*
* Payload shape
*   { "dd04v": { data element fields } }
*   Unlike the table bridge there is no dependent row table; ROLLNAME is
*   stamped from iv_name so header and object name cannot drift apart.
*
* Write sequence
*   1 RS_CORR_INSERT               claim TADIR under the package
*   2 DDIF_DTEL_PUT                store the inactive version
...
*   caller finishes with ZSAPKIT_ADT_DDIC_ACTIVATE, type DTEL.

  TYPES: BEGIN OF ty_dtel_payload,
           dd04v TYPE dd04v,
         END OF ty_dtel_payload.

  DATA ls_payload       TYPE ty_dtel_payload.
  DATA ls_header        TYPE dd04v.
      ...
      CASE iv_action.
        ...
        WHEN 'CREATE' OR 'UPDATE'.
          ...
          IF lv_transportable = abap_true.
            PERFORM tadir_claim_dtel USING lv_obj iv_devclass iv_transport
          ...
          ls_header          = ls_payload-dd04v.
          ls_header-rollname = lv_obj.
          ...
          IF ev_subrc <> 0.
            ev_message = |DDIF_DTEL_PUT refused { lv_obj }, sy-subrc={ ev_subrc }|.
            RETURN.
          ENDIF.

          IF lv_transportable = abap_true.
            IF iv_transport IS NOT INITIAL.
              PERFORM request_attach_dtel USING lv_obj iv_devclass iv_transport
                                       CHANGING ev_subrc ev_message.
              IF ev_subrc <> 0.
                RETURN.
              ENDIF.
            ENDIF.
            PERFORM browser_refresh_dtel USING iv_devclass.
          ENDIF.

          ev_subrc   = 0.
          ev_message = |Data element { lv_obj } written in the inactive version. | &&
                       |Finish with ZSAPKIT_ADT_DDIC_ACTIVATE type DTEL.|.
        ...
        WHEN 'DELETE'.
          ...
          IF ev_subrc <> 0.
            ev_message = |RS_DD_DELETE_OBJ could not drop DTEL { lv_obj }, sy-subrc={ ev_subrc }|.
            RETURN.
          ENDIF.
          ...
          IF lv_transportable = abap_true.
            PERFORM browser_refresh_dtel USING iv_devclass.
          ENDIF.
          ev_message = |Data element { lv_obj } deleted|.
...
*----------------------------------------------------------------------*
* tadir_claim_dtel - reserve the TADIR entry so the object lands in the
...
*----------------------------------------------------------------------*
* request_attach_dtel - put the object on the workbench request. Runs
* after the PUT, so a rejected write never leaves an entry behind.
*----------------------------------------------------------------------*
FORM request_attach_dtel
  USING    iv_obj       TYPE ddobjname
           iv_devclass  TYPE string
           iv_transport TYPE string
  CHANGING cv_subrc     TYPE i
  ...
  TRY.
      ...
      cv_subrc    = 0.
      cv_message  = |DTEL { iv_obj } to { iv_transport }, | &&
...
*----------------------------------------------------------------------*
* browser_refresh_dtel - rebuild the SE80 object list for the package so
...
*----------------------------------------------------------------------*
FORM browser_refresh_dtel USING iv_devclass TYPE string.
