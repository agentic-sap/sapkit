...
Same root, different entry point: **`ReloadProfile` answering `restartRequired: true` means the published tool list is still the one decided at launch**, whatever the reloaded profile now says. Attach an on-prem profile to a server whose tool list was fixed at launch under a different profile (inspection-only, or a cloud axis) and the connection and tier come from the new profile while `UpdateClass`, `ActivateObjects` and the other write tools are simply **not in the tool list** — they cannot be called, and no later response explains why (field-verified 2026-09-09). Reconnect the MCP server (`/mcp` on the Claude adapter) instead of retrying the call; absent write tools here are not a permission or tier problem.
...
Responses that look like a failure, a block, or a truncation but are none of those — plus a few that look like success and are not, and one absence that reads as a limit. Each one below was actually mis-read in project work (measured 2026-07-28 → 2026-09-04 across two S/4 systems), and the wrong reading was the expensive part — not the tool.
...
### `UpdateSourceByPatch` — write-side traps (serial loss · CRLF · main source only · FUNC)

Three independent behaviors, each returning success while doing less than the response implies. ⓐ **Serial patches with `activate: false` lose the earlier ones** — the patch is written to the *inactive* version, but each next call fetches the *active* source to apply onto, so with activation deferred only the last patch survives (responses stay `success` + `occurrences_replaced: 1` + a plausible diff; field-verified on two systems, CLAS 3-in-a-row → 1 survivor). Tell: the `diff_preview` context of patch N does not show patch N−1's change. Safe pattern: `activate: true` on **every** patch plus a `GrepObjects` read-back of the changed token — with that, even serial CLAS patching is safe (6-in-a-row verified 2026-08-05). ⓑ **Multi-line `old_string` never matches** — server source is CRLF, an LF-joined pattern fails with `old_string not found` even though the text is visibly present. Use single-line anchors only. ⓒ **CLAS patches reach the main source only** — test includes (CCAU) and local types are out of range; use `UpdateLocalTestClass` / `UpdateLocalTypes` (full-text) for those, and see the CCIMP entry above for reading them. ⓓ **`object_type: "FUNC"` is refused outright** in the current bundle, with an explicit message that the write it delegates to is not implemented and that nothing was read from or written to SAP (field-verified 2026-08-31) — a clean refusal, not a false success. Edit function modules as `GetFunctionModule` → change locally → `UpdateFunctionModule` (full source); FM sources are usually short enough that a full write costs little.

**Choosing the anchor is where most of the remaining failures live.** In the current bundle `old_string` matches as a plain substring — not a whole line, not a word, and with no option to ask for either — so `FORM do_show.` also matches inside `PERFORM do_show.` and is refused as two locations (2026-08-04); a two-space-indented statement matches inside its four-space-indented twin (2026-08-12); and in a program that keeps commented-out copies of live lines (`*****      LOOP AT …`), every live line in that region is a substring of its own comment copy, so no single-line anchor is unique there at all (2026-07-31). Measure instead of guessing: `GrepObjects` the intended anchor first and count the matches. Where a bare statement is ambiguous, anchor on something unique beside it — a neighbouring comment line — or start the anchor with a newline (`"\nFORM do_show."`), which excludes the `PERFORM` case because CRLF contains LF. Common lines (`ELSE.`, `ENDIF.`) are never anchors. Related: when `GetInclude` spills to a file because the source exceeds the token limit, the line numbers in that file are the server's own line numbers and can be used directly for this work (field-verified 2026-07-31).

Deleting is where the CRLF limit bites, again as the current bundle behaves. A multi-line `old_string` cannot be expressed at all, so **lines cannot be removed as a block**, while insertion works fine because `new_string` may be multi-line. Two consequences worth knowing before reaching for a full-source write. Writing `\r\n` **as characters** in `new_string` yields one long line and the write is refused with `The line N exceeds 255 characters` (HTTP 400) — send real newlines. And where exactly one line has to go, a `new_string` of a single space blanks it and passes (2026-08-12, four sites). Escalate to a full-source `UpdateInclude` / `UpdateClass` when several lines go or the structure changes.
...
A run can answer `success: true`, `activated: false`, `checked: false`, `generated: true`, `failed_count: 0` while every entry in `objects[]` reports `status: "activated"` with an empty `errors[]` — and **nothing has been activated**. The two run-level flags are the signal; the per-object status is not. A genuine activation reports `activated: true` **and** `checked: true` (field-verified 2026-09-04, against successful runs in the same session). `checked: false` says the syntax-check stage never ran, so "no errors" there means nothing was examined rather than nothing was wrong.
...
**A second false negative from the same tool, different mechanism.** `CheckSyntax` on an include **alone** — no `source_code`, no main program in play — can answer `success: false` with `errors: []` and `warnings: []`, twice in a row, while the very next `ActivateObjects` compiles the same source with zero errors (field-verified 2026-07-31). An include has no main-program context of its own, so this reads as "cannot judge" expressed as failure, and the empty error list makes it indistinguishable from a real one. Treat only `success: true` with `errors: []` as a pass; `success: false` with `errors: []` is not a result. Where a check and an activation disagree, the activation is the stronger evidence — but read the fixed-point-arithmetic entry below before concluding that a write is blocked.
