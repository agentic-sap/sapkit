...
- **RAP / OData artifacts**: Read [rap-odata-rules](../knowledge/abap/conventions/rap-odata-rules.md) before creating or editing any Behavior Definition / Behavior Implementation / Service Definition / Service Binding / Metadata Extension (DDLX) or any OData-exposed CDS view. It lists the field-verified **silent** failure modes — projection `use etag` non-inheritance, DDLX facet scope, conversion-exit fields, backend cache masking `$metadata`, `/srvd/` vs `/srvd_a2x/`, on-premise publish paths — that activation, syntax check, and ATC all pass through without warning.
...
2. **IMPLEMENT** — write initial implementation via the matching `Update*` MCP tool; if `vsp` is installed, run the 13-rule offline analysis (`AnalyzeABAPCode` via `vsp --offline`) on the source first — optional, not a gate; on the Claude adapter the `offline-code-analysis` hook runs it automatically after each source write ([troubleshooting §7](troubleshooting.md#7-sapkit-verifier--local-verification-optional)). For Class / Program / Interface, `CheckSyntax` with `source_code` compiles the proposed source server-side **without writing anything** — a cheap pre-write check that spares an activation round-trip (those three object types only; include/FM variants check the server's inactive version instead, field-verified 2026-07):
