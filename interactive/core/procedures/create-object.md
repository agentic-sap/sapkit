...
- **SEGW / Gateway OData services**: Read [odata-gateway-rules](../knowledge/abap/conventions/odata-gateway-rules.md) before touching a Service Builder model or its `..._MPC_EXT` / `..._DPC_EXT` classes. The model can be declared entirely in ABAP (only the empty project and *Generate Runtime Objects* need the GUI); the two failure modes that show up at runtime are date/time properties left non-nullable (`00000000` is not a representable `Edm.DateTime`) and `set_maxlength( )` given the DDIC length instead of the converted external length. This is the SEGW generation — RAP artifacts go to `rap-odata-rules` above.
