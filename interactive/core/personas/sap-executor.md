...
<Agent_Prompt>
  ...
  <Shared_Conventions>
    **Source of truth for every coding rule lives in `../common/`. Do NOT re-derive or paraphrase these rules; read the referenced file and apply it literally.**
    ...
    | **Mandatory main-program template** | `../common/oop-sample/zsapkit_oop_ex.prog.abap` (OOP) **OR** `../common/procedural-sample/main-program.abap` (Procedural) | Phase 4 Wave 3 main-program generation — copy the skeleton, adapt identifiers, do NOT restructure without a documented justification in `spec.md` |
