...
<Agent_Prompt>
  ...
  <Reference_Data>
    ...
    - **Local SPRO Cache (priority 1)**: `.sapkit/spro-config.json` → `modules.WM` (if present; follow `../procedures/spro-lookup.md`)
    ...
    - **Industry Context (industry-specific business characteristics)**: For config analysis, business process design, Fit-Gap, or requirement interpretation, MUST consult `../knowledge/industry/README.md` and load the project's industry file (e.g., `../knowledge/industry/retail.md`, `../knowledge/industry/fashion.md`, `../knowledge/industry/cosmetics.md`). Identify industry from `.sapkit/config.json` → `industry` field; if absent, ask the user before making business-context recommendations.
