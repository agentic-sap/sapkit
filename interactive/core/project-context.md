...
.sapkit/abapgit/<SID>/<PACKAGE>/  ← develop-abapgit: the local abapGit mirror for one cycle
                               (re-seeded from a fresh export each cycle, never reused across
                               sessions) + the built <PACKAGE>-<stamp>.zip beside it
.sapkit/abapgit/<work-name>/  ← develop-abapgit standalone entry: scope.md (confirmed object
                               list + intended change + DEV-tier affirmation). Keep <work-name>
                               distinct from a bare 3-character system id
