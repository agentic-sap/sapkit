...
- **Writing a group include goes through the group, not the include.** A function-group include (`L<fugr>TOP`, `L<fugr>F01`, …) is not a standalone include: it is reachable for reading at the standalone address but not for locking. *On older bundles* `UpdateInclude` and `UpdateSourceByPatch(INCL)` locked it at `/programs/includes/<name>` and died at the lock step with 403 *"This syntax cannot be used for an object name"* (field-verified 2026-07-30). Since the D-147 engine repair (server engine 1.4.0 / plugin 0.10.3) both derive the group from the include name (`L<group>` + `TOP` / `UXX` / a two-digit suffix such as `F01`) and lock, write, and activate at the group's include address, reporting `type: "FUGR/I"` and `function_group` in the response. Offline-verified only — confirm on first live use. A group in a `/NS/` namespace falls outside the name rule and still takes the standalone address, so the 403 stands there; and if the derivation mis-fires on a genuinely standalone `L*` include, the lock failure says which group it derived, which is the tell.
