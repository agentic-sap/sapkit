...
Every OOP-mode ABAP program in sc4sap shares the convention set out below. Its canonical reference is [`common/oop-sample/zsapkit_oop_ex*`](../templates/oop-sample/), derived from `babamba2/OOALV` / `YRPAEK001`.
...
- **What the reference set holds**: `zsapkit_oop_ex.prog.abap`, its includes `*a` (ALV class) `*c` (DATA class) `*e` (event handler) `*f` (forms) `*i` (PAI) `*o` (PBO) `*s` (selection) `*t` (TOP) `*tst` (test classes), and screens `0100`/`0200`.
