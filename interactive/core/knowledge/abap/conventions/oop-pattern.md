...
- **What the reference set holds**: `zsapkit_oop_ex.prog.abap`, its includes `*a` (ALV class) `*c` (DATA class) `*e` (event handler) `*f` (forms) `*i` (PAI) `*o` (PBO) `*s` (selection) `*t` (TOP) `*_tst` (test classes), and screens `0100`/`0200`.
- **Reuse before writing**: the sample leans on the reusable handlers kept in [`abap/alv-oop-handlers/`](../../../../server/sap-assets/alv-oop-handlers/) — `ZCL_SAPKIT_CM_ALV`, `ZCL_SAPKIT_CM_OALV`, `ZCL_SAPKIT_CM_OTREE`, `ZCL_SAPKIT_CM_ALV_EVENT`, `ZCL_SAPKIT_CM_TREE_EVENT`, `ZIF_SAPKIT_CM`, `ZCX_SAPKIT_EXCP`. In a generated program these should be instantiated or extended, not duplicated.
- **Message class**: the standard message class to reach for is `S_UNIFIED_CON` (`013 No data found`, `000 &1 &2 &3 &4`). A custom `ZMC` class is never to be created — both `ZCX_SAPKIT_EXCP` and the sample are already pointed at `S_UNIFIED_CON`.
- **If anything is unclear**: take over the sample's include split (`a/c/e/f/i/o/s/t/_tst`), its event handler wiring, its container creation, the shape of its field catalog builder, and its PAI/PBO module names. A layout that departs from this breaks what the create-program procedure expects and what the OOP reviewer checks look for.
