# Cloud ABAP Constraints — S/4HANA Cloud Public
...
The rest of the file follows the order a request actually travels: §2 is the blocked
surface, §3 turns a blocked request into a redirect decision, §4 gives the program shape
that replaces the classic one, and §5 supplies the API-level substitutions needed while
writing it.

## 2. What Cloud Public takes away

Everything in the middle column is prohibited. Where the platform offers a named
successor for the whole category, it is in the last column.
...
## 4. The program shape that survives

- **ABAP freestyle reports are not allowed** — the event blocks they are built from are
  themselves prohibited.
- Carry the logic in a **class method or a function module** instead.
- To make a class directly runnable, implement the `if_oo_adt_classrun` interface:
...
## 5. Replacement APIs — grouped by what you were trying to do

### 5.1 Language and runtime primitives
...
### 5.2 Dates, calendars and value conversion
...
### 5.5 Runtime services — locking, numbering, scheduling
...
### 5.7 Authorization, translation and lifecycle
