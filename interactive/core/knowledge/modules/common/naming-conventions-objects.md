...
Naming patterns broken out by ABAP object type. This page is the companion to [naming-conventions.md](naming-conventions.md) — turn to that parent for General Rules, Module Codes, Code-Level Naming, Special Prefixes, Validation Rules, and the Recommended Approach.
...
**Authoritative pattern (company standard — confirmed 2026-04-19). `Z` runs straight into the 2-letter module with no underscore, and the `FG` / `FM` tag is what divides module from purpose.**
...
- `Z_{MODULE}_{NAME}` — a `Z_MM_...` style opening is no longer current.
- `ZFG_{MODULE}_...` / `ZFM_{MODULE}_...` — carrying the tag ahead of the module as a prefix departs from the company standard.
- Letting an FM and its FG carry different module codes (an FM takes the same module as the parent FG holding it).

For an FM placed under an FG that already exists, its purpose string should stay brief and separate from the FG purpose — e.g., FG `ZMMFG_HISTORY` + FM `ZMMFM_GET_HISTORY` / `ZMMFM_POST_HISTORY`.
...
| Type | Pattern | Example |
|------|---------|---------|
| Dynpro Screen | 4-digit number within 0100-9999; main takes `0100` | `0100`, `0200`, `9000` |
| GUI Status | All-caps identifier | `STATUS_0100`, `MAIN_STATUS` |
| GUI Title | All-caps identifier | `TITLE_0100` |
| Selection Screen | Lives within the program | `AT SELECTION-SCREEN` |
...
| VOFM Routine | 3-digit; custom occupies 600-999 | `901`, `905` (application form routines) |

### Configuration / 설정

| Type | Pattern | Example |
|------|---------|---------|
| Package (Local) | `$TMP` | Where local/temporary objects go |
| Package (Custom) | `Z{MODULE}_{NAME}` | `ZSD_ORDER_MGMT` |
| Transport Request | SAP generates it | `ABCK900123` |
