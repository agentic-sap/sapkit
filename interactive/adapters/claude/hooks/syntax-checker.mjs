 ...
 * automatically suggests running CheckSyntax to diagnose
  ...
  try {
    ...
    if (isAbapSyntaxError(error)) {
      guidance = `[SC4SAP SYNTAX CHECK] ABAP error detected${objectLabel}. ` +
        `Run CheckSyntax (server-side ADT check) to get detailed syntax diagnostics. ` +
      ...
      guidance = `[SC4SAP ERROR] MCP ABAP tool "${toolName}" failed${objectLabel}. ` +
        `Consider running CheckSyntax to check for underlying syntax issues. ` +
