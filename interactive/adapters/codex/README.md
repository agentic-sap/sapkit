...
- `node interactive/scripts/doctor.mjs`가 이 충돌(전역 등록 + 번들 공존)을 감지해
  보고한다. 단 **command 없는 오버라이드 전용 항목**(아래 "실데이터 2종 하드 차단"
  절의 `disabled_tools` 레시피)은 제거 대상이 아니다 — doctor도 이 경우를 INFO로
  구분해 보고한다.
- 제거는 **사용자 확인 후에만** — `codex mcp remove sap`를 실행하기 전에 반드시
  사용자에게 물을 것(자동 제거 금지). 그 항목에 `disabled_tools` 실데이터 하드차단이
  있다면 **제거 전에 대체 차단을 먼저** 마련한다.
...
아래 검증 명령으로 항상 실제 적용 여부를 확인할 것. 이 항목은 위 "기존 사용자
마이그레이션" 절의 전역 그림자와 모양이 비슷하지만 **command가 없으므로 제거 대상이
아니다** — doctor ⑦도 command 유무로 두 경우를 구분한다.)
...
## 질문 형식 — 선택 창 (`request_user_input`)

말투 정본은 [`core/policies/plain-language.md`](../../core/policies/plain-language.md)다. 그
정책은 **호스트가 선택 창을 주면 선택 창, 없으면 같은 내용을 번호 붙은 글로** 묻게 한다.
Codex CLI에서는 `request_user_input`이 그 자리다 — 한 창에 질문 ≤4, 질문당 선택지 ≤4(열린
칸 포함), 추천안을 첫 번째에 두고 「(추천)」을 붙인다. 후보가 4를 넘는 칸은 **추천 3 + 열린
칸으로 접고** 나머지 후보는 질문 본문에 한 줄로 나열한다(자르지 않는다).

⚠ **주의점 둘.**

1. **핀된 버전·모드에서의 동작은 미실측이다.** 그 도구의 존재는 공개 자료로 확인했으나, 이
   어댑터가 핀한 Codex 버전(`compatibility.json`이 정본)과 실행 모드에서 실제로 노출되는지는
   재 보지 않았다. **없으면 글로 간다** — 선택 창 호출이 실패하면 즉시 같은 내용을 번호 붙은
   글로 다시 묻는다. 정책이 폴백을 의무로 두므로 도구가 없어도 대화는 막히지 않는다.
2. `codex exec`처럼 사람이 없는 실행 모드에서는 선택 창 자체가 의미가 없다. 그 모드는
   인터뷰·승인이 있는 절차(create-program·deep-interview·create-object)의 실행 모드가
   아니다 — 그 절차들은 **사람이 곁에 있는 대화**를 전제한다.

## 권한 안내 — sapkit이 병합할 허용 목록이 없다

Claude Code 어댑터에는 설계 승인 직후 「한 번에 허용」을 제안하는 **부분 목록**이 있다. **이
어댑터에는 그 기제가 없다** — Codex의 도구별 승인 모드는 **사용자 config 전용**이라 플러그인이
선언·배포할 수 없다(위 「안전 모델 주의」의 근거와 같다). 그래서 이 호스트에서 절차는 질문이
아니라 **한 줄 안내**를 낸다:

> 이 하네스에서는 도구 호출마다 허용 창이 뜹니다 — 그 설정은 호스트 쪽 사용자 설정이고
> sapkit이 바꾸지 않습니다. 실데이터 조회 2종은 어디서든 매번 묻습니다.

실데이터 2종은 여기서 **`disabled_tools` 하드 차단**으로 아예 막혀 있다(위 절 — 필수). 즉
「매번 묻는다」보다 강하다.
