# interactive (구 sc4sap-lite) — 3사 하네스 공통 라이트 버전 설계서

> **상태 정본은 레포 루트 `HANDOFF.md` — 본 문서는 설계 정본(구현 상태는 여기 기록하지 않음).**
>
> 이 문서는 컨텍스트 클리어 후 어떤 세션이든 이 문서만 읽고 구현을 시작할 수 있도록 작성된
> 단일 소스다. **정본: 본 파일** (`sapkit/interactive/DESIGN.md`) — L0에서 스펙 문서로부터
> 이동, 이후 별도 레포(sc4sap-lite)를 거쳐 2026-07-10 본 레포에 subtree 병합됐다.

## 0. 한 문장 요약

`sc4sap-lite`는 sc4sap-custom(Claude Code 전용 SAP 플러그인)의 전 자산(모듈 지식 14+BC,
업종 14, 국가 16, ABAP 규약·템플릿, 페르소나 26, 절차 노하우, MCP 서버)을 **하네스 중립
코어 + 얇은 어댑터 3벌(Claude Code / Codex / Antigravity)** 구조로 재편성하되, 멀티에이전트
자동 디스패치·team 협업이라는 무게를 버리고 "1명 작업 + 1명 새-컨텍스트 리뷰 + SAP 기계
검증" 품질 모델로 단순화한 신규 레포다.

## 1. 배경과 확정 결정

### 1-1. 발단 (사용자 요구, 2026-07-10)

- sc4sap-custom은 Claude 전용이라 Codex 버전 변환 비용이 크다 → **Codex와 Antigravity에서도
  사용 가능하게** 발전시키고 싶다.
- 그대로 옮기는 게 아니라 **"다 옮기되 가벼운 버전"**으로 재구성한다.
- sc4sap의 모듈 에이전트·업종별·ABAP 자산을 (sap-agentic-harness packs에서도) 가져올 수
  있는 구조로 설계한다.

### 1-2. 대화에서 확정된 결정 (재논의 불필요)

1. **3사 통일**: Claude Code도 라이트 버전으로 갈아탄다. 기존 sc4sap-custom은 동결
   보존(삭제 안 함, 지식 수정 금지).
2. **전 자산 이식**: 지식 100% + 페르소나 26개 + 절차 노하우 전부. 버리는 것은
   **멀티에이전트 자동 디스패치와 team 협업 두 가지뿐** — 나머지는 형태 변환.
3. **품질 모델**: 1명 작업 + 1명 새-컨텍스트 리뷰 + SAP 기계 검증(CheckSyntax→활성화→
   ABAP Unit→ATC). 리뷰어는 read-only 판정만, 수정은 작업자가(독립성 보존).
4. **안전훅 유지**: 사전 차단 훅(안전 3종)은 Claude 어댑터에 옵션으로 유지. (2026-08-02
   설계 v2로 현행화 — 훅 6종은 기본 미설치·스위치 보존으로 강등되고 안전의 정본은 서버
   번들 자체 게이트다. 상세: `docs/reference/designs/2026-08-02-claude-onboarding-codex-parity-no-engine.md`
   §7, `adapters/claude/hooks/README.md`.)
5. **접근 방식 A 채택**: 신규 레포 sc4sap-lite (B: 인플레이스 개조, C: sap-agentic-harness
   흡수는 기각 — B는 동작 중 플러그인 수술+동결 결정과 충돌, C는 DESIGN.md §2 역할 분리 위반).

### 1-3. 선행 설계와의 관계 (중요)

`sc4sap-custom/private/lightweight-redesign-design.md`(2026-07-06, 구현 상당 부분 완료)의
성과를 **그대로 승계**한다 — 재작업 없음:

- **엔진 번들화 완료**: `engine/server.bundle.cjs`(8.25MB) + npm 채널
  `@hjaewon/abap-mcp-adt-powerup@4.9.x`. vendor 771MB 공정은 이미 제거됨.
  (⚠ 이 줄은 **2026-07 시점의 승계 기록**이다. 번들의 소스는 2026-08-19 판7-b에서
  자체 저작 `sapkit-engine`으로 교체됐다 — 아래 §1-4와 `docs/BLUEPRINT.md` ⑴ 참조.
  번들화라는 **형태**는 그대로 승계됐고, 바뀐 것은 그 안의 소스다.)
- **훅 다이어트 완료**: 편의성 훅 15종은 `SC4SAP_LEAN` 기본 오프. 감사 결과 주입기들은
  run.cjs stdin 버그로 애초에 동작한 적이 없었음이 실측됨.
- **안전 훅 경계 확정**: 안전 3종(block-forbidden-tables, tier-readonly-guard,
  prefer-sqlquery-explicit-fields)은 hooks.json이 아닌 install-hooks.mjs → 프로젝트
  settings 주입 방식으로 분리되어 있고, 서버 자체에 L4 가드(blocklist·SAP_TIER) 내장.
- **지식 성장 규약**(data-only 마크다운 + 인덱스 1줄 + 트리거 로드, 상시 로드 진입 금지)을
  core/knowledge의 로딩 규약으로 채택.

충돌 정리: 선행 설계의 "기능 손실 0" 원칙과 본 설계의 "파이프라인 폐기"는 목적이 다르다 —
선행 설계는 "Claude 안에서의 무게" 문제, 본 설계는 "3사 이식성" 문제이며, **본 설계가
후속 결정으로 우선한다.**

### 1-4. 레포 좌표

| 역할 | 위치 (2026-07 기준) |
|---|---|
| 이식 원천 (동결 예정) | `D:\claude for SAP\sc4sap-custom` — `hjaewon/sc4sap-custom` (MIT, 업스트림 babamba2/superclaude-for-sap) |
| 본 설계 산출물 | `sapkit/interactive/` — L0에서 sc4sap-lite 레포로 생성 후 2026-07-10 subtree 병합·개명 (플러그인명 `sapkit` — D-041 개명 전 `sap-agentic-harness`) |
| **MCP 엔진 소스 정본** | **`sapkit-engine/`** (자체 저작 · 2026-08-19 판7-b · D-095). 번들러 `sapkit-engine/tools/bundle.mjs` → `interactive/server/server.bundle.cjs`. npm 발행 없음 |
| ~~MCP 엔진 업스트림~~ | ~~`hjaewon/abap-mcp-adt-powerup`~~ — 교체로 배포물에서 빠졌다. 포크 소스 `engine/`도 **판7.5(2026-08-22)에 은퇴했다** |
| 무인 하네스 트랙 (별개) | `D:\claude for SAP\sap-agentic-harness` — DESIGN.md v2.1. packs가 lite core를 지식 원천으로 사용 (§5-3) |
| 선행 설계 문서 | `sc4sap-custom/private/lightweight-redesign-design.md` (비공개 유지) |

## 2. 무엇을 버리고 무엇이 남는가

| 자산 | 처리 |
|---|---|
| 지식(모듈·업종·국가·ABAP) | 100% 유지 — 무변환 복사(경로·링크만 갱신) |
| MCP 서버(SAP 실행 능력 ~150 도구) | 100% 유지 — server/로 이관, 3사 공통 등록 |
| 페르소나 26개 | 유지 — 서브에이전트에서 "호출 가능한 중립 문서"로 형태 변환 |
| 스킬 17개 중 10개 | 절차 문서로 변환 (관문·순서 유지, 자동 교대만 제거) |
| 스킬 중 sap-abap | 원래 순수 지식 — `knowledge/abap/reference/`로 편입. **2026-08-09 전량 삭제**(GPL-3.0 편집 저작물 사본 — 고지는 「제거로 해소」로 전환) |
| 스킬 중 setup·mcp-setup·sap-doctor·sap-option | 설치 스크립트 + doctor + troubleshooting 문서로 변환 |
| **스킬 중 team·trust-session** | **명시 폐기** (team=Claude 전용 개념, trust-session=정적 권한 템플릿으로 대체) |
| hooks.json의 편의성 훅 15종 | 폐기 (선행 설계에서 무동작 실증) |
| 안전 훅 3종 | 유지 — Claude 어댑터에 승계 (2026-08-02 v2: 기본 미설치·선택 스위치로 강등, 정본은 서버 게이트 — §4-3·`adapters/claude/hooks/README.md`) |
| 멀티에이전트 파이프라인(8역할 자동 교대) | 폐기 — "1+1+기계검증" 모델로 대체 |

파이프라인 폐기로 진짜 잃는 것: 역할별 모델 라우팅(비용 최적화), 병렬 속도, 대형 작업의
컨텍스트 분리. 품질의 본질(교차 검증)은 새-컨텍스트 리뷰 패스로 보존한다.

## 3. 코어 구조 (§1 확정안 — 이중 리뷰 v2)

### 3-1. 원칙

- **코어는 어떤 하네스도 모른다. 하네스를 아는 것은 어댑터뿐이다.**
- 하네스 중립의 기준은 "단어 금지"가 아니라 **"의미 보존 + 표준 동작 어휘집"**이다
  (Codex 리뷰 1.1로 정정 — 지식 파일에 이미 구형 MCP 도구명이 포함되어 있어 단어 금지는
  무변환 복사와 양립 불가). core는 중립 capability명을 쓰고 어댑터가 실제 도구명으로
  매핑하며, 실제 하네스 식별자(`.claude/`, `mcp__` 프리픽스 등)만 CI에서 검사한다.
  구형 도구명(`mcp__mcp-abap-adt__*`)은 이식 시 1회 정규화한다.
- 지식 로딩 규약: data-only + 인덱스 + 트리거 로드 (선행 설계 S6 승계). 상시 주입 금지.

### 3-2. 디렉토리 구조

```text
sc4sap-lite/
  core/
    knowledge/
      modules/{FI,CO,MM,SD,PP,QM,PM,PS,HCM,TR,TM,WM,BW,Ariba}/  ← 14개, 원본 대소문자 보존
      modules/common/                  ← 교차모듈 지식 (configs/common 7파일)
      modules/BC/                      ← agent_details/bc 2파일 기반 특수 모듈 (신규 합성임을 명시)
      industry/  country/              ← 14업종 + 16개국
      abap/
        (reference/ 는 없다)            ← 이식 당시 `skills/sap-abap` 레퍼런스 31파일이 있었으나
                                          GPL-3.0 편집 저작물의 사본이라 2026-08-09 전량 삭제 —
                                          고지는 「제거로 해소」 기록으로 전환(THIRD_PARTY_NOTICES).
                                          빈자리는 자체 집필이 아니라 조회로 처리한다(D-086) —
                                          동봉 help-portal 체계(procedures/help-portal-fetch.md +
                                          tools/fetch/)로 공식 문서를 필요할 때 가져오고,
                                          사전(辭典) 층은 재수록하지 않는다. 사다리 진행 상태는 docs/BLUEPRINT.md ⑶-a
        conventions/                   ← common/ 규약 (include-structure, oop-pattern, alv-rules,
                                          clean-code 3종, naming, text-element, constant 등)
        templates/                     ← oop/procedural/alv 샘플 + ecc/ (DDIC 헬퍼 템플릿 3종)
    personas/
      INDEX.md                         ← 유일한 발견 표면 (셀렉터). 본문 26개는 온디맨드 로드
      *.md                             ← 최소 헤더(name/description/지식참조/capability 태그
                                          readonly|readwrite) + 역할 본문. Claude 배선 제거
    procedures/
      create-program.md  create-object.md  release.md  analyze-symptom.md
      analyze-code.md  program-to-spec.md  compare-programs.md
      analyze-cbo-obj.md  deep-interview.md  ask-consultant.md  troubleshooting.md
      review-checklist.md              ← 인덱스 + 조건부 참조 구조 (거대 단일문서 금지)
      interview-sweep.md               ← 「파내기」 공용 하위 절차(판I · §11). 인터뷰가 닫은 칸
                                          **밖**의 결정 — 예외·오류·권한·데이터량·실행 방식·
                                          잠금·통화/단위·출력·로그·완료 기준 — 을 지도 → 빈자리
                                          열거 → **전부 묻기** → 기록의 4단계로 캔다. 스킬 래퍼
                                          없음(review-checklist와 같은 지위) · 호출자 3
                                          (create-program · deep-interview · create-object) ·
                                          기록은 그 런이 이미 가진 산출물에 고정 토큰
                                          `<!-- SAPKIT:DIG-OUT -->`로 — 번역하지 않는다
      develop-abapgit.md               ← abapGit 오프라인 ZIP 인도 통로(판D · §10). 에이전트는
                                          로컬 미러에 쓰고 동봉 검사기로 검사하며 반입 전까지
                                          SAP에 닿지 않는다 — 반입·활성화·이송은 사용자 소유
      modify-object.md                 ← Minimal 강도 절차, 흔적 0 (D-047 · aegis direct 각색)
      lesson.md                        ← LESSONS→RULES 5단계 (D-047 · aegis lesson 각색)
      knowledge.md                     ← lesson의 대칭짝: **실패가 아니라 몰랐던 사실**을 누적
                                          (D-052). .sapkit/knowledge/{domain,system}.md ·
                                          KD-/KS- 인용 ID(제품 D-0xx 결정번호와 분리) ·
                                          정정은 삭제 아님 · 읽기는 인터뷰 계열 2곳
      package-to-process.md            ← 패키지 1개 → E2E 업무 흐름 문서(D-056). 객체 단위
                                          (analyze-cbo-obj)·프로그램 단위(program-to-spec) 위의
                                          **프로세스 계층**. 메인이 7단계 전부 소유(디스패치 0,
                                          페르소나 채택) · P0/P1 · 산출물에 PNG 다이어그램 2종
      verify-applied.md                ← 「반영 확인(기계 확인)」. SAP에 이미 반영한 오브젝트를
                                          되읽어 보낸 것과 대조하고 구문·활성 상태를 확인한다.
                                          완료의 기계 절반이며 나머지 절반은 독립 새-컨텍스트
                                          리뷰다. 도구 성공 응답은 어느 절반도 아니다.
                                          (renew 1차에서 옛 이름의 전용 절차·스킬·러너를
                                          개명·삭제하고 재저작 — 도장 의식과 별도 산출물
                                          파일은 폐지. 경위는 결정 로그)
      setup.md  troubleshooting.md     ← 온보딩 마법사 · 하네스 중립 진단/운용
      install-sap-assets.md            ← SAP측 헬퍼 오브젝트 설치
      handoff.md                       ← 세션 마감: 프로젝트 루트 HANDOFF.md·RUN-PLAN.md 갱신
                                          (사용자 데이터 · fail-closed 소유권 확인)
      spro-lookup.md  customization-lookup.md  help-portal-fetch.md
                                       ← 조회 보조 절차 (SPRO 경로 · Customizing 값 · 공식 문서)
      schemas/                         ← approval·review-request·review-result·verification 스키마
                                          (승인 = spec 해시 + SID/client + transport에 결합)

      절차 수·스킬 수의 **정본은 `plugin-metadata.json`의 자산 계수**다(여기 복제하지 않는다 —
      복제하면 갱신 누락이 곧 거짓말이 된다). 둘은 같지 않다 — 스킬 진입점으로 노출되지 않는
      절차가 있다(예: `review-checklist.md`·`interview-sweep.md`는 부르는 절차만 있는 하위 절차).
    policies/
      knowledge-sourcing.md            ← 성숙층 사다리(D-059): RULES→지식원자→스냅샷→볼트→번들
                                          우선순위·읽기 의무·write-back — 전 페르소나·절차 우산
      plain-language.md                ← 사용자 대면 말투의 정본(판I · §11). 적용 범위(채팅 +
                                          사용자용 파일 · 기계용 json 제외) · 배관 비노출 ·
                                          「용어(뜻)」 병기 · 선택지 = 뜻+추천+이유 · 질문의
                                          맥락과 진행 표시 · 질문 형식(호스트 선택 창 → 글
                                          폴백) · 묶음 규칙의 일반형. `project-context.md`에
                                          always-on 요약 블록(D-059 사다리 표와 같은 자리)
      sap-standards.md                 ← Z/Y 접두어, transport 필수, 활성화 의무, 버전 가드
      verification-policy.md           ← CheckSyntax→활성화→ABAP Unit→ATC 체인, 차단 severity
      approval-gates.md                ← 관문=문서 규약임을 명시 (기계 강제의 정본은 서버
                                          tier 게이트 — Claude 훅은 2026-08-02 v2부터 선택
                                          이중화)
      development-loop.md              ← 강도 축(Minimal/Standard/Full)·execution_owner·
                                          보증 등급 매트릭스 (D-047)
      data-protection/                 ← exceptions/ 12파일 이식 + 실데이터 조회 승인 정책
      credential-handling.md
    vocabulary.md                      ← 표준 동작 어휘집 (중립 capability명 ↔ 어댑터 도구명 계약)
    project-context.md                 ← 다섯 번째 요소: .sapkit/{config.json, active-profile,
                                          sap.env} 프로젝트 런타임 상태 규약 + 누적 산출물
                                          (RULES/LESSONS/knowledge) — 전부 로컬 전용
  server/
    server.bundle.cjs  VERSION  integrity.json
    runtime-deps/keyring/              ← NODE_PATH 배선 포함 (keychain 비밀번호 저장)
    tool-catalog/                      ← sapkit-mcp-tools*.md 4파일 — 엔진 등록점 생성물
                                          (판10 · D-107 · read 90 / write 79 / runtime 15 ·
                                          행 데이터 2종은 섹션 밖·인덱스에 사유). 노출 프리셋
                                          원천. 생성기 harness/render-tool-catalog.mjs · 대조
                                          게이트 gates/catalog.mjs (둘 다 sapkit-engine/) —
                                          손으로 고치면 거부된다
    sap-assets/                        ← abap/ 헬퍼 17파일 + ALV 핸들러 + 설치 manifest
    UPDATE-RUNBOOK.md                  ← 업스트림 재번들·무결성 검증·capability diff 절차
  adapters/                            ← 콘텐츠 복제 없음. manifest·정책 매핑·설치물만
    claude/  codex/  antigravity/  compatibility.json
    claude/permissions-template.json   ← 전체 허용 템플릿 (setup 4a가 제안)
    claude/permissions-build.json      ← **부분 목록**(판I · §11) — 설계 승인 직후 「한 번에
                                          허용」이 병합하는 자리. 템플릿에서 필터로 파생하며
                                          (`gen-permissions.mjs --derive-build`) 손으로 유지하지
                                          않는다. 제외 = `Delete*` · `CreateTransport` ·
                                          `ReleaseTransport` · `CreatePackage` · `Runtime*` ·
                                          실데이터 2종 —
                                          되돌리기 어려운 일과 실데이터는 계속 허용 창이 뜬다.
                                          검사는 핀 스냅샷 `provenance/mcp-surface.json`의
                                          `adapter_deny.claude_build`에 걸린다
  scripts/
    build-adapters.mjs                 ← manifest 생성 + 링크 체커 + 이식 커버리지 검증
    install-{claude,codex,antigravity} ← Node 단일 (크로스플랫폼, Windows 우선 실측)
    doctor.mjs                         ← 3사 어댑터 버전/코어 해시 불일치 감지
    setup-state.mjs                    ← 결정론 상태 mutator: status/plan/apply/verify
                                          (2026-08-02 v2 §8-2 — secret 필드 불허)
    codex-wire-mcp.mjs                 ← Codex bundled MCP 토큰→절대경로 재작성, 멱등
                                          (status/apply, 2026-08-02 v2 §6-1)
  MIGRATION-MANIFEST.md                ← 원본 전 경로 5분류 (§5-1)
  LICENSE  THIRD_PARTY_NOTICES.md
```

### 3-3. 핵심 규약

1. **단일 원천 규칙**: 페르소나=역할·관점, 절차=순서·관문, 정책=불변조건만 소유.
   절차가 페르소나 본문을 복사하지 않는다 — 참조만. (이중 보관 드리프트 방지)
2. **프로젝트 컨텍스트**: `.sapkit/` 규약(sapVersion·abapRelease·activeModules·industry·
   country·active-profile)은 선행 설계의 멀티 프로파일 체계를 유지한다. 구 세대
   디렉터리 폴백은 **renew R5에서 제거**돼 지금은 단일 세대다(D-057 이행 완료 후).
   **"루트 .env 하나로 통일" 발상은 기각** (서버 tier 가드가 `.sapkit`을 읽으므로
   우회 위험 + DEV/QAS/PRD 전환 현실과 충돌 — 이중 리뷰 합의 지적).
3. **설치 프로필 2종**: `knowledge-only`(지식+페르소나, 서버 미설치 — SAP 연결 없는
   컨설팅용)와 `connected`(서버+SAP 연결). 서버 8.25MB+런타임 4.5MB는 후자에서만.
4. **지식 정본 선언**: 이식 완료 시점부터 지식 정본 = sc4sap-lite. 동결된 sc4sap-custom은
   지식 수정 금지. sap-agentic-harness packs는 lite의 스냅샷 버전을 provenance로 명기.
5. **동봉 로컬 검사기 (sapkit 검사기)**: SAP 반영 전 오프라인 lint/parse/analyze는
   자체 저작 검사기(소스 정본 `sapkit-cli/`)가 맡고, 그 단일 파일 번들
   `checker/sapkit-checker.bundle.cjs`를 **플러그인에 동봉**한다. 제품 MCP 서버에
   병합하지 않는 이유는 그대로다(번들 도구와 표면 중복) — 별도 CLI로 둔다. 검사기에는
   **SAP 접속도 MCP 모드도 없다**. 동봉이므로 설치 단계가 아예 없고(내려받기 0),
   **설치가 완전 오프라인**이 된다 — 릴리스 자산을 1회 내려받던 구 방식(D-044)이 해소한
   문제 자체가 사라졌다. 미보유 상태에서도 하네스는 정상 동작한다(훅은 무음 통과).
   무결성·출처 게이트 = `scripts/verify-checker.mjs`, 재번들 절차 =
   `checker/UPDATE-RUNBOOK.md`.
6. **방법론 강도 축**: `development-loop.md`(정책)가 강도 축(Minimal/Standard/Full)·
   execution_owner·보증 등급 매트릭스를 소유한다. 강도 축은 **Track A 실행 구조
   (Direct/Guided) 라우팅과 직교**한다 — 어느 쪽도 서로를 바꾸지 않는다(D-047).
7. **구현 위임의 실행체와 발동**: D-047이 들여온 `execution_owner`는 정책만 있고 실행체가
   없어 `auto`가 항상 `main`으로 폴백했고(D-051이 워커 1기 `agents/sap-worker.md` + 선택
   프롬프트로 1차 수리), 실사용에서는 그 프롬프트·디스패치가 발동하지 않아 위임이 여전히
   0이었다(D-066). 현행: create-program(Full)은 Phase 3.5에서 소유자를 **결정론 해석**한다
   — 사용자 명시 > 기본값(워커를 무개입 호출 가능한 어댑터에서 `delegated`, 그 외 `main`)
   — 별도 질문 없이 **진행 안내의 한 줄**로 표기하고(판I가 Phase 3.5를 그 이름으로 바꿨다 —
   §11) 같은 응답에서 번복 가능하며, `delegated`면
   Phase 4 디스패치가 **의무**이고 기동 실패는 fail-closed(명시 요청은 침묵 폴백 금지,
   기본값 폴백은 고지 + `effective_owner` 기록)다. **8역할 자동 교대 파이프라인은 여전히
   폐기 상태** — §2의 "자동 디스패치 폐기"는 8역할 자동 교대의 폐기이지, 메인이 배정하는
   워커 1기의 기본 기동을 막는 것이 아니다. 품질 모델(1작업 + 1리뷰 + 기계검증)은 불변이다.
   위임 불가 두 축의 MCP 도구 4종(P2 실데이터 2·P4 이송 2)은 Claude 어댑터에서
   `disallowedTools` 4줄로 **기계 차단**하고 게이트가 그 생존을 assert한다(셸 경유 CLI 등
   도구 밖 경로는 절차 규범 — D-051·D-066).

## 4. 어댑터 3벌 (§2 확정안)

### 4-1. 수렴 구조

이중 리뷰로 확인: 3사 모두 현재 공식 배포 표면이 **플러그인(스킬+MCP 동봉)**이다 —
Claude(`.claude-plugin`), Codex 0.144+(`.codex-plugin`, custom prompts는 **deprecated**),
Antigravity(agy CLI 1.0.7 `plugin install/validate`). 어댑터는 동일한 6개 계약을 각사
매니페스트 문법으로 표현한 것이다.

**어댑터 공통 계약 6가지:**

1. **MCP 등록** — `server/server.bundle.cjs` stdio 등록 (NODE_PATH keyring 배선 포함)
2. **페르소나 진입** — INDEX 셀렉터만 발견 표면에 노출, 본문 26개는 온디맨드
3. **절차 진입** — core/procedures를 스킬/워크플로우로 래핑 (본문 복사 금지, 참조만)
4. **정책 주입** — sap-standards 요약본만 상시, 상세는 참조 경로
5. **도구명 매핑** — vocabulary.md의 capability명 → 하네스 실제 도구명 표
6. **안전장치 매핑** — 3층 안전 모델의 L3을 자기 기계 수단으로 구현

### 4-2. 하네스별 실체

**adapters/claude/ (기준 구현 — 기존 자산 최대 재사용):**

```text
.claude-plugin/plugin.json               ← 생성물 (정본 = plugin-metadata.json)
adapters/claude/hooks/                   ← **안전훅 6종**(안전 4 + 품질 조언 2), 2026-08-02
                                            v2부터 기본 미설치·선택 스위치 — 설치는
                                            install-hooks.mjs 명시 실행, 문서는 hooks/README.md
adapters/claude/hooks/continuity/        ← **연속성 훅 1종 + 자기 설치기**(판C · D-139 ⓒ).
                                            안전훅이 아니라 편의 스위치라 디렉터리로 갈랐다 —
                                            그 구분이 「안전훅 6종」 정체성과, 훅 파일 집합을
                                            세어 대조하는 기존 스위치 시험을 무수정으로
                                            지킨다. 여기도 기본 미설치(D-062)
adapters/claude/permissions-template.json ← 정적 allowlist 템플릿 (trust-session 대체.
                                            GetTableContents/GetSqlQuery 제외 유지 — 매번 승인)
adapters/claude/lib/                     ← 프로파일 해석·stdin 등 **Claude 전용** 훅 모듈
                                            (하네스 중립 공용 모듈은 여기가 아니라
                                             scripts/lib/ — 어댑터가 중립 트리에
                                             의존하는 방향만 허용하고 그 역은 막는다.
                                             atomic-write.mjs가 그래서 저쪽에 산다)
```

**스킬·에이전트는 어댑터가 아니라 플러그인 루트가 소유한다** — 실측: `skills/` 18개
(절차 중 진입점으로 노출되는 것들의 얇은 래퍼, "core/procedures/X.md를 읽고 수행"
수준) · `agents/` 2개(`sap-reviewer` read-only 리뷰 패스용 · `sap-worker` P2/P4 차단,
execution_owner=delegated 실행체 — D-051). 3사가 같은 실물을 공유하므로 어댑터별 복제는
없다(§4-1 계약 3).

**adapters/codex/:**

```text
.codex-plugin/ 매니페스트 + skills/ 래퍼 + MCP 등록 (플러그인 동봉 .mcp.json)
AGENTS.md        ← 인덱스만 (합산 32KiB 한도 준수): 표준 요약 + core 경로 + 셀렉터 안내
toolSurface 결정  ← .sapkit/config.json의 toolSurface(readonly 기본/development는
                    SAP_TIER=DEV+명시 선택)를 launch.cjs가 exposition으로 변환(2026-08-02
                    v2 §7-1, 고정 --exposition 인자는 제거) — Codex는 지연 로딩이 없어
                    도구 전부가 컨텍스트에 올라가므로 read 중심 기본이 특히 중요
```

- 설치는 **전역** (프로젝트 config는 trusted 게이트로 신뢰 불가 — installer가 처리)
- 리뷰 패스: `codex exec` 새 세션 (read-only 샌드박스)

**adapters/antigravity/:**

```text
플러그인(agy) 동봉: rules 1~2개 (안전 규칙 요약 — 파일당 12,000자 한도 준수)
                  + workflows (절차·페르소나 진입점, /create-program 형태)
                  + MCP 설정 (~/.gemini/config/mcp_config.json에 merge-write)
```

- 페르소나는 rules가 아니라 **workflows**로 (rules는 상시 주입이라 컨텍스트 비용)
- 리뷰 패스: Agent Manager에서 리뷰 에이전트 별도 실행
- 주의: Antigravity 문서가 2.0 표면으로 이행 중 — compatibility.json에 검증 버전
  (agy 1.0.7) 고정, L5 착수 시 재검증. (claude-code도 동일하게 exact 버전 핀 유지 — codex만
  2026-08-02부터 capability 기반(minimumSupported/probeCandidates/lastVerified/
  requiredCapabilities)으로 전환. 상세:
  `docs/reference/designs/2026-08-02-claude-onboarding-codex-parity-no-engine.md` §9-1,
  `adapters/compatibility.json`.)
- **L5 실측 갱신 (2026-07-10)**: 예상(rules/workflows 신규 제작)보다 단순하게 해결 —
  루트 `plugin.json` 매니페스트 하나 추가로 `agy plugin install <repo>` 직설치가 동작
  (skills 11 + agents 1 임포트, core/server 포함 전체 복사). MCP 번들은 agy 1.0.7
  미지원이라 전역 mcp_config 수동 등록. 상세: `adapters/antigravity/README.md`,
  `docs/research/L0-cli-surface.md`

### 4-3. 안전 모델 — 3층

| 층 | 실체 | Claude | Codex | Antigravity |
|---|---|---|---|---|
| L1 문서 정책 | core/policies | ✅ 동일 | ✅ 동일 | ✅ 동일 |
| **L2 서버 내장 가드** | 번들의 blocklist + SAP_TIER readonly 가드 (`.sapkit` 프로파일 해석) — **3사 공통 기계 방어선** | ✅ | ✅ | ✅ |
| L3 하네스 기계 장치 | 각사 고유 | allowlist(permissions 템플릿) + 안전훅 3종(2026-08-02 v2부터 기본 미설치·선택 스위치) | approval 모드 + 샌드박스 | 자체 권한 설정 |

- 실데이터 조회 2종(GetTableContents/GetSqlQuery): Claude는 allowlist 제외로 매번 승인.
  Codex/Antigravity는 L2+L1뿐 — **Claude보다 한 겹 약함을 어댑터 README에 정직하게 명시.**
- **connected 설치 수용 기준**: 프로파일 미설정 시 서버가 write 도구를 노출하지 않을 것
  (L2가 최후 방어선이므로 — Phase L2 검증 항목).

### 4-4. 리뷰 패스 인계

작업자가 `schemas/review-request.json`(변경 객체 목록 + spec 해시 + SID/client + transport)을
남기면 → 새 컨텍스트의 리뷰어가 review-checklist.md로 판정 → `review-result.json` 기록.
리뷰어는 read-only 판정만, 수정은 작업자가.

## 5. 이식 매핑 · 로드맵 · 상위 설계 반영 (§3 확정안)

### 5-1. 이식 매핑표 (MIGRATION-MANIFEST 골격 — 미분류 0이 완료 조건)

| 원본 | 분류 | 목적지 / 처리 |
|---|---|---|
| `configs/` 14모듈+common | copy | `core/knowledge/modules/` (대소문자 보존) |
| `industry/` `country/` | copy | `core/knowledge/{industry,country}/` |
| `skills/sap-abap/` | copy | `core/knowledge/abap/reference/` |
| `common/` 50파일 | transform | 규약→`abap/conventions/`, 정책→`policies/`, 절차→`procedures/`, Claude 전용 4종(model-routing-rule·team-consultation-protocol·context-loading-protocol·multi-profile-artifact-resolution)→obsolete |
| `agents/` 26개 | transform | `core/personas/` (배선 제거 + capability 태그) |
| `agents/agent_details/bc/` | transform | `core/knowledge/modules/BC/` |
| `skills/` 10개 | transform | `core/procedures/` (1인 절차화. troubleshooting.md는 sap-doctor 노하우에서 신규 합성) |
| `skills/` setup·mcp-setup·sap-doctor·sap-option | transform | `scripts/install-*` + `doctor.mjs` + troubleshooting.md |
| `skills/` **team·trust-session** | **obsolete** | 명시 폐기 |
| `hooks/hooks.json` + LEAN 훅 15종 | obsolete | 선행 설계에서 무동작 실증 |
| `scripts/hooks/` 안전 3종 + install-hooks.mjs | copy | `adapters/claude/hooks/` |
| `engine/` (번들·VERSION·integrity) | copy | `server/` |
| `runtime-deps/keyring/` | copy | `server/runtime-deps/` |
| `data/` 4파일 | copy | `server/tool-catalog/` |
| `abap/` 17파일 | copy | `server/sap-assets/` + 설치 manifest |
| `asset/template_base.xlsx` + `scripts/spec/` | copy | 스펙 산출 파이프라인 유지 (program-to-spec 의존) |
| `skills/create-object/ecc/` 템플릿 3종 | copy | `core/knowledge/abap/templates/ecc/` |
| `exceptions/` 12파일 | copy | `core/policies/data-protection/` |
| `tests/validation/` | transform | 링크 체커 + 이식 커버리지 테스트로 개조 |
| `docs/` | transform | 선별 (bundle-integrity → UPDATE-RUNBOOK 등) |
| **`private/`** | **exclude-private** | 영구 denylist — 어떤 산출물에도 미포함 |
| `src/` `.omc/` CHANGELOG 등 | obsolete/archive | 구형 stub·기록물 |

### 5-2. 로드맵 (완료 기준 포함)

| Phase | 작업 | 완료 기준 |
|---|---|---|
| **L0 부트스트랩** | 레포 생성, MIGRATION-MANIFEST 전 파일 분류, 링크체커 작성, Codex `.codex-plugin`·agy plugin 표면 실측 스모크 | 미분류 파일 0 + 두 CLI 스모크 로그 존재 |
| **L1 코어 이식** | knowledge→personas→procedures→policies 순 이식, 구형 도구명 정규화, 대소문자 보존 | 커버리지 테스트 통과 + 링크 체커 0 에러 |
| **L2 서버 이관** | server/ 구성, connected 설치 검증 | tools/list 원본 일치 ✅ + 프로파일 미설정 시 write 도구 미노출 — **실측 미충족(목록 노출됨, inspection-only로 완화)**: 엔진 포크 이슈로 이관, `--exposition`으로 노출 축소 가능. 상세 `docs/research/L2-server-verification.md` |
| **L3 Claude 어댑터** (기준 구현) | 플러그인+안전훅+권한 템플릿 | FI 상담 1건 + create-program 절차 1건 E2E (리뷰 패스 포함) |
| **L4 Codex 어댑터** | 플러그인+AGENTS 인덱스+exposition 프리셋 | 동일 상담 + 객체 조회 E2E, AGENTS 32KiB 한도 준수 확인 |
| **L5 Antigravity 어댑터** | 플러그인(rules+workflows)+MCP merge-write | 동일 E2E, rule 12,000자 한도 준수 확인 |
| **L6 교차 검증** | 동일 과제를 3사에서 완주, 리뷰 인계 스키마 검증, doctor 동기화 점검 | 3사 완주 로그 + 인계 스키마 왕복 1회 실증 |

L3까지가 "지금과 동등"의 회복점, L4~L6이 본 프로젝트의 신규 가치다.

### 5-3. sap-agentic-harness DESIGN.md 반영 (소폭 3건 — 구현 계획에 포함)

1. §1 좌표 표에 `sc4sap-lite` 행 추가 — 역할: "지식 정본 + 3사 대화형 트랙"
2. §12 packs 콜드스타트 출처를 sc4sap-custom → **sc4sap-lite core**로 변경 + 스냅샷
   버전 명기(provenance) 규약 추가
3. §2 그림의 대화형 트랙 주석 갱신 — sc4sap-custom은 "동결(지식 수정 금지)" 표기

### 5-4. 미결 사항 (구현 시 결정)

| # | 항목 | 시점 |
|---|---|---|
| 1 | Codex `.codex-plugin` manifest 정확 스키마 실측 | L0 |
| 2 | Antigravity 2.0 표면 이행 여부 재검증 | L0, L5 착수 시 재확인 |
| 3 | 페르소나 INDEX 셀렉터 형식 (표 vs frontmatter 집계) | L1 |
| 4 | review-request/result 스키마 필드 상세 | L3 |
| 5 | Codex exposition 프리셋의 read/write 경계 (tool-catalog 기준) — **✅ 2026-08-02 toolSurface로 종결(D-062 ⑦)**: 경계는 클라이언트 프리셋이 아니라 런처의 `toolSurface` 판정이 담당 | L4 |

## 6. 검토 이력 (이중 리뷰, 2026-07-10)

Fable 5 서브에이전트(지적 26건)와 Codex CLI 0.144.1(지적 28건)이 동일 브리프로 독립 검토.
v1 → v2(본문 §3·§4) 반영의 결정적 지적:

- **합의 지적 12건 전부 반영**: 모듈 목록 정정(14+common, BC는 합성), 프로젝트 런타임 상태
  계층 신설, exceptions/ 12파일 이식, keyring/VERSION/integrity 동반 이식, 단일 .env 기각,
  abap/ 헬퍼의 server 분리, xlsx 파이프라인·ECC 템플릿 이식, 절차 11개로 확장, 페르소나
  INDEX 셀렉터, 어댑터 콘텐츠 복제 금지, 이식 매니페스트+링크 체커, doctor 동기화 점검.
- **설계 원칙 정정 2건**: "mcp__ 단어 금지" → "의미 보존 + 어휘집"(Codex 1.1 실측 반증),
  Codex 어댑터 표면을 custom prompts → 플러그인+스킬로 교체(공식 문서 근거).
- **단독 채택**: 설치 프로필 2종(Codex 4.3), review-checklist 인덱스+조건부 참조(Codex 4.4),
  승인의 spec 해시+SID 결합(Codex 5.5), capability 태그로 리뷰어 R/O 보존(Fable 4-3),
  서버 L2 가드의 3사 공통 방어선 규정(Fable 2-1).

## 7. 근거 실측 사실 (구현 세션의 재검증 좌표)

검증일 2026-07-10. 경로는 §1-4 기준.

- R1. sc4sap-custom `configs/`는 14개 모듈 + `common/` — BC 디렉토리 없음. BC 지식은
  `agents/agent_details/bc/` 2파일 (diagnostic-flows, transaction-codes).
- R2. 페르소나 26/26이 `.sc4sap/config.json`(sapVersion·abapRelease) 확인을 MUST로 요구.
- R3. 서버 번들은 시작 시 `.sc4sap/active-profile.txt`/`sap.env`에서 tier를 해석하고
  guardTool이 그 tier로 write를 차단 — 루트 `.env`만으로는 tier 가드가 실효 없음.
- R4. `.mcp.json`은 `NODE_PATH=runtime-deps/keyring/node_modules` 주입 — 번들 단독으로는
  keychain 저장 불가. `engine/VERSION`이 node-rfc·keyring의 external을 명시.
- R5. `hooks/hooks.json`은 11~12이벤트/23~30호출이나 편의성 훅은 LEAN 기본 오프이며,
  안전 3종은 `scripts/hooks/` + `install-hooks.mjs` 별도 경로 (선행 설계 M4 실측).
- R6. 지식 파일에 구형 도구명 잔존 — 예: `configs/FI/workflows.md`의
  `mcp__mcp-abap-adt__*` (Codex 리뷰 실측).
- R7. `skills/program-to-spec`은 `asset/template_base.xlsx` + `scripts/spec/*.mjs`에
  실행 의존. `skills/create-object/ecc/`에 DDIC 헬퍼 ABAP 템플릿 3종.
- R8. Codex CLI 0.144.1 로컬 설치 확인. 공식 문서 기준 custom prompts deprecated,
  플러그인(`.codex-plugin`)이 skill·MCP·hook 동봉, AGENTS.md 합산 기본 한도 32KiB,
  프로젝트 config는 trusted 게이트.
- R9. Antigravity 로컬 설치 확인(`~/.gemini/` 하위 antigravity*, config). agy CLI 1.0.7이
  plugin install/import/validate 제공. rules 워크스페이스 경로 `.agents/rules`
  (`.agent/rules` 하위호환), rule/workflow 파일당 12,000자 제한, MCP는
  `~/.gemini/config/mcp_config.json` 전역 관리. 문서가 2.0 표면 병기 중 — L0/L5 재검증.
- R10. sc4sap-custom `tests/validation/` 4종(agents·plugin-structure·skills·spro-configs)
  존재 — 링크/구조 검증 개조 원천.

## 8. 확장 설계 — 지식 3층 · knowledgeRoots · 제품명 개편 (2026-07-21, D-040 후속)

### 8-1. 관점 정리: 페르소나 = 모듈 하네스의 진입구

조립 관점에서 "모듈 하네스"는 세 부품의 세트다 — **페르소나**(진입구·관점, 호출
시에만 로드) + **로딩 규약**(INDEX 먼저·본문 온디맨드) + **지식층**(내용물). 사용자
입장에선 "컨설턴트를 부르면 모듈 전문성이 통째로 작동"하는 것이 맞고, 확장 지점은
항상 지식층이다: 전문성 증가 = 지식 파일 추가이지 페르소나 본문 수정이 아니다
(§3-3 단일 원천 규칙 유지). 따라서 지식이 커져도 페르소나 호출 고정비는 불변이다.

### 8-2. 지식 3층 모델

| 층 | 내용 | 위치 | 공개성 |
|---|---|---|---|
| L1 내장 공용 지식 | 범용 SAP 지식 (`core/knowledge/`) | 본 레포 | 공개 |
| L2 개인 위키 | 사용자 경험·범용 노트 (LLM-Wiki 구조) | 레포 밖 로컬 | 사적 |
| L3 고객/프로젝트 위키 | 특정 고객 Customizing·시스템 특성 | 레포 밖 로컬 | 사적·기밀 |

**불변 경계 — 본 레포는 공개다.** L2·L3는 복사·커밋 금지, 포인터 참조만 한다.
실제 경로 문자열(고객명이 포함될 수 있음)도 레포에 커밋하지 않는다: 포인터는 각
사용 프로젝트의 `.sapkit/`(git 미추적 런타임 상태)에만 존재하고, 본 레포에는 이 필드의
스키마 설명만 둔다.

### 8-3. knowledgeRoots 규약 (L2·L3 참조 배선)

`.sapkit/config.json`의 선택 필드:

```json
"knowledgeRoots": [
  { "path": "<로컬 위키 절대경로>", "scope": "personal" },
  { "path": "<고객 위키 절대경로>", "scope": "client" }
]
```

- **로딩**: 각 root의 `index.md`가 진입점(LLM-Wiki 구조 전제 — 없으면 루트 목록으로
  대체) · read-only · 온디맨드(상시 로드 금지 — §3-3 지식 성장 규약과 동일).
- **반영 지점**: ask-consultant 절차 step 4의 지식 참조 목록 + 페르소나 공통 규약
  (project context 해석 시 knowledgeRoots가 있으면 참조 후보에 포함). 절차·페르소나
  텍스트 외 코드 변경 없음 — 읽기는 하네스의 파일 도구로 수행하며 서버·훅 무관.
- **scope 라벨 용도**: 답변에 출처 구분 표시(예: "고객 위키 기준") + §8-4 환류
  분류의 기본값.

### 8-4. 지식 환류 (write-back 방향)

실전에서 배운 것의 귀속: 고객 특수(해당 시스템의 Customizing·버릇) → L3 · 개인 범용
경험 → L2 · 보편 SAP 지식 → L1. 단 L1 승격은 D-040 ④ provenance 정책을 따른다 —
출처·적용 버전 frontmatter 필수, 도그푸딩 성공만으로 정본 승격 금지. 상세 분류
규약은 선규정하지 않고 도그푸딩 실전에서 정련한다(D-040 정신).

### 8-5. 제품명 개편 (Phase 1 완료 · Phase 2 집행 착수)

배경: D-040으로 하네스(ENGINE)는 제품 정체성에서 빠졌다 — `sap-agentic-harness`는
옛 정체성 이름이며, 설치 명령의 중복(`이름@이름`)과 길이도 실사용 지적이다.

- **Phase 1 — ✅ 집행 완료 (2026-07-21, D-041)**: 이름은 **세 층으로 갈린다**(집행 중
  발견 — 원안의 "전부 sapkit"은 D-041 ②로 supersede):
  · **플러그인 = `sapkit`** — 스킬 접두어 `/sapkit:*` · 도구 네임스페이스
    `mcp__plugin_sapkit_sap__*`. 매 호출 등장하므로 짧게 유지한다(길이 × 도구 수 =
    세션 고정 토큰, §D-040 ② KPI 3순위).
  · **마켓플레이스 = `agentic-sap`** — 설치 명령이 `플러그인@마켓`이라 동명이면
    `sapkit@sapkit`으로 이 절이 지적한 중복이 그대로 남는다. 조직명과 맞춰
    `sapkit@agentic-sap`으로 해소. kebab-case 소문자 강제.
  · **레포 = `agentic-sap/sapkit`** — GitHub 조직 신설 + 이전(옛 경로는 리다이렉트).
  파급 = 권한 템플릿 재생성(gen-permissions) · 매니페스트 5종 재생성(단일 정본 게이트) ·
  문서 접두어 일괄 수정 · **설치 머신 재설치**. 후보 비교: **sapkit**(채택 — 상담+개발
  폭을 표현) / zdev(개발 편향) / abapkit(모듈 상담이 가려짐).
  집행 시 얻은 규약 3건: ⓐ 도구 네임스페이스는 **정본에서 파생**한다(하드코딩 시
  개명에서 부정 단언이 공허해짐 — smoke-mcp 실측) ⓑ 플러그인 개명은 `renames`로
  자동 이관되나 **마켓 이름 개명은 이관 경로가 없다** → 마켓 이름은 되돌리기 비싼 값
  ⓒ **MCP 도구 접두어 `mcp__plugin_<X>_sap__`의 `<X>`는 플러그인 이름이다**(마켓
  이름 아님) — 공식 문서에 없어 개명 전엔 두 값이 같아 관측 불가였고, 2026-07-21
  재시작 실측으로 확정했다. 스킬 접두어·에이전트 접두어도 동일 규칙이다. 따라서
  **플러그인 이름만 짧게 유지하면 세션 고정 토큰이 줄고, 마켓·조직명 길이는 무관하다.**
- **Phase 2 — ✅ 완결 (착수 2026-08-02 · 호환층 제거 2026-08-09)**: 아래는 착수 시점의
  설계 기록이다. **지금은 단일 세대다** — 사람 선행 게이트(이행 완료·두 스코프
  COEXIST_OK·구 env 부재)를 통과한 뒤 renew R5가 `SC4SAP_HOME_DIR` 폴백·구 런타임
  디렉터리 폴백·마이그레이터와 그 시험을 걷어냈고, 개명 게이트
  (`check-runtime-path-rename.mjs`)는 "구 세대 토큰 재등장 금지"만 지키는 단순
  게이트로 재작성됐다. 그 판에서 엔진은 5.0.0으로 major 범프·재번들됐다.
  아래 원문(착수 시점): `.sc4sap/` 디렉토리명·`~/.sah/` 프로파일 홈·
  `SC4SAP_*` 환경 변수 개명. 설계 정본 =
  `docs/reference/designs/2026-08-01-runtime-path-rename-sapkit.md`(v4, Codex 이종
  교차 리뷰 4회 — BLOCKER 8건 원문 확증). 대원칙 **R-PRESERVE**: 개명은 legacy-only
  입력의 결과와 각 소비자의 채택 기준(깊이·경계·state 정의)을 그대로 보존하며,
  `.sc4sap`을 보던 모든 자리에 `.sapkit`을 후보로 추가할 뿐이다(탐색기 통일·state
  재검토는 범위 밖 — 별도 D-결정 필요). git 밖 상태는 copy-not-move crash-safe
  마이그레이터로만 이행하고, 기존 프로젝트는 사용자가 그것을 실행하기 전까지 폴더명이
  그대로다(2026-08-01 사용자 확정: 수동 이사, setup 자동 실행·플러그인 자동 이사는
  기각). 집행 완료 후 **D-057**을 append한다.

## 9. 연속성 층 — 사용자 프로젝트의 세션 간 기억 (2026-08-25 · 판C · D-138 · D-139)

**문제.** 제품이 깔린 **사용자 프로젝트**에서 「어디까지 했나」가 세션 사이에 남지
않았다. 실측된 공백이 셋이다 — 제품이 거는 훅 6종은 전부 안전·분석용이고 `SessionStart`는
0건 · 절차·스킬 중 「어디까지 했나」를 다루는 것이 없다(`knowledge`·`lesson`이 `.sapkit/`에
쌓는 것은 「무엇을 알아냈나」와 「무엇이 실패했나」다 — D-052의 대칭) · 다른 도구가
채워 줄 것이라 가정할 수 없다. 근거는 D-138.

### 9-1. 부품 넷

```text
assets/continuity/HANDOFF-template.md    ← 사용자 프로젝트 루트에 세울 **상태 스냅샷** 원본
assets/continuity/RUN-PLAN-template.md   ← 같은 자리의 **작업 큐** 원본
                                            둘 다 1행에 표식 주석 · 영어 · 비밀 기록 금지 경고
core/procedures/handoff.md               ← 마감 절차(소유 판별 → 갱신/생성 → 크기 검사 →
skills/handoff/SKILL.md                     초과 시 이사). 래퍼는 기존 패턴 그대로
adapters/claude/hooks/continuity/        ← 선택층: SessionStart 훅 + 자기 설치기(§9-3)
scripts/check-doc-size.mjs               ← 크기 상한. 판B의 레포 게이트를 대상 인자화해
                                            제품이 같은 부품을 쓴다(§9-4)
```

파일이 루트에 사는 것과 이름이 이 레포의 관례와 같은 것은 **같은 설계 계보**이기 때문이다
— 상태와 큐를 가르고, 상한을 두고, 넘치면 삭제가 아니라 이사한다.

### 9-2. 소유 판별 — fail-closed가 이 층의 중심이다

사용자 프로젝트에는 같은 이름의 파일이 **이미 있을 수 있고**, 그것은 남의 것이다. 그래서
파일마다 상태가 셋이다 — **부재 | 표식 있음 | 표식 없음**. 표식은 렌더링에 보이지 않는
주석 한 줄(`<!-- sapkit:continuity -->`)이고 탐지는 앞 20줄 스캔이다.

| 상황 | 행동 |
|---|---|
| 둘 다 부재 | 쌍 생성(표식 포함) — 승인 하에 |
| **표식 없는 동명 파일이 하나라도 있음** | **세우지도 갱신하지도 않는다.** 한 번 보고하고 물러난다 — 크기조차 재지 않는다 |
| 표식 있는 파일만 있음 | 갱신 대상. 한쪽이 없으면 없는 쪽만 재생성 제안 |

**읽기 실패·판정 불능은 「표식 없음」과 같다.** 잘못 「우리 것」이라 하면 되돌릴 수 없는
남의 작업을 덮고, 잘못 「남의 것」이라 하면 설명 한 문장을 치른다 — 비용이 대칭이 아니다.
**불변식: 모든 갱신은 표식을 보존한다.** 잃으면 다음 세션이 스스로 잠근다. 뒤집으면 그것이
사용자의 탈출구다.

파일은 생성 순간부터 **사용자 소유 데이터**다 — 제품은 삭제·초기화하지 않으며, 훅 스위치의
uninstall도 훅만 제거한다.

### 9-3. 시작 안내는 2층이다

- **기본층 (하네스 중립 · Claude에서는 CLAUDE.md)** — setup 4d가 사용자 `CLAUDE.md`에
  영어 한 줄을 **승인 하에** 더한다(가산적·멱등). **그 줄은 자기 조건을 품는다** —
  「표식을 가진 `HANDOFF.md`가 있을 때」로 스스로를 가둔다. 사용자가 나중에 표식을 지우거나
  (제품이 광고하는 탈출구다) 자기 파일을 놓으면 무조건문은 **이행 불가능한 상시 지시**가
  되기 때문이다. Codex 병합 블록 규칙 7도 같은 이유로 같은 게이트를 품는다.
  Antigravity는 어댑터 README가 그 자리다 — 두 하네스에 훅 등가물은 없다.
- **선택층 (Claude 전용)** — `SessionStart` 훅. **기본 미설치**(D-062 불가침)이고 **안전훅
  6종과 분리된 자기 설치기**로 켠다. 표식 있는 재개점이 있을 때만 **짧은 포인터**를 주입하고
  (파일 전문 주입 금지 — 무게의 척도는 세션 토큰이다 · D-040) 그 밖은 전부 무음이다.

**왜 전부를 훅 뒤에 두지 않았나**: 시작 안내 전체가 옵트인이면 D-138이 갈래 ⓑ를 기각한
논리(「제품의 약속을 옵트인에 의존시킬 수 없다」)가 제품 안에서 재발한다.

### 9-4. 크기 상한은 같은 게이트를 두 곳이 쓴다

`check-doc-size.mjs`는 판B가 레포용으로 지었고 대상 2파일이 상수로 박혀 있었다 —
제품 패키지에 통째로 실려 나가므로 남의 프로젝트에서 돌리면 「부재 = 위반」으로 거짓
red가 났다. 판C가 `--file <경로>:<상한>[:<아카이브>]`를 필수로 만들고 **무인자를 exit 2**
(위반 1과 구분)로 갈랐다. 레포 판정은 동등하게 유지된다.

### 9-5. 범위 밖 (실수요가 생기면 별판)

장기 절차 22종에 갱신 배선 · unattended 자동 갱신 · 타 도구와의 협조나 감지 로직
(**타 도구 비인지** — D-139 ⓑ).

## 10. abapGit 오프라인 ZIP — 두 번째 인도 통로 (2026-08-26 · 판D · D-141)

**문제.** sapkit의 기본 통로는 살아 있는 ADT 접속(MCP)으로 ABAP을 SAP에 적용한다. 그 접속이
없는 자리에서 제품은 일을 끝내지 못했다. 겹친 사실이 셋이고 **세 번째가 범위를 지배한다** —
① 정책(`AGENTS.md` P3)은 user-operated abapGit을 **이미 허용**하는데 그것을 받치는 절차
배선이 0이었다 · ② `core/knowledge/abap/conventions/source-repair-protocol.md`는 다수 FM
일괄 수리에 abapGit 경로를 **이미 권한다** — 권고만 있고 그 뒤가 비어 있었다 · ③ **MCP를 쓸
수 없는 환경에서도 제품이 일을 끝낼 수 있어야 한다.** 근거는 D-141.

③이 지배한다는 말은 범위 결정에서 드러난다. 이 경로가 **유일한 길인 사용자**가 있으므로
객체 범위를 안전을 이유로 좁히지 않았다 — 화면(DYNPRO)·GUI 상태(CUA)는 **소유자 결정으로
범위 안**이고, 신규 XML의 반입 왕복 위험은 **빼기가 아니라 씨앗 모사 + 사전 고지**로 다룬다.
빼면 다른 길이 없는 사용자를 가둔다.

### 10-1. 통로의 모양 — 에이전트는 반입 전까지 SAP에 닿지 않는다

```text
core/procedures/develop-abapgit.md   ← 절차 정본 (씨앗 → 미러 저작·검사 → 전체 ZIP → 인계)
skills/develop-abapgit/              ← 스킬 래퍼. 기존 패턴 그대로
```

에이전트는 로컬 abapGit **미러** 안에서 ABAP을 쓰고 오프라인으로 검사한 뒤 **패키지 전체
ZIP**을 지어 건넨다. 거기서 멈춘다 — **반입은 사용자가 abapGit UI에서 직접** 한다. 진입은
둘이다: `create-program`의 적용 단계에서 갈라져 오는 길과, 단독 호출(일괄 수리 · 기존 패키지에
객체 추가). 자산 계수의 정본은 `interactive/plugin-metadata.json`, 상태 수치의 정본은 루트
`HANDOFF.md`이며 **여기에 복제하지 않는다.**

### 10-2. 안전 등급 — 이 층의 핵은 「기계 게이트가 아니라 절차 규범」이라는 것이다

ZIP 인계까지 에이전트가 밟는 모든 걸음은 **P0 오프라인**이고, **SAP을 바꾸는 행위는 P3이며
그것은 사용자의 것**이다(반입 후 `COMPLETE`를 구하는 검증 국면은 §10-3 — MCP가 있는 자리에서만
성립한다). 아무것도 접속돼 있지 않으므로 **MCP 경로를 지키는 서버측 tier 게이트가
발화할 수 없다** — 그래서 DEV 한정은 착수 시점의 **기록된 사용자 확약**으로 실린다. 등급으로
말하면 기계 게이트가 아니라 **절차 규범**이고, **이것은 MCP 경로보다 약한 보장이다.** 절차는
그 약함을 감추지 않고 **말하는 것으로** 벌충한다 — 확약을 받을 때, 검사가 일어난 것처럼
읽히지 않게 그 사실을 함께 말한다. 확약이 기계 PASS로 둔갑하지 않도록 그 기록은 기계 전용
검증 계약 **밖의 별도 파일**에 둔다(D-141 ⓑ — 스키마 확장은 기각됐다).

- **P4는 무접촉**이다 — 반입 · transport 배정 · 릴리스 전부 사용자 소유.
- **abapGit을 에이전트가 자동으로 조작하도록 배선하는 것은 금지**다. 미룬 것이 아니라 금지다:
  그렇게 하면 SAP으로 들어가는 **게이트 없는 두 번째 경로**가 MCP 옆에 서고, 게이트는 둘 중
  하나만 지키게 된다.

### 10-3. 상태 의미론 — 사용자의 보고는 확약이지 기계 PASS가 아니다

| 무슨 일이 있었나 | 상태 |
|---|---|
| 미러 저작 + 오프라인 검사 통과 + 리뷰 — SAP 무변경 | **`DRAFT`** |
| 사용자가 반입·활성화 성공을 보고 | **`PROVISIONAL_WRITE`** |
| 되읽기 확인 + 반입 후 시험·ATC 증거 **그리고** 독립 리뷰 통과 | **`COMPLETE`** |
| 사용자가 반입 **실패**를 보고 | **`PROVISIONAL_WRITE`** + 미상 상태 부기 |

`COMPLETE`는 **MCP read가 가능한 자리에서만 닿는다** — `core/procedures/verify-applied.md`의
되읽기 대조 + `CheckSyntax` + `GetInactiveObjects` 0 + **반입 후 시험·ATC 증거**(`RunUnitTest`·
`GetAtcFindings` 각각 `PASS` 또는 사유 있는 `SKIPPED` — MCP 지선의 Phase 8 행렬과 같은 급.
D-144: 되읽기를 가능하게 하는 그 MCP 가용성이 시험·ATC도 가능하게 하므로, 요구를 올려도
도달 불가능해지는 환경이 없다. 시험 실행은 P3이고 DEV-only가 그대로 걸린다) + 독립
새-컨텍스트 `R-PASS`. read가 없는
시스템에서 이 경로는 **`PROVISIONAL_WRITE`에서 멈추고 그렇게 기록한다** — 거기서 `COMPLETE`로
가는 길은 없고, 있다고 말하는 것이 과대보고다. **실패 보고는 `DRAFT`로 되돌아가지 않는다**:
실패한 반입은 서버가 **미상 상태**라는 뜻이고(부분 적용이 얼마든지 가능하다), 되돌아감은
「SAP은 안 바뀌었다」는 거짓 주장이 된다.

### 10-4. 설계상 fail-closed 둘

⑴ **씨앗** — 사용자가 **대상 패키지의 abapGit 전체 export**를 대지 않으면 절차는 시작하지
않는다. 부분 재구성도, 백지 저작도 없다. 그리고 직렬화의 권위는 규칙 문서가 아니라 **씨앗**이다
(BOM 유무까지) — 직렬화는 그 서버에 깔린 abapGit 빌드의 것이기 때문이다.

⑵ **전체 패키지 ZIP만** — 오프라인 반입은 ZIP을 **원격의 전체 상태**로 읽는다. 빠진 것은 곧
삭제 후보가 되고, FUGR pull은 병합이 아니라 delete-and-recreate다. 완전한 ZIP이면 그 목록이
**구조적으로 빈다** — 사고를 규율이 아니라 구조로 막는다.

### 10-5. 경계와 정직 유보

**범위 밖**: 온라인 abapGit(리포지터리 연결 흐름)은 이 경로가 아니다 — **오프라인 ZIP
전용**이다. **코드 변경 0** — 엔진 · 검사기 · 훅 무접촉이고 MCP 도구 표면도 그대로다.
RAP/CDS 계열은 v1에서 약속하지 않는다.

**정직 유보 둘.** 이 판은 **SAP 실기 0**이다 — 절차 문서 제품이므로 **첫 실사용이 시험대**다.
그리고 씨앗과의 **XML 구조 대조에는 기계 검증기가 없다** — 에이전트 자신의 리뷰이고, 이 사슬
에서 가장 약한 고리다. 절차는 그것이 인계 시 통과한 검사처럼 읽히지 않도록 명시할 것을
요구한다.

## 11. 사용자 대면 말투 · 파내기 · 승인 후 자율 진행 (2026-09-18 · 판I · D-149)

소유자의 실사용 지적 두 축에서 나왔다. ① 「용어나 단어들이 어렵게 표현되서 아밥개발자인
내가 보기에도 읽기가 너무 어려워」 ② 「설계가 승인 난 후에도 너무 사람 확인을 많이 요구해 …
구조적으로 설계에 훨씬 암묵지까지 파고들 수 있게 집중하고 이후 구현은 진짜 필요한 확인
사항 아니면 알아서 진행」. **앞단은 더 깊게, 뒷단은 더 조용히**가 이 층의 한 문장이다.

### 11-1. 14칸을 왜 남겼나 — 불만의 과녁은 개수가 아니었다

「질문이 많다」로 읽고 칸을 줄이는 것이 가장 쉬운 오답이었다. 소유자에게 **「아는 칸은
건너뛸까요」를 두 번 물었고 두 번 다 거부**했다 — 14칸도, 파내기 후보도 「전부 묻기」다.
불만은 **묻는 양이 아니라 묻는 방식**(맥락 없는 기술 선택지·플러그인 내부 용어·추천 없는
나열)과 **승인 뒤의 반복 확인**이었다. 그래서 이 판은 **승인 전 질문을 하나도 줄이지
않았고**, 대신 ⑴ 한 칸씩을 **관련 칸 2~4개 묶음**으로 바꾸고(묶음 방식도 소유자가 스스로
제안했다) ⑵ 모든 질문에 뜻·추천·이유·맥락을 붙이고 ⑶ **줄인 것은 승인 뒤의 확인**이다.
줄일 것과 줄이면 안 되는 것을 가르는 선이 여기 있다 — **승인 전은 늘리고 승인 뒤는 줄인다.**

### 11-2. 파내기 — 인터뷰가 닫는 것은 「누군가 목록에 적어 둔 칸」뿐이다

14칸은 절차를 만든 사람이 물어야겠다고 **미리 안** 것이다. 실제 프로그램이 필요로 하는
결정은 그 목록 밖에 있다 — 대상이 0건이면 · 마감된 기간이면 · 두 번 돌리면 · 중단 뒤
재실행하면 · 누가 돌릴 수 있나 · 통화와 단위는 · 무엇을 로그로 남기나. 이것들이 안 물어진
채 남으면 **코드를 쓰는 쪽이 조용히 정한다.** 그 자리를 절차로 세운 것이 `interview-sweep.md`
(파내기)다: **지도 보여주기 → 빈자리 열거 → 전부 묻기 → 기록**.

설계상 중요한 셋.

**⑴ 확신은 건너뛸 권리가 아니라 추천을 붙일 권리다.** 초안은 「지도가 답을 강제하는 후보는
뺀다」는 조항을 달았는데, 독립 대조 리뷰가 그것이 **「전부 묻는다」를 한 단계 앞에서
무효화**한다고 잡았다 — 「답이 하나뿐」은 곧 에이전트의 확신이고, 뺀 후보는 종료 기준(열거된
것에만 걸린다)에도 닿지 않아 보이지 않게 사라진다. 지금은 **후보가 아예 하나도 안 나오는
종류**만 빠지고, 확신하는 자리는 추천을 붙여 묻고, 지도가 이미 말한 자리는 **확인 재진술**로
닫는다.

**⑵ 끝나는 기준이 관측 가능해야 한다.** 「충분히 물었다」는 세지 못한다. 기준은 **「내가
멋대로 정하면 안 될 결정이 하나도 안 남았다」** — 열거한 자리 전부가 사용자 확인으로 닫혔고
남은 것은 변수명·인클루드 분할처럼 사용자가 선호를 갖지 않는 기술 세부뿐일 때다. 이것이
모호도 점수(`≤5%`)를 대신한다. 점수는 **자기가 매기고 자기가 통과**시키는 게이트였다.

**⑶ 기록은 그 런이 이미 가진 산출물에 고정 토큰으로 든다.** `create-program`은
`interview.md`, `deep-interview`는 자기 브리프, 단독 `create-object`는 만들기 직전 확인문이
그 자리다(새 파일 0). 절 제목은 **언어 무관 고정 토큰 `<!-- SAPKIT:DIG-OUT -->`**이고
번역하지 않는다 — 계획 단계의 존재 검사가 **그 주석을 grep**하기 때문이다. 헤딩을 찾게 하면
사용자 언어로 렌더된 순간 「파내기가 없었다」로 읽힌다.

### 11-3. 승인 — 목록 단어에서 「뜻이 분명한 긍정」으로

옛 계약은 `승인`/`approve`/`proceed` 같은 **정해진 단어**만 승인으로 받았다. 사람은 그렇게
말하지 않는다(「좋아요, 이대로 만들어요」). 지금은 뜻이 분명한 긍정이면 승인이고, 애매하면
(「음… 일단 해봐」) **한 번만** 되묻고, 바꾸자는 말은 수정→재표시→재질문, 침묵·화제 전환은
승인이 아니다. 기록도 `approval_keyword`(enum)에서 **`approval_phrase`(사용자가 실제로 한 말
그대로)**로 바뀌었다 — 기록이 「목록의 어느 단어와 맞았는가」가 아니라 **「무엇이라고 했는가」**를
남긴다. **해시 결합과 재승인 규칙은 그대로다**: 설계문서가 바뀌면 해시가 바뀌고 다시 받는다.

그리고 **승인 답 자체가 「구현에 들어간다」는 명시적 진행 신호**다. SAP에 쓰는 단계로 들어가는
것을 사람이 허락하는 자리가 바로 그 답이고, 그 뒤에 별도의 「진행합니까?」는 없다. 모드 선택
화면(auto/manual/hybrid)은 그래서 폐지했다 — 능력은 남았고(「단계마다 확인받고 싶다」 한 줄로
옵트인), 사라진 것은 **고르라고 내미는 화면**이다.

### 11-4. 멈추는 자리는 닫힌 일곱이다 — 그래서 셀 수 있다

승인 뒤 기본은 **끝까지 진행**이다: 구현 → 자기 검사 → 독립 리뷰 → 되읽기 확인 → 완료 보고를
질문 없이 잇고, 단계마다 **한 줄 진행 보고**만 낸다(내는 것이지 묻는 것이 아니다). 멈추는
자리는 일곱이고 **그것이 전부**다 — 실데이터 조회 · 승인된 설계대로 안 되는 것 발견 · 자동
수리 한도 소진 · ECC에서 사람이 SE11로 활성화해야 하는 단계 · 사용자가 직접 해야 하는 단계 ·
위임을 명시했는데 작업자를 못 띄울 때 · 되읽기 대조가 어긋날 때.

**이송 생성이 이 목록에 없는 것은 P4를 푼 것이 아니다.** 새 이송은 기술 인터뷰에서 정해
**승인 전에 확정**되고 승인 기록이 그 번호를 요구한다 — 승인 뒤 구간에 「이송을 만들까요」라는
질문이 **남아 있지 않다.** 릴리스는 이 흐름 밖의 별도 절차다. 같은 이유로 남의 객체를
지우거나 고치는 일도 목록에 없다 — `create-program`은 승인된 설계가 이름 댄 것을 만들고,
기존 객체를 건드려야 한다는 발견은 **그 자체가 「설계대로 안 된다」**여서 이미 두 번째 항목이다.

실제 방어는 두 겹이다. ⑴ 위 일곱은 **절차가 멈추고 묻는 자리**이고 ⑵ 그와 별개로 **호스트의
허용 창이 계속 뜨는 자리**가 있다 — 부분 목록이 일부러 뺀 것들(삭제·이송 생명주기·런타임
실행·실데이터 2종). 진행 안내가 이 둘을 **나눠서** 말하는 것이 중요하다. 나누지 않으면
「이 일곱 말고는 아무것도 안 뜬다」가 거짓말이 된다.

### 11-5. 되읽기를 왜 자동으로 — 자동화한 것은 시점이지 기준이 아니다

`COMPLETE`의 정의는 **되읽기 기계 확인 + 독립 새-컨텍스트 리뷰** 둘 다이고 그대로다. 바뀐
것은 **되읽기를 언제 돌리느냐**다. 전에는 사람이 따로 불러야 했고, 그래서 안 불리면
`PROVISIONAL_WRITE`에서 멈춘 채 완료처럼 보고되는 자리가 생겼다. 지금은 리뷰 통과 뒤
`verify-applied`가 **자동으로** 돈다(읽기만 — 승인 불요). `verify-applied`의 「실행 전 명시
확인」은 **일반형 단서**로 풀었다: 부르는 절차가 이미 사용자 확인을 받은 오브젝트 목록으로
부를 때는 그 확인이 곧 이 확인이다(단독 호출은 현행대로 묻는다). 어긋나면 그 자리에서 멈춘다.
`create-object`도 같은 되읽기를 자동으로 하지만 **리뷰 게이트가 없으므로 COMPLETE는 아니고**
「임시 반영 + 되읽기 확인됨」으로 보고한다.

완료 기준을 검증 가능하게 만든 짝이 둘 더 있다 — OOP이고 완료 기준 예시가 있으면 **그 예시가
단위테스트 메서드의 최소 집합**이 되고(§F-1), 리뷰 체크리스트에 **§13 「완료 기준 대조」**가
생겼다(리뷰어가 「이 소스가 예시대로 동작하겠는가」를 한 항목으로 판정).

### 11-6. 권한 부분 목록 — 원칙은 「되돌리기 어려운 것은 계속 묻는다」

승인 직후 「한 번에 허용」이 병합하는 대상은 전체 템플릿이 아니라 **부분 목록**
`adapters/claude/permissions-build.json`이다. 템플릿에서 **필터로 파생**하고(손으로 유지하지
않는다) 제외 집합은 `Delete*` · `CreateTransport` · `ReleaseTransport` · **`CreatePackage`** ·
`Runtime*` · 실데이터 2종이다. **`CreatePackage`는 마감 리뷰가 더했다** — `AGENTS.md`의 P4
정의가 「package/request create」이고 표면에 `DeletePackage`가 없어 **우리 도구로 되돌릴 수
없다**(D-098 ①). 제외 원칙 「되돌리기 어려운 일」에 정확히 걸리는데 초판이 빠뜨렸다(D-151). 병합은 **추가만**(삭제·재정렬 금지 · 개수가 줄면 되돌림)이고 **메인 대화만**
한다(위임 작업자는 하지 않는다). Codex·Antigravity에는 sapkit이 병합할 허용 목록 기제가
없으므로 그 두 호스트에서는 **질문이 아니라 한 줄 안내**로 대체된다.

제외를 **도구 이름 패턴**으로 정한 것은 정직 유보를 하나 남긴다 — 그리고 그 방향은
**fail-open이다.** 파생은 「템플릿에서 제외 패턴에 걸리는 것만 뺀다」이므로, 목록을 다시 낼 때
**어느 패턴에도 안 걸리는 새 도구는 그대로 부분 목록에 실린다**(= 승인 직후 병합에 포함되어
허용 창 없이 지나간다). 안 실리는 쪽이 아니다. 그래서 **재생성 때 새 이름을 사람이 확인하는
것이 이 기제의 안전장치**이고, 그 문장을 생성물 `_comment`에 박아 두었다. 마감 리뷰가 이
서술이 거꾸로 적혀 있던 것을 잡았다(D-151).

### 11-7. 질문 형식 — 호스트가 선택 창을 주면 선택 창, 없으면 글

Claude Code(`AskUserQuestion`)와 Codex CLI(`request_user_input`)는 선택 창, Antigravity는 글.
한 창에 질문 ≤4, 질문당 선택지 ≤4(열린 칸 포함), 추천안은 첫 번째에 「(추천)」. 후보가 4를
넘는 칸(프로그램 유형·모듈 등)은 **추천 3 + 열린 칸으로 접고** 나머지는 질문 본문에 한 줄로
나열한다 — 자르는 것이 아니라 접는 것이다. 선택 창 호출이 실패하면 **즉시 같은 내용을 글로**
다시 묻는다.

**Codex 선택 창은 소유자가 직접 요청했다**(「코덱스도 선택창 띄울 수 있던데 되게 해줘」).
하네스 중립성을 이유로 빼지 않았고, 대신 유보를 정직하게 적는다 — `request_user_input`의
존재는 공개 자료로 확인했으나 **핀된 버전·모드에서의 동작은 미실측**이다. 어댑터 README가
그 주의점을 든다.

### 11-8. 읽기 시연 — 한국어 렌더링 한 벌

절차 문서는 **영어로 남고**(제품은 다국어) 에이전트가 사용자 언어로 렌더링한다. 그래서 「쉬운가」는
문서를 읽어서는 잴 수 없다. 아래는 `create-program`의 세 자리를 §A로 렌더링한 **한 벌의 예시**다
— 붙여넣을 대본이 아니라 **말의 높이를 재는 자**다.

**⑴ 설계 승인을 여쭙는 자리**

> 설계문서를 처음부터 끝까지 보여 드렸습니다. 이대로 만들까요?
>
> 이 답이 곧 시작 신호입니다 — 「네」라고 하시면 바로 SAP에 만들기 시작하고, 그 뒤로는 아래
> 적어 드릴 일곱 자리 말고는 여쭙지 않습니다. 고칠 곳이 있으면 지금 말씀해 주세요. 고쳐서
> 다시 보여 드리고 다시 여쭙겠습니다.

**⑵ 진행 안내 (여쭙는 것은 1번 하나뿐)**

> 시작하기 전에 네 가지만 알려 드립니다.
>
> **1. 만드는 동안 쓸 도구를 한 번에 허용해 둘까요?** (예 / 아니오)
> 허용해 두시면 만들고·고치고·활성화하는 동안 허용 창이 뜨지 않습니다. 허용하셔도 **지울
> 때 · 이송(transport — 개발 내용을 다른 시스템으로 옮기는 묶음)을 새로 만들거나 풀 때 ·
> 새 패키지를 만들 때 · 시스템에서 무언가를 실행할 때 · 실제 업무 데이터를 꺼낼 때**는 창이
> 그대로 뜹니다. 되돌리기 어려운 일이라 일부러 뺐습니다.
> (허용하신 뒤에도 창이 한 번 더 뜨면 그때 한 번만 허용해 주세요. 늦어도 다음 세션부터는
> 안 뜹니다.)
>
> **2. 곁에 계셔 주세요.** 제가 멈추고 여쭙는 자리는 이 일곱뿐입니다.
> · 실제 업무 데이터를 표에서 꺼내야 할 때 — 무엇을·어느 항목을·몇 줄까지 먼저 보여 드립니다
> · 승인하신 설계대로는 만들 수 없는 것이 나왔을 때
> · 혼자 고쳐 보는 횟수를 다 썼을 때
> · 옛 시스템이라 사람이 SE11(테이블·구조를 만드는 화면)에서 직접 활성화해야 할 때
> · 파일을 직접 들여오시는 것처럼 사용자만 할 수 있는 단계일 때
> · 따로 작업자에게 맡기라고 하셨는데 여기서는 띄울 수 없을 때
> · 만든 것을 다시 읽어 맞춰 봤는데 보낸 것과 다를 때
>
> **3. 단계마다 확인받으며 가고 싶으시면 지금 말씀해 주세요.** 그러면 매 단계 앞에서 여쭙습니다.
>
> **4. 만드는 사람과 넣는 방법.** 제가 따로 작업자에게 맡기고 결과를 검토하겠습니다(직접
> 하길 원하시면 말씀해 주세요). 만든 것은 SAP에 바로 넣습니다.

**⑶ 완료 보고**

> 다 됐습니다. **완료(COMPLETE — 만든 것을 SAP에서 다시 읽어 보낸 것과 맞춰 봤고, 다른 사람
> 눈으로 한 번 더 검토까지 통과했다는 뜻)**입니다.
>
> · 만든 것: 리포트 1개, 인클루드 7개, 선택화면 1개
> · 이송: DEVK900123
> · 검사: 구문 통과 · 활성화 완료 · 단위테스트 4개 전부 통과
> · 검토: 통과 (막는 지적 0 · 권고 2 — 둘 다 반영했습니다)
> · 걸린 시간: 약 40분
>
> 완료 기준으로 잡으신 두 가지도 그대로 동작합니다.
> · 대상이 0건이면 「대상 없음」만 알리고 끝난다 → 그렇게 됩니다
> · 마감된 기간을 넣으면 거부한다 → 그렇게 됩니다
>
> 이번에 알게 된 「미결 전표는 BSIK가 아니라 ACDOCA에서 본다」를 프로젝트 지식으로 남겨
> 둘까요? 다음에 같은 걸 다시 여쭙지 않아도 됩니다.

### 11-9. 경계와 정직 유보

**범위**: 구조 변경(파내기·승인·승인 후 자율 진행)은 `create-program` · `deep-interview` ·
`create-object` 셋에만 걸었다. 분석 절차와 `modify-object`는 **말투만** — 질문 순서·묶음·승인
루프는 손대지 않았다(그래서 `package-to-process`·`analyze-cbo-obj`가 여전히 한 칸씩 묻는
것은 결함이 아니라 설계다). **코드 변경 0** — 엔진·검사기·훅·MCP 도구 표면·번들 무접촉이고,
손댄 것은 게이트 2종과 권한 목록 파생뿐이다.

**정직 유보 넷.** ⑴ **실사용 0** — 새 인터뷰·승인·자율 진행은 이 판에서 한 번도 실행되지
않았다. 절차 규범이라 모델이 어길 수 있고(D-070 ⓑ와 같은 등급) **첫 create-program 실사용이
시험대**다. ⑵ Codex `request_user_input`의 핀된 버전·모드 동작은 **미실측**이며 Antigravity에는
선택 창이 없다고 **가정**한다. ⑶ `.claude/settings.local.json` 병합이 **같은 세션에 즉시
먹는지 미실측** — 안내문이 두 경우를 다 덮는다. ⑷ 권한 부분 목록의 제외는 **이름 패턴**이고 방향은
**fail-open이다** — 어느 패턴에도 안 걸리는 새 도구는 그대로 목록에 실린다. 재생성 때
새 이름을 사람이 확인하는 것이 안전장치다(§11-6 · 마감 리뷰가 잡았다 · D-151).
