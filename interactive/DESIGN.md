...
sc4sap-lite/
  ...
  adapters/                            ← 콘텐츠 복제 없음. manifest·정책 매핑·설치물만
    ...
    claude/permissions-build.json      ← **부분 목록**(판I · §11) — 설계 승인 직후 「한 번에
                                          ...
                                          `ReleaseTransport` · `Runtime*` · 실데이터 2종 —
...
않는다) 제외 집합은 `Delete*` · `CreateTransport` · `ReleaseTransport` · `Runtime*` ·
실데이터 2종이다. 병합은 **추가만**(삭제·재정렬 금지 · 개수가 줄면 되돌림)이고 **메인 대화만**
한다(위임 작업자는 하지 않는다). Codex·Antigravity에는 sapkit이 병합할 허용 목록 기제가
없으므로 그 두 호스트에서는 **질문이 아니라 한 줄 안내**로 대체된다.

제외를 **도구 이름 패턴**으로 정한 것은 정직 유보를 하나 남긴다 — 목록을 다시 낼 때 새 도구가
패턴 밖으로 새면 그 도구는 **부분 목록에 안 실릴 뿐**이고(허용 창이 뜬다) 열리지는 않는다.
방향이 fail-closed라 택했고, 그래도 재생성 때 새 이름을 확인하라는 문장을 생성물 `_comment`에
박아 두었다.
...
> 실제 업무 데이터를 꺼낼 때**는 창이 그대로 뜹니다. 되돌리기 어려운 일이라 일부러 뺐습니다.
...
먹는지 미실측** — 안내문이 두 경우를 다 덮는다. ⑷ 권한 부분 목록의 제외는 **이름 패턴**이라
새 도구가 패턴 밖으로 새면 목록에 안 실릴 뿐이다(fail-closed 방향).
