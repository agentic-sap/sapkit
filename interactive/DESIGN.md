...
에이전트가 밟는 모든 걸음은 **P0 오프라인**이고, **SAP을 바꾸는 행위는 P3이며 그것은
사용자의 것**이다. 아무것도 접속돼 있지 않으므로 **MCP 경로를 지키는 서버측 tier 게이트가
