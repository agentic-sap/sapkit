...
**2026-08-23 · 재저작이 끝났다 (D-109~D-112).** 상류 고지 1건(`interactive/LICENSE`)은
`interactive/` **서브트리 전체**에 걸린 채 남는다 — 파일 목록으로 좁히려던 두 번의
시도가 리뷰에서 물러났고(D-104 · D-106), 재저작 완주가 그 이유를 확정했다: 상류
저작물로 남은 것은 **파일이 아니라 산재한 계약행**(~600행/40파일 — 참조표 · 출력
템플릿 · print-verbatim 블록 · 사용자 노출 문구 · 승인 키워드)이라 경로로 표현할 수
없다. 파일 단위 차용은 이제 **`server/sap-assets/` 30**(영구 존치 · D-079 ⑥)뿐이다.
