...
`troubleshooting.md` 11)을 2026-08-24에 재저작했고, 위 §3.4 수치가 그만큼 내려간 것이다.
~~파일 단위 차용은 **`server/sap-assets/` 30**(영구 존치 · D-079 ⑥)뿐이다.~~
**정정 (2026-09-09 · D-130~D-136)** — **파일 단위 차용은 이제 0이다.** 그 마지막 한 갈래였던
`server/sap-assets/`가 2026-08-23~24에 **자작 재저작·개명**됐다(아래 표). 그러므로 상류
저작물로 남는 것은 **위의 산재 계약행뿐**이고, 그것이 `interactive/LICENSE`가 서브트리
전체에 남는 유일한 이유다(D-112 ⓐⓓ).
...
| `server/sap-assets/` — **30파일** (2026-09-09 실측 · `.abap` 22 + `.xml` 7 + `README.md` 1 · 하위 `alv-oop-handlers/` 포함 · 이 디렉터리만 세고 `interactive/`의 다른 곳은 세지 않는다) | ~~`hjaewon/sc4sap-custom` 차용분. …이식 커밋 `038085c2` 이후 **변경 커밋 0**~~ → **정정 (2026-09-09)**: **자체 저작이다.** 2026-08-23 판S1~S5가 **자산 29파일 7,087행을 참조-재저작하고 개명**했다(`ZMCP_ADT_*` → `ZSAPKIT_ADT_*` · `Z*_S4SAP_*` → `Z*_SAPKIT_*` · D-130·D-131). 이후 **SAP 실기**가 9종 설치·활성화를 확인했고(D-132), 실기가 드러낸 결함 3건을 수리해 **런타임 왕복까지 실증**했으며(D-134), 오배치된 수리 1건을 레포에서 되돌렸다(D-136). 30번째 파일은 이 디렉터리 자신의 `README.md`(설치 안내)로 재저작 명부 대상이 아니고 이식 이후 판Z·판S5가 갱신했다 | ~~MIT — `interactive/LICENSE`에 고지 승계. **영구 존치** — D-079 ⑥(SAP 오브젝트 무접촉)이 정했고 D-103 ⓓ가 끝그림에 명시했다~~ → **정정 (2026-09-09)**: MIT (루트 LICENSE). **이 갈래는 더 이상 고지의 존재 이유가 아니다** — D-130 ⓑ가 D-079 ⑥과 D-103 ⓓ를 함께 뒤집었다. 상류와 축자로 겹치는 것은 **달라질 수 없는 계약행뿐**이고 그 내역은 `interactive/LICENSE`의 표가 든다 |
