...
> ⚠ **여기 있는 것은 「공방 훅」이다** — 이 레포에서 작업하는 **개발 세션**을 지킨다.
> 제품이 최종 사용자 머신에 까는 **어댑터 훅**(`interactive/adapters/claude/hooks/`)은
> 전혀 다른 층이고 **배포 기본이 0개**다(D-062 — 선택 기능 · 스위치 보존). 그 「훅 0개
> 기본」을 결함으로 보고 되돌리지 말 것(`CLAUDE.md` 안전 규칙). 이 파일에 3개가 배선돼
> 있다는 사실은 그 정책과 **무관하다.**
