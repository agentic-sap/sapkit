...
- `session-start-context.py` — `SessionStart`(`startup|resume`). 현재 브랜치/변경 파일에 더해, 실행 대기 중인 phase(pending)·조치 필요 phase(error/blocked)·미조사 엔진 LESSONS·docs 갱신 검토(완료 phase가 docs/*.md 최신 갱신보다 새로운 경우, harness-docs 안내)를 컨텍스트에 주입해 세션 시작 시 사용자에게 "지금 할 일"을 알린다. 강제 계층이 아닌 안내용 편의 훅이며, 엔진의 무인 세션(`HARNESS_RUN=1`)에는 아무것도 주입하지 않는다.
