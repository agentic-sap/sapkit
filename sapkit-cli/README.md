...
승계 규율은 「계약 표면(명령 이름·규칙 키·심각도·플래그·JSON 형태)은 기능 계약이므로
그대로, 메시지·설명 **문구는 새로**」였다. 참고서로 쓰던 `vsp/` 소스는 **은퇴해 레포에
없다**(읽기 전용 참고서·복사 금지 규율은 그 시기의 것). **그래서 지금부터 판정을 바꾸는
변경은 `fixtures/baseline/` 채록본과 `DIVERGENCES.md`로만 정당화된다** — 구 구현을 다시
읽어 확인할 길이 없으므로, 채록본과 갈리는 변경은 등재 없이는 결함이다.
