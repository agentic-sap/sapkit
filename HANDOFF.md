...
>
> **⚠ 판6.3이 사용자에게 남긴 숙제 둘 — SAP에서 직접 정리해야 한다.**
> P4 두 종은 우리 도구로 지울 수 없다(표면 186종에 `DeleteTransport`·`DeletePackage`가 없다).
>
> | 남은 것 | 상태 | 정리 |
> |---|---|---|
> | 이송요청 `DEVK901065` (+ 태스크 `DEVK901066`) | **미해제**(status `D`) · 객체 **0건** · target `로컬변경요청` | **SE01** — 미해제 요청 삭제 |
> | 패키지 `ZSAPKIT_63_PKG` | **반쯤 만들어졌다** — 이름 검증은 「이미 있다」, `GetPackageContents`는 빈 목록(오류 아님), 그런데 `GetPackage`·`SearchObject`는 못 찾는다 | **SE80/SE21** — 실재 확인 후 삭제 |
>
> **연습 객체 잔재는 0**이고 **보호 자산 8종은 세 번 조회 모두 무사**하다. P2 0건.
>
