...
> ⓖ **같은 자리에서 세 번 걸렸다** — D-102 ⓐ(「안 본 범주가 또 있는가」) → D-104 ⓒ(판8이
> 그것을 인용해 두고도 transform을 빠뜨림) → **D-106 ⓓ**(회수마저 도출 누락). 유보를 넓게
> 쓰는 것만으로는 부족했고 **방법을 바꾸는 것**이 답이었다(역방향 스캔).
...
> 움직인다. **코드 리뷰로만 확인했다.**
> ⓙ **판10의 독립 새-컨텍스트 리뷰가 아직**이다(D-107 유보 ⓐ) — 생성기·게이트·재저작본은
> **새 코드**이고, 그 검증은 음성시험 9종 + 사보타주 실측 + 그 리뷰가 나눠 진다.
>
