...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판M2-a` (⑴ M2 오프라인 배선 · 새 D-결정 필요 · 오프라인)**
>
...
>
> **판M2-a는 배선 판이다** — `--mcp=<destination>`의 **destination/UAA 토큰을 기동 경로에
> 잇는다.** 코드는 판5에서 토큰 계층까지 이미 섰고(D-091), 남은 자리는 **기동이 취득을
> 돌리지 않는 것**이다. ⚠ **착수 전에 새 D-결정이 필요하다** — D-091 ⓑ가 그은 **정지선을
> 뒤집는 판**이기 때문이다. **오프라인 판**이고, 실접속은 **판M2-b**(BTP 계정 — 사용자만
> 답을 가진 것)의 몫이다. 브리프는 `docs/RUN-PLAN.md` 판M2-a.
>
> 1. `git fetch` + `main..origin/main` 확인(재개 전 원격 대조 — 안전 규칙)
> 2. **판11의 회수는 이미 끝났다**(D-113 — R-FAIL · 차단 2 인정 · 수치·문장 정정 · ⑷ 완료
>    판정 유지). 경량 재리뷰까지 끝났고(닫는 조건 반영 `54c10a37`),
>    **그것이 판M2-a보다 앞선다.**
> 3. **착수 기준선을 네 묶음 전부** 뜬다(제품 게이트 9 + 음성시험 9 + `sapkit-engine`
...
> 4. 판M2-a를 잡는다 — **새 D-결정을 먼저 낸다**(D-091 ⓑ 정지선을 뒤집는 판이므로).
>
...
> **판M2-a**(**다음** · ⑴ M2 오프라인 배선 · **새 D-결정 필요** · 오프라인) ·
> 판M2-b(**BTP 계정 — 사용자만 답을 가진 것**). **별건 5**(판 아님): `atomic-write` 배선 ·
...
>
> **묻는 자리는 마지막 하나** — 병합·push. 그 밖은 정하고 간다.
>
...
> | **도구** | **186** | **186** — 전량 등록 |
> | 번들 무게 | **3,813,330 B** (구 8,304,513 B의 **46%**) | — |
...
interactive/
  ...
  server/                       ← MCP 번들(sapkit-engine 1.0.0) + keyring + tool-catalog + sap-assets + UPDATE-RUNBOOK
