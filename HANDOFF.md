...
> ═══════════════════════════════════════════════════════════════════
>
> **▶ 현재 착수점 (2026-07-31 갱신) = 백로그 ⑥ 집행 2단계 — 프로세스 계층 3건**
> (`core/knowledge/modules/common/document-flows.md` · `core/procedures/package-to-process.md`
> 재작성 · 15줄 스킬 래퍼). 재작성 규범은 **D-053 ④**에 이미 못박혀 있다(메인 컨텍스트가
> 전 단계 소유 · `Agent(...)` 디스패치 0 · 모델 배너/frontmatter 제거 = 3사 중립 · 살릴 것
> 7종 명시). **BPML은 범위 밖**(재검토 트리거 2개 — D-053 ②). **`trust-session`은 만들지
> 않는다**(권한 템플릿 187건이 Layer 1을 이미 대체 — D-053 ③).
>
...
>   E2E 체크리스트 마지막 줄에 등재.
> ⑥ **상류 `package-to-process` 이식 — 범위 확정 ✅(D-053) · 1단계 도구층 ✅ 집행 완료
>   (07-31 · D-055 · v0.4.2) · ★남은 착수점 = 2단계 프로세스 계층 3건**.
...
>   매니페스트에 **제2출처** 기록 + `tools/spec/` 트리 재핀(5→7파일). 잔여 = `render-md-images`
>   미배선(D-055 ⓓ).
