...
> 작성 2026-07-10 · 최종 갱신 2026-08-13. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
> 정확한 표현이다.
>
> **2026-08-13부터 이 남은 양에 눈금이 생겼다.** 사다리 ⑴의 마일스톤 체계
> **M1 잔여 ~ M6**을 확정했다(D-081) — 위 표의 각 축이 어느 M에서 닫히는지가
> `docs/BLUEPRINT.md` 사다리 ⑴ 절의 「마일스톤 체계」 표에 적혀 있다.
> **M1은 아직 열려 있다** — 아래 ①이 그 잔여다.
>
> ## 🔻 내일 첫 작업 (2026-08-13 세션 종료 시점)
>
> ### ⓪ 이건 끝났다 — **M2 이후 마일스톤 설계 (D-081)**
>
> **다시 하지 말 것.** 사다리 ⑴의 **M1 잔여 ~ M6**을 확정했고, 각 M의 범위·완료
> 판정과 **전 M 공통 완료 요건 6개**, M6의 제작 금지 원칙과 예외까지 들어갔다.
> 정본은 `docs/BLUEPRINT.md` 사다리 ⑴ 절의 「마일스톤 체계」 표이고, 결정의 '왜'는
> `docs/reference/DECISIONS.md`의 **D-081**이다. 없는 번호를 가리키던
> `sapkit-engine/harness/DIVERGENCES.md`의 두 줄도 함께 결선했다 —
> **D8 → M6**(교체 직전 관문) · **D15 → M2**(접속 방식 확장).
>
> 순서는 **M1 → M2 → M3 → M4 → M5 → M6으로 고정**됐다. 그리고 D-081은
> **D-079를 뒤집지 않는다** — 도구 186종 전량 승계·개명 금지는 그대로다.
>
> ### ① 먼저 — **M1 잔여 둘을 닫는다** (M2는 그 뒤다)
>
