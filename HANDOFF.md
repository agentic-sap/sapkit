...
> 작성 2026-07-10 · 최종 갱신 2026-08-12. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
> 정확한 표현이다.
>
> ## 🔻 내일 첫 작업 (2026-08-13 세션 종료 시점)
>
> ### ⓪ 먼저 답할 것 — **D-081과 M-표가 `main`에 없다** ← 착수 전 사용자 판단 필요
>
> **실측(2026-08-13):** `main`(= `b52afc5`)의 결정 로그 마지막 항목은 **D-080**이고,
> `docs/BLUEPRINT.md`에 「M1/M2/M6」 문자열이 **0건**이다. **D-081과 사다리 ⑴ M-표는
> 미머지 가지 `dryforge/tmp-practice-scenarios`(head `c5088d3`)에만 있다** —
> PR #2가 그 커밋들보다 **먼저** 머지돼서 D-081 정직 유보 ⓔ의 경고가 **반대 방향으로**
> 실현됐다. (검증: `git merge-base --is-ancestor c5088d3 main` → 아니오)
>
> 이 판(`.dryforge/spec.md` §2)은 D-081·M-표를 전제로 그것과의 충돌을 해소하는 구조다.
> 그대로 두면 **T35·T38이 고칠 표가 없고**, D-082가 실재하지 않는 항목을 인용한다.
>
> **택일**: ⓐ `dryforge/tmp-practice-scenarios`를 작업 가지에 먼저 합류시킨다(권장 —
> 결정 로그 꼬리에서 D-081↔D-082 충돌 1건 예상, 해소는 "D-081 다음 D-082"로 자명) /
> ⓑ 합류하지 않는다 → **D-082를 D-081로 개번하고 인용을 다시 쓴다**.
>
> ### ① 그다음 — **오프라인 대량 제작 판 (3-doc 실행 중)**
>
> - **작업 가지**: `dryforge/offline-bulk-build` (`main`에서 분기 · **push 안 함**)
> - **3-doc**: `.dryforge/{handoff,spec,plan}.md` — 과제 38개 · 6단계.
>   `/dryforge:go` 로 이어서 실행한다.
> - **끝난 것 (1건)**: **T37** — `docs/reference/DECISIONS.md`에 **D-082** append
>   (마일스톤 완료를 `짓기 완료`/`증거 완료` 두 단계로 분리 · 판 단위 순차 확정).
>   커밋 `c4a8e73`, 머지 `c2f87fa`. **D-081 번호는 비워 뒀다**(위 ⓪).
> - **내일 첫 묶음 (0단계 나머지 4건 — 병렬 가능)**: **T38**(실사용 축 재측정 →
>   `BLUEPRINT.md` §4.7에 도구별 표 · §4.7은 base에 실재) · **T2**(구 번들 표면을
>   19종 → **186종 전량 + 4조건 이름 집합**으로 채록) · **T3**(증거 급 넷째 추가 ·
>   상태 3칸 · `replay-attended.mjs` 커버리지 배선) · **T6**(하네스 사전 검사를
>   도구 선언에서 읽게).
>   그다음 T4(대장) → T5(표면 게이트+CI) → T7·T8. **0단계가 전부 끝나기 전에는
>   도구를 짓지 않는다.**
> - **기준선 (2026-08-13 실측, 착수 전)**: `sapkit-engine` build 0 · typecheck 0 ·
>   **jest 816 passed / 1 skipped** · 자체 게이트 전종 통과 · **기존 제품 게이트
>   8종 전부 exit 0**. 내일 뭔가 빨가면 **이 판이 낸 것**이다.
...
> - **남은 수 (대장 생기기 전 · 문서 기준)**: 도구 **19 / 186** · 전송 1/3 ·
>   RFC 1/5 · 인증 Basic만. 이 판은 **증거를 하나도 얻지 않는다** — 끝나도
>   `지음·증거 대기`가 수백 종 남는 것이 정상이다.
>
> ### ② 보류 — M2 이후 마일스톤 설계 (D-081이 이미 함 · 위 ⓪이 정리되면 소멸)
>
> D-081(미머지 가지에 있음)이 M2~M6 경계·완료 판정·D8/D15/D18 배정·꼬리 49종
> 재평가 위치를 **이미 다 정했다.** 위 ⓪에서 그 가지를 합류시키면 이 항목은
> 그대로 소멸한다. 합류하지 않기로 하면 그때 다시 설계해야 한다.
>
> ### ③ 미룬 것 — 남은 12종 C1 시나리오 확장 (읽기 계열 6종부터 · **SAP 접속 필요**)
>
...
> **이건 attended 세션 몫이다** — 지금 도는 판은 SAP에 붙지 않는다(D-082 ②·③).
>
> ```powershell
> cd sapkit-engine
...
> ```
>
> - **가지**: `dryforge/tmp-practice-scenarios` — **PR #2는 머지됐다**(머지 커밋
>   `b52afc5`). 다만 그 뒤에 D-081·M-표 커밋 7건이 **그 가지에만 더 쌓였고 미머지**다
>   (head `c5088d3`) → 위 ⓪.
