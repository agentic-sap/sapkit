...
>
> ## ✅ 직전 판 — **판3.5: ⑶-b 5차 배치 — 1-A 모듈 후반부 48편 (재저작 35 + 검증 체크 13)** (2026-08-18)
>
> **1-A 모듈 91편이 소진됐다 — 코드 0줄, 지식 `.md`만.** D-087 ⑧의 **소비 실증** 기준으로
> **Ariba·BW·HCM·PS·QM·TM·TR·WM 8모듈 × 6편 = 48편**을 처리했다. 처리는 판3.4와 같이 두
> 갈래로 갈렸다 — **재저작 35편 + 검증 체크(무변경) 13편**(spro 7 · tcodes 6)이고, 무변경
> 13편은 **사양이 미리 지목한 후보 명단과 정확히 일치**했다(근사-무변경 → 재저작 전환 **0건**).
> **새 D-결정은 없다** — 전부 **D-087·D-088의 적용**이다. **잔량 77 → 29**(명부
> `docs/reference/copy-baseline.md`). 명부 1부에 남은 것은 **1-E 견본 5**뿐이다.
>
> - **⚠ 회수 0 — 판3.1~3.4는 매 판 회수가 났다**(3회 · 9곳 · 12곳 · 12곳). 이번 판은 그
>   **실패 모드와 판정 폭을 재저작 프롬프트에 선재 주입**해서 회수가 **0**이 됐다. **후속 판은
>   이 주입을 승계할 것** — 효과가 실측된 유일한 예방책이다.
> - **검증**: 독립 새-컨텍스트 리뷰 **6건 전원 PASS** — **전수 대조 4그룹 48/48**
>   (RA=Ariba+BW · RB=HCM+PS · RC=QM+TM · RD=TR+WM · 누락 0 · 추가 0 양방향 · 식별자·수치·
>   한글 다중집합 기계 대조 · **Get\* 동결 55곳이 행 좌표까지 동일** · blockquote·fence·
>   코드스팬 sha256 동일) · **RE 총괄 재검 13/13 CONFIRM**(blob **3중 대조** · **477 데이터
>   행 전수** 재분류 · 어휘 전수 덤프 EN 530 · KR 364 토큰) · **RF 통합**(소비 표면
>   **14지점 유효** · 새로 깨진 것 0 · **자작 경고 블록 8편 무접촉**). 제품 게이트 9종 +
>   doctor 전부 exit 0 · 지식 `.md` **148 유지** · 버전 무변.
> - **⚠ 판3.4 체크 완료분에 잔존 1건 — 판3.6이 처리한다**: `SD/bapi.md:31`의
>   `Internal FM: read pricing procedure results` 셀이 **미재저작으로 남아 있다**(판3.4 회수
>   2차 `d24e18a4`가 같은 파일 `:7`은 고치고 이 셀을 누락한 것으로 추정). 판3.5는 **체크 완료
>   93편 무접촉 계약**이라 손대지 않았다. 명부 체크는 이미 서 있으므로 수리 커밋 1건이면 닫힌다.
> - **⚠ 참조표 사실 의심 밀도 경보 (SAP 접속 필요 — 판6 이후 소관)**: BW·QM·TR·TM·WM의
>   참조표는 의심 밀도가 판3.4 배치보다 뚜렷히 높다(BW spro 33/34 무대응 + 내부 모순 5건 ·
>   QM spro 뷰명 계통 의심 · TR spro 「stem+1글자」 패턴 생성 의심 · TM `V_TM*` 27행 ·
>   **WM bapi↔tables의 TO/TR 테이블 쌍 역전 — 2워커 교차 확증**). **문장 재저작과 별개의 사실
>   검증 백로그 후보**이고 전부 원문 보존했다. 계열별 상세는 `docs/RUN-PLAN.md` 판3.5 브리프.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1~3.4에서 승계, 이 판도 동일)**: 판3.5는 **버전을
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.6: ⑶-b 6차 배치 — 잔량 29 (1-E 견본 5 + 2부 24)**
>
> **잔량 29가 명부의 마지막 두 갈래다** — **1-E 견본 5**(`.abap` 코드) + **2부 24**(지식 폴더
> 밖 copy). ⚠ **둘 다 그 갈래를 여는 판이 계약을 먼저 정한다**(판3.3 브리프의 명문 승계) —
> **산문 재저작 계약(D-088)이 그대로 적용되지 않는다.** 판3.6의 `/ready`는 **어느 갈래를 열지 +
> 그 갈래의 계약**을 먼저 물어야 한다.
>
...
> 판5까지가 상한이고 그 뒤는 기본 규칙 복귀).
>
> **계수 방식·체크 규칙·선정 기준은 `D-087`, 표 지배 참조 파일 규칙은 `D-088`이 확정했고**
> 명부(`docs/reference/copy-baseline.md`)가 그 집행 자리다. **잔량 29/170**(체크 누계 141 ·
> **1부는 1-E 5편만 남았다**).
>
> - **1-E 견본 5** — `.abap` **코드 파일**이라 재저작이 곧 **생성 코드 골격 변경**이 되고,
>   그러면 판4급 **SAP 확인(판6 이연)** 이 걸린다. D-088의 「표 셀 분류」는 여기 쓸 수 없다.
> - **2부 24 — 성격이 셋으로 갈린다**: ⓐ **훅 4 + 사양서 도구 5 + 조회 도구 2 = 11편은
>   `.mjs` 실행 코드**이므로 **동작 계약 유지가 필수**다(`tools/fetch/` 2종은
>   `core/procedures/help-portal-fetch.md`가 기대하는 계약 · 훅 4종은 PreToolUse 안전 배선) ·
>   ⓑ **데이터보호 정책 12는 `.md`**라 **D-088 적용이 가능**하다(2부 중 유일하게 산문 계약이
>   그대로 서는 갈래 — **판3.6의 유력 배치 후보**) · ⓒ **xlsx 1은 바이너리**로 **재제작 방법
>   자체가 미정**이다.
> - **⚠ 먼저 닫을 잔존 1건**: `SD/bapi.md:31`의 `Internal FM: read pricing procedure results`
>   셀(판3.4 체크 완료분 · 회수 2차 `d24e18a4` 누락 추정). 명부 체크는 이미 서 있으므로
>   **수리 커밋 1건**이면 닫힌다 — 판3.6이 처리한다.
> - **이월 전량 승계 + 판3.5 신규 등재**: 소비자 오인용 4건 · 사실 의심 2건 · 제품명 표기
>   갈림(`SAPKIT` vs `sc4sap`) · **이식 전 참조 보존 목록 21곳**(판3.5가 `Ariba/tables.md:43-44`
>   1곳 추가 · ⚠ 그중 1곳은 소비자 오인용 ④와 같은 건 — **이중 계수 금지**) · 소비자 서술
>   불일치 4건 · 헤딩 재작성 기록 2건 · 판3.3·판3.4 등재 사실 의심 · 배치 내 기존 불일치 5건 ·
>   재저작 구조 기록 2건 — 여기에 **판3.5가 새로 등재한 것**(**낡은 도구명 동결 55곳** = 1-A
>   전체 실측치로 판3.4의 36곳을 대체 · **참조표 사실 의심 밀도 경보 BW·QM·TR·TM·WM** ·
>   `BW/workflows:9` `AEDАТ` 키릴 동형 이체자 · `PS/workflows:82` DDLS 행 무접촉 권고 ·
>   **KR 「전기/계상」 갈림은 이 배치에서 재현되지 않아 열린 채**)가 더해진다.
>   **상세는 `docs/RUN-PLAN.md` 판3.5 브리프의 「판3.6 재료」.**
> - **⚠ 재저작의 함정 — 판3.1~3.4 넷 다 밟았고, 판3.5가 처음 0으로 막았다.** 「사실을 잃지
>   않았는가」만 재면 부족하다. 판3.1은 저자가 설명을 돕겠다고 넣은 문장이 **원문에 없던 SAP
>   단언·지시**가 돼 5편 전부에서 **세 차례 회수**했고, 판3.2는 **강도·범위 미세 이탈 9곳**
>   (`bcbe36f6`), 판3.3은 **강도·프레이밍·표기 12곳**을 두 차례(`05643eda` 10곳 · `41110412`
>   2곳), 판3.4는 **규범 voice·과잉 환언·어미체 이탈 12곳**을 두 차례(`88165eeb` 9곳 ·
>   `d24e18a4` 3곳) 회수했다. **판3.5는 그 실패 모드와 판정 폭을 재저작 프롬프트에 선재
>   주입해 회수 0을 달성했다 — 후속 판은 이 주입을 승계할 것.** 재저작은 표현·구성만 바꾸는
>   일이므로 **더하는 것도 범위 밖**이고, 리뷰 근거는 **누락 0 + 추가 0** 양방향으로 잰다.
>   명부 §2에 명문화돼 있다.
...
> 든다). 시작 명령: **`/dryforge:ready docs/RUN-PLAN.md 판3.6`** → 3-doc 승인 후 같은
