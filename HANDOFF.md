...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판6.3` (attended · 사용자 참여 필요)**
>
> **교체는 끝났다** (2026-08-19 · 판7-b · D-095/D-096). 제품 번들
> `interactive/server/server.bundle.cjs`는 이제 **자체 저작 `sapkit-engine` 1.0.0**의
> 산출물이다.
>
> **그런데 다음이 판7.5(`engine/` 은퇴)가 아니다.** BLUEPRINT §3.1 ③이 못 박는다 —
> 「교체는 되돌릴 수 있어야 한다. 전환 후 **최소 1회의 실사용 사이클** 동안 구 부품을
> 남겨 두고, 그 사이클이 무사하면 ④(은퇴)로 간다.」 **판7-b는 SAP에 한 번도 접속하지
> 않았다.** 그러므로 실사용 사이클이 먼저이고, 그 자리가 **판6.3**이다(`$TMP`에서
> `Create*`→`Delete*` 왕복 46종 — attended 실기가 곧 실사용이고 대장도 함께 줄어든다).
> **이 순서는 판7-b가 정했다**(D-096 ⓔ) — 판 큐도 그렇게 세워 뒀다.
>
...
> 3. `/dryforge:ready`로 **판6.3**을 잡고 `go`로 실행한다.
>    **브리프는 `docs/RUN-PLAN.md`에 있다**(2026-08-19 · D-097로 신설) — 대상 46종
>    전량과 **착수 전에 알아야 할 함정 넷**이 거기 든다. 함정 ①(`CreateTransport`·
>    `CreatePackage`가 **P4**라 `$TMP`에서 안 끝난다)은 **착수 시 사용자에게 먼저 물을
>    자리**다.
>
> ⚠ **판6.3은 SAP write를 동반하는 attended 판이다** — 안전 정본이 사람 승인을 요구하는
> 자리가 그 안에 있다(P3 · `$TMP` 밖 객체 금지 · 되돌릴 수 없는 삭제). 사용자가 붙어
> 있어야 하고, 자동 주행하지 말 것.
>
> **판6.3이 끝나면 판7.5**(`engine/` 은퇴). 그때 함께 판정할 것은 판7.5 브리프에 적어 뒀다 —
> 은퇴가 **채록기와 CI 롤백 확인을 함께 데려간다**는 것, 그리고 **롤백 경로가 바뀐다**는 것.
>
> **큐 전체**(2026-08-19 정리 · D-097): **판6.3**(다음 · attended) → 판7.5 → 판8 →
> 판9 · 그리고 판M2-a(새 D-결정 필요) · 판M2-b(BTP 계정 — 사용자 결정). 브리프는 전부
> `docs/RUN-PLAN.md`에 있다. **판9는 이번에 신설**했다 — 꼬리 49종 재평가의 「최종 교체
> 직전 1회」가 **교체로 트리거를 잃어** 갈 곳이 없던 것을 옮긴 자리다(M6의 D8 관찰은
> 판6.3으로 갔다).
>
