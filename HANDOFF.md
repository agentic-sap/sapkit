...
> 작성 2026-07-10 · 최종 갱신 2026-08-20. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판7.5` (`engine/` 은퇴 · 오프라인)**
>
> **판6.3이 끝났다** (2026-08-20 · **D-098**). 그 판이 판7.5의 전제였던
> **「전환 후 최소 1회의 실사용 사이클」을 세웠다**(BLUEPRINT §3.1 ③ · D-096 ⓔ).
> 판7-b는 SAP에 한 번도 접속하지 않았고, 판6.3이 **신 엔진으로 실 SAP을 밀었다.**
>
> **근거를 이름으로 적는다** — `sapkit-engine/fixtures/attended-only/zsapkit63-domain-dataelement.json`의
> **0번 단계 `CreateDomain`**(`engine.name: "sapkit-engine"` · `1.0.0` · `$TMP`에
> `ZSAPKIT_63_DOM`을 `status: "active"`로 생성, 1번 단계가 되읽고 6번 단계가 삭제).
> 픽스처 9편 · 107단계 전부 같은 엔진이다. **「돌았다」가 아니라 이 파일의 이 단계다.**
>
...
>    ⚠ `doctor`는 **exit 1이 정상 상태**다 — 이 머신의 Antigravity 설치본(1.1.1)이 고정값
>    (1.1.4)과 달라서이고, 판6.3 착수·마감 실측에서 **동일**했다. 레포 문제가 아니다.
> 3. `/dryforge:ready`로 **판7.5**를 잡고 `go`로 실행한다. 브리프는 `docs/RUN-PLAN.md`.
>
...
>
> **큐 전체**(2026-08-20 갱신): **판7.5**(다음) → 판8 → 판9 · 그리고 판M2-a(새 D-결정
> 필요) · 판M2-b(BTP 계정 — 사용자 결정). **판9의 전제도 판6.3이 채웠다**(실사용 분포 갱신).
...
> | 5 · 그때까지 **구 번들이 현역** | **지켜졌고, 형태가 바뀌었다** | 유지 기간이 교체 이후로 연장됐다(D-093 ⓐ) — 현역 자리는 넘겼고 **소스 `engine/`이 롤백 자산으로** 남는다. 은퇴는 판7.5 |
>
...
>
> **▶ 다음 세션이 먼저 볼 것**: **`판7.5`**(`engine/` 은퇴) — 단, 그 전제인 「교체가
> 실사용에서 안정」은 **이 판이 재지 못했다**(SAP 접속 0회). 최상단 재개점 참조.
>
