...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판7-b` 착수 (묻지 말고)**
>
> **전제는 충족됐다.** 검증 기준 1~4가 전부 서 있고(아래 🚦 표), 판7-b는 **SAP 접속이
> 필요 없는 오프라인 판**이다. `docs/RUN-PLAN.md` 판 큐가 이미 `다음`으로 세워 뒀다.
> **「이제 교체할까요?」를 묻지 마라 — 큐가 답한 질문이다**(CLAUDE.md 「판 실행 규약」).
>
...
> 3. `/dryforge:ready`로 **판7-b**를 잡고 `go`로 실행한다.
>
> **판7-b가 반드시 판정해야 할 것 둘** (판7-a 최종 리뷰가 짚었고 RUN-PLAN 판7-a 브리프에
> 전문이 있다):
> - **① 안전에 닿는다** — 교체하면 `interactive/adapters/claude/hooks/README.md`의
>   「blocklist 노브 3종은 활성 프로파일 `sap.env`에만 유효하다」가 **거짓이 된다**(신
>   엔진은 프로세스 env도 받는다 · D6). 문서 한 줄 문제가 아니다: **D-043의 소유자 머신
>   예외가 그 「서버측 blocklist 하한」에 기대고 있어** 교체는 그 하한을 **누가 설정할 수
>   있는가**를 바꾼다.
> - **② 게이트 기대값 구조** — `checkRecordedGap`의 `recorded`가 대상 범위 밖이라 신
>   엔진이 D6 수리를 도로 잃어도 조용히 초록이 된다. `byTarget.bundle`로 옮기면 닫히는데
>   **집행하면 구 번들 무인자 출력이 바뀐다**(그 시점엔 교체 중이라 무방하나, 모르고 하면
>   무접촉 증명의 기준선이 조용히 움직인다).
>
