...
>
> **판7.5가 끝났다** (2026-08-22 · **D-101** · 은퇴 커밋 **`843a6aba`** + 재핀
> `ac26030b` · **push 완료** · CI 전종 green). **구 포크 `engine/`이 레포를 떠났다** —
> 추적 파일 **1,604개 삭제 + 1건 이관**(482,714줄). 루트 `LICENSE`의 상류 고지 표가 **2행 → 1행**이 됐고, 남는
> 1건이 바로 판8이 다룰 `interactive/LICENSE`다. 검증은 **34/34 exit 0**
> (제품 게이트 9 + `doctor` · 음성시험 9 · `sapkit-engine` 10 · `sapkit-cli` 4 ·
> 재현 빌드).
>
