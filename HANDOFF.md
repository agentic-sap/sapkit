...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ **완주했다** (2026-08-23) — 다음 세션은 이 절만 읽으면 된다
>
> **D-120이 「판 4개면 끝」이라고 못박은 그 넷이 전부 닫혔다.** 판R(D-121) ·
> 판E1·판E2(D-122) · 판Z(D-123). 사다리 ⑴~⑷가 모두 완료이고 끝그림 ①~④가 도달이다.
>
> ⚠ **가장 먼저 알아야 할 것 — 「완주」는 「전부 검증됐다」가 아니다.**
> 증거 대기 **30종은 여전히 실 SAP에서 요구 급 증거가 없다.** 완주는 D-120 ⓓ가 정의한
> 대로 **「✅ 또는 이유와 함께 못 섰다」**로 닫힌 것이고, 그 30종은 **후자**다.
> 이 둘을 섞어 읽으면 D-082에서 실제로 났던 오해를 되풀이한다.
>
> ## ✅ 완주 판정 — D-120의 기준 넷에 그대로 대조한다
>
> | 기준 | 판정 | 근거 |
> |---|---|---|
> | ⓐ M-표 **증거 칸** M2~M5가 ✅ 또는 「이유와 함께 못 섰다」로 닫힌다 | ✅ | **D-122** — 넷 다 「이유와 함께 닫힘」. 각 칸에 **왜 못 섰는지**와 **「검증됨이 아니다」**를 함께 박았다 |
> | ⓑ 끝그림 **④ 브랜드**가 닫힌다 | ✅ | **D-123** — 카탈로그 4파일 개명 + 훅 마커 14(태그 3 + 접두어 11 — **그중 「모델이 읽는 출력」은 9** · 2는 JSDoc 헤더) + 자기지칭 **20줄 / 14파일**(초기 보고 「16」은 과소계수 · D-124 ⓓ). `docs/BLUEPRINT.md` §1.1 ④ 행이 정본 |
> | ⓒ **잡무 5건** 처리 | ✅ | **D-123** — ⓐ atomic-write **배선**(폐기 아님) ⓑ 승인 파일명 통일 ⓒ executor 문장 수리 ⓓ 브랜드(=ⓑ 기준) ⓔ 체크박스 규범항은 **보류 유지**(D-113 ⓒ⑤ — 미룬 일이지 보존 판정이 아니다) |
> | ⓓ **이월**(D-118·D-119 리뷰)이 닫힌다 | ✅ | **D-121** — 대상을 먼저 5커밋으로 고정하고 돌렸다. R-FAIL·차단 1·권고 7 |
>
> ## ⚠ 정직하게 — 완주가 데려가지 못한 것 (다음 사람이 읽을 목록)
>
> **⑴ 증거 대기 30종은 검증되지 않았다.** 판E1이 착수 실측에서 **브리프의 전제가 무너진 것**을
> 발견했다(D-122). 다섯 가지다: ① 재생기는 `fixtures/` **최상위만** 수집하는데(재귀 없음)
> 대상 18종은 `attended-only/`에서만 태워졌다 ② 그런데 **요구 급 산식은 재귀로 배정**해
> `replay` 급을 매겼다 — **수집 단위 ≠ 배정 단위**라 「요구 급은 재생인데 재생 대상이 될 수
> 없는」 도구가 생겼다 ③ 새 기준선은 `attended-guard`가 **무조건 거부**(자기 대조)하고 구 엔진은
> 판7.5에서 은퇴해 **만들 수단이 없다** ④ **접속 3곳이 전부 막혔다** ⑤ **재생 러너에 keyring
> `NODE_PATH` 배선이 없다**(짝인 녹화 러너엔 있다).
>
> **⑵ 살아 있는 갈래가 하나 있다 — 접속이 돌아오면 가장 먼저 시험할 것.**
> `attended-only/` 픽스처를 `--fixture`로 **직접 재생**하는 길이다. **코드 변경 0**이고,
> 그 픽스처는 엔진 **1.0.0**에 찍혔는데 지금은 **1.2.1**이라 자기 대조가 아니라
> **버전 간 회귀 대조**다. 18종 전부가 픽스처 **6편** 안에 있음을 확인했다
> (`cds-view-mde-test` 5 · `rap-bdef-bimp-service` 7 · `class-locals-unittest` 4 ·
> `domain-dataelement` 2 · `function` 1 · `interface` 1). ⚠ **시험되지 않았다** —
> 접속이 막혀 대조 단계에 닿지도 못했다. 상세는 장부의 「도구 결함 등재」 절.
>
> **⑶ 접속 상태 (2026-08-23 이 머신 실측)** — 증거 작업을 재개하려면 여기부터 본다.
>
> | 프로파일 | 상태 |
> |---|---|
> | **KR-DEV** (`192.168.1.225` · 판6.3이 픽스처를 찍은 시스템) | ❌ **TCP 미도달** — 이 머신이 다른 대역(`192.168.75.x`)이다. 사내망/VPN 필요. ⚠ 자격이 **keychain 참조**라 ⑴⑤ 결함에도 걸린다 |
> | **IDES-DEV** | ❌ **401** — 서버는 살아 있다. 자격증명 문제. ⚠ 러너의 잠금 경고(`login/fails_to_user_lock`)를 따라 **1회에서 멈췄다** |
> | **IDEA-JNC** | ❓ **미시도** — 서버는 살아 있다(401 응답). 계정 잠금 위험 대비 기대값이 낮다고 판단해 시도하지 않았다 |
>
> **⑷ 고치지 않고 등재만 한 결함 2건**(장부 「도구 결함 등재」 절 · D-122 ⓒ) — 재생 러너의
> keyring 미배선 · 재생 수집/배정 단위 불일치. **고쳤는지 확인할 수단이 없어서** 고치지 않았다
> (검증 없는 수리를 제품에 싣지 않는다). ⚠ **산식은 일부러 안 고쳤다** — 고치면 18종이 그 자리에서
> `증거 있음`이 되는데 그 모양이 **D-120 ⓐ가 경계한 「요구를 낮춰 증거를 만드는」 것과 구별되지
> 않는다.**
>
> **⑸ 판E2 12종의 부류별 처방은 하나도 집행되지 않았다** — ⓑ 거짓 음성 2종의 엔진 수리는
> **진단이 실기 1회의 가설**이라 SAP 재현 없이 고치면 맞는지 증명할 수 없고, ⓐ 면제 2종의
> 재분류는 산식을 고쳐야 해서 ⑷의 사유에 걸린다.
>
> **⑹ 그 밖의 부채** — D-118 attended 실측의 **채록본 부재**(수치가 산문 4곳에만 있고 넷이
> 서로를 인용한다 · D-121 유보 ⓑ) · **훅 출력 대소문자 미통일**(태그 대문자 · 접두어 소문자 —
> 브랜드가 아니라 스타일이라 별건) · **체크박스 규범항 재저작 보류**(review-checklist 24 +
> troubleshooting 10 — **미룬 일이지 보존 판정이 아니다**).
>
> **⑺ 최종 리뷰(D-124)가 새로 등재한 것 셋 — 전부 고치지 않았다.**
> ⓐ **`atomicWriteFileSync`의 0600은 Windows에서 무효다** — Node/Windows가 read-only 비트만
> 매핑해 실측 `666`으로 남는다. **회귀가 아니라 선재 한계**이고 옛 구현도 무제한이었다 —
> **자격증명 보호 주장은 POSIX 한정**이다. ⓑ **계층이 섞였다** — 하네스 중립인
> `interactive/scripts/setup-state.mjs`가 `interactive/adapters/claude/lib/atomic-write.mjs`를
> import 한다. 트리 전체가 동봉되므로 오늘 깨지는 것은 없으나 **Codex·Antigravity setup 경로에
> Claude 어댑터 전용 모듈이 들어왔다** — 모듈을 중립 자리로 옮기는 **별건**이다.
> ⓒ **`docs/reference/designs/2026-07-15-track-a-rebase-v2.md`의 `file:line` 인용 2건이
> 죽은 경로를 가리킨다**(판Z 개명 이후). **`AGENTS.md`가 그 문서를 트랙 A 라우팅의
> controlling source로 지목**하므로 살아 있는 권위가 죽은 경로로 이어진다. **고치지 않았다** —
> 날짜 박힌 실측 기록이라 **그 시점의 파일명이 실제로 그것이었고**, 고치면 기록이 거짓이 된다.
>
> ## 🔧 이 세션이 남긴 상태 (2026-08-23)
>
> | 축 | 값 |
> |---|---|
> | **제품 번들** | `sapkit-engine` **1.2.1** · **3,859,183 바이트** · sha256 `ddcbf31d87b0…` · 소스 핀 **`ad393e60`** |
> | **판 올린 이유** | 1.2.0 바이트는 `4706e0f7`로 **이미 push되어 나갔다** — D-096 선례(제품에 나간 적 없는 바이트 교정이면 판 유지)의 밖이고, D-115가 같은 이유로 올렸다 |
> | **도구 표면** | **186 / 155** 불변 (개명은 파일명이지 도구가 아니다 — capability diff 0) |
> | **시험** | jest **4,261 passed** / 1 skipped (**4,262 total**) — **이 판의 증분은 +3**(R3 1 · R7 2). 직전 값은 판R 마감 실측 **4,258 passed**이고, 「4,251」은 D-117 시점 값이다(그 사이 +7은 **이미 push된 D-119 회수분**). ⚠ 장부 8차는 **total** 기준(4,259)으로 적었다 — **basis가 다르다** |
> | **게이트** | 제품 **9종** · 엔진 **전종** · 음성시험 **9종** 전부 exit 0 |
> | **CI** | `782e3e3c` **success**(run `32627288013`) · `448adc0a` **success**(run `32627625688`) — 둘 다 `offline-gates` `completed | success`. 공통 요건 ①②③을 받친다. ⚠ **이 문서를 고치는 마지막 커밋 자신의 CI는 다음 세션이 본다**(문서만 바뀌므로 회귀 위험 없음 · 선례 `a73a236f`) |
> | **대장** | 증거 대기 **30** · 증거 있음 **156** (**변하지 않았다** — 이 세션은 증거를 만들지 않았다) |
>
> **커밋 8종** — `8ba7c92e`(D-121·D-122) · `b76de866`(M-표·장부 8차) · `7e120f79`(D-123) ·
> `25ecc4a9`(잡무 4건) · `75ee4f63`(브랜드+이월) · `ad393e60`(1.2.1) · `99f74648`(재번들) ·
> 그리고 이 문서 갱신. ⚠ **`25ecc4a9`는 카탈로그 rename 4건을 딸려 담았다** — `git mv`가
> index에 올라간 사이 그 커밋이 나가 **「새 이름 + 옛 본문 + 옛 생성기」**가 됐다(자기 신고보다 넓다 — 본문 6줄뿐 아니라 **생성기 8곳·게이트 상수·census**까지 옛 이름이라 **그 커밋에 서면 카탈로그 게이트가 실패한다**). `75ee4f63`이 본문과 생성기를
> 채워 정합했다. **두 커밋을 함께 봐야 원자적이다.**
>
> ## ▶ 다음 세션이 할 일
>
> **① push·CI는 끝났다.** `3b632c4a..782e3e3c`가 `origin/main`에 올라갔고 CI
> (`offline-gates`)가 그 SHA에서 **`completed | success`**로 완주했다(run `32627288013`).
> **다음 세션이 할 일은 아래 ②뿐이다.** 무인증 재확인법:
> `curl -s "https://api.github.com/repos/agentic-sap/sapkit/actions/runs?per_page=1&branch=main"`
> ⚠ **M3를 올리려던 계획은 접었다**(2026-08-23 · **D-125**). D-124 ⓒ가 「CI green이면
> 올릴 수 있다」고 적었으나 **그것이 오독이었다** — 「전 M 공통 완료 요건」은 CI green
> 하나가 아니라 **여섯 항목**이고, **⑥ 「마일스톤별 실 SAP attended 확인 1건」**이
> M2~M5 **전부에 대해 미충족**이다(접속 차단). CI green이 받치는 것은 ①②③이다.
> **그 요건 하나가 네 칸의 「이유와 함께 못 섰다」를 한 번에 설명한다.**
>
> ⚠ **완주 후 회수가 한 번 더 있었다**(2026-08-23 · **D-126**). 사용자가 GitHub README
> 첫 화면에서 옛 브랜드 문구를 찾았고, 파 보니 **살아 있는 환경변수 2종**이 더 있었다
> (`SC4SAP_LITE_NS`·`SC4SAP_AGENTS_DIR` → `SAPKIT_TOOL_NS`·`SAPKIT_AGENTS_DIR`).
> **리뷰 2회와 게이트 전종이 모두 초록인 채로** 제품 첫 화면에 옛 브랜드가 나가 있었다 —
...
> | 산재 **계약행** (`server/` 밖 서브트리 전체) | **~2,200행 / ~130파일** | **의도된 보존**(D-112 ⓓ) — 참조표 · 펜스 템플릿 · print-verbatim · **체크박스 규범항** · 사용자 노출 문구 · 승인 키워드 · 공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다. ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다**(**D-113 ⓐ**) — 구성 **3갈래** = §3.4 **~600/40** + `copy` 재저작 체크분 **~950/64** + 무변경 검증 참조표 **~660/25** |
>
