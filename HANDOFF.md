...
> **40파일 · ~598행**(페르소나 21 · 절차 13 · 지식 3 · 정책 1 · 도구 2)을 얻었고, **고지 4곳**
...
> | 산재 **계약행** | **~600행 / 40파일** | **의도된 보존**(D-112 ⓓ) — 참조표 · 펜스 템플릿 · print-verbatim · 사용자 노출 문구 · 승인 키워드 · 공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다 |
>
> ⚠ **「전부 자작」을 행 단위로 읽지 말 것** — ①의 도달은 **파일 단위** 기준이고, 행 단위로는
> 위 계약행이 남는다(D-112 유보 ⓒ). **이 구분을 흐리면 과대 주장이 된다** — 고지 4곳이
> 그 구분을 명시한다.
>
> ⚠ **이 판정은 아직 잠정이다** — **독립 새-컨텍스트 리뷰가 이 기록 커밋 뒤에 돈다**
> (D-112 유보 ⓐ). ㉮는 두 번 데인 자리이므로 **리뷰 통과 전까지 확정으로 읽지 않는다.**
> 리뷰가 차단을 내면 **그 회수가 판M2-a보다 앞선다.**
>
...
> 두 번 틀렸다.** 파일별 판정은 **판3.7이 사람 손으로 했고**(56/56 · 리뷰 4회 · D-109~D-111), **판11의 마감 스캔이 40파일 · ~598행**을 실측했다 — 그 한계(과소·과대)는 **그대로 승계된다**(D-112 유보 ⓑ).
> ⓖ **같은 자리에서 네 번 걸렸다** — D-102 ⓐ(「안 본 범주가 또 있는가」) → D-104 ⓒ(판8이
...
> ⓚ **판11(⑷ ㉮ · D-112)의 독립 새-컨텍스트 리뷰가 아직이다** — 기록 커밋 뒤에 돈다
> (D-112 유보 ⓐ). **㉮는 두 번 데인 자리**이므로 리뷰 통과 전까지 「⑷ 완료 · 끝그림
> ①~③ 도달」은 **잠정**이다.
>
...
> 2. **판11의 독립 리뷰 결과를 먼저 확인한다** — 회수가 나오면 그것이 판M2-a보다 앞선다
>    (D-112 유보 ⓐ · 리뷰 통과 전까지 ⑷ 완료는 **잠정**이다).
...
> (⑷ ㉮ 고지 범위 정합 · **사다리 ⑷ 완료** · **D-112** · ⚠ 독립 리뷰 뒤 확정) →
> **판M2-a**(**다음** · ⑴ M2 오프라인 배선 · **새 D-결정 필요** · 오프라인) ·
> 판M2-b(**BTP 계정 — 사용자만 답을 가진 것**). **별건 4**(판 아님): `atomic-write` 배선 ·
> `data-access-approval` 파일명 3자 · executor 문장 · **브랜드**(끝그림 ④).
