...
> 작성 2026-07-10 · 최종 갱신 2026-08-17. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
> ═══════════════════════════════════════════════════════════════════
>
> ## 📊 사다리 ⑴ 진척 — **이 표를 보고할 때 함께 말한다** (2026-08-14 실측)
>
...
> | 인증 | Basic **접속** · `--env=<name>` **접속** · `--mcp=<destination>` **설정 조립까지** · 브로커 **저장소 재료까지** | 4통로 전부 접속 |
>
...
> | 지음 · **증거 대기** | **143** | 재생 대조 **96** + attended 실기 **47** — 전부 SAP 접속 필요 |
> | 증거 있음 | **43** | 42종은 요구 급이 `계약 시험`(오프라인으로 참) · 실 SAP을 밟은 것은 `CreateProgram` **1종**뿐 |
>
> **⚠ 「증거 대기」를 완료로 읽지 마라** (D-082 ①). 이 판이 도달한 것은
> **M2·M3·M4·M5의 `짓기 완료`**이고 **네 M 모두 `증거 완료`는 열려 있다.** M1도
> 열려 있다. 사다리 ⑴은 닫히지 않았고 마일스톤 하나도 닫히지 않았다.
>
> **⚠ 위 「증거 있음 43」과 아래 옛 서술의 「재생 증거 7종」은 갈린다 — 숫자가 줄어든
> 것이 아니다.** 그 7종(`SearchObject` `CheckSyntax` `UpdateProgram` `UpdateClass`
> `ActivateObjects` `GetProgram` `GetClass`)은 **콘솔 출력과 문서 서술로만 존재하고
> 레포에 커밋된 판정 파일이 없다** — 대장은 파일만 입력으로 받으므로 재생 증거를
> 전량 「미기록」으로 센다(`evidence/replay/*.json` = 0건). 재생 러너는 SAP에 붙으므로
> 오프라인 판에서 돌릴 수 없었고, 이 판은 **판정 파일의 경로·형식만 만들어 뒀다**
> (`sapkit-engine/evidence/replay/`). 채우는 것은 다음 attended 세션이다.
>
> ## ✅ 직전 판 — **판3.4: ⑶-b 4차 배치 — 1-A 모듈 전반부 43편 (재저작 30 + 검증 체크 13)** (2026-08-17)
>
> **1-A 모듈 91편의 전반부가 처리됐다 — 코드 0줄, 지식 `.md`만.** D-087 ⑧의 **소비 실증**
> 기준으로 `common` 7 + **FI·CO·MM·SD·PP·PM 6모듈 × 6편 = 43편**을 처리했다. 실증한 소비
> 표면은 넷 — **모듈 컨설턴트 페르소나 14편의 파일별 지시 로드**(예 `sap-fi-consultant.md:10`
> + MANDATORY `:37`·`:64`·`:69`) · **컨설턴트 공통 블록의 `common` 5편**(bapi·tcodes·tables·
> spro·enhancements) · **naming 2편의 소비자**(`analyze-code.md:73`·`:82` ·
> `create-object.md:77` · `sap-bc-consultant.md:10` 글롭) · **모듈 폴더 패턴 로더 6편**이다.
> **처리가 두 갈래로 갈렸다 — 재저작 30편 + 검증 체크(무변경) 13편**이고, 후자를 위해
> **D-088을 신설**했다. **잔량 120 → 77**(명부 `docs/reference/copy-baseline.md`).
>
> - **⚠ D-088 신설 — 표 지배 참조 파일의 규칙**(판3.5도 이 규칙을 그대로 쓴다): 표 셀을
>   **식별자·공식 명칭 열**(보존 = 사실 데이터)과 **문장형 서술 셀**(재저작 대상)로 가르고,
>   그 적용 후에도 재저작 표면이 0인 **순수 참조표 파일**은 「문장형 차용 산문 잔존 0」을
>   전수 확인 + 독립 리뷰 재확인으로 **체크를 인정**한다(명부 커밋 칸에 해시 대신
>   **「검증 체크(무변경)」**). **D-087 ⑤의 해석 보충이지 삭제 예외가 아니다 — 파일은 그대로
>   남는다.** **사용자 상시 위임 하 「산출자 권고 채택」**(사후 거부권 — 되돌리려면 D-089
>   정정 + 명부 §2 보충 철회 + 13편 체크 해제). 무변경 13편 = 6모듈의 `spro`·`tcodes` 각 6 +
>   `SD/tables`.
> - **검증**: 독립 새-컨텍스트 리뷰 **6건** 전부 통과 — **전수 대조 4그룹 43/43 PASS**
>   (누락 0 · 추가 0 양방향 · 셀 **1,356+** · 산문 **445행+** · 식별자·수치·한글 다중집합
>   기계 대조 · **식별자 열 밀림 0**) · **무변경 13편 총괄 재검**(551셀 전수 · **blob 해시
>   무변 13/13** · 모듈 spro 경고 블록은 차용분이 아니라 **자작 신설분** — 커밋 `5fb1d901`) ·
>   **통합 리뷰**(구성 독립성 43/43 · **소비 표면 28지점 유효** · 새로 깨진 것 0) ·
>   **회수 2회**(`88165eeb` 9곳 · `d24e18a4` 3곳) 후 재확인 정상. 제품 게이트 9종 + doctor
>   전부 exit 0 · 지식 `.md` **148 유지** · 버전 무변.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1~3.3에서 승계, 이 판도 동일)**: 판3.4는 **버전을
>   올리지 않는다.** 콘텐츠 변경은 판1이 이미 올려 둔 **미배포 0.7.0 범프에 편승**해 전달된다
>   (D-060 — 설치본 내용이 바뀌는 커밋은 버전이 올라야 기설치자에게 닿는다). 그러므로
>   **push 직전에 설치본 버전을 확인한다** — `node interactive/scripts/doctor.mjs`가 설치된
>   플러그인의 manifest version을 레포 버전과 병기해 보여 준다. **이미 0.7.0 설치본이
>   존재하는 머신이 발견되면 패치 범프 후 push.**
> - **사용자 상시 위임 (2026-08-17)**: 판 루프의 **열린 결정은 산출자 추천 자동 채택 +
>   사후 거부권**, **판 종료 후 다음 판 자동 개시**. 모델 배정 — ready 점검 **fable** ·
>   재저작·전수 대조·마감·최종 리뷰 **opus** · 구조 대조·기계 작업 **sonnet 이하**.
>   **경계**: 판6+(SAP attended)는 자동 불가 · push는 커밋까지만 · 굵직한 결정을 기록할 때
>   **「위임 하 권고 채택」을 명시**한다.
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.5: ⑶-b 5차 배치 — 1-A 모듈 후반부 48편**
>
> **▶ 판3.5 ready 완료(2026-08-17) — 3-doc 확정: `.dryforge/spec.md` · `plan.md` ·
> `handoff.md`(독립 점검 2회 통과 · 차단 0 · 위임 채택 내역은 spec §9). 남은 것 =
> 실행뿐: 새 세션에서 `/dryforge:go` 입력(3-doc이 자기 완결이라 대화 맥락 불요 ·
> 상시 위임 유효).**
>
> ⚠ **dryforge `go`/`ready`는 사용자 직접 입력 전용이다**(disable-model-invocation —
> 모델의 Skill 호출·우회 재현 금지, 판3.5 ready 종료 시 실측). 상시 위임의 「자동
> 개시」의 실제 형태 = **판 안 전자동(질문 0) + 판 사이 사용자 한 줄 입력.** 판이
> 끝나면 다음 입력 명령을 보고에 명시한다.
>
> **결정은 전부 서 있다 — 판3.5는 새 규칙이 필요 없다.** 계수 방식·체크 규칙·선정 기준은
> **D-087**, 표 지배 참조 파일 규칙은 **D-088**이 확정했고 명부
> (`docs/reference/copy-baseline.md`)가 그 집행 자리다. **잔량 77/170.**
>
> - **배치 = 1-A 후반부 48편**: **Ariba·BW·HCM·PS·QM·TM·TR·WM 8모듈 × 6.** 계약은 판3.4와
>   같다(**D-088 그대로**). 이 48편으로 **1-A 갈래가 소진**된다(43 + 48 = 91).
> - **⚠ 정찰 2건 — 무변경 선례를 전제하지 말 것**: ⓐ **`PS/spro.md`는 표 밖 산문이 실재한다**
>   (L82-84 불릿 — Fiori 대체 · Hierarchical Projects · ACDOCA). 다른 모듈 spro가 무변경이었던
>   사실을 PS에 그대로 옮기면 틀린다. ⓑ **spro 술어 탐침의 Title-Case 명사 함정** —
>   `Sets`(QM/PM Cycle Sets) · `Means`(TM Means of Transport)를 동사로 읽으면 오분류된다.
> - **이월 전량 승계 + 판3.4 신규 등재**: 소비자 오인용 4건 · 사실 의심 2건 · 제품명 표기
>   갈림(`SAPKIT` vs `sc4sap`) · **이식 전 참조 보존 목록 20곳**(⚠ 그중 1곳은 소비자 오인용
>   ④와 같은 건 — **이중 계수 금지**) · 소비자 서술 불일치 4건 · 헤딩 재작성 기록 2건 ·
>   판3.3 등재 사실 의심 · 배치 내 기존 불일치 5건 · 재저작 구조 기록 2건 — 여기에 **판3.4가
>   새로 등재한 것**(모듈별 사실 의심 다수 · **낡은 도구명 동결 36곳** · **판정 폭 기록** ·
>   `FI/bapi` KR 「전기→계상」 5셀 **용어 갈림 = 판3.5 일관성 재료**)이 더해진다.
>   **상세는 `docs/RUN-PLAN.md` 판3.4 브리프의 「판3.5 재료」.**
> - **⚠ 재저작의 함정 — 판3.1~3.4 넷 다 밟았다.** 「사실을 잃지 않았는가」만 재면 부족하다.
>   판3.1은 저자가 설명을 돕겠다고 넣은 문장이 **원문에 없던 SAP 단언·지시**가 돼 5편 전부에서
>   **세 차례 회수**했고, 판3.2는 **강도·범위 미세 이탈 9곳**(`bcbe36f6`), 판3.3은 **강도·
>   프레이밍·표기 12곳**을 두 차례(`05643eda` 10곳 · `41110412` 2곳), 판3.4는 **규범 voice·
>   과잉 환언·어미체 이탈 12곳**을 두 차례(`88165eeb` 9곳 · `d24e18a4` 3곳) 회수했다.
>   재저작은 표현·구성만 바꾸는 일이므로 **더하는 것도 범위 밖**이다 — 리뷰 근거는
>   **누락 0 + 추가 0** 양방향으로 잰다. 명부 §2에 명문화돼 있다.
> - **⚠ 배치 내 상호 인용**: 14개 모듈 `spro.md:9`가 `common/spro.md`의 헤딩
>   **「IMG Activity Verification」**을 인용한다. 판3.4 재저작으로 그 절의 **행 좌표는
>   이동했고 헤딩 문구는 보존**됐다 — **절 이름으로만** 다루고 행 좌표를 박지 말 것.
>
> **진행 방식은 D-084 그대로다 — 「짓기 우선 완주, SAP 증거는 최종 테스트 판으로 이연」.**
> ⑵는 판1로, **⑶-a는 판2로 닫혔다.** 남은 오프라인 갈래는 ⑶-b(판3.x)·⑶-c(판4)와
> ⑴ 잔여 코드(판5)다. attended 증거는 **판6(최종 테스트 판)** 하나로 모은다.
> §3.2(검증 없는 교체·은퇴 금지)와 증거 순서 M1→M6(D-081·D-082 ④)은 불변 —
> **이연이지 재배열이 아니다.**
>
> **판 큐의 정본 = `docs/RUN-PLAN.md`** (판 = dryforge ready→go 1사이클 · 판 완료 시
> 그 표 갱신까지가 작업의 일부 · 판 사이 맥락은 .dryforge 아카이브가 아니라 그 파일이
> 든다). 시작 명령: **`/dryforge:go`**(판3.5 실행 — ready는 완료). 판3.5 마감 후의
> 다음 판부터 `/dryforge:ready docs/RUN-PLAN.md 판3.6`.
> 「M1 잔여부터」는 **판6 몫으로 이연** — 상세는 아래 절에 재료로 보존.
>
> ## 📦 이연 재료 — M1 잔여 (판6 몫 · D-084로 이연 · 내용은 그대로 유효)
>
> **원래의 순서 논리(여전히 유효)**: D-082 ④가 D-081의 순서 고정을 뒤집지 않았다 —
> **증거를 태우는 순서는 여전히 M1 → M2 → M3 → M4 → M5 → M6**이다. 판6이 열리면
> 첫 작업은 새 도구가 아니라 **M1의 SAP 증거**다.
>
> M1(19종)의 잔여 셋 — 전부 attended 세션 몫이다(D-082 ②·③):
>
> 1. **재생 대상 10종의 녹화·재생 대조** — 읽기 계열 6종(`GetInclude`
>    `GetFunctionModule` `GetTable` `GetStructure` `GrepObjects` `GetSourceDiff`)이
>    먼저다. write가 없어 되돌릴 게 없고, 대상은 이미 `$TMP`에 있는
>    `ZSAPKIT_M1_DEMO`(PROG)·`ZCL_SAPKIT_M1_DEMO`(CLAS)를 쓰면 된다.
>    `UpdateInclude`도 `ZSAPKIT_M1_DEMO`에 붙이면 된다.
> 2. **`CreateInclude` 실기** — `Create*`는 재생으로 증거를 못 얻는다(두 번째 실행이
>    "이미 있다"로 실패). 요구 급이 `attended 실기`다. `CreateProgram`은 이미 닫혔다.
> 3. **대표 절차 실기(C3)** — 프로그램 생성→활성→기계 확인.
>
> ```powershell
> cd sapkit-engine
...
> - **판정 파일은 `sapkit-engine/evidence/replay/`에 쌓인다** — 지금은 비어 있고,
>   그것이 쌓여야 대장의 「증거 대기」가 실제로 줄어든다.
>
> ## 🚧 막힌 것
>
...
> **⑵는 2026-08-16 판1로, ⑶-a는 같은 날 판2로 소진됐다**(D-085 · D-086) — 남은 오프라인
> 활주로는 ⑶-b(판3.x)·⑶-c(판4)와 ⑴ 잔여 코드(판5)다.
>
> ### 고지 은퇴는 훨씬 멀다 (혼동 방지)
>
