...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **증거 판 (대장 증거 대기 30종)**
>
> ## 🔔 상태 셋 (2026-08-23 판M2-c 완료 시점)
>
> **① M2가 섰다 — 사다리 ⑴의 접속 갈래가 닫혔다.** 엔진이 **사람 로그인 뒤 SAP와
> 직접 대화**했다(D-118 · `GetSystemInfo` → `{"supported":true,"system_id":"TRL",…}`).
> **M-표 짓기 칸은 이제 M1~M5 전부 ✅**이고 D15는 **해소**다. **2026-08-10 D-079가 연
> 사다리 ⑴이 여기서 닫힌다.**
>
> **② 그러나 `증거` 칸은 M2~M5가 전부 열려 있다.** 「접속이 된다」와 「도구들이
> 증명됐다」는 **다른 사실**이다(D-082 ① · D-118 ⓒ). 이 판이 잰 것은 **`GetSystemInfo`
> 하나**이고, 대장은 **증거 대기 30종**이다. ⚠ 그리고 **증거 있음 156 중 62종은 실기가
> 아니라 요구 급 인하**다(D-092 ⓐ) — 인하 전 기준이면 **증거 대기 92**다. 대장 머리말이
> 두 수를 병기한다.
>
> **③ 세션마다 로그인이 실제 마찰이다.** 토큰은 프로세스 메모리에만 살고(D-091 ⓒ①)
> 갱신 배선이 없다(D-117 ⓕ). **증거 판은 SAP에 여러 번 붙어야 하므로** 그 판을 열기 전에
> **토큰 보존/갱신을 먼저 볼 것인가**가 판단이다. 토큰 보존은 **D-091 ⓒ①을 뒤집는 새
> 결정**이 필요하다.
>
> **다음 판을 고르는 자리** — 둘 중 하나다:
> ⓐ **증거 판**(대장 30종 · attended) — 사다리 ⑴의 `증거`를 채운다. 매번 로그인 마찰을 진다.
> ⓑ **토큰 보존/갱신 판** — 먼저 마찰을 없애고 ⓐ를 수월하게 한다. **새 D-결정 필요**
> (갱신만이면 D-117 ⓕ의 범위 확장 · 보존까지면 D-091 ⓒ① 개정).
> **메인이 정한다** — 규약상 판 사이 전환은 묻지 않는다.
>
> **잡무는 그대로 남아 있다**(아래 「남은 것」 표) — 브랜드 이름(끝그림 ④) 포함.
>
...
> | 항목 | 성격 | 막고 있는 것 |
> |---|---|---|
> | **증거 판** | 판(attended) | **막는 것 없음** — 대장 **증거 대기 30종** · 사다리 ⑴의 `증거` 칸 |
> | **토큰 보존/갱신** | 판(미개시) | **세션마다 로그인** 마찰 — 증거 판 전에 볼 것인가가 판단. 보존은 **D-091 ⓒ① 개정** 필요 |
