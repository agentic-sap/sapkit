...
>
> ## ✅ 직전 판 — **판2: ⑶-a GPL 빈자리 = 조회 확정 + 집행** (2026-08-16 · D-086)
>
> **사다리 ⑶-a가 닫혔다 — 코드 0줄, 문서만.** 2026-08-09 커밋 `54fd84de`로 삭제된
> GPL-3.0 ABAP 문법 사전 31파일(그 처분의 결정 기록은 **D-074 · 08-10** — 삭제일과
> 기록일은 다르다)의 빈자리를, **자체 집필이 아니라 조회로** 처리하기로 확정하고
> 집행했다. 문법 사전은 제품에 다시 싣지 않는다 — 제품에 남는 참조 지식은 실전 검증
> 함정 모음(`interactive/core/knowledge/abap/conventions/` 21편)이다.
>
> - **조회처 = 이미 동봉된 help.sap.com 조회 체계**
>   (`interactive/core/procedures/help-portal-fetch.md` + `interactive/tools/fetch/` 2종).
>   새로 만든 배선이 없고 **소비자만 늘렸다** — 그래서 판2가 **새로 여는 외부 노출은 0**
>   이고, 수요 지점에는 **레포 내부 상대 링크만** 남는다. 그 절차와 fetcher 2종은 무수정.
> - **집행 6곳**: `docs/reference/DECISIONS.md`(D-086 append) ·
>   `interactive/THIRD_PARTY_NOTICES.md`(§GPL 「향후 경로」 문단) · `docs/BLUEPRINT.md`
>   (§⑶-a) · `interactive/DESIGN.md`(트리 주석 1곳) · **수요 지점 포인터 2곳 복원**
>   (`…/conventions/rap-odata-rules.md` · `…/procedures/create-object.md` — 삭제 커밋이
>   지운 자리에 "사전 링크" 대신 "조회 절차 링크"). 여기에 이 문서와 `docs/RUN-PLAN.md`.
>   **지식 `.md` 148 유지**(신설·삭제 0) · 게이트·훅·번들 2종·`engine/`·`sapkit-engine/`·
>   `sapkit-cli/` 무접촉 · 제품 게이트 9종 전부 exit 0.
> - **조회 경로 생존 스모크 1회 통과** — `node interactive/tools/fetch/fetch-abap-keyword-doc.mjs
>   abenwhere_all_entries` exit 0(공식 ABAP Keyword Documentation 본문 수신).
>   ⚠ **상설 게이트가 아니다** — 외부 문서 사이트의 이동·폐쇄는 감시하지 않기로
>   수용했고(D-086 ⑤ · 정직 유보 ⓐ), 발견 경로는 **실사용 중의 조회 실패 보고뿐**이다.
> - **⚠ push 전 확인 의무 (편승 — 이 판의 전달 계약)**: 판2는 **버전을 올리지 않는다.**
>   콘텐츠 변경은 판1이 이미 올려 둔 **미배포 0.7.0 범프에 편승**해 전달된다(D-060 —
>   설치본 내용이 바뀌는 커밋은 버전이 올라야 기설치자에게 닿는다). 그러므로 **push 직전에
>   설치본 버전을 확인한다** — `node interactive/scripts/doctor.mjs`가 설치된 플러그인의
>   manifest version을 레포 버전과 병기해 보여 준다. **이미 0.7.0 설치본이 존재하는
>   머신이 발견되면 패치 범프 후 push.**
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.1: ⑶-b copy 지식 치환 (반복 판의 첫 판)**
>
> **첫 안건 = `copy` 잔량 계수 방식의 재정의.** 분류 장부(`interactive/MIGRATION-MANIFEST.md`
> ·`interactive/provenance/`)와 그것을 검사하던 게이트가 은퇴하면서 **잔량을 세는 방법이
> 공백**이 됐고, `docs/BLUEPRINT.md` §⑶-b 주석이 그 재정의를 **착수 시점에** 요구한다.
> 이 판이 그것을 정하지 않으면 ⑶-b의 완료 기준(「잔량 단조 감소」)이 측정 불가로 남는다.
> 둘째 안건은 **1차 치환 대상 선정 기준**(실전에서 검증된 것부터).
>
...
> 든다). 시작 명령: `/dryforge:ready docs/RUN-PLAN.md 판3.1`.
