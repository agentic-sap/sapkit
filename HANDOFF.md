...
>
> 오프라인 증거 — **판6.2 실측(2026-08-19 · 마감 시점 전종 재실행)**: 제품 게이트 **9종
> exit 0** · 음성시험 **9종 exit 0** · `sapkit-engine` verify·gates·음성시험·
> `render-ledger --check`·`build-plan --check` **전부 0**(jest **4,153 통과 / 1 skip** ·
> 242 스위트) · `sapkit-cli` verify **355/355**·gates(**코퍼스 47파일 × 표면 3종 갈림 0**)·
> 음성시험 2종 **전부 0**. `doctor`만 **exit 1**이고 **착수 전 기준선과 동일**(FAIL 1 =
> Antigravity 설치본 1.1.1 ≠ 고정 1.1.4 · WARN 4 — 전부 머신 환경 사유). ⚠ **머신 함정**:
> `sapkit-cli/node_modules`가 이 머신에 **없어서** 첫 실행이 실패했다 — `npm install`
> 선행이 필요하다(레포 회귀가 아니다). **다음 판의 착수 기준선은 네 묶음 전부 뜰 것** —
> 판6.2 기준선이 `doctor` + 제품 게이트 9종만 떠서 이 공백을 못 걸렀다.
>
...
>
> **⚠ 그러나 이 126은 이제 「교체까지 얼마나 남았는가」의 답이 아니다** (**D-093 ⓐ**,
> 2026-08-19). **교체(판7)의 관문은 BLUEPRINT 「검증 기준」 1~4**이고 M-표·대장은
> **제작 진도표**다. 판6.2 실사 결과 **기준 1·3·4는 지금도 서고 기준 2만 배선이 필요**하다
> — 즉 교체까지 남은 것은 이 126이 아니라 **오프라인 배선 하나**다. 대장의 126은 교체
> 뒤에도 실사용으로 계속 줄어든다.
>
...
>
> ## ✅ 직전 판 — **판6.2: ⑶-c 마감 + 저비용 확인 + 사다리 방향 확정 (D-093 신설)** (2026-08-19)
>
...
> 관문 실사 1회** ⓓ **결정 셋(D-093)**.
>
> **▶ 다음 세션이 먼저 볼 것**: `docs/RUN-PLAN.md`의 **판6.2 브리프 안 「판7에 남은 것」**
> — 그 목록이 곧 다음 작업 목록이다.
>
...
> - **⚠ 이월 1순위 — `verify-applied.md` ②가 틀린 도구를 1순위로 지정한다.** REPS
>   되읽기 1순위가 `GetProgFullCode`인데 그 도구는 **공백·들여쓰기를 붕괴시켜**
>   (`*& What    :` → `*& What :`) **거짓 불일치**를 만든다. 이 판은 `GetProgram`/
>   `GetInclude`로 대조해 피했다. **기계 확인의 절반이 걸린 자리**이고, 절차 정본 수정이
>   이 판 범위 밖이라 고치지 않았다.
...
>   **판7** 소관 · 토큰 keychain 영속 유보 · `CLAUDE.md` 엔진 게이트 서술 느슨함).
...
> **판7의 전제는 BLUEPRINT 「검증 기준」 1~4**이고 M-표 완주가 아니다(실사 기준 남은 것은
> **기준 2 배선 하나**). **판8의 전제는 ⑶ 세 갈래 소진**이고 판6.2가 그 마지막 조각을
> 닫았다. 그리고 **교체와 은퇴를 한 판에 묶지 않는다** — 되돌아갈 자리를 남기려고
> 판7 / 판7.5로 갈랐다(BLUEPRINT §3.2 · 검증 기준 5의 유지 기간 연장).
>
