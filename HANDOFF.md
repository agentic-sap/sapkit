...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판7.5` 착수 (묻지 말고)**
>
> **교체는 끝났다** (2026-08-19 · 판7-b · D-095). 제품 번들
> `interactive/server/server.bundle.cjs`는 이제 **자체 저작 `sapkit-engine` 1.0.0**의
> 산출물이다. `docs/RUN-PLAN.md` 판 큐가 **판7.5**(`engine/` 은퇴)를 `다음`으로 세워 뒀다.
>
...
> 3. `/dryforge:ready`로 **판7.5**를 잡고 `go`로 실행한다.
>
> **⚠ 그런데 판7.5의 전제는 「교체가 실사용에서 안정」이다** — 이 판은 그것을 **재지
> 못했다**(D-095 정직 유보 ⓔ · **SAP 접속 0회**). 오프라인 게이트는 전종 초록이지만
> 신 엔진이 제품 자리에서 실제 SAP 업무를 한 번도 수행한 적이 없다. **그러므로 판7.5에
> 들어가기 전에 판단할 것 하나**: 실사용 사이클을 먼저 돌릴 것인가(그러면 **판6.3**이
> 그 자리다 — attended 46종 왕복이 곧 실사용이다), 아니면 오프라인 증거만으로 은퇴할
> 것인가. **되돌아갈 자리를 없애는 판이므로 그 순서를 뒤집지 말 것.**
>
> **판7.5가 반드시 함께 판정해야 할 것** — 이 판이 넘긴 재료다:
> - **`engine/` 은퇴가 무엇을 데려가는가.** `engine/LICENSE` 고지는 이제 `engine/`
>   **하나만** 덮는다(번들은 2026-08-19에 빠졌다 — BLUEPRINT §1.3 갱신됨). 그러나
>   `harness/old-surface/capture.mjs`가 **롤백 소스에서 지은 번들**을 채록 대상으로
>   삼고 CI `engine-tests` 잡이 「롤백 번들이 여전히 지어진다」를 확인한다 — 은퇴하면
>   **둘 다 사라진다.** 채록본 `m1-tools.json`은 그때부터 `sapkit-cli/fixtures/baseline/`과
>   같은 지위(되뜰 수 없는 기준)가 된다. 그 사실을 문서에 박고 은퇴할 것.
> - **롤백 경로가 바뀐다.** 지금은 「교체 커밋 revert」로 끝나지만, `engine/`이 떠나면
>   revert가 소스 없는 번들을 되살리는 꼴이 된다.
>
...
> | 2 · 제품 게이트 2종이 신 엔진에 대해 통과 | **선다** | `--target=engine`으로 `smoke-mcp`·`conformance-server-gates` **exit 0**. **교체 뒤에는 무인자(`bundle`)도 같은 엔진**이라 둘이 같이 초록이어야 한다 — CI가 매 푸시 갱신 |
> | 3 · 실데이터 2종 상시 게이트 + 음성시험 | **선다** | 안전 24 + 음성 44 + jest 4,153 · `GetTableContents` MCP 왕복 포함 |
...
> exit 0** · 음성시험 **9종 exit 0**(`test-smoke-mcp`는 **29건**으로 늘었다 — 판7-b) ·
> **신 엔진(dist) 대상 2종 exit 0** ·
> `sapkit-engine` verify·gates·음성시험 3종·**번들 스모크**·`render-ledger --check`·
> `build-plan --check` **전부 0**(jest **4,153 통과 / 1 skip** · 242 스위트) ·
...
>
> ## ✅ 직전 판 — **판7-b: ⑴ 교체(swap) 실행 (D-095 신설)** (2026-08-19)
>
...
> - **⚠ 안전에 닿는 변화가 하나 있다 — 판7-a 이월 ①의 판정.** 신 엔진은 blocklist
>   노브 3종을 **프로세스 env로도 받는다**(`resolveSafetyEnv` = `{...processEnv,
>   ...profileEnv}`). 그 자체는 이 레포가 옛 **GAP-2**로 등재해 둔 격차의 **수리**다.
>   그러나 `off`·`MCP_ALLOW_TABLE`은 **푸는** 노브라 「누가 서버측 바닥선을 낮출 수
>   있는가」가 넓어지고, **D-043의 소유자 머신 예외가 바로 그 바닥선에 기대고 있다.**
>   존치하되 세 겹으로 막았다: ① 기본값은 잠긴 채(노브 0개 → `standard` · 오타는 조이는
>   쪽으로만) ② **같은 키면 활성 프로파일이 이긴다**(새 단언 **B2p3**) ③ **제품이
>   발행하는 MCP 배선에 노브 0개**(`smoke-mcp` 검사 ⑦ 신설 — 매니페스트 드리프트
>   검사로는 못 막는다: **생성기 자체에** 노브가 들어가면 생성물과 생성기는 여전히 일치).
>   `adapters/claude/hooks/README.md`의 층별 한계 표를 그에 맞춰 정정했다.
...
>   ② **`MCP_ALLOW_TABLE`이 프로세스 env로 드는지는 여전히 미증명**(B2p가 `off`와
>      함께 주입해 가린다). 방향이 푸는 쪽이라 미증명이 안전한 쪽으로 남는다.
