...
> **`class: transform` 42를 통째로 빠뜨렸고**, 그중 **38파일이 상류 원본과 60자 초과 동일
> 행 1,326행을 공유**한다(**페르소나 26이 전부 이 범주** · 25가 **54~95%**).
> **`interactive/LICENSE`는 서브트리 전체에 걸린 채다.**
>
...
> ⓕ **`transform` 42의 잔존 수치는 상한도 하한도 아니다**(60자 초과 완전 일치만 셌다) —
> **파일별 판정은 판3.7이 사람 손으로.**
> ⓖ **D-102 ⓐ의 교훈이 판8에서 다시 반복됐다** — 「내가 안 본 같은 범주가 또 있는가」를
> 재개점이 인용까지 해 뒀는데 같은 자리에서 걸렸다. 증상은 **유보를 한 방향으로만 연 것**
> (「36보다 **적을** 수 있다」 — **많을 수 있는가**는 묻지 않았다).
>
...
> ~~판9~~ **완료** → **판10**(다음 · ⑷ ㉯ 활성 차용 6 소진) · **판3.7**(⑶-b `transform` 42
> 재저작 — **D-104가 신설**) · 판M2-a(새 D-결정 필요) ·
...
**실측 (2026-08-23 · 번들 stdio `tools/list` 바이트 · 이 머신 프로파일 활성 = connected)**:
