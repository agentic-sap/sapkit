...
> ═══════════════════════════════════════════════════════════════════
>
> **▶ 현재 착수점 (2026-07-31 갱신) = 백로그 ⑥ 집행 1단계 — 도구층 4건 이식**.
> 범위 산정은 **끝났다(D-053)**. 8파일 전문 + `build-bpml.mjs` 827줄 정독 + `tools/spec/`
> 5종 실측 대조 완료. **채택** = 도구층 4건(`screen-image-renderer` 1180→1732 ·
> `render-process-images`(신규) · `build-spec` +75 언어혼용가드 · `render-md-images`(신규))
> **→ 프로세스 계층 3건**(`knowledge/modules/common/document-flows.md` ·
> `procedures/package-to-process.md` 재작성 · 15줄 스킬 래퍼). **BPML은 범위 밖**(재검토
> 트리거 2개 명문 — D-053 ②). **`trust-session`은 만들지 않는다 — 최대 장벽이 아니라
> 오판이었다**(권한 템플릿 187건이 Layer 1을 이미 대체 · 이 스킬이 쓰는 26도구 누락 0 ·
> `dontAsk`는 디스패치의 부산물). 진짜 장벽 = 없는 서브에이전트 3종 전제 → 재작성으로 해소.
> 백로그 ⑦은 **사실만 기록하고 배선 유보로 종결(D-054)**. 최신 집행분은 **v0.4.1
> `knowledge` 절차**(D-052, 아래 첫 ▶ 블록) — 이번 세션은 **코드 0줄, 문서 2건뿐**이다.
>
...
>   E2E 체크리스트 마지막 줄에 등재.
> ⑥ **상류 `package-to-process` 이식 — 범위 확정 ✅(07-31 · D-053) · 집행 미착수 ★착수점**.
