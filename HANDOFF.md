...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판9` (⑴ 표면 재평가 — 꼬리 49종 · 오프라인)**
>
> **판8이 끝났고, 그 집행의 절반이 독립 리뷰로 되돌려졌다** (2026-08-22 **D-103** →
> 2026-08-23 **D-104** · 전 구간 오프라인 · SAP 접속 0회). **판정은 선다 — 철회된 것은
> 고지 범위를 좁힌 집행이다.**
>
> | 무엇 | 결과 |
> |---|---|
> | 판정 (**유효**) | ①(36건 재저작) **기각** · 36을 **동결 30 / 활성 6**으로 갈랐다 · ⑷ 완료 정의를 「고지 0건」 → **「범위 정합 + 활성 차용분 0」**으로 고쳤다 |
> | 끝그림 ③ (**유효**) | ~~최종 0건~~ → **도달 불가로 명시.** 이유는 미완이 아니라 **D-079 ⑥**(SAP 오브젝트 무접촉) |
> | 고지 범위 좁히기 (**철회**) | `interactive/LICENSE`는 **서브트리 전체**에 걸린 채다. 판8이 `class: copy`만 세고 **`class: transform`을 통째로 빠뜨렸다** |
> | 새로 드러난 것 | `transform` **42건 현존** · **38파일이 상류 원본과 60자 초과 동일 행 1,326행 공유** · **페르소나 26이 전부 이 범주**이고 25가 **54~95%** |
>
> **판정의 열쇠**(유효): 차용의 무게를 **파일 수가 아니라 「우리가 그것을 떠안고 있는가」**로
> 쟀다. 끝그림 ②의 정의가 이미 「레포 안에서 **개작·유지보수하지 않는다**」였고, git 이력이
> `sap-assets/` 30은 그 조건을 **이미 만족**함을 보였다.
>
> **⚠ 명부 170의 「완주」를 넓게 읽지 말 것.** 그것은 **`copy` 범주 안에서의 완주**다 —
> 페르소나·절차는 그 명부에 **한 줄도 없다**. transform 명부는 `copy-baseline.md` **§3.4**
> 신설이고 재저작은 **판3.7**이다. ⚠ **이건 새로 생긴 일이 아니라 아무도 세지 않던 일이다.**
>
> **⚠ 여기서 얻은 규칙**: 고지를 **넓게 두는 것**은 부정확할 뿐이지만 **좁히면서 「나머지는
> 우리 것」이라 적는 것**은 방향이 다르다 — 판정이 설 때까지 **넓은 쪽**에 둔다.
>
> **⚠ 지금 열려 있는 유보** (판7.5 것 셋 + 판8 것 둘 + 회수가 남긴 것):
> ⓐ **롤백 두 걸음을 실제로 해 본 적이 없다** — 아래 🔙 절. 그것을 재던 CI 잡이 없으므로
> **자동으로 해소되지 않는다.** 판8은 그 경로를 **서술한 문장만** 고쳤다(D-103 ⓕ).
> ⓑ **채록본 셋**(`m1-tools.json`·`handler-tree.json`·`retired-evidence-paths.json`)은
> **틀린 값이 있어도 원본과 대조해 발견할 길이 없다.**
> ⓒ `sapkit-engine/src/**`의 JSDoc 계보 인용 **425파일**이 전부 죽은 경로를 가리킨다 —
> **명시 이월.**
> ⓓ **36건이 「지금도」 상류 문장인지는 파일 단위로 대조하지 않았다**(동결 레포 무접촉).
> 분류는 명부의 `class: copy`를 그대로 받았고, 그래서 `tool-catalog/` 4와
> `verify-engine.mjs`는 **보수적으로** 차용에 남아 있다 — **실제 잔존 차용 문장은 36파일보다
> 적을 수 있다.**
> ⓔ **`bundle-keyring.mjs`의 처분은 정하지 않았다** — 판10에 열어 뒀다.
> ⓕ **`transform` 42의 잔존 수치는 상한도 하한도 아니다** — 60자 초과 행의 완전 일치만
> 셌다(부분 일치·재배열은 과소 · 정형 문구는 과대). **파일별 판정은 판3.7이 사람 손으로.**
...
>
> **판9는 「표면을 줄이는 판」이 아니라 「근거를 가진 1회 판정을 내는 판」이다.** 결과가
> 「전량 유지」여도 성립한다(D-083이 이미 한 번 그렇게 답했다). ⚠ **묻는 무게가 달라졌다** —
> 교체 전이라면 아직 아무에게도 안 나간 표면을 줄이는 일이었지만, 지금은 **이미 배포된
> 제품에서 도구를 빼는 일**이고 **검증 기준 1(표면 186 동일 이름·호출 계약)과 정면으로
> 부딪힌다.** 빼기로 정하면 스냅샷·tool-catalog·어댑터 프리셋·권한 정책이 함께 움직이고
> 그 자체가 새 D-결정이다. 브리프는 `docs/RUN-PLAN.md` 판9.
>
...
>    ⚠ **머신 함정 둘** — ⓐ `sapkit-engine/node_modules`에 `esbuild`가 없으면
>    `npm run verify`가 `ERR_MODULE_NOT_FOUND`로 죽고 `bundle-smoke`가 연쇄 실패한다.
>    `cd sapkit-engine && npm ci`로 해소. ⓑ `sapkit-cli/node_modules`도 없으면 첫 실행이
>    실패한다. **둘 다 레포 회귀가 아니다.**
>    ⚠ **판8 착수·마감 실측은 34/34 exit 0**(`doctor` 포함)이다 — 판7.5 마감과 동일.
...
> **판9**(다음) → **판10**(⑷ ㉯ 활성 차용 6 소진) · **판3.7**(⑶-b `transform` 42 재저작 —
> **D-104가 신설**) · 판M2-a(새 D-결정 필요) · 판M2-b(**BTP 계정 — 사용자만 답을 가진 것**).
> **판9의 전제는 판6.3이 채웠다**(실사용 분포 갱신). 브리프는 전부 `docs/RUN-PLAN.md`.
>
