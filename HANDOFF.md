...
>
> ## ✅ 직전 판 — **판3.1: ⑶-b 계수 방식 확정 + 잔량 명부 신설 + 1차 배치 5편** (2026-08-17 · D-087)
>
> **⑶-b의 눈금이 생겼고 루프가 한 바퀴 돌았다 — 코드 0줄, 문서만.** 분류 장부 은퇴로
> 공백이던 **`copy` 잔량 계수 방식**과 **1차 치환 대상 선정 기준**을 결정으로 닫고, 그
> 결정대로 **잔량 명부를 신설**한 뒤 **상시 로드 규약 5편을 실제로 재저작**했다. 이로써
> ⑶-b의 완료 기준(「잔량 단조 감소」)이 **측정 가능**해졌다. 결정 기록 = **D-087**.
>
> - **잔량 명부 정본 = `docs/reference/copy-baseline.md`** · **총계 170** = 1부 지식 갈래
>   146 + 2부 편입분 24 · **현재 잔량 165**. **잔량 = 총계 − 체크 누계**이고, 체크는
>   **재저작만** 인정하며(**삭제로 갚는 길은 없다** — 그 예외는 사용자가 기각했다),
>   **독립 새-컨텍스트 리뷰 통과 후**에만 찍고 커밋 해시 + 리뷰 근거를 병기한다.
>   ⚠ **이 명부를 게이트·스크립트로 만들지 말 것** — 만드는 순간 은퇴한 이식 게이트와
>   같은 물건이 된다. 진척은 git 이력이 증명하고 명부는 그 요약이다.
> - **편입 24 = 이번 판이 연 새 범위** — 지식 폴더 **밖**이라 어느 판도 맡지 않던 copy
>   (안전 훅 4 · `tools/spec/` 5 · `tools/fetch/` 2 · `core/policies/data-protection/` 12 ·
>   사양서 xlsx 1)를 판3.x 범위로 **편입했다**(사용자 결정 — 산출자 권고는 이연이었다).
>   **이번 판은 등재만** 했고, 갚는 방법은 그 파일을 맡는 후속 판3.x가 정한다.
> - **셈 밖 소관 명기**: 서버 결부 copy 39건(`server/sap-assets/` 30 · `tool-catalog/` 4 ·
>   유틸 2 · 번들 계열 3) → **판7**(⑴ 교체가 은퇴·대체 범위에서 판정) ·
>   `server/runtime-deps/keyring/` 21 → 끝그림 ② · 루트 `LICENSE` 1 → ⑷ 자체.
>   **판8(⑷) 착수의 실질 전제 = 명부 170 소진 + 판4(oop 20) + 판7 소관분 판정.**
> - **1차 배치 = 상시 로드 규약 5편**(선정 기준 = **소비 실증**: 정책이 실제로 로드하는
>   지식부터 — `interactive/core/policies/sap-standards.md`가 직접 로드하는 전부):
>   `naming-conventions` · `sap-version-reference` · `cloud-abap-constraints` ·
>   `ecc-ddic-fallback` · `abap-release-reference`. **잔량 170 → 165.**
> - **검증**: 독립 새-컨텍스트 리뷰 3건 전부 통과 — **사실 누락 0**(대조 항목 793 · 표 셀
>   전수 일치 · 릴리스 번호 49쌍 양방향 일치) · 구성 독립성 5/5 · 명부 **170/170 멤버십
>   일치**(레시피 독립 재실행) · 소비자 정합에 **이번 판이 만든 어긋남 0**. 리뷰가 잡은
>   「원문에 없던 주장·지시 4건」은 회수했다(재저작은 사실을 **더하는 것도** 범위 밖).
>   제품 게이트 9종 + doctor 전부 exit 0 · 지식 `.md` **148 유지** · 버전 무변.
> - **⚠ push 전 확인 의무 (편승 — 판2에서 승계, 이 판도 동일)**: 판3.1은 **버전을 올리지
>   않는다.** 콘텐츠 변경은 판1이 이미 올려 둔 **미배포 0.7.0 범프에 편승**해 전달된다
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.2: ⑶-b 2차 배치 (반복 판 계속)**
>
> **결정은 이미 서 있다 — 판3.2는 「무엇을 다음에 다시 쓸까」만 고르면 된다.** 계수
> 방식·체크 규칙·선정 기준은 **D-087**이 확정했고 명부(`docs/reference/copy-baseline.md`)가
> 그 집행 자리다. 판3.2는 D-087 ⑧의 **소비 실증** 기준을 그대로 적용해 다음 배치를 고르고,
> 재저작 → 독립 리뷰 → 명부 체크 → 잔량 이력 append의 같은 루프를 돈다.
>
> - **다음 배치 후보**: 규약 잔여 13편(명부 §6의 1-D). 그중 `clean-code-oop` ·
>   `include-structure` · `oop-pattern` 3편은 **앵커 문자열 `zrsc4sap_oop_ex` 보존 필수** —
>   개명 게이트(`check-runtime-path-rename.mjs`)가 그 정확 문자열을 지킨다.
> - **이월 지적 4건**(이번 판이 소비자 무수정 원칙 때문에 손대지 않은 **기존** 어긋남 —
>   구본 대조로 이번 판 이전부터 있던 것임을 확인했다): `sap-standards.md:56`의 Cloud
>   Private 행 오인용 · Z/Y 네임스페이스 서술과 `abap/conventions/`(Z 전용)의 간극 ·
>   변수 접두사·상수 규칙을 `abap/conventions/`에 있다고 가리키는 소비자 2곳(실제 위치는
>   `modules/common/naming-conventions.md`) · `clean-code.md:5`의 이식 전 경로.
>   **고치려면 소비자 문서를 여는 별도 결정이 필요하다.** 상세는 `docs/RUN-PLAN.md`
>   판3.1 브리프의 「판3.2 재료」.
> - **⚠ 재저작의 함정 하나 — 판3.1이 실제로 밟았다.** 「사실을 잃지 않았는가」만 재면
>   부족하다. 5편 **전부**에서 저자가 설명을 돕겠다고 넣은 문장이 **원문에 없던 SAP 단언·
>   지시**가 됐고(두 건은 자기 문서 안의 표와 모순), **세 차례 회수**해서야 정리됐다.
>   재저작은 표현·구성만 바꾸는 일이므로 **더하는 것도 범위 밖**이다 — 리뷰 근거는
>   **누락 0 + 추가 0** 양방향으로 잰다. 명부 §2에 명문화해 뒀다.
> - **제품명 표기 갈림 (이월)**: 5편 중 `ecc-ddic-fallback.md`만 `SAPKIT`을 쓰고 나머지
>   4편·코퍼스 다수는 `sc4sap`이다. 코퍼스 일괄 정리 안건이지 파일 단위 재저작 판의 몫이
>   아니다.
> - **사실 자체가 의심스러운 지점 2건**(재저작 범위 밖이라 원문 그대로 보존):
>   `sap-version-reference.md`의 `MATNR 40 characters (1909+)`(외부 확인으로는 1511 ·
>   기본 비활성) · `abap-release-reference.md` 체크리스트의 `GetPackage`(실제 도구 표면은
>   `GetPackageContents`/`ReadPackage`). **내용 개정 판의 안건이다.**
>
...
> 든다). 시작 명령: `/dryforge:ready docs/RUN-PLAN.md 판3.2`.
