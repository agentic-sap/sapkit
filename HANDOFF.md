...
>
> ## ✅ 직전 판 — **판4: ⑶-c OOP 템플릿 정본 전환 — 자체 저작 22파일 신설 (D-090 신설)** (2026-08-18)
>
> **OOP 템플릿의 정본이 `zsapkit_oop_ex` 계열로 옮겨졌다.** 차용 20파일(`zrsc4sap_*`)을 대체할
> **자체 저작 22파일**(prog 10쌍 + 화면 2)을 신설하고, **소비자 6문서와 개명 게이트의 정본
> 지위를 신 템플릿으로 전환**했다. 이 판이 연 결정이 **D-090**이다(사용자 상시 위임 + 밤샘
> 위임 하 **산출자 권고 채택**) — 판3.6이 판4에 넘긴 「옛 앵커 **보존**」 지시를
> **앵커 전환**으로 뒤집은 것이 핵심이다. **옛 20파일은 무참조로 존치**하며 제거는
> **판6 검증 뒤**다.
> **명부(`copy-baseline.md`)는 무접촉** — oop 20은 ⑶-b 셈 밖(⑶-c 눈금)이고 **170/170·잔량 0**은
> 이 판으로 변하지 않는다.
>
> - **⚠ D-090 신설 — 정본 전환 5종**: ⓐ **앵커 전환**(개명 게이트 ANCHORS의 ABAP 앵커 **6항을
>   `zrsc4sap_oop_ex`→`zsapkit_oop_ex`로 교체** + 음성시험 literal 동반 갱신 · keychain 1항
>   무접촉 — **게이트 데이터 갱신이고 판정 로직은 무접촉**이며 판정 능력은 복제 트리 실증
>   **6/6**으로 확인. `CLAUDE.md`의 「게이트 수정 금지」 경고는 `sapkit-engine`·`sapkit-cli`의
>   무접촉 증명을 지키는 조항이라 **무충돌**) · ⓑ **신·구 병존**(신 22가 정본 · 옛 20 존치 ·
>   제거는 판6 뒤 — BLUEPRINT §3.2) · ⓒ **소비자 인용의 정본 = 실코드**(옛 인용의 생성자 모순
>   `NEW lcl_alv( go_data )`를 신 실코드 축자로 해소) · ⓓ **체크 완료편 최소 수정 2곳**
>   (`oop-pattern.md` :3 출처 서술 → 역사 서술 · :45 열거에 `tst` — 앵커 전환이 **만든** 어긋남의
>   즉시 정합) · ⓔ **`sapkit-cli/fixtures/corpus/templates/` 무접촉**.
> - **⚠ 코퍼스 픽스처를 신 템플릿으로 「동기화」하지 말 것** (D-090 ⓔ) — 그 11파일은 **옛
>   템플릿의 바이트 스냅샷**이자 `vsp/` 은퇴 뒤 **구 vsp 판정의 유일 잔존 기준선**이다(D-085).
>   갈아 끼우면 기준선이 **영구 소실**된다. **낡아 보이는 것이 정상이다.**
> - **옛 결함 3종 해소**(신 템플릿에 미승계): 댕글링 `INCLUDE ZRSC4SAP_OOP_EXTST`(**`_tst`
>   신설**로 해소) · 미선언 변수 `LV_001`/`LV_002` · 비존재 구문 `AT SELECTION-SCREEN OUTPUT
>   FOR FIELD`. 트리 부모키 오배정·미사용 상수 26 등도 승계하지 않았다.
> - **검증**: 독립 새-컨텍스트 리뷰 **2팀** · **회수 2건**. **R-TP** = **FAIL 1(MINOR)** — Main
>   헤더 주석의 INCLUDE 계수 `ten`↔실측 9 → `a66c4415`로 `nine` 정정. 그 밖 전 축 PASS(**상속
>   API 22종 시그니처 대조** · TOP 고아 0 · 스크러빙 완결 grep 신 파일 **0건** · **게이트 판정
>   능력 독립 실증 6/6** · 게이트 11종 exit 0) · **RF** = **PASS** — 게이트 **13종 exit 0** ·
>   **변경 전모 30파일 정확**(신설 22 + 소비자 6 + 게이트 1 + 음성시험 1) · 코퍼스 픽스처 diff
>   **0** · 명부 무접촉. **오프라인 검사** = 신 `.abap` **lint 12/12 무소견** · parse 미해석 0.
>   커밋 = TP1 `bc86bfc9` · TP2 `aad03850` · `afb1e786`(음성시험 13/13 복귀) · 회수 `a66c4415`.
>   지식 `.md` **148 유지**(`templates/`는 계수 밖) · 버전 무변 · 생성물 7종 무접촉.
> - **⚠ 이 판의 산출물은 실 SAP에서 한 번도 검증되지 않았다 — 판6 몫**: 22파일 전량이 생성·활성·
>   기계 확인 미이행이고, **구문 3곳은 릴리스 하한조차 미확인**이다(`CALL TRANSACTION …
>   WITH AUTHORITY-CHECK` 7.52+ · `UP TO @gc_max_rows ROWS` · `SELECTION-SCREEN COMMENT`
>   전역명 8자 축약). **lint 무소견은 오프라인 검사기의 판정이지 SAP 구문 검사가 아니다.**
>   대상 목록·제거 조건·`_tst` 접미 재고(F3)는 `docs/RUN-PLAN.md` 판4 브리프의 「판6 재료」.
> - **⚠ 이월된 것 (이 판이 손대지 않음)**: **소비자 4곳 INCLUDE 순서 정렬**(판3.6이 판4에
>   넘겼으나 **판4 사양이 산문 개정을 범위 밖으로 명시**해 그대로 — `include-structure.md:24`·
>   `:32` · `clean-code-procedural.md:13` · `review-checklist.md:208`) · **`SD/bapi.md:31`
>   잔존 1건**(수리 커밋 1건이면 닫힌다) · 판3.6까지의 이월 전량 · 판4 신규 **F3**(`_tst` 표기
>   갈림) · **F4**(「6필드 머리글」 규범 자기 붕괴 — 선재) · **F9**(`../common/` 경로 표기 —
>   선재) · **F8**(검사기 `parse`의 `Unknown` 도달 불가). 상세는 `docs/RUN-PLAN.md` 판4 브리프.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1~3.6에서 승계, 이 판도 동일)**: 판4는 **버전을
...
>   **「위임 하 권고 채택」을 명시**한다.
> - **⚠ 위임 1회성 확장 (2026-08-18 밤 — 이 밤에 한함)**: 사용자가 밤샘 위임으로 **① 판5까지
>   연속 주행 ② 판 사이 `ready`/`go` 대행 ③ 커밋·푸시·머지 ④ 완료 후 시스템 종료**를 허용했다.
>   **이 확장은 2026-08-18 밤의 연속 주행에만 유효하다** — 그 뒤의 판은 **기본 규칙으로 복귀**
>   한다(판 사이는 사용자 한 줄 입력 · push는 요청 시에만). 위 「상시 위임」의 경계 중
>   **판6+(SAP attended) 자동 불가는 이 확장에도 그대로 남는다**(판5까지가 확장의 상한이다).
>   **⚠ 판4가 끝났으므로 이 확장의 잔여는 판5 하나다** — 판5가 밤샘 위임의 **마지막 판**이고,
>   완료 후 **push·머지 → 시스템 종료**가 예정된 종착점이다.
...
>   것**: `test-hook-decisions.mjs`는 아직 **로컬 러너**이고 **CI·게이트 목록 편입 판단이 판5
>   몫**이다(D-089 ⓔ) · 동결 승계한 갭 전량(훅 파서 갭 4종 · `fetch-sap-help-doc` exit 크래시 ·
...
>   한다**).
>
> ## 🔻 다음 판 — **판5: ⑴ 엔진 잔여 코드 (M2 인증 · D18 · keyring CI · 러너 CI 편입 판단)**
>
> **⑶-c가 닫혔으므로 오프라인 ⑶ 갈래는 전부 끝났다** — 명부 170/170 · OOP 템플릿 정본 전환
> 완료. 남은 오프라인 판은 **판5 하나**이고, 대상은 **`sapkit-engine/`의 잔여 코드**다
> (제품이 아니다 — 구 번들 엔진 5.0.0이 여전히 현역이고 교체는 판7 소관).
>
...
> 끝나면 다음 입력 명령을 보고에 명시한다. **단 2026-08-18 밤의 1회성 확장 동안은
> 판 사이 `ready`/`go` 대행과 push·머지가 허용된다**(위 직전 판 절의 확장 항목 —
> **판5가 그 확장의 마지막 판**이고 그 뒤는 기본 규칙 복귀 · 판6+는 확장에서도 자동 불가).
>
> - **판5의 네 갈래** (상세는 `docs/RUN-PLAN.md` 판5 브리프): ⓐ **M2 인증 확장 코드** — UAA
>   토큰 취득 + 그 위의 JWT 접속 계층(신 엔진 `AdtClient`는 Basic만 다룬다 — M-표에서 M2만
>   `일부`인 이유) · ⓑ **D18** — 무접속 거부 어휘를 `conformance-server-gates.mjs`가 구·신
>   **둘 다** 인식하게 넓히기(**신 문구로 교체 금지** — 구 번들이 현역인 동안 그쪽 판정이
>   깨진다) · ⓒ **keyring 폴백 CI 잡**(`npm ci --omit=optional` — fallback 설계가 아직 어떤
>   CI로도 입증되지 않았다) · ⓓ **`test-hook-decisions.mjs` CI 편입 판단**(D-089 ⓔ가 판5로
>   이연한 몫 — 지금은 로컬 러너다. CI `offline-gates.yml`과 `CLAUDE.md` 게이트 목록에 올릴지를
>   이 판이 정한다).
> - **⚠ `sapkit-engine/`을 만질 때의 상시 제약**: 제품(`engine/`·`interactive/server/`)과
>   **섞지 말 것**이고, **제품 게이트 스크립트를 고치지 말 것**이다 — 「제품 게이트 전종 여전히
>   green」이 구 부품 무접촉의 기계 증명이기 때문이다(`CLAUDE.md`). ⚠ 단 **판4의 D-090 ⓐ는
>   그 경고의 예외가 아니라 범위 밖**이다(제품 게이트의 **데이터** 갱신이었고 `sapkit-engine`·
>   `sapkit-cli`를 건드리지 않았다) — 판5는 그 선례를 자기 작업에 확대 적용하지 말 것.
> - **⚠ 판4가 남긴 SAP 부채는 판5가 아니라 판6 몫이다** — 신 템플릿 22파일의 생성·활성·기계
>   확인, 미검증 구문 3곳, 옛 20파일 제거, `_tst` 표기 재고. 판5는 **오프라인 판**이다.
> - **이월 전량 승계**: 판3.6까지의 이월 전부(소비자 오인용 4건 · 사실 의심 · 제품명 표기 갈림 ·
>   이식 전 참조 보존 목록 21곳(⚠ 그중 1곳은 소비자 오인용 ④와 같은 건 — **이중 계수 금지**) ·
>   **참조표 사실 의심 밀도 경보 BW·QM·TR·TM·WM** · 낡은 도구명 동결 55곳 · 훅 파서 갭 4종 ·
>   문서↔훅 불일치 6건 · 브랜드 표기 4갈래 혼재 · `screen-image-renderer` QUIRK 5건 ·
>   `fetch-sap-help-doc` exit 크래시 · **md 블록리스트 상설 게이트 무방비**) + **판4 신규**
>   (F3 `_tst` 표기 갈림 · F4 「6필드 머리글」 규범 자기 붕괴 · F9 `../common/` 경로 표기 ·
>   F8 검사기 `parse`의 `Unknown` 도달 불가) + **소비자 4곳 INCLUDE 순서 정렬**(판3.6→판4로
>   넘어왔으나 판4 사양이 범위 밖으로 명시해 **여전히 열려 있다**) + **`SD/bapi.md:31` 잔존
>   1건**(수리 커밋 1건이면 닫힌다). **상세는 `docs/RUN-PLAN.md` 판4 브리프의 「이월 승계」.**
>
> **진행 방식은 D-084 그대로다 — 「짓기 우선 완주, SAP 증거는 최종 테스트 판으로 이연」.**
> ⑵는 판1로, **⑶-a는 판2로, ⑶-b는 판3.x(판3.6 완주)로, ⑶-c는 판4로 닫혔다.** 남은 오프라인
> 갈래는 **⑴ 잔여 코드(판5)** 하나다. attended 증거는 **판6(최종 테스트 판)** 하나로 모은다.
...
> 든다). 시작 명령: **`/dryforge:ready docs/RUN-PLAN.md 판5`** → 3-doc 승인 후 같은
> 세션에서 **`/dryforge:go`**. ⚠ **push 직전 `node interactive/scripts/doctor.mjs`로 설치본
> 버전 확인**(미배포 0.7.0 편승 — 0.7.0 설치본이 있는 머신이 발견되면 패치 범프 후 push).
> **밤샘 위임(2026-08-18 밤)의 잔여 범위 = 판5 하나다 — 판4는 닫혔다.** 판 사이 `ready`/`go`
> 대행 · 커밋·머지·push까지가 그 범위이고, **판5 완료 후 push·머지 → 시스템 종료**가 예정된
> 종착점이다. **판6+(SAP attended)는 확장에서도 자동 불가**이며 그 뒤의 판은 기본 규칙(판
> 사이 사용자 한 줄 입력 · push는 요청 시에만)으로 복귀한다. 「M1 잔여부터」는 **판6 몫으로
> 이연** — 상세는 아래 절에 재료로 보존.
>
...
> 소진됐다**(D-085 · D-086 · D-089 · D-090) — 남은 오프라인 활주로는 **⑴ 잔여 코드(판5)**
> 하나다.
>
> ### 고지 은퇴는 훨씬 멀다 (혼동 방지)
>
