...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **아래 「완주 로드맵」을 읽고 ①부터**
>
> ## 🔔 상태 셋 (2026-08-23 세션 마감 시점)
>
> **① 사다리 ⑴의 「짓기」가 전부 끝났다.** M-표 짓기 칸이 **M1~M5 전부 ✅**다 — 오늘
> 판M2-c가 마지막 M2를 세웠고(D-117 배선 + **D-118 attended 확인**) **D15가 해소**됐다.
> **2026-08-10 D-079가 연 사다리 ⑴의 접속 갈래가 여기서 닫힌다.**
>
> **② 남은 것은 「증거」와 잡무뿐이고, 그 지도는 아래 「완주 로드맵」이 정본이다.**
> 네 덩어리(A 재생 18 · B attended 12 · C 잡무+브랜드 · D 이월 리뷰)와 **권장 순서
> ①D→②A→③B→④C**, 그리고 **완주 판정 기준**까지 그 절에 적혀 있다.
...
> 보존/갱신을 먼저 보는 게 낫다」고 적은 적이 있는데 **틀렸다.** 증거 작업은 **BTP가
> 아니라 기존 DEV 프로파일**에서 하므로 「세션마다 로그인」 마찰이 **거기엔 없다.**
> 근거는 로드맵 ⑴.
>
> ## 🗺 완주 로드맵 — **「최종」까지 남은 전부** (2026-08-23 작성 · 다음 세션이 이것으로 달린다)
>
> **완주의 정의** = ① 끝그림 ①~⑤(`docs/BLUEPRINT.md` §1.1) + ② M-표 **증거 칸** + ③ 잡무.
> **①은 ④(브랜드)만 남았고, ②가 진짜 덩어리다.**
>
> ### ⚠ 먼저 — 이 두 사실을 모르면 계획을 잘못 세운다
>
> **⑴ 증거 작업은 BTP가 아니라 기존 DEV 프로파일에서 한다.** `harness/record-attended.mjs`는
...
> **BTP destination은 tier=UNKNOWN이라 write가 fail-closed라서 이 작업에 쓸 수 없다.**
> → **「세션마다 로그인」 마찰은 증거 작업에 해당하지 않는다.** 판M2-c가 푼 것은 **기술적
> 의존이 아니라 순서**였다(D-081의 M1→M6 고정이 증거 축으로 존속 — D-082).
> **그러므로 토큰 보존/갱신 판은 증거 판의 선행이 아니다.** 앞선 세션에서 그렇게 말한
> 적이 있는데 **틀린 조언이었다.**
>
> **⑵ attended 실기 12종은 이미 한 번 실패했다 — 재시도로 풀리지 않는다.** 판6.3(D-098)이
> 46종을 시도해 **34종은 섰고 12종은 이유와 함께 못 섰다.** **지금 증거 대기의 attended
> 12종이 정확히 그 12종이다**(이름 대조 완료). 「다시 해 보면 되겠지」로 판을 열면 그대로
> 다시 막힌다.
>
> ### 남은 것 — 네 덩어리
>
> **A. 증거 · 재생 대조 18종** — 판 1개 규모 · **기계적**
> `GetLocalTestClass` `GetLocalTypes` `UpdateTable` `GetView` `ReadBehaviorDefinition`
> `GetDataElement` `GetServiceBinding` `GetDomain` `GetMetadataExtension`
> `UpdateMetadataExtension` `GetServiceDefinition` `UpdateServiceDefinition`
> `ReadBehaviorImplementation` `GetFunctionGroup` `GetInterface` (15) +
> **재생 대조 + 대체** `UpdateLocalTestClass` `UpdateLocalTypes` `UpdateView` (3).
> 방법 = DEV 프로파일에서 **녹화 → 재생 → 정규화 diff 0**. 판6.1이 9종으로 같은 일을 했다.
>
> **B. 증거 · attended 실기 12종** — **부류마다 처방이 다르다. 한 판이 아닐 수 있다.**
>
> | 부류 | 도구 | 왜 못 섰나 (판6.3 실측) | 처방 |
> |---|---|---|---|
> | ⓐ **원리상 불가** 2 | `DeleteUnitTest` · `CreateCdsUnitTest` | SAP 호출 **0회** — 엔진 소스가 이미 사연을 적어 뒀고 실기로 확인됐다 | **판정 사안**(면제·재분류) — 시도가 아니다 |
> | ⓑ **거짓 음성** 2 | `CreateMetadataExtension` · `CreateServiceDefinition` | **객체는 생겼는데 도구가 오류를 보고한다** — 사슬이 생성→잠금→검사라 검사가 깨져도 생성은 끝난 뒤다 | **엔진 수리** — 진짜 결함이다 |
> | ⓒ **시스템 측 고장** 6 | 텍스트·화면·GUI 계열(`CreateTextElement` `CreateScreen` `CreateGuiStatus` `DeleteTextElement` `DeleteScreen` `DeleteGuiStatus`) | **`ZMCP_ADT_SRV`가 HTTP 500** — 텍스트 엘리먼트도 같은 서비스를 탄다는 것이 판6.3의 새 관측 | **SAP 측 자산 문제** — `interactive/server/sap-assets/` 재설치 점검 또는 다른 DEV 시스템. ⚠ **우리 레포에서 못 고칠 수 있다** |
> | ⓓ **엔드포인트/P4** 2 | `CreateUnitTest`(`/abapunit/runs` **404**) · `CreatePackage`(세 갈래 다 거부) | 시스템 기능 부재 · P4 권한 | **진단 선행** — P4는 사람 승인 자리다 |
>
> **C. 잡무 5 + 끝그림 ④(브랜드)** — 반나절 규모 · 아래 「남은 것」 표 참조.
>
> **D. 이월 리뷰** — **D-118(attended 확인) + D-119(회수)를 함께 보는 독립 리뷰.**
> 판M2-c 리뷰가 그 둘을 보지 못했다(리뷰 중 대상이 움직였다 — D-119 유보 ⓑ).
>
> ### 권장 순서
>
> **① D(이월 리뷰) → ② A(재생 18) → ③ B(부류별) → ④ C(잡무)**
> D를 먼저 두는 이유: 부채이고 짧다. A를 B보다 먼저 두는 이유: **기계적이라 예측 가능**하고,
> B는 그 안에서 다시 갈라진다.
>
> ### 완주의 규모 — **판 4개면 끝이다** (D-120 ⓒ)
>
> **① 판R**(리뷰 · 짧음) → **② 판E1**(재생 18 · 기계적) → **③ 판E2**(attended 12 ·
> **대부분 「이유와 함께 못 섰다」로 닫힌다**) → **④ 판Z**(잡무+브랜드 · 반나절).
> **이 넷이 끝나면 완주다.** 규모는 판E2가 부류별로 갈릴 수 있어 **±1**이다.
>
> ### 완주 판정 기준 — **무엇이면 끝인가**
>
> ⓐ M-표 **증거 칸**이 M2~M5 전부 **✅ 또는 「이유와 함께 못 섰다」**로 닫힌다
> (D-098 선례 — **못 선 것도 이유가 있으면 닫힌 것**이다) ⓑ 끝그림 **④ 브랜드**가 닫힌다
> ⓒ 잡무 5건이 처리된다 ⓓ 이월(D-118·D-119 리뷰)이 닫힌다.
>
> ### ⚠ 정직하게 — 완주가 「전부 ✅」로 안 끝날 자리들
>
> - **ⓒ 6종은 SAP 시스템 측 고장**이라 우리 레포에서 못 고칠 수 있다. 그러면 **「이유와
>   함께 못 섰다」로 닫힌다** — 그것이 이 프로젝트의 완료 형태다(D-098이 이미 그렇게 했다).
> - **ⓐ 2종은 원리상 불가** — 「증거 있음」이 아니라 **면제**로 닫힐 것이다.
> - **요구 급 인하 62종은 되돌리지 않는다 — 완주는 「증거 대기 30」 기준이다**(**D-120** ⓐⓑ).
>   ⚠ **92는 「더 생긴 일」이 아니다** — 도구 **186종은 그대로**이고, 그 62종의 **요구 급**을
>   어느 잣대로 보느냐의 차이다. D-092 ⓐ가 「판6에서 한 번도 안 쓰인 꼬리」의 요구 급을
>   `재생 대조` → `계약 시험`으로 인하했고 **그것들은 이미 계약 시험을 통과한 상태**여서
>   같은 순간 `증거 있음`으로 넘어갔다. **인하가 증거를 만든 것은 아니므로** 대장 머리말은
>   두 수를 계속 병기하지만, **완주 판정의 잣대는 30이다.**
> - **끝그림 ③은 「최종 0건」이 도달 불가로 이미 판정됐다**(D-103) — `sap-assets/` 30은
>   영구 존치다.
>
