...
>
> ## ✅ 직전 판 — **판3.2: ⑶-b 2차 배치 — 1-D 규약 잔여 13편 재저작** (2026-08-17)
>
> **1-D(규약) 갈래가 소진됐다 — 코드 0줄, 지식 `.md`만.** D-087 ⑧의 **소비 실증** 기준으로
> 명부 1-D의 **잔여 13편을 전량 재저작**했다. 앵커 3편(`clean-code-oop` ·
> `include-structure` · `oop-pattern`)은 `zrsc4sap_oop_ex`를 **그대로 보존**했고 개명
> 게이트로 확인했다. **1-D 18/18** · **잔량 165 → 152**(명부
> `docs/reference/copy-baseline.md`). **새 D-결정은 없다** — 이 판의 선택은 전부 D-087
> 적용이거나 기존 규율 승계, 또는 사용자 확정 답이었다.
>
...
>   `procedural-form-naming` · `text-element-rule`. 커밋 해시·리뷰 근거는 명부 §6 1-D가 든다.
...
>   버전 무변.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1에서 승계, 이 판도 동일)**: 판3.2는 **버전을
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.3: ⑶-b 3차 배치 (반복 판 계속)**
>
> **결정은 이미 서 있다 — 판3.3도 「무엇을 다음에 다시 쓸까」만 고르면 된다.** 계수
> 방식·체크 규칙·선정 기준은 **D-087**이 확정했고 명부(`docs/reference/copy-baseline.md`)가
> 그 집행 자리다. **1-D 갈래는 판3.2로 소진됐으므로**(18/18) 판3.3은 명부 1부의 **나머지
> 갈래**에서 D-087 ⑧의 **소비 실증** 기준을 그대로 적용해 다음 배치를 고르고, 재저작 →
> 독립 리뷰 → 명부 체크 → 잔량 이력 append의 같은 루프를 돈다.
>
> - **다음 배치 후보**: 명부 §6의 나머지 갈래 — **1-A 모듈 91 · 1-B 산업 15 ·
>   1-C 국가 17 · 1-E 견본 5**.
> - **이월 목록**(판3.2가 손대지 않고 그대로 넘긴 것): 소비자 오인용 4건 · 사실 의심 2건 ·
>   제품명 표기 갈림(`SAPKIT` vs `sc4sap`) · **이식 전 참조 보존 목록**(재저작이 원문 그대로
>   둔 옛 경로 표기 20곳 — 일괄 정비 안건) · **소비자 서술 불일치 신규 발견 4건**(전부
>   재저작 이전부터 존재 — 구본 대조 확인 · 내용 개정 판 소관) · `constant-rule.md` 절 헤딩
>   재작성 기록 1건. **상세는 `docs/RUN-PLAN.md` 판3.2 브리프의 「판3.3 재료」.**
> - **⚠ 재저작의 함정 — 판3.1·판3.2 둘 다 밟았다.** 「사실을 잃지 않았는가」만 재면
>   부족하다. 판3.1은 저자가 설명을 돕겠다고 넣은 문장이 **원문에 없던 SAP 단언·지시**가
>   돼 5편 전부에서 **세 차례 회수**했고, 판3.2도 **강도·범위 미세 이탈 9곳**을 회수했다
>   (`bcbe36f6`). 재저작은 표현·구성만 바꾸는 일이므로 **더하는 것도 범위 밖**이다 —
...
> 든다). 시작 명령: `/dryforge:ready docs/RUN-PLAN.md 판3.3`.
