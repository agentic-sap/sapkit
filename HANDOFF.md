...
>
> ## ✅ 직전 판 — **판3.6: ⑶-b 6차 배치 — 잔량 29편 5갈래 전량 (⑶-b 완주 · D-089 신설)** (2026-08-18)
>
> **명부가 소진됐다 — 170/170 · 잔량 0.** 잔량 **29편을 한 판에 전량** 처리했다: 견본 `.abap` 5 ·
> 안전 훅 `.mjs` 4 · 사양서 도구 `.mjs` 5 · 조회 도구 `.mjs` 2 · 데이터보호 정책 `.md` 12 ·
> xlsx 바이너리 1. **⑶-b 갈래가 완주했다**(명부 `docs/reference/copy-baseline.md`). 산문 재저작
> 계약(D-087·D-088)이 그대로 서지 않는 갈래들이라 **새 계약을 정했고 그것이 D-089**다(사용자
> 상시 위임 + 밤샘 위임 하 **산출자 권고 채택**). **판3.x 중 유일하게 코드를 만진 판**이다.
>
> - **⚠ D-089 신설 — 잔량 5갈래의 재저작 계약**: ⓐ **정책 12 = D-088 변형 + 「구문 층」 신설**
>   (훅이 정규식으로 읽는 서식 — 헤딩 구조·표 헤더 열 이름·tier/action 주석·식별자·구분자·
>   와일드카드·파일명 — 을 **재저작 원천 배제** · **게이트 하드코딩 6종**(USR02·BNKA·KNA1·
>   VBRK·BALDAT·PA0008) 문자 불변 · **체크 전제 = 파싱 동치 기계 증명**) · ⓑ **코드류 16편 =
>   동작 동결 재구현**(판정·출력·exit·export 동치를 채록·차분으로 증명 · **알려진 갭도 함께
>   동결** — 개선은 별건) · ⓒ **xlsx = 생성 스크립트 재제작**(전 구성 자체 산출 + 재현성 기계
>   증명) · ⓓ **`alv-rules.md` fence 1곳 동기화 예외**(체크 완료편 무접촉의 **명시 예외**) ·
>   ⓔ **훅 판정 채록 러너 신설**(74케이스 — 안전 계층 L3 판정의 **첫 회귀 기준** · **CI 편입은
>   판5에서 판단**).
> - **검증**: 독립 새-컨텍스트 리뷰 **4팀 전원 PASS** · **회수 1건 + 경량 수정 1건**.
>   **R-DP** = 파싱 동치 독립 재검 11/11 + **실훅 바이너리 530회 종단 대조**(5프로파일 ·
>   **판정 차이 0**) + 하드코딩 **6/6 바이트 동일** + 구문 층 **12/12 불변** · **R-CODE** =
>   11 PASS / 1 FAIL → `378db9c7` 회수(견본 closing line의 **축자 계약 이탈** · Major) +
>   **74건 스위트를 원본 훅에도 걸어 동일 통과** + 변이 3종 검출 실증 · **R-SPEC** = build-spec
>   **E2E 43/43** + export **18종** 확정 + 템플릿 신조 **재현성 기계 증명** +
>   `program-to-spec.md` 수치·예시 5곳 정합(`a5f5279a`) · **RF** = 게이트 **12종 green**
>   (제품 9종 + doctor + `test-hook-switch` 13/13 + 신설 `test-hook-decisions` 74/74).
>   지식 `.md` **148 유지** · 버전 무변 · 생성물 7종 무접촉.
> - **⚠ 신설 파일 2종 — 다음 판이 알아야 한다**: `interactive/scripts/test-hook-decisions.mjs`
>   (훅 판정 채록 러너 74케이스 · **아직 로컬 러너다** — CI와 `CLAUDE.md` 게이트 목록 등록은
>   **판5 판단**) · `interactive/tools/spec/gen-template-base.mjs`(xlsx 생성 스크립트 —
>   `template_base.xlsx`는 이제 **생성물**이고 92,153B → **45,357B 신조**로 대체됐다).
> - **⚠ 동결 승계한 갭 (전부 선재 · 수리는 별건 — D-089 정직 유보)**: 훅 파서 갭 4종
>   (`Y*` 커버리지 0 · 유령 엔트리 `VIEW` · 헤더 행 미스킵 · **비객체 JSON = 실질 fail-open**) ·
>   `fetch-sap-help-doc` 전송 실패 exit 크래시 · 문서↔훅 불일치 5건 · 브랜드 표기 3갈래 혼재 ·
>   **md 블록리스트는 상설 게이트가 읽지 않는다**(`conformance-server-gates`는 서버 번들
>   문자열만 대조 — 파싱 동치 검증이 유일 방어 · 상설화는 후속 후보). 계열별 상세는
>   `docs/RUN-PLAN.md` 판3.6 브리프.
> - **⚠ `SD/bapi.md:31` 잔존 1건은 아직 열려 있다** — 판3.5가 「판3.6이 처리한다」고 넘겼으나
>   **판3.6 사양이 범위 밖으로 명시**했다(이 판의 배치는 잔량 29편 소진이었다). 명부 체크는
>   이미 서 있으므로 **수리 커밋 1건**이면 닫힌다.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1~3.5에서 승계, 이 판도 동일)**: 판3.6은 **버전을
...
>   한다**).
>
> ## 🔻 다음 판 — **판4: ⑶-c ZRSC4SAP 템플릿 20파일 재생성**
>
> **⑶-b가 닫혔으므로 판3.x는 끝났다** — 명부 170/170 · 잔량 0. 다음은 **판4**이고, 대상은
> `interactive/core/knowledge/abap/templates/oop-sample/`의 `zrsc4sap_*` **20파일**(프로그램 9 +
> 화면 2 외)을 SAPKIT 이름으로 새로 쓰는 일이다. **레포 템플릿 원본만** — SAP 시스템 안의 기존
> 오브젝트는 무접촉이고, **실 SAP DEV 생성→활성→기계 확인은 판6으로 이연**된다(옛 템플릿 제거는
> 그 확인 뒤 — 판6 완료 전 삭제 금지).
>
...
> **판5까지가 상한**이고 그 뒤는 기본 규칙 복귀 · 판6+는 확장에서도 자동 불가).
>
> - **⚠ 판3.6의 1-E 계약이 판4의 선례다.** 판4가 다시 쓰는 20파일은 이 판이 처리한 견본 5편과
>   **같은 성격**(코드 견본 · 소비자가 파일명·구조로 인용)이다. 그러므로 **D-089 ⓑ**(동작 동결
>   재구현 — 시연 패턴·FM 흐름·교육 목적 동치 · 주석·메시지·변수명·구현 스타일은 자체 저작)와
>   판3.6의 실측 관습을 준용한다: **ecc 트리오가 헤더 라벨 `What`/`How`/`Caution`으로 독립
>   수렴**했고, **`ecc-ddic-fallback.md` §4의 축자 요소는 계약**이다(이 판의 유일한 리뷰 FAIL이
>   그 축자 이탈이었다 — `378db9c7`). 판4는 **소비자 문서가 견본 문자열을 축자로 고정한 자리를
>   먼저 찾고** 시작할 것.
> - **⚠ 앵커 제약**: 20파일 재생성이 `zrsc4sap_oop_ex`를 건드리면 개명 게이트
>   (`check-runtime-path-rename.mjs`)가 거부한다. 규약 3편(`clean-code-oop`·`include-structure`·
>   `oop-pattern`)도 같은 앵커를 품고 있다.
> - **⚠ 소비자 4곳 INCLUDE 순서 정렬 (판4가 함께 정리하는 것이 자연스럽다)**: 판3.6의
...
>   전에도 완전 일치는 아니었다. 함께 드러난 자기모순 6건(S1~S6)의 상세는
>   `docs/RUN-PLAN.md` 판3.6 브리프의 「판4 재료」. **판3.6은 소비자 무접촉 계약이라 손대지
>   않았다.**
> - **이월 전량 승계 + 판3.6 신규 등재**: 판3.5까지의 이월(소비자 오인용 4건 · 사실 의심 2건 ·
>   제품명 표기 갈림 · 이식 전 참조 보존 목록 21곳(⚠ 그중 1곳은 소비자 오인용 ④와 같은 건 —
>   **이중 계수 금지**) · 소비자 서술 불일치 4건 · 헤딩 재작성 기록 2건 · 판3.3~3.5 등재 사실
>   의심 전부 · **참조표 사실 의심 밀도 경보 BW·QM·TR·TM·WM** · 낡은 도구명 동결 55곳 ·
>   `BW/workflows:9` 키릴 동형 이체자 · `PS/workflows:82` DDLS 행 무접촉 권고 · KR
>   「전기/계상」 갈림) **전량 그대로**이고, 여기에 **판3.6이 새로 등재한 것**(훅 파서 갭 4종 ·
>   문서↔훅 불일치 5건 · 브랜드 표기 3갈래 혼재 · `sap-stocker.md:29`·`:48`과
>   `data-extraction-policy.md:116` 구경로 · `screen-image-renderer` QUIRK 5건 ·
>   `fetch-sap-help-doc` exit 크래시 · **신설 러너 CI 편입 = 판5 소관**)이 더해진다.
>   **상세는 `docs/RUN-PLAN.md` 판3.6 브리프의 「판4 재료」.**
> - **⚠ `SD/bapi.md:31` 잔존 1건** — 판3.5→판3.6으로 넘겼으나 판3.6 사양이 범위 밖으로
>   명시했다. **수리 커밋 1건**이면 닫히므로 판4가 편승 처리해도 되고, 내용 개정 판 소관으로
>   두어도 된다.
> - **⚠ 재저작의 함정 — 판3.1~3.4 넷 다 밟았고, 판3.5·판3.6이 선재 주입으로 막았다.**
>   「사실을 잃지 않았는가」만 재면 부족하다. 판3.1은 원문에 없던 SAP 단언·지시를 **세 차례**,
>   판3.2는 강도·범위 미세 이탈 9곳(`bcbe36f6`), 판3.3은 12곳(`05643eda`·`41110412`), 판3.4는
>   12곳(`88165eeb`·`d24e18a4`) 회수했다. **판3.5는 실패 모드와 판정 폭을 재저작 프롬프트에
>   선재 주입해 회수 0**, **판3.6은 같은 주입으로 회수를 1건**(축자 계약 이탈)**까지 줄였다 —
>   후속 판은 이 주입을 승계할 것.** 재저작은 표현·구성만 바꾸는 일이므로 **더하는 것도 범위
>   밖**이고, 리뷰 근거는 **누락 0 + 추가 0** 양방향으로 잰다(명부 §2).
> - **⚠ 상호 인용**: 14개 모듈 `spro.md:9`가 `common/spro.md`의 헤딩
...
> ⑵는 판1로, **⑶-a는 판2로, ⑶-b는 판3.x(판3.6 완주)로 닫혔다.** 남은 오프라인 갈래는
> **⑶-c(판4)** 와 **⑴ 잔여 코드(판5)** 다. attended 증거는 **판6(최종 테스트 판)** 하나로
> 모은다. §3.2(검증 없는 교체·은퇴 금지)와 증거 순서 M1→M6(D-081·D-082 ④)은 불변 —
> **이연이지 재배열이 아니다.**
>
> **판 큐의 정본 = `docs/RUN-PLAN.md`** (판 = dryforge ready→go 1사이클 · 판 완료 시
> 그 표 갱신까지가 작업의 일부 · 판 사이 맥락은 .dryforge 아카이브가 아니라 그 파일이
> 든다). 시작 명령: **`/dryforge:ready docs/RUN-PLAN.md 판4`** → 3-doc 승인 후 같은
> 세션에서 **`/dryforge:go`**. ⚠ **push 직전 `node interactive/scripts/doctor.mjs`로 설치본
> 버전 확인**(미배포 0.7.0 편승 — 0.7.0 설치본이 있는 머신이 발견되면 패치 범프 후 push).
> **밤샘 위임(2026-08-18 밤)의 잔여 범위 = 판4 → 판5 연속 주행 · 판 사이 `ready`/`go` 대행 ·
> 커밋·머지·push · 완료 후 종료.** 그 상한이 판5이고, **판6+(SAP attended)는 확장에서도
> 자동 불가**이며 그 뒤의 판은 기본 규칙(판 사이 사용자 한 줄 입력 · push는 요청 시에만)으로
> 복귀한다. 「M1 잔여부터」는 **판6 몫으로 이연** — 상세는 아래 절에 재료로 보존.
>
