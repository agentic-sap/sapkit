...
> 작성 2026-07-10 · 최종 갱신 2026-08-14. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
>
> ## 🔻 다음 세션 첫 작업 — **M1 잔여부터** (attended · **SAP 접속 필요**)
>
> **왜 M1부터인가**: D-082 ④가 D-081의 순서 고정을 뒤집지 않았다 — **증거를 태우는
> 순서는 여전히 M1 → M2 → M3 → M4 → M5 → M6**이다. 이 판이 앞당긴 것은 `짓기` 축
> 하나뿐이므로, 다음에 열 것은 새 도구가 아니라 **M1의 SAP 증거**다.
>
...
> ## 🚧 막힌 것
>
> ### ⓐ **D-081과 사다리 ⑴ M-표가 이 가지에 없다** ← 착수 전 사용자 판단 필요
>
> **실측(2026-08-14 재확인):** 이 가지의 결정 로그는 D-080 다음이 **D-082**이고
> (D-081 번호는 비워 뒀다), `docs/BLUEPRINT.md`에 「M1」 문자열이 **0건**이다.
> **D-081과 사다리 ⑴ M-표는 미머지 가지 `dryforge/tmp-practice-scenarios`
> (head `c5088d3` · 7커밋)에만 있다** — PR #2가 그 커밋들보다 **먼저** 머지돼서
> (`b52afc5`) D-081 정직 유보 ⓔ의 경고가 **반대 방향으로** 실현됐다.
> (검증: `git merge-base --is-ancestor c5088d3 HEAD` → 아니오)
>
> 그래서 D-082 ⑦이 배정한 **「M-표에 `짓기 완료`/`증거 완료` 두 칸 반영」은 수행하지
> 못했다.** 없는 표를 창작하지 않았고, 대신 두 단계의 정의와 이 판의 도달 지점을
> `docs/BLUEPRINT.md` 사다리 ⑴ 절에 산문으로 적어 뒀다.
>
> **택일**: ⓐ `dryforge/tmp-practice-scenarios`를 합류시킨다(권장 — 결정 로그 꼬리에서
> D-081↔D-082 충돌 1건 예상, 해소는 "D-081 다음 D-082"로 자명. 합류 뒤 두 칸을 M-표로
> 옮기는 후속 1건) / ⓑ 합류하지 않는다 → **D-082·D-083이 인용하는 D-081과 M-표가
> 실재하지 않는 문서로 남는다** — 그때는 개번과 인용 재작성이 필요하다.
>
> ### ⓑ **남은 것은 전부 attended 세션에 묶여 있다**
>
> 대장의 143종과 M1 잔여 셋이 전부 SAP 접속을 요구한다. **오프라인으로 더 밀 수 있는
> 것이 없다** — 이 판이 오프라인 몫을 다 썼다.
>
> ### 고지 은퇴는 훨씬 멀다 (혼동 방지)
>
