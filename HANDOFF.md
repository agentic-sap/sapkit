...
> 작성 2026-07-10 · 최종 갱신 2026-08-23. 새 세션은 ① 이 문서 → ② 필요 시 해당 트랙
...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ **여기부터 읽는다** — 2026-08-23 마감 · 다음 세션은 이 절만 읽으면 복원된다
>
> ## 🔑 한 문단 요약
>
...
> (`192.168.1.225:8000` · 사내망). 프로파일 3종은 `~/.sapkit/profiles/`에 있고 전부
> `SAP_TIER=DEV`다. ⚠ **IDES-DEV는 2026-08-23 기준 비밀번호가 거부된다(401)** —
> 두 번 실패했고 계정 잠금을 피하려 멈췄다. 고치려면 그 프로파일의 `SAP_PASSWORD`를
> 갱신하면 된다(파일은 git 밖이다).
>
...
> | **도구 표면** | **186 / 155** 불변 |
> | **시험** | jest **4,261** |
> | **게이트** | 제품 9종 · 엔진 전종 · 음성시험 9종 전부 exit 0 |
> | **CI** | `4421b904` **success**(run `32635983723`) — 그 뒤 문서·자산 커밋의 CI는 다음 세션이 본다 |
> | **대장** | 증거 대기 **30** · 증거 있음 **156** (변하지 않았다) |
> | **SAP** | `$TMP`에 `ZSAPKIT_*` **10종 설치·활성**(FUGR 1 + FM 2 + INTF 1 + CLAS 6) |
>
> ## ✅ 오늘 닫힌 것 (D-121 ~ D-132 · D140)
>
> | 갈래 | 결과 |
> |---|---|
...
> - **비추적 `engine/` 753MB가 제품 게이트를 red로 만든다** —
>   `check-runtime-path-rename.mjs` 위반 6건이 전부 `engine/dist/*.d.ts`다. 판7.5에서
>   은퇴한 포크의 빌드 잔재로, 게이트가 git이 아니라 파일시스템을 훑어 집는다.
>   **CI는 초록**(깨끗한 체크아웃). 지우면 로컬도 초록이 된다 — 안에 소스도
>   미커밋 작업물도 없다(`dist`+`node_modules`뿐).
...
>
> ## ✅ 직전 판 — **판M2-c: ⑴ M2 `authorization_code` (D-117 · D-118)** (2026-08-23 · 완료)
>
> **배선 + attended 확인이 한 판에서 닫혔다.** 정지선을 **명시 옵트인 하나**로 열었고,
> 그 통로로 **엔진이 SAP와 실제로 대화**했다.
>
> | 축 | 결과 |
> |---|---|
> | **제품 번들** | `sapkit-engine` **1.2.0** · **3,858,719 바이트** · 소스 핀 **`9f69479c`** — ⚠ 배선 직후 값(3,856,996 B · `a126f6a8`)이 아니다. **독립 리뷰 R-FAIL 회수(D-119)가 재번들했다** |
> | **기동 배선** | `--auth-interactive`(옵트인) · `--callback-host`(기본 **`localhost`**) · `--callback-port`(기본 **8080**) — ⚠ `--host`/`--port`는 **HTTP/SSE 전송이 이미 쓴다** |
> | **합류 지점** | **`TokenSource`의 `acquire` 훅** — 두 그랜트가 `accessToken()` 한 줄에서 만나 접속 조립·`replace()`를 **그대로 재사용** |
> | **브라우저** | **열지 않는다.** URL은 **stderr**로만 · 실기동 문면이 **「Nothing opens on its own」** |
> | **attended 실왕복** | **`GetSystemInfo` → `{"supported":true,"system_id":"TRL","client":"100","language":"EN","user_name":"CB9980000836","adt_stack_type":"modern"}`** — 판M2-b에서 `client_credentials`로 `supported:false`였던 **바로 그 자리** |
> | **진단 개명** | `MCP_DESTINATION_CONNECTED` → **`TOKEN_ACQUIRED`** · **`CONNECTED`는 여전히 방출 안 함**(확인 주체가 기동이 아니라 사람이 부른 도구다 — D-118 ⓓ) |
> | **시험** | **+26**(4,225 → **4,251**) · 실 네트워크 0 · **이월 수령 2**(`ReloadProfile` 재적재 갈래) |
> | **도구 표면** | **등록 186 · 노출 155 불변** — 도구를 새로 짓지 않았다 |
> | **검증** | 착수 **34/34** · 마감 **37/37 exit 0**(재현 빌드 바이트 일치 · jest **4,251**) |
> | **SAP** | **P1 read 1건** · P2 0 · P3 0 · P4 0 |
>
> **`src/auth/**`는 한 줄도 고치지 않았다** — 기구는 판5가 이미 완성했고 **이 판이 지은
> 것은 부르는 자리 하나**다. **도구도 새로 짓지 않았다**(D-117 ⓒ — `build-plan.mjs`가
> **되뜰 수 없는 채록본**의 186종을 정본으로 삼아 187번째는 그 기계에 없는 개념이다).
>
> **핸드셰이크 지연은 실재한다** — 기동이 로그인만큼 멈추므로 **클라이언트 기동 시한이
> 짧으면 토큰이 도착해도 실패로 셀 수 있다.** 진단이 그 사실을 미리 말한다(실기동 확인).
>
> **커밋 5**: `2d988203` D-117 · `a126f6a8` 배선 · `092eace8` 재번들+장부 5차 ·
> `8f448ba5` 문서 · `65ed3f0f` D-118.
>
> ## ⚠ 판M2-c의 독립 리뷰가 돌았고 **R-FAIL이었다 — 회수는 끝났다** (2026-08-23 · **D-119**)
>
> **주장 7개 중 6개가 참으로 확인됐다.** 리뷰어가 **돌연변이 시험**을 원본에 넣고
> 돌려(뒤에 원복) 플래그 가드를 지우면 **2건**, 옵트인을 강제로 켜면 **7건**이 red가
> 되는 것을 실측했다 — **「플래그 없으면 동일」이 말이 아니라 기계에 물려 있다.**
> 비밀 누출 0 · 브라우저 여는 코드 0(번들 포함) · 표면 186/155 · 재현 빌드 바이트
> 일치까지 전량 재현됐다. **무너진 것은 새로 연 노브의 입력 검증 하나다.**
>
> **차단 — `--callback-host`에 검증이 없었다.** `.trim()`만 하고 통과시켜
> **`0.0.0.0`·`192.168.1.50`이 그대로 `server.listen(port, host)`까지** 내려갔다
> (리뷰어 실측). 그 서버가 받는 것은 **인가 코드**이고 **평문 HTTP**다 —
> `src/auth/callback.ts`가 그 모듈의 「지키는 것 ①」로 **「loopback에만 바인딩한다 …
> 랜 어디서나 닿는 자리에 열면 같은 망의 다른 프로세스가 코드를 가로챌 수 있다」**를
> 적어 둔 바로 그 문장을 어긴다. **이 판이 그 인자를 argv에 처음 노출하면서 열렸다.**
> → 허용 집합을 **`localhost`·`127.0.0.1`**로 못박고 그 밖은 되돌리며 이유를 말한다.
> **`callback.ts`는 안 고쳤다** — 지켜야 할 보장이지 완화할 서술이 아니다.
>
> ⚠ **`::1`은 회수 초안에 넣었다가 뺐다 — 시험이 잡았다.** loopback이 맞지만 이 통로가
> 만드는 URL이 깨진다(`http://::1:8080/callback` — IPv6는 대괄호를 요구한다). 조립을
> 고치려면 `src/auth/**`를 건드려야 하므로, **받아 놓고 깨진 `redirect_uri`를 보내느니
> 거부하고 이유를 말한다.**
>
> **권고 5건 전건 채택** — ⓐ 핸드셰이크 경고를 **`printAuthorizeUrl`로**(`bootstrap`이
> transport 연결 **전에** await 하므로 **대기 중 보이는 유일한 출력**이다 · D-117 유보
> ⓔ가 「미리 말해야 한다」고 적은 자리) ⓑ 진단을 **`enabled`로 게이트**(플래그 없는
> 기동의 진단 수가 **무인자 기동과 같다** — 실측 2=2) ⓒ `UAA_REJECTED`에 **PKCE 절**
> (D-117 유보 ⓒ가 예고한 실패 모드가 정확히 그 코드로 떨어진다) ⓓ `OPAQUE_CAUSE`
> **3종 → 5종**(선재 결함) ⓔ 시험 공백 2.
>
> **판 번호는 1.2.0을 유지했다** — **아직 push되지 않은 바이트**의 교정이다(D-096 선례).
> 번들 **3,858,719 B**.
>
> ⚠ **리뷰가 보지 못한 것 둘** — ⓐ **D-118의 attended 실측**(리뷰어는 SAP에 접속하지
> 않았다) ⓑ 리뷰 도중 대상이 두 번 움직였다. **다음 판은 리뷰 대상 커밋을 고정한 뒤
> 돌린다**를 규칙으로 남긴다(D-119 유보 ⓑ). **D-118 + 이 회수를 함께 보는 리뷰가 다음
> 판 몫이다.**
>
> **회수 커밋 3**: `b8a315f3` D-119 결정 · `9f69479c` 코드+시험 · `4706e0f7` 재번들+장부 7차.
>
> ## 판M2-c 배선 상세 (D-117) — 위 표가 요약이다
>
>
...
> 12 — 대상별 2종 포함 · 음성시험 9 · 엔진 11 · cli 4 · **재현 빌드 바이트 일치**
> `5a13a81ba2ac…` 3,849,834 B). ⚠ 두 **33**과 **37**은 **구성이 다르다** — 분모를
> 그대로 비교하지 말 것. ⚠ `check-links`는 **`interactive/` 전용**이라 이 판의 신설
> 파일 2종을 보지 않는다 — 그 링크는 **손으로 확인했다**(29개 검사 · 깨짐 0).
...
> 읽혔다. 실측에서 그 문구를 낸 바로 그 접속이 전 경로 401이었다. **문구만 고쳤다**
...
>
> ## ⚠ 판M2-b의 독립 리뷰가 돌았고 **R-FAIL이었다 — 회수는 끝났다** (2026-08-23 · **D-116**)
>
> **코드는 옹호됐다.** 리뷰어가 `src`를 복제해 문구를 옛 것으로 되돌리는 **돌연변이
> 시험**을 돌렸고 **정확히 신설 시험 1건만 FAIL · 나머지 18건 PASS** — 「문구만
> 고쳤다」와 「시험이 회귀를 잡는다」가 한 번에 증명됐다. 게이트도 전량 재현됐고
> 비밀정보 0 · P2/P3/P4 흔적 0도 확인됐다. **무너진 것은 문서 계약과 장부 규약이다.**
>
> **차단 ① `DIVERGENCES.md` append-only 위반** — D15 **3차 갱신 표**의 칸을 고쳤는데
> **그 표를 여는 문장이 「1차·2차 갱신 표도 고치지 않는다」**였다. 게다가 그 편집은
> **판M2-a의 기록이 판M2-a가 알지 못했던 사실을 말하게** 만들었다. → 원문 원복 +
> **「D15 4차」 절 append**.
> **차단 ② `docs/BLUEPRINT.md` M2 행이 여전히 「실접속은 판M2-b」** — 계획 정본만 읽는
> 세션은 「실접속 = 판M2-b」 → RUN-PLAN에서 **완료** 확인 → **M2가 섰다고 읽는다.**
> 이 판이 가장 막으려는 오독을 정본이 만들고 있었다. → M2 행을 판M2-c로 + 각주 ¹에
> 판M2-b 블록.
>
> **함께 정정한 것 셋** — ⓐ **「입구가 아예 없다」는 덜 말했다**: 운영자가 건네줄
> 입구는 없지만 **기구(`src/auth/callback.ts`·`uaa.ts`)는 이미 지어져 있고 시험도
> 있다.** 없는 것은 **진입점**이다 — 이 과장은 판M2-c가 **이미 있는 것을 다시 짓게**
> 만들 뻔했다. ⓑ **「ADT 전면 개방」은 세다** — 실측은 **잰 3경로 200**이고 401 쪽은
> 4경로였다. ⓒ **콜백은 포트만이 아니라 호스트도 설계 입력이다** — 통과한 것은
> `localhost:8080`인데 엔진 기본값은 **`127.0.0.1`**이라 **포트만 맞추면 여전히
> 실패한다.**
>
> **측정 재현 수단을 남겼다**(권고 9 — 이 판의 산출물은 측정인데 산문으로만 남아
> 재현 불가였다): **`sapkit-engine/harness/probe-destination.mjs`**(⚠ **게이트 아님** ·
> attended 전용 · P1 read만 · 비밀 무출력 — `record-attended.mjs`와 같은 지위) +
> 채록본 **`docs/reference/audits/2026-08-23-btp-abap-trial-grant-probe.md`**.
>
> **판M2-c로 넘긴 것**: 진단 코드 이름 이동 — `MCP_DESTINATION_CONNECTED`의 본문이
> **다섯 문장을 자기 이름을 취소하는 데** 쓴다. 그 판이 **진짜로 붙은 상태**를 만들 때
> 쓸 이름이 남아 있지 않으므로, 토큰 취득 자리를 `..._TOKEN_ACQUIRED`류로 옮기고
> **`CONNECTED`를 실접속 자리에 예약**한다.
>
> **회수 커밋 3**: `26d4d71c` 결정 · `6612346b` 집행 · `ed02e521` 재핀(시험이 `src/`
> 아래라 소스 핀이 움직였다 — **번들 바이트는 불변**).
>
> **이월 2**:
> ⓐ **destination 재적재 갈래 전용 시험**은 여전히 없다(판M2-a에서 승계 · 이번 판도
> 추가하지 않았다). **판M2-c의 리뷰가 본다.**
> ⓑ **기동이 「접속이 실제로 서는가」를 기계로 재지 않는다** — D-115가 대안 (c)
...
> 않고 **그랜트 성격으로 좁혔다** — 사람 몫의 상호작용이 있는 쪽만 세운다.
>
> | 축 | 판M2-a 결과 |
> |---|---|
> | **제품 번들** | `sapkit-engine` **1.1.0** · **3,849,389 바이트** · 소스 핀 **`0b4b985b`** |
> | **기동 배선** | `client_credentials` = **자동 취득** · `authorization_code` = **정지선 유지**(진단이 다음 걸음을 안내) · **선언 없는 키의 기본값 = `authorization_code`**(자동 취득은 **키가 말할 때만**) · destination **tier=UNKNOWN read-only fail-closed** |
> | **진단** | `TOKEN_PENDING`을 **`authorization_code` 전용으로 좁힘** + 신설 4종(`TOKEN_REQUIRED`·`CONNECTED`·`TOKEN_FAILED`·`NO_SERVICE_URL`) — 리뷰가 **「읽을 만하다」** 판정(D-091 유보 ⓓ가 남긴 질문이 닫혔다) |
> | **시험** | **32건 신설**(전 구간 목 · **실 네트워크 0**) · jest **4,224** · **Basic 회귀 0**(기계 근거) |
> | **도구 표면** | **186 불변** |
> | **독립 리뷰** | **R-PASS · 차단 0 · 권고 5** — 코드 3건 회수 완료(`0b4b985b`+`a37e7efc`) · 권고 4 = 이 문서 갱신 · 권고 5는 기록만 |
> | **커밋 5** | `ce6e5dbd` 배선 · `c0180f0c` 1.1.0 · `d89dba6b` 재번들 · `0b4b985b` 리뷰 회수 · `a37e7efc` 회수 재번들 |
> | **SAP** | **접속 0회** · P2 0건 · P4 0건 · 전 구간 P0 |
>
> ⚠ **이 판은 오프라인 반쪽이다 — M-표 M2는 승격하지 않았다**(D-114 유보 ⓐ). **실 UAA
> 왕복 0회**이고 M2 `짓기 완료` 기준 **ⓐ가 실접속**이기 때문이다. 「배선했다」를
> 「접속된다」로 읽지 말 것.
>
> **리뷰 회수 3건**(코드): ⓐ `ReloadProfile`의 `restartRequired`가 **destination 접속
> 상실**을 신호한다 ⓑ `TOKEN_FAILED` 문구를 영문으로 일관 ⓒ 표기 정리. **판 번호는
> 1.1.0을 유지했다** — 같은 판 안의 회수이고 제품에 나간 적 없는 바이트의 교정이다
> (**D-096 선례**).
>
> **이월 2** (판M2-a가 남긴 것):
> ⓐ **destination 재적재 갈래 전용 시험을 추가하지 않았다** — 근거가 **리뷰어의 실기동
> 실측**이라 자동 회귀가 없다. **다음 판의 리뷰가 본다.**
...
> | 항목 | 성격 | 막고 있는 것 |
> |---|---|---|
> | **D 이월 리뷰** | 리뷰 1회 | **막는 것 없음** — D-118+D-119를 함께 본다(판M2-c 리뷰가 못 봤다) |
> | **A 재생 대조 18종** | 판(attended · DEV 프로파일) | **막는 것 없음** — 기계적. **BTP 아님** |
> | **B attended 실기 12종** | 판 1~2개 | **부류별 처방**(면제 2 · 엔진 수리 2 · SAP 측 고장 6 · 진단 2) — 재시도 아님 |
> | 토큰 보존/갱신 | 판(미개시) | **증거 판의 선행이 아니다.** `--mcp` 통로를 상시 쓰게 될 때의 문제 · 보존은 **D-091 ⓒ① 개정** 필요 |
...
> | 별건 ⓓ **브랜드**(끝그림 ④) | 잡무 | `sc4sap-mcp-tools*.md` 파일명 · `[SC4SAP …]` 훅 태그 3 — **이름의 문제** |
> | 별건 ⓔ **체크박스 규범항 재저작 보류**(24 + 10) | 잡무 | **미룬 일이지 보존 판정이 아니다**(D-113 ⓒ⑤) |
> | 이월 ⓐ destination 재적재 시험 | 이월 | 판M2-c 리뷰가 본다(판M2-b도 추가 안 함) |
> | 이월 ⓑ 기동의 접속 확인 왕복 부재 | 이월 | D-115가 대안 (c)를 기각한 대가 — 문구가 말할 뿐 |
>
> ---
>
> **판11이 끝났다 — ⑷ ㉮ 고지 범위 정합이 섰다** (2026-08-23 · **D-112** · 오프라인 ·
> SAP 접속 0회 · P2 0건 · P4 0건 · 전 구간 P0). **답은 「좁히기」가 아니라 「서브트리 유지 +
> 내역의 정확성」이었다** — 상류 저작물로 남는 것은 **파일이 아니라 산재한 계약행**이라
> **파일 목록이라는 형태 자체가 틀린 답**이었다(D-103 → D-104 → D-106에서 **두 번 데인
> 자리**다). 착수 선행인 **역방향 스캔 1회**(D-106 ⓒ)를 마감 실측으로 돌려 겹침 잔존
> **40파일 · ~598행**(페르소나 21 · 절차 13 · 지식 3 · 정책 1 · 도구 2)을 얻었고 — ⚠ **그 값은
> §3.4 갈래만이었다**(아래 회수 · D-113 ⓐ) — **고지 4곳**
> (루트 `LICENSE` · `interactive/LICENSE` · `THIRD_PARTY_NOTICES.md` · `interactive/README.md`)을
> 그 내역으로 갱신했다. **MIT 전문·상류 저작권 문구는 불변.**
>
> **그래서 사다리 ⑷가 완료됐고, 사다리 전체가 닫혔다** — ㉮(판11) · ㉯(판10 · D-107) ·
> ㉰(판3.7 · D-109~D-111) 셋 다 섰다. **2026-08-10 D-079가 착수한 교체 사다리가 2026-08-23에
> 끝난다.** 끝그림 판정(D-112 ⓒ): ① 몸통 전부 자작 **도달** · ② 외부는 정식 의존성만
> **도달** · ③ 계보성 고지 **도달**(D-103 재정의 기준) · ④ 브랜드 단일 **미완으로 명시** ·
> ⑤ 무게는 상시 원칙. **「포크 0」의 도달 형태 = ①~③.**
>
> | 남은 상류 유래 | 수 | 성격 |
> |---|---|---|
> | `interactive/server/sap-assets/` | **30파일** | **영구 존치**(D-079 ⑥) — 개작·유지보수하지 않는 **무접촉 동봉 자산**. 이 고지가 영구히 남는 이유다 |
> | 산재 **계약행** (`server/` 밖 서브트리 전체) | **~2,200행 / ~130파일** | **의도된 보존**(D-112 ⓓ) — 참조표 · 펜스 템플릿 · print-verbatim · 사용자 노출 문구 · 승인 키워드 · 공유 라벨. 재저작하면 제품 동작·데이터 정합이 깨진다. ⚠ **판11이 공표한 ~600행/40파일은 §3.4 갈래만의 값이었다**(**D-113 ⓐ**) — 구성 **3갈래** = §3.4 **~570/39** + `copy` 재저작 체크분 **~950/64** + 무변경 검증 참조표 **~660/25**. ⚠ **~~체크박스 규범항~~은 2026-08-24에 재저작으로 빠졌다**(`review-checklist.md` 24 · `troubleshooting.md` 11 — 그것이 §3.4가 ~600/40 → ~570/39로 내려간 몫이다) |
>
> ⚠ **「전부 자작」을 행 단위로 읽지 말 것** — ①의 도달은 **파일 단위** 기준이고, 행 단위로는
> 위 계약행이 남는다(D-112 유보 ⓒ). **이 구분을 흐리면 과대 주장이 된다** — 고지 4곳이
> 그 구분을 명시한다.
>
> ⚠ **판11의 독립 새-컨텍스트 리뷰가 돌았고 R-FAIL이었다 — 회수는 끝났다** (2026-08-23 · **D-113** ·
> **종결 — `54c10a37`**). 기계 재현은 전부 섰고(마감 스캔 40/603 재현 · 명부 56/56·170/170 ·
> MIT 전문 불변 · 게이트 10/10 + `--rebuild` 바이트 일치) **방향과 성격 판정도 옹호됐다.**
> 무너진 것은 **내역의 완전성**이고 차단 2를 전부 인정했다:
> ⑴ **잔존 총계가 §3.4 갈래만의 값이었다** — 도출 4단계가 설계상 `copy` 명부 170의 목적지를
> 빼기 때문이다. 리뷰어가 같은 스캔을 그쪽에도 돌려 **`server/` 밖 실제 총계 129파일/~2,216행**
> (공표치의 3.2~3.7배)을 실측했다. ⚠ **고지가 「장부」로 가리키는 `copy-baseline.md` §3.5
> 자신이 「128건에서도 겹침을 본다」고 이미 적고 있었다.**
> ⑵ **「전부 재저작」이 27파일에 거짓이었다** — `copy` 명부 226 중 27은 D-088 ②의
> **「검증 체크(무변경)」**(순수 참조표 · 바이트 동일 보존)이다.
> **⑷ 완료 판정 자체는 유지된다 — 리뷰어의 결론이기도 하다.** ㉮의 정의(내역 일치)가
> **정정된 내역**으로 성립하고, `copy` 갈래 잔존의 성격도 같은 계약행임이 확인됐다.
> 고쳐진 것은 **총계와 두 문장**이며 **권고 5건도 전부 채택**했다(고지 산문 문구 완화 ·
> 명부 낡은 문장 3곳 마킹 · §3.4 시점 주석 · BLUEPRINT ② keyring 각주 + **분류 정정** ·
> **「보류는 보류로」**). ⚠ **다음 판(판M2-a)은 코드 판이라 고지를 자연히 보지 않으므로**,
> 회수 무재리뷰 선례와 달리 **고지 4곳 + 장부 정합만 겨눈 경량 리뷰 1회를 돌린다**(D-113 ⓓ).
>
> **판3.7이 완주됐다** (2026-08-23 · D-109~**D-111** · §3.4 **56/56 · 잔량 0**): 배치3이
> 절차 16 · 코드 8 · 지식·정책 7 = 31건을 닫았다(리뷰 30/31 · 차단 1은 `and`→`or` 한
> 단어 — 복원). **차단 이력이 지형을 그린다**: 배치1 `explicitly` · 배치3 `and` — 전부
> **논리 연산자·한정어 한 단어**였다. 코드 갈래는 계약 실증(훅 74+13 · extract 13케이스
> 바이트 동일)이 증거이고, 참조표는 바이트 보존이 정답이었다.
>
> **별건 대기 5** (판이 아니다 · 판M2-a와 독립): ⓐ `atomic-write.mjs` 레포 내 소비자 0
> (**배선** 여부 확인) ⓑ `data-access-approval` 파일명 **3자 불일치**(안전 정본이라 별도
> 판에서 통일) ⓒ executor 깨진 문장 수리 ⓓ **브랜드**(끝그림 ④ — `sc4sap-mcp-tools*.md`
> 파일명 · `[SC4SAP …]` 훅 태그 3 · **저작권이 아니라 이름의 문제**. ~~keyring 21 정식 귀속~~
> → **끝그림 ② 소관으로 이동**했다 — 이름이 아니라 **귀속·의존성 형태**의 문제였다
> (**D-113 ⓒ④** — D-112 ⓒ의 오분류 정정. vendored 21파일이 레포에 커밋된 채 실리고,
> **개작 0이라 ②의 운용 정의는 만족하나 형태는 미달**이다)
> ⓔ **체크박스 규범항 재저작 — 「보류」다**(`review-checklist` §7 하위 **24** +
> `core/procedures/troubleshooting.md` **10**). ⚠ **미룬 일이지 보존 판정이 아니다** —
> D-111이 **의도적 보류**로 기각한 것(밀도 높은 기술 단정문은 패러프레이즈 자체가 의미
> 표류 위험)을 D-112 ⓓ가 「의도된 보존」으로 흡수하며 성격을 바꿨고, **D-113 ⓒ⑤가 그것을
> 되돌렸다**. **보류는 보류로** 남고, 고지의 잔존 범주에는 「체크박스 규범항」이 별도로 들어갔다.
>
> **배치2도 끝났다** (2026-08-23 · **D-110** · 본체 `03992a7d`): 페르소나 나머지 **20편**
> 재저작 — 워커 4 병렬 · 독립 리뷰어 2 병렬 **20/20 PASS**(배치1 차단 유형 0건 · 워커
> 자기 검증이 초안 회귀를 스스로 잡음) · 공유 블록 6종 **3차 일괄**(상류 축자 판별 후
> 새 정본 한 벌 · md5 1종) · 거짓 지칭 「config file」 14곳 정정 · 참조 동사 84행 `See`
> 통일. **체크 25 / 잔량 31 — 페르소나 갈래 소진.** 남은 것 = **절차 16 · 어댑터 5 ·
> 지식 5 · 정책 2 · 도구 3.**
>
> ⚠ **병렬 재저작 규칙 (배치2가 얻은 것)**: 여러 파일이 공유하는 문자열은 워커에게
> 나누지 말고 **일괄 패스로만** 다룬다 — 나누는 순간 제품 문자열이 워커 수만큼 갈라진다
> (실제로 4갈래가 났고 3차 일괄이 닫았다). ⚠ 3차 일괄 문장 자체는 다음 배치 리뷰가
> 본다(D-110 유보 ⓐ). ⚠ executor의 깨진 문장(BLOCKED 뒤 재전환)은 원문 결함 보존 —
> 수리는 별건 결정.
>
> **배치1이 끝났다** (2026-08-23 · **D-109** · 재저작 `b1df26e2`): 페르소나 상위 잔존
> 5편(architect·debugger·qa-tester·critic·analyst) 재저작 — 독립 리뷰 4 PASS · critic
> 1 FAIL(explicitly 소실 한 단어)을 복원으로 해소 · **체크 5 / 잔량 51**. §3.5 표본
> 판정도 닫혔다 — **체크 정당(ⓐ)** · spro 띠 서술은 사실 오류(그 띠는 전부 검증
> 체크) · 진짜 의심 자리(enhancements 「명칭 — 서술」 11행)는 회수 재저작으로 갚음.
>
> **⚠ 배치2 이후가 물려받는 것**: ⓐ **26편 공통 문장 부채**(`.sapkit/config.json`
> MUST — 상류 축자 246자 · **26편 일괄 1회로만** 재저작 · D-109 ⓔ) ⓑ 잔존 서술행
> 11행 재저작은 이번 배치 리뷰를 안 거쳤다(다음 배치 리뷰가 본다 · D-109 유보 ⓑ)
> ⓒ 「볼드 라벨 보존 정책이 §2를 만족하는가」 열린 질문(유보 ⓒ).
>
> **판10이 끝났다** (2026-08-23 · **D-107** · 오프라인 · SAP 접속 0회 · 전 구간 P0).
> 판8이 확정한 과녁 **활성 6이 소진됐다** — `tool-catalog/` 4는 **엔진 등록점에서 만드는
> 기계 생성물**이 됐고 유틸 2는 **골격 재저작**됐다. 그리고 **판6.1부터 이월돼 온
> 「tool-catalog 자동 대조 게이트 부재」가 같은 작업으로 닫혔다** — 하나로 둘을 갚았다.
>
> **⚠ 그 판의 독립 리뷰가 R-FAIL을 냈고 회수까지 끝났다** (2026-08-23 · **D-108**).
> **엔지니어링은 전부 재현됐다**(표면 diff 0 · 사보타주에서 게이트가 행 단위로 짚음 ·
> 강화 1건을 구판과 나란히 실증 · clean 트리 `--rebuild` 재현 빌드 일치). 차단은 코드가
> 아니라 **문서 계약**이었다 — 판10이 카탈로그에 준 「기계 생성물」 지위가 **그 파일을
> 만지라고 말하는 기존 절차 정본 2곳에 전파되지 않았다**(`UPDATE-RUNBOOK.md` 4단계는
> 여전히 「함께 **고친다**」고 지시했고 — 제품에 실려 배포되는 파일이다 —
> `ADDING-A-TOOL.md`의 「한 커밋」에는 카탈로그 재생성이 없었다). **둘 다 고쳤고 권고
> 4건도 전부 채택했다. 코드 변경 0 · 재리뷰는 돌리지 않는다**(D-102 선례 · D-108 ⓓ).
> ⚠ **그중 하나는 수치 해석의 정정이다** — 「tool-catalog 4파일 겹침 0행」은 **전환의
> 효과가 아니다**(구판도 이미 0 · 도구 이름 줄은 60자 미만이라 측정 밖). **delta 증거는
> `bundle-keyring.mjs` 하나**이고, 전환의 가치는 **생성물 지위 + 대조 게이트**다.
>
> | 대상 | 처분 | 근거 |
> |---|---|---|
> | `tool-catalog/sc4sap-mcp-tools*.md` 4 | **등록점 생성물로 전환** + 대조 게이트 신설 | 생성 목록이 구 카탈로그와 **diff 0**(read 90 · write 79 · runtime 15) · 역방향 스캔 겹침 **0행** |
> | `verify-engine.mjs` | **골격 재저작** 84 → **90줄** (계약 불변 + 강화 1건) | 겹침 **1/17** — `VERSION`에 commit 없으면 이제 **FAIL**(옛 판은 null끼리 비교해 조용히 통과했다) |
> | `bundle-keyring.mjs` | 처분 = **재저작** 277 → **226줄** | 착수 스캔 **46/51 = 90%**가 근거 · 마감 겹침 **1/35** · 4모드 exit 계약·핀 형식 보존 |
>
> **⚠ 게이트가 여태 못 서던 원인은 「입력」이었다** — 카탈로그를 번들 `tools/list`로 재면
> 프로파일 유무로 **155/186**이 갈려 CI에서 답이 달라진다. 입력을 **정적 등록점**
> (`TOOL_REGISTRY` 186종)으로 바꾸자 게이트가 성립했다. **엔진 게이트 5 → 6종** ·
> **음성시험 43 → 52** · CI는 `sapkit-engine` 잡이 이미 그 둘을 돌리므로 **워크플로 수정 0**.
> 카탈로그는 이제 **손으로 고치면 거부된다** — `TOOL-LEDGER.md`와 같은 지위다.
>
> **⚠ 재저작 검증은 역방향 스캔으로 했다** — D-106 ⓒ가 절차로 못박은 방법의 **첫 이행**이다.
> 6파일 전부 **겹침 3행 미만**이고 잔여 2행의 정체까지 확인했다(Node import 관용구 1 ·
> 핀 형식 계약 문자열 1). **차용 산문 잔존 0.**
>
> **⚠ 고지는 갱신했으나 범위는 좁히지 않았다**(D-107 ⓕ) — **§3.4의 상류 잔존 56**이
> 남아 있으므로 `interactive/LICENSE`는 여전히 서브트리 전체에 걸린다. 바뀐 것은 **알려진
> 내역**이고, 갚을 것이 「활성 6 + 56」에서 **「56」**으로 줄었다. **그 56이 판3.7이다.**
>
> **판9는 그 앞에서 끝났다** (2026-08-23 · **D-105**). D-079 ⑧이 2026-08-10에 예약한
> **「재평가 1회」가 소진**됐고 답은 **전량 유지**다 — 도구 **186종은 이름·인자·응답
> 형태까지 불변**이다.
>
> **⚠ 판9의 핵심은 「유지」가 아니라 실측이 과녁을 옮긴 것이다.** 유일한 찬성 논거였던
> 「세션이 무겁다」를 재 보니 **기본 프리셋 `readonly`는 74종·~13.7k 토큰**이고 그중 꼬리는
> **11종(18.9%)**뿐이며 **`Delete*` 25종은 한 종도 뜨지 않는다**. 일상 세션에서 꼬리를 다
> 빼도 절감은 **~2.6k 토큰**인데, 대가는 **제품 7파일 + 엔진 135파일 + 새 D-결정 +
> 배포 제품의 breaking change**다. 겨눌 곳은 **기본 표면 74종의 설명문 밀도**(평균
> **740 B/도구**)이고 **§5-16 백로그**로 분리했다. ⚠ **둘을 섞으면 잘못된 해법에
> 도달한다** — 꼬리를 다 빼도 137종이 남아 무게는 그대로다.
>
> **판8은 완료됐으나 집행 일부가 회수됐다** (2026-08-22 **D-103** → 08-23 **D-104**).
> **판정은 선다** — ①(36건 재저작) 기각 · 36을 **동결 30 / 활성 6**으로 갈랐다 · ⑷ 완료
> 정의를 **「범위 정합 + 활성 차용분 0」**으로 고쳤다 · 끝그림 ③의 「최종 0건」은 **도달
> 불가**(D-079 ⑥). ⚠ **철회된 것은 고지 범위를 좁힌 집행**이다 — `class: copy`만 세고
> **`class: transform`을 통째로 빠뜨렸다.** **`interactive/LICENSE`는 서브트리 전체에
> 걸린 채다.**
>
> **⚠ 그리고 2차 리뷰가 그 회수마저 R-FAIL을 냈다** (2026-08-23 · **D-106**). 대체
> 수치 42도 미달이었다 — **§4 레시피의 목적지 도출이 절차 12파일을 아예 만들어 내지
> 못했다**(`skills/<스킬>/**` → `<스킬>.md`로 납작해졌기 때문). **그래서 방법을 바꿨다**:
> §3.4는 이제 장부에서 도출하지 않고 **레포 파일에서 출발하는 역방향 스캔**으로 만들며,
> 원본을 **디렉터리 2단계까지 묶어** 비교한다. 결과 **42 → 56**(페르소나 25 · 절차 16 ·
> 어댑터 5 · 지식 5 · 정책 2 · 도구 3 · `program-to-spec.md` **90/138 = 65%**).
>
> **⚠ 차용 범위를 판정하거나 고지를 좁히려는 판은 착수 시 역방향 스캔을 1회 돌린다**
> (D-106 ⓒ · 게이트로 만들지 않는다 — 명부 §1과 충돌). 장부 방향만으로 판정하면 장부의
> **도출 결함을 그대로 물려받는다** — 이 프로젝트에서 **세 번 연속 그렇게 걸렸다.**
>
> **⚠ 명부 170의 「완주」를 넓게 읽지 말 것** — 그것은 **`copy` 범주 안에서의 완주**이고
> 페르소나·절차는 그 명부에 **한 줄도 없다**. transform 명부 = `copy-baseline.md` **§3.4** ·
> 재저작 = **판3.7 — 2026-08-23 완주**(56/56 · D-109~D-111). **새로 생긴 일이 아니라 아무도 세지 않던 일이었다.**
>
> **⚠ §3.5도 새로 생겼다** — 역방향 스캔은 **`copy` 체크가 끝난 128건에서도 겹침**을
> 본다(`spro.md` 85~92% · `enhancements.md` 85% · `conventions/` 12편). `tcodes.md`류
> 100%는 **D-088 ②로 정상**이지만 나머지는 **재저작 커밋으로 체크된 것들**이다.
> ~~**단정하지 않고** 판3.7이 표본을 떠서 판정한다~~ → **판정됐다: 「체크 정당」**(D-109 ⓐ — spro 띠 서술이 사실 오류였고, 진짜 의심 자리 11행은 회수 재저작으로 갚았다).
>
> **⚠ 여기서 얻은 규칙**: 고지를 **넓게 두는 것**은 부정확할 뿐이지만 **좁히면서 「나머지는
> 우리 것」이라 적는 것**은 방향이 다르다 — 판정이 설 때까지 **넓은 쪽**에 둔다.
>
> **⚠ 지금 열려 있는 유보**:
> ⓐ **롤백 두 걸음을 실제로 해 본 적이 없다** — 아래 🔙 절. 그것을 재던 CI 잡이 없으므로
> **자동으로 해소되지 않는다.**
> ⓑ **채록본 셋**(`m1-tools.json`·`handler-tree.json`·`retired-evidence-paths.json`)은
> **틀린 값이 있어도 원본과 대조해 발견할 길이 없다.**
> ⓒ `sapkit-engine/src/**`의 JSDoc 계보 인용 **425파일**이 죽은 경로를 가리킨다 — **명시 이월.**
> ⓓ **`copy` 동결 30이 「지금도」 상류 문장인지는 파일 단위로 대조하지 않았다**(무접촉 원칙).
> ~~활성 6도 그랬으나~~ **판10이 그 6은 실제로 대조했다**(D-107 ⓔ — `bundle-keyring.mjs`는
> 착수 스캔에서 **90%**로 짙게 남아 있었다). 남은 미대조는 **동결 30**이다.
> ⓔ ~~**`bundle-keyring.mjs`의 처분은 정하지 않았다**~~ → **해소 (2026-08-23 · D-107 ⓓ) —
> 처분은 재저작이다.**
> ⓕ **§3.4의 56은 수치도 목록도 전수라는 보장이 없다**(60자 초과 완전 일치만 셌다 —
> 쪼갠 문장·짧은 실질 행·번역은 과소, 표·정형 문구는 과대). **두 번 「전수」로 읽었다가
> 두 번 틀렸다.** 파일별 판정은 **판3.7이 사람 손으로 했고**(56/56 · 리뷰 4회 · D-109~D-111), **판11의 마감 스캔이 40파일 · ~598행**을 실측했다 — 그 한계(과소·과대)는 **그대로 승계된다**(D-112 유보 ⓑ). ⚠ **그리고 세 번째로 틀렸다** — 그 40/~598조차 **`copy` 갈래를 뺀 §3.4 갈래만의 값**이었고, 리뷰가 `server/` 밖 총계를 **129파일/~2,216행**으로 정정했다(2026-08-23 · **D-113 ⓐ**).
> ⓖ **같은 자리에서 다섯 번 걸렸다** — D-102 ⓐ(「안 본 범주가 또 있는가」) → D-104 ⓒ(판8이
> 그것을 인용해 두고도 transform을 빠뜨림) → **D-106 ⓓ**(회수마저 도출 누락) →
> **D-108 ⓐ**(판10이 카탈로그의 「생성물」 지위를 절차 정본 2곳에 전파하지 않음) →
> **D-113 ⓐ**(판11이 `copy` 갈래를 뺀 값을 **전체 총계로 공표**).
> 유보를 넓게 쓰는 것만으로는 부족했고 **방법을 바꾸는 것**이 답이었다(역방향 스캔).
> ⚠ **네 번째가 더한 것**: 전파 대상은 **수치·범주·고지만이 아니라 「지위」이기도 하다** —
> 파일의 성격이 바뀌면(수동 → 생성물) **그 파일을 만지라고 말하는 모든 절차 문서**가
> 갱신 대상이 된다. **성격을 바꾼 판은 그 파일 이름으로 레포를 grep하고 절차 문서를
> 전부 훑는다.**
> ⚠ **다섯 번째가 더한 것**: **방법을 바꿔도(역방향 스캔) 「무엇을 빼고 세는가」가 틀리면
> 틀린 총계가 나온다.** 그러므로 — **총계를 공표할 때는 제외 집합을 문장에 같이 적는다**
> (D-113 유보 ⓑ). 판11의 도출 절차 4단계는 설계상 `copy` 명부의 목적지를 뺐고, 그 「설계상」이
> 문장에 남지 않아 갈래별 값이 전체 값으로 읽혔다.
> ⓗ **`bundle-keyring.mjs`는 어떤 게이트·CI·runbook에도 물려 있지 않다**(D-107 유보 ⓑ —
> 레포 전체 grep에서 자기 참조뿐). 재저작해도 「자동으로 지켜지는가」는 아무도 재지 않는다.
> **손으로 도는 도구**라는 성격은 존치하고 **게이트化는 하지 않기로 했다**(populate 모드가
> 런타임 의존을 다시 쓰므로 CI에 넣을 물건이 아니다).
> ⓘ **그 populate 모드는 실행해 보지 않았다**(D-107 유보 ⓒ) — 이 머신에 소스
> (`server/node_modules/@napi-rs`)가 없어 설계대로 exit 1이고, 돌리면 런타임 의존 바이트가
> 움직인다. **코드 리뷰로만 확인했다.**
> ⓙ ~~**판10의 독립 새-컨텍스트 리뷰가 아직**이다~~ → **해소 (2026-08-23 · D-108) —
> 돌았고 R-FAIL이었으며 회수까지 끝났다.** 코드 축은 전부 재현·통과였고, 차단은 문서
> 계약이었다(위 판10 블록). ⚠ **다만 회수분 자체는 재리뷰하지 않았다**(D-108 ⓓ · 문서만 ·
> 코드 변경 0) — **다음 판의 리뷰가 자연히 본다.**
> ⓚ ~~**판11(⑷ ㉮ · D-112)의 독립 새-컨텍스트 리뷰가 아직이다**~~ → **돌았고 R-FAIL이었으며
> 회수까지 끝났다** (2026-08-23 · **D-113** · 차단 2 = 총계가 §3.4 갈래만 · 「전부 재저작」이
> 27파일에 거짓 · 권고 5 전부 채택). **방향·성격 판정은 옹호됐고 「⑷ 완료 · 끝그림 ①~③
> 도달」은 정정된 내역 위에서 유지된다.** 경량 재리뷰의 닫는 조건(계수 26 · 제외 집합 라벨)까지 `54c10a37`이 반영해 **종결됐다**
> (D-113 ⓓ — 고지 4곳 + 장부 정합만). **고지 판의 세 번째 FAIL**이고 다음 판이 코드 판이라
> 자연히 보지 않기 때문이며, 그 재리뷰 전까지 **정정된 내역은 아직 독립 확인 밖**이다.
>
> **판M2-b는 실접속 판이다** — destination 프로파일로 **실접속 성공**(inspection-only
> 강등이 아니라) · `--mcp` 진단 `MCP_DESTINATION_UNSUPPORTED` 소멸 · Basic 회귀 0 →
> **D15 해소** → **M-표 M2 승격**. 코드는 판M2-a가 기동까지 이었으므로(D-114) 남은 것은
> **실 UAA 왕복 한 번**이다. ⚠ **그 왕복이 BTP 계정을 요구하고, 그것이 사용자 결정이다** —
> 이 재개점이 혼자 열 수 없는 유일한 이유다. 브리프는 `docs/RUN-PLAN.md` 판M2-b.
>
> 1. `git fetch` + `main..origin/main` 확인(재개 전 원격 대조 — 안전 규칙)
> 2. **판M2-a는 끝났고 리뷰까지 R-PASS다**(D-114 · 차단 0 · 권고 5 전건 처리 —
>    코드 3은 `0b4b985b`+`a37e7efc`, 문서 1은 이 갱신, 1은 기록만). **회수 대기 없음.**
> 3. **사용자에게 묻는다 — BTP/클라우드 계정을 열 것인가**(비용·계약 · CLAUDE.md
>    묻는 자리 ③ · D-092 ⓑ). **예면 판M2-b, 아니면 위 별건 5 중에서 고른다.**
>    ⚠ **판M2-b를 우회하려고 세션 파일을 스테이징하지 말 것** — `--env=<name>` 통로의
>    저장소는 `<home>/Documents/mcp-abap-adt/sessions/<name>.env`이고, 그것을 채우는 것은
>    **자격증명 복제**라 사용자가 하지 않기로 정했다.
> 4. **착수 기준선을 네 묶음 전부** 뜬다(제품 게이트 9 + 음성시험 9 + `sapkit-engine`
>    전종 + `sapkit-cli` 전종 + `doctor`). PowerShell로.
>    ⚠ **엔진 게이트가 6종이 됐다** — 판10이 **카탈로그 대조**(`gates/catalog.mjs`)를 더했고
>    `npm run gates`가 그것까지 돈다. 음성시험 `test-gates.mjs`도 **43 → 52건**이다.
>    ⚠ **머신 함정 둘** — `sapkit-engine/node_modules`에 `esbuild`가 없으면 `npm run verify`가
>    `ERR_MODULE_NOT_FOUND`로 죽고, `sapkit-cli/node_modules`도 없으면 첫 실행이 실패한다.
>    각각 `npm ci`로 해소. **둘 다 레포 회귀가 아니다.**
>    ⚠ **판8·회수·판9 실측은 모두 34/34 exit 0**(`doctor` 포함)이다.
>    ⚠ `--rebuild`는 **작업 트리가 clean일 때만** 그 갈래로 간다(D-095 정직 유보 ⓑ).
>
> **✅ 판6.3이 SAP에 남긴 흔적은 0건이다 — 숙제 없음** (2026-08-20 사용자 확인 · **D-100**).
> 이송요청 `DEVK901065`(+태스크 `DEVK901066`)는 **사용자가 SE01에서 삭제**했고, 패키지
> `ZSAPKIT_63_PKG`는 **애초에 만들어지지 않았다.**
>
> ⚠ **판 기록이 한 번 틀렸고 그 자리를 D-100이 정정했다.** 판 마감 시점에는 그 패키지를
> 「반쯤 만들어졌다」로 적었다 — 읽기 경로 다섯이 갈렸고(`CreatePackage`의 「이미 있다」와
> `GetPackageContents`의 빈 목록이 **존재**를, `GetPackage`·`SearchObject`가 **부재**를 가리켰다)
> 그중 존재 쪽으로 기울인 것이 오판이었다. **`GetPackage`·`SearchObject`가 옳았다.**
> 여기서 남는 교훈은 **증거가 있는 자리와 추정한 자리의 신뢰도가 다르다**는 것이다 — 46종
> 갈림과 대장 수치는 픽스처에서 기계로 나왔고 이 정정의 영향을 받지 않는다.
>
> **연습 객체 잔재 0** · **보호 자산 8종 세 조회 모두 무사** · **P2 0건.**
>
> ## 🔙 롤백 좌표 — **판7.5가 이것을 바꿨다 (외우지 말고 여기서 읽을 것)**
>
> 판7-b 시점의 「**교체 커밋 revert** 한 번」은 **더 이상 성립하지 않는다** — 그 revert는
> 이제 **소스 없는 번들**을 되살린다. 현행 롤백은 두 걸음이다:
>
> 1. `engine/`이 온전한 마지막 커밋 **`2264f89d`**에서 그 트리를 되뜬다
> 2. 그 위에서 교체분(`97b533a4` + 회수 `e32d4799`·`60a62706`)을 되돌린다
>
> ⚠ **롤백 가능성은 이제 상시로 기계 확인되지 않는다** — 「롤백 번들이 여전히 지어진다」를
> 매 푸시 확인하던 CI `engine-tests` 잡이 은퇴와 함께 제거됐다. 그리고 **이 두 걸음을
> 실제로 해 본 적이 없다**(D-101 정직 유보 ⓐ) — 논리로만 세운 경로다.
>
> ⚠ **되뜰 수 없는 기준이 둘 늘었다** — `sapkit-engine/harness/old-surface/m1-tools.json`
> (표면 채록본)과 `handler-tree.json`(구 핸들러 폴더 + 위임형 판정)은 이제 **재채록 수단이
> 없다**. `sapkit-cli/fixtures/baseline/`과 같은 지위이고, **틀린 값이 있어도 원본과
> 대조해 발견할 길이 없다.** 손으로 고치지 말 것.
>
> **⚠ 대장 수치를 읽는 법이 바뀌었다.** 지금 **증거 대기 30 · 증거 있음 156**인데,
> 판6.3에서 **두 가지가 함께 움직였다** — 실기 34종(126 → 92, **증거다**)과 요구 급
> **인하 62종**(92 → 30, **증거가 아니라 요구를 낮춘 것**이다 · D-092 ⓐ 집행).
> 인하된 62종은 실 SAP에서 검증된 적이 없다. 대장 머리말이 인하 전 수치를 병기한다.
>
> **큐 전체**(2026-08-23 갱신): ~~판7.5~~ **완료** → ~~판8~~ **완료(집행 일부 회수)** →
> ~~판9~~ **완료** → ~~판10~~ **완료**(⑷ ㉯ 활성 차용 6 소진 · D-107) → ~~판3.7~~ **완료**
> (⑶-b 상류 잔존 **56** 재저작 · §3.4 **56/56** · D-109~**D-111**) → ~~판11~~ **완료**
> (⑷ ㉮ 고지 범위 정합 · **사다리 ⑷ 완료** · **D-112** · **회수 D-113** — 수치·문장 정정 ·
> 판정 유지 · 경량 재리뷰 후 **종결** `54c10a37`) →
> ~~판M2-a~~ **완료**(⑴ M2 오프라인 배선 · **D-114** · 엔진 **1.1.0** · 리뷰 **R-PASS ·
> 차단 0** · **M-표 M2 승격은 없다** — 실 UAA 왕복 0) →
> **판M2-b**(**다음** · ⑴ M2 attended 실접속 · ⛔ **BTP 계정 — 사용자 결정 대기 · 모델이
> 열 수 없다**). **별건 5**(판 아님): `atomic-write` 배선 ·
> `data-access-approval` 파일명 3자 · executor 문장 · **브랜드**(끝그림 ④ — keyring은
> **② 소관으로 이동** · D-113 ⓒ④) · **체크박스 규범항 재저작 보류**(24 + 10 · D-113 ⓒ⑤).
> **이월 2**(판M2-a): destination 재적재 갈래 전용 시험 · 결정 append를 첫 커밋으로.
> 브리프는 전부 `docs/RUN-PLAN.md`.
>
> ⚠ **판이 늘어난 것으로 읽지 말 것** — 판9는 D-079 ⑧이 2026-08-10에 예약한 「1회」가
> M6 소멸로 옮겨 온 것이고(D-097), 판3.7은 **명부가 처음부터 세지 않던 범주**가 드러난
> 것이며, 판11은 판8이 시도했다 철회된 ㉮의 **재시도**다. 새로 만든 일은 판10 하나였고
> 그것도 판8이 미룬 몫이었으며, **그 넷이 다 끝났다.**
>
> **묻는 자리는 이제 둘이다** — ① 병합·push ② **BTP 계정 개설 여부**(판M2-b의 전제 ·
> 비용·계약이 걸린 것). 그 밖은 정하고 간다.
>
> ## 🚦 교체 관문 — **넷이 다 서서 교체했다** (2026-08-19 · 판7-b 실행 · D-095)
>
> **교체(판7-b)를 여는 조건은 이 넷이었다**(D-093 ⓐ — M-표는 제작 진도표이지 교체
> 조건이 아니다). 판6.2 실사에서 **기준 2만 안 섰고**, 판7-a가 그 배선을 놓아 넷이 다
> 섰으며, **판7-b가 실제로 갈아끼웠다.**
>
> | 기준 | 상태 | 근거 |
> |---|---|---|
> | 1 · 표면 186 동일 이름·호출 계약 | **선다** | 문서 3종 실측 184 ↔ 등록점 186(차 2종은 행 데이터 도구 **의도적 제외**) · 채록본과 186/186 선언 일치 (판6.2) |
> | 2 · 제품 게이트 2종이 신 엔진에 대해 통과 | **선다** | `--target=engine`으로 `smoke-mcp`·`conformance-server-gates` **exit 0**. **교체 뒤에는 무인자(`bundle`)도 같은 엔진**이라 둘이 같이 초록이어야 한다 — CI가 갱신하되 **트리거는 `main` 푸시·PR·수동뿐**(feature 브랜치 단독 푸시로는 안 돈다) |
> | 3 · 실데이터 2종 상시 게이트 + 음성시험 | **선다** | 신 엔진 안전 게이트 **15** + 게이트 음성시험 **44** + jest **4,161** · 제품 쪽 서버 안전 게이트 **26건**(판7-b에서 22 → 24 → 25 → **26**(2차 회수) · `GetTableContents` MCP 왕복 포함) |
> | 4 · 실 SAP 대표 절차 구·신 동일 결과 | **선다** (M1 범위) | 판6.1 C3 · 재생 판정 파일 2건 전 스텝 `match` |
> | 5 · 그때까지 **구 번들이 현역** | **지켜졌고, 끝났다** | 유지 기간이 교체 이후로 연장됐고(D-093 ⓐ) 소스 `engine/`이 롤백 자산으로 남아 있었다 — **2026-08-22 판7.5에서 은퇴**(D-101). 롤백 좌표는 위 🔙 절 |
>
> ⚠ **「교체했으니 검증됐다」로 읽지 말 것** (D-095 정직 유보 ⓐ·ⓔ).
> **기준 1·3·4의 근거는 판6.2·판6.1 기록이지 판7-b의 재실측이 아니고**, 이 판은
> **SAP에 한 번도 접속하지 않았다**. 대장의 `증거 대기` **126은 그대로**다.
>
> ~~**되돌리는 법**: 교체 커밋을 revert한다~~ — ⚠ **이 서술은 판7.5로 무효가 됐다**
> (2026-08-22 · D-101 · 정정은 판8 · D-103 ⓕ). 지금 그 revert는 **소스 없는 번들**을
> 되살린다. **현행 롤백은 두 걸음이고 위 🔙 절이 정본이다.** CI `engine-tests` 잡도
> 함께 제거됐으므로 **롤백 가능성은 상시로 기계 확인되지 않는다.**
>
> ## 📊 사다리 ⑴ 진척 — **이 표를 보고할 때 함께 말한다** (인증 행은 판5 · 교체 행은 판7-b)
>
...
> | 축 | 현재 | 목표 |
> |---|---|---|
> | **제품 자리** | **차지했다** — `interactive/server/server.bundle.cjs` = `sapkit-engine` **1.1.0** (교체 2026-08-19 · D-095 → 기동 토큰 배선 2026-08-23 · D-114) | 〃 |
> | **도구** | **186** | **186** — 전량 등록 |
> | 번들 무게 | **3,849,389 B** (구 8,304,513 B의 **46%**) | — |
> | 전송 | stdio · HTTP · SSE **3** | 3 |
> | RFC 백엔드 | `odata`·`soap`·`native`·`gateway`·`zrfc` **5** | 5 |
> | 인증 | Basic **접속** · `--env=<name>` **해석기는 접속하나 저장소가 다른 자리다**(판6.2 실측 — `~/.sapkit/profiles/`가 아니라 `<home>/Documents/mcp-abap-adt/sessions/<name>.env`. 같은 해석기를 `--env-path`로 태우면 실접속 성립) · `--mcp=<destination>` **기동 배선까지**(판M2-a · D-114 — `client_credentials`는 기동이 자동 취득 · `authorization_code`는 정지선 유지 · **실 UAA 왕복 0회**라 실접속은 여전히 `판M2-b`) · 브로커 **저장소 재료까지** | 4통로 전부 접속 |
>
...
> `sapkit-engine` verify·gates(표면 202 · **안전 15** · 대장 3 · HTTP 7 · SSE 11)·
...
> 단서가 붙어 있었다. 판7-b가 npm을 거치지 않고 node로 직접 빌드하게 바꿨다(빌드 명령은
> package.json과 **대조**해 드리프트를 막는다). **번들을 치우고 재빌드해 바이트 일치 확인.**
>
> ⚠ **머신 함정**: `sapkit-cli/node_modules`는 판6.2에서 **없어서** 첫 실행이 실패했다.
> 없으면 `npm install` 선행 — 레포 회귀가 아니다.
> **착수 기준선은 네 묶음 전부 뜰 것**(판6.2가 `doctor` + 제품 게이트 9종만 떠서 이
> 공백을 못 걸렀다 · 판7-a·판7-b는 넷 다 떴다).
>
...
> **제작 진도표**다. 판6.2 실사에서 기준 1·3·4가 섰고 기준 2는 판7-a가 세웠으며,
> **판7-b가 2026-08-19에 실제로 교체했다**(위 🚦 표). 대장의 126은 **교체 뒤에도 그대로**
> 남아 실사용으로 줄어드는 **제작 진도**다 — 교체가 그 수를 한 건도 줄이지 않았다.
>
...
>
> ## ✅ 이전 판 — **판7-b: ⑴ 교체(swap) 실행 (D-095 · 회수 D-096)** (2026-08-19 · 역사)
>
> **제품 번들이 자체 저작 엔진이 됐다.** `interactive/server/server.bundle.cjs` =
> `sapkit-engine` **1.0.0** · **3,813,330 바이트**(구 8,304,513의 **46%**).
...
>
> **▶ 그 뒤**: 전제였던 「교체가 실사용에서 안정」을 **판6.3이 세웠고**(D-098),
> **판7.5가 2026-08-22에 `engine/`을 은퇴시켰다**(D-101). 최상단 재개점 참조.
>
> - **없던 것을 만들어야 했다 — `sapkit-engine`에 번들러가 없었다.** 제품은 설치 중
>   `npm install`을 돌리지 않으므로 엔진은 `node_modules` 없는 **파일 하나**여야 하는데
>   산출물이 tsc `dist/` 트리뿐이었다. external 2종(`node-rfc`·`@napi-rs/keyring`)만
>   남기고 나머지(`@modelcontextprotocol/sdk`·`zod`·`fast-xml-parser`)를 인라인했다.
>   구 external의 `pino`·`pino-pretty`는 이 엔진이 **참조 0건**이라 빠졌다.
> - **⚠ 판 스탬프가 없으면 제품에서 무조건 `0.0.0`이다.** `readEngineVersion()`이
>   위로 훑어도 `interactive/server/` 위에는 `sapkit-engine` package.json이 없다.
>   **번들 스모크는 그래서 번들을 임시 디렉터리로 복사해 띄운다** — `dist/`에 둔 채
>   띄우면 폴백이 제 package.json을 만나 **단언이 공허해진다**(사보타주로 실측: define을
>   지우고 빌드해도 통과했고, 임시 디렉터리로 옮긴 뒤에야 `0.0.0`으로 검출됐다).
> - **🔴 안전 바닥선에 실제 구멍이 있었고, 독립 리뷰가 잡았다 (D-096).** 판7-a 이월 ①의
>   첫 판정은 「프로세스 env 통로를 존치하되 세 겹으로 막았다」였는데, 그 세 겹 중 하나
>   (「같은 키면 프로파일이 이긴다」)가 **`MCP_ALLOW_TABLE`에는 성립하지 않았다** —
>   병합은 **프로파일이 그 키를 실제로 적었을 때만** 이기고, 제품 마법사
>   (`setup-state.mjs` `ENV_KEYS`)는 셋 중 하나만 쓴다. **실측**(죽은 루프백): blocklist
>   키 없는 프로파일 + 프로세스 env `MCP_ALLOW_TABLE=BNKA` → **REACHED_SAP**(은행 계좌 ·
>   `minimal`층 deny). `MCP_BLOCKLIST_PROFILE=off`도 같았다.
>   **회수**: 노브를 **방향별로** 합성한다 — `MCP_ALLOW_TABLE`은 **프로파일 파일만** ·
>   층 이름은 프로파일보다 **더 조일 때만** · `MCP_BLOCKLIST_EXTEND`는 **합집합**.
>   한 줄로 **「프로세스 env는 조일 수는 있어도 풀 수는 없다」**. 옛 GAP-2가 요구한
>   수리는 조이는 방향으로 그대로 산다(D6를 통째로 되돌린 것이 아니다 · 장부에 방향
>   변경 append). 게이트는 **B2p·B2p2·B2p3·B2p4** 넷이 되고 **사보타주 4회**로 각각
>   load-bearing임을 확인했다.
> - **그리고 같은 거짓 문장이 두 곳 더 있었다** — `core/procedures/setup.md`(그것도
>   `Tell the user plainly:` 아래 = **에이전트가 사용자에게 읽어 주는** 문구)와
>   **`HANDOFF.md` §8 불변 규칙 4**(안전 정본). 첫 판정이 「문서 **한 줄**」이라고 적은
>   것이 틀렸다. `troubleshooting.md` 2곳·`tool-catalog` 귀속도 함께 고쳤다.
>   ③ **제품이 발행하는 MCP 배선에 노브 0개**(`smoke-mcp` 검사 ⑦)는 그대로 유효하다 —
>   매니페스트 드리프트 검사로는 못 막는 자리다(**생성기 자체에** 노브가 들어가면
>   생성물과 생성기는 여전히 일치).
> - **게이트에서 「기록된 격차」 기구가 사라졌다.** GAP-1은 엔진 4.14.1에서, GAP-2는
>   이 교체로 닫혀 남은 격차가 0이 됐다. B2p를 **셋으로 갈랐다** — B2p(푸는 노브가
>   든다) · **B2p2**(조이는 노브 `EXTEND`만 주입 — **판7-a가 「별건」으로 남긴 증명을
>   이 판이 세웠다**) · **B2p3**(프로파일 우선). **사보타주 2회로 증명**: 번들의
>   `resolveSafetyEnv`를 `{...profileEnv}`로 바꾸면 B2p·B2p2 FAIL, 순서를 뒤집으면
>   B2p3만 FAIL. 총계 22 → **24건 · PASS 24 · FAIL 0**.
> - **⑶-b 「서버 결부 copy 39건」 판정 — 3 해소 · 36 잔존.** 대체된 것은 번들 계열 3뿐.
>   `sap-assets/` 30은 **존치 확정**(D-079 ⑥ — 신 엔진도 기설치 `ZMCP_ADT_*`를 그대로
>   호출하고 SAP 오브젝트 재생성은 곧 재설치다) · `tool-catalog/` 4와 유틸 2는 이월.
>   ⚠ **그 결과가 판8의 목표를 바꾼다**: 루트 `LICENSE`는 `interactive/LICENSE`를
>   **서브트리 전체**에 걸어 두었으므로 36건이 남는 한 **고지 은퇴는 사실과 어긋난다.**
>   판8은 「고지 2→0」이 아니라 셋 중 하나를 고르는 판이 됐다(재저작 / 범위 좁혀 다시
>   적기 / ⑷의 완료 정의를 고치고 **포크 0 미달 명시**).
>   → **판8이 골랐다** (2026-08-22 · **D-103**): 뒤의 둘을 **함께** 채택하고 재저작은
>   기각. 위 재개점 표가 결과의 정본이다.
> - **`--rebuild`가 처음으로 소유자 머신에서 섰다.** npm을 거치지 않고 node로 직접
>   빌드하게 바꿨다(Windows `spawnSync npm.cmd EINVAL` 제약 해소). 빌드 명령은
>   package.json과 **대조**해 드리프트를 막는다. ⚠ 그 대조 갈래 자체는 **끝까지
>   실행되지 않았다** — 작업트리가 clean일 때만 도달하는데 시험하려면 트리를 더럽혀야 한다.
> - **CI 재배치**: `--rebuild`가 `engine-tests` → `sapkit-engine` 잡으로 옮겨 갔고
>   그 잡에 `fetch-depth: 0`이 켜졌다. **`engine-tests`는 이제 「롤백 소스 건강 확인」**
>   (구 포크 jest + 번들이 여전히 지어지는지)이다. **「구 번들 발행 표면 표류 확인」
>   스텝은 내렸다** — 구 번들이 현역일 때만 뜻이 있던 질문이다. `capture.mjs`도
>   제품 경로가 아니라 `engine/dist/`를 가리키게 바꿨다(그대로 두면 **신 엔진으로
>   「구 번들」 채록본을 덮어쓴다**). 재채록 결과 `m1-tools.json`은 **바이트 동일**이었다.
> - **🔴 이 판이 남긴 것 — 다음 판이 볼 자리**:
>   ① **실사용 증거 0.** 신 엔진이 제품 자리에서 SAP 업무를 한 번도 하지 않았다.
>      판7.5(은퇴)의 전제가 「실사용 안정」이므로 **순서 판단이 먼저다.**
>   ② ~~`MCP_ALLOW_TABLE`이 프로세스 env로 드는지 미증명~~ → **들었다. 그리고 막았다**
>      (D-096). **푸는 노브에서 「미증명」은 안전한 쪽이 아니라 모르는 쪽**이다 —
>      첫 판정이 그렇게 읽어 넘긴 자리이고, 다음에 같은 문장을 쓰지 말 것.
>   ③ **번들 스모크의 external 검사 ④는 게이트까지 도달하지 못했다** — external을
>      빼면 esbuild가 `.node` 로더 부재로 **빌드에서 먼저 죽는다**(더 이른 방어).
>   ④ **「여는 권한은 프로파일 파일에만」은 파일의 *내용*에 대한 규칙이다** —
>      `MCP_ENV_PATH`·`SAPKIT_HOME_DIR`은 여전히 프로세스 env가 정하므로 자기가 쓴
>      `sap.env`를 고르게 만들면 바닥선이 열린다(2차 리뷰 실측). 레포가 발행하는
>      배선은 다섯 키를 다 막지만(`smoke-mcp` 검사 ⑦), 셸·argv 쪽 절반은 D-043 ③의
>      "여는 쪽이 옵트인" 그대로다. **절대문으로 읽지 말 것.**
>   ⑤ **제품 진입점(셰임 + 실 번들) 상시 단언이 얇다** — 없는 게 아니다.
>      `conformance-runtime-dir.mjs:616-648`이 **진짜 번들**을 `launch.cjs`로 띄워
>      `initialize` 응답을 단언한다(전건 실행일 때만 · `--case`/`--consumer-root`에서는
>      생략). 다만 **판·도구 수·`NO_PROFILE` 강등 문구는 보지 않는다.** 이 판이 손으로
>      1회 더 확인했다(`sapkit-engine@1.0.0` · 65종 · 강등 관측). 두껍게 하는 것은 이월.
>   ⑥ 판6.1 구조적 이월 셋 · 꼬리 기준 인하 집행 · `conformance-server-gates`의 상설
>      음성시험 부재 · tool-catalog 자동 대조 게이트 부재 — **넷 다 그대로**다.
>      **→ 그중 「tool-catalog 자동 대조 게이트 부재」는 2026-08-23 판10이 닫았다**(D-107 ⓑ).
>
> ## 판7-a — **교체 관문 배선 (D-094 신설)** (2026-08-19)
>
...
>
> (당시 포인터: **`판7-b`(교체)** — **2026-08-19에 실행됐다.** 현행 포인터는 위 판7-b 절이다.)
>
...
> 그 파일이 든다). ⚠ **아래 목록은 2026-08-19 시점의 것이고 그 뒤로 다섯이 닫혔다** —
> 판6.3 ✅ · 판7-b ✅ · 판7.5 ✅ · 판8 ✅ · (판9는 대기). **남은 판의 정본은 위 재개점의
> 「큐 전체」와 `docs/RUN-PLAN.md`다.** 특히 **판8의 결과가 그때의 서술과 다르다** —
> 「고지 0 = 포크 0 도달」은 **D-103이 도달 불가로 판정하고 끝그림에서 내렸다.**
>
...
> 15. **`ActivateObjects`가 `BDEF/BDO`를 틀린 ADT 주소로 푼다** — `activateObjects.ts:126-127`이
>     `/sap/bc/adt/ddic/bdef/sources/{이름}`으로 푸는데 SAP이 인정하는 것은
>     `/sap/bc/adt/bo/behaviordefinitions/{이름}`이다. **같은 엔진의 읽기 쪽**
>     (`read/internal/behaviorRead.ts:124`)은 후자를 쓴다 — 엔진이 자기 안에 두 주소를 갖고 있다.
>     ⚠ **구·신 공통**(`engine/src/lib/resolveAdtUri.ts:117`)이라 교체가 만든 회귀가 아니고,
>     **신 엔진에서 아직 안 고쳐졌다**(위 1~14와 성격이 다르다). 실 SAP에서 판6.3이 처음 관측했고
>     `uri`를 명시하면 활성화된다. `GetInactiveObjects` 재조회 오라클이 이 거짓 성공을 잡았다. (2026-08-20 · D-098)
> 16. **`CreatePackage`의 발행 설명이 실제 동작과 어긋난다** — 선언은 `software_component`를
>     「안 주면 SAP이 기본값(보통 ZLOCAL)을 정한다」고 적는데 `createPackage.ts:299`가
>     **필수로 던진다**(벤더 가드를 그대로 되살린 자리). **사용자에게 나가는 제품 표면의 서술
>     결함**이고 신 엔진에도 그대로 있다. (2026-08-20 · D-098)
> 17. **엔진이 실패를 접을 때 요청 URL을 오류 문구에 싣는 경로가 있다** — 그 문구가 픽스처에
>     실리면 마스킹이 옳게 거부하므로 **한 단계의 오류가 시퀀스 전체의 증거를 날린다**(판6.3에서
>     세 번 겪었다). 근본 처방은 오류 문구에서 호스트를 정규화하는 것이고, 그 전까지는
>     시나리오 쪽에서 그 단계를 피하는 수밖에 없다 — 그것은 「증거를 못 만든다」와 같은 말이다.
>     (2026-08-20 · D-098)
> 18. **없는 객체를 「있다」로 답하는 자리 — 「거짓 성공」의 반대편** (2026-08-20 · **D-100**).
>     대상이 없다는 것이 **지상 진실로 확정된** 패키지 이름으로 다시 떠서 얻은 실측이다:
>     `GetPackageContents` → **`[]`**(「있는데 비었다」와 구별 불가) · `ReadPackage` →
>     **`success: true` · `metadata: null`**. 같은 대상에 `GetPackage`는 HTTP 400,
>     `SearchObject`는 0건으로 **옳게 부재를 말한다** — 읽기 도구 넷이 갈린다.
>     아울러 `CreatePackage`의 **「이미 있다」 판정이 거짓이었다** — 그 판정 조건이
>     `createPackage.ts:435-448`에서 **본문에 `already exists`가 들어 있기만 하면** 성립할 만큼
>     넓다. ⚠ **그 판정을 만든 SAP 응답의 원문을 못 봤으므로 인과는 미확정**이다.
>     1~14와 달리 **신 엔진에도 그대로 있다**(15·16과 같은 부류).
>
...
### 5-16. 도구 설명문 밀도 축소 (2026-08-23 등록 — 판9 · D-105 ⓒ)

**왜 여기 있나**: 판9가 「꼬리 49종을 표면에서 뺄 것인가」를 판정하며 **전량 유지**로
답했는데(D-105 ⓐ), 그 과정의 실측이 **「세션이 무겁다」의 과녁이 꼬리가 아님**을 보였다.
표면을 깎는 대신 **설명문을 다듬는 것**이 효과가 크고 파괴적이지 않다.

**실측 (2026-08-23 · 번들 stdio `tools/list` 바이트)** — ⚠ **아래는 프로파일이 활성인 이
머신의 connected 표면**이다. 프로파일 없는 환경(inspection-only)의 독립 리뷰 실측은
**`readonly` 65종 48,925 B(평균 753) · `readonly,high` 155종 130,741 B(평균 843)**이고
**결론은 두 표면에서 같다**(D-106 ⓔ④):

| 프리셋 | 도구 | `tools/list` | ≈토큰 | 평균 |
|---|---:|---:|---:|---:|
| `readonly` (**기본값**) | 74 | 54,763 B | ~13.7k | **740 B/도구** |
| `readonly,high` | 186 | 159,627 B | ~39.9k | 858 B/도구 |

기본 표면 상위: `GrepPackages` 1,683 · `GetCallGraph` 1,662 · `CheckSyntax` 1,547 ·
`GrepObjects` 1,393 · `ReadFunctionModule` 1,341 · `GetSqlQuery` 1,322 B —
**상위 10 중 꼬리는 4개뿐이고 나머지는 실사용 상위 도구다.**

**범위**: 도구 **설명문(description)과 inputSchema 서술만**. 이름·인자·응답 형태는
건드리지 않으므로 **검증 기준 1과 충돌하지 않고** tool-catalog·어댑터 프리셋·권한
정책도 움직이지 않는다.

**⚠ 착수 전에 먼저 물어야 할 것 — 이것이 이 항목이 판이 아니라 백로그인 이유다**:
설명문을 줄이면 **모델의 도구 선택 정확도가 떨어질 수 있다.** 줄이는 것 자체가 품질
위험이고, 그 균형은 **실사용으로만** 잰다. 「몇 바이트 줄였나」를 성과로 삼는 순간
제품이 나빠지는 자리다. 착수한다면 **줄인 뒤 오선택이 늘지 않는지 재는 방법**을 먼저
정할 것.

**재현**: 번들을 `node`로 spawn → `initialize` → `notifications/initialized` →
`tools/list` → `result.tools`의 `JSON.stringify` 바이트. `--exposition=<값>`으로 프리셋을
가른다. ⚠ **`development`는 무효한 값**이다(`launch.cjs`가 `readonly,high`로 번역한다 —
그대로 주면 도구 0종). ⚠ 프로파일이 없으면 inspection-only **155/65**로 잡힌다.
...
sapkit-engine/                  ← **MCP 엔진 소스 정본** (자체 저작 · 2026-08-19 D-095).
                                   tools/bundle.mjs → interactive/server/server.bundle.cjs.
                                   수리→bundle→반영은 UPDATE-RUNBOOK
                                   (구 포크 engine/ 은 2026-08-22 판7.5에서 은퇴 — D-101.
                                    되뜨려면 커밋 2264f89d)
sapkit-cli/                     ← 동봉 오프라인 검사기의 소스 정본 (번들은 interactive/checker/)
interactive/
...
4. 실데이터 조회 2종(GetTableContents/GetSqlQuery): **배포 기본값은 어떤 하네스에서도
   ...
   **소유자 머신은 D-043으로 서버 바닥선 모델** — 묻는 층 없이 서버 blocklist(기본
   standard)가 보호 테이블만 거부하며, 작업 테이블 예외는 프로파일 sap.env
   `MCP_ALLOW_TABLE`로 등록한다. **통로 규칙(2026-08-19 D-096으로 갱신)**: 노브 3종은
   프로파일 `sap.env`와 서버 프로세스 env 두 통로로 들되 **프로세스 env는 조일 수는
   있어도 풀 수는 없다** — `MCP_ALLOW_TABLE`은 프로파일 파일만, 층 이름은 프로파일보다
   **더 조일 때만**, `MCP_BLOCKLIST_EXTEND`는 **합집합**. (구 서술 「노브 3종은 sap.env
   안에서만 유효 — D-062 ⑥」는 구 엔진 번들의 동작이었고 교체로 바뀌었다. **여는 쪽이
   프로파일 파일 소유라는 결론은 그대로다.**)
