...
>
> **⑴ 런타임 왕복이 실증되지 않았다.** 확인된 것은 **설치·활성화·되읽기**까지다.
> 도구가 실제로 화면을 읽어 오려면 OData 서비스 **`ZSAPKIT_ADT_SRV`**가 있어야 하는데
> 이 시스템엔 구 `ZMCP_ADT_SRV`만 있다. 프로파일의 `SAP_RFC_ODATA_SERVICE_URL`도
> 구 서비스를 가리킨다. **번들 4**(`install-sap-assets.md` Step 4)가 그 자리다.
>
> **⑵ 29파일 중 9종만 SAP에서 검증됐다.** ECC 8종은 S/4에서 밟히지 않고
> (절차가 Step 2를 건너뛰라고 못박는다) OData 2종·`flush_cache`는 미설치다.
>
> **⑶ 신·구 오브젝트가 SAP에 함께 있다.** 옛 `ZMCP_*`·`ZCL_S4SAP_*`를 지우지 않았다
> (읽기만 했다). 정리는 소유자 판단이다. ⚠ **제품은 이제 `ZSAPKIT_*`를 부른다** —
> 옛 이름으로만 설치된 시스템은 재설치가 필요하다.
>
...
>
> ## ⛔ 지금 막힌 것 — SAP 자격증명 (2026-08-24 실측)
>
> 프로파일은 **2종만 남았다**(IDES-DEV는 이 머신에 없다). 둘 다 **TCP는 열리고**
> 서버가 `Basic` 챌린지를 정상 반환하는데 **자격증명이 거부된다(401)**.
>
> | 프로파일 | 호스트 | sap-system | TCP | ADT discovery |
> |---|---|---|---|---|
> | KR-DEV(활성) | `192.168.1.225:8000` | DEV | ✅ | ❌ 401 |
> | IDEA-JNC | `sap2.remoteides.com:8021` | S4H | ✅ | ❌ 401 |
>
> **프로파일당 1회만 시도하고 멈췄다**(기본 잠금 임계 5회). 고치려면
> `~/.sapkit/profiles/<alias>/sap.env`의 `SAP_PASSWORD`를 갱신한다(git 밖).
> **⑴⑵⑶⑸가 전부 이 한 줄에 걸려 있다.**
>
> ## 🧭 다음에 뭘 하면 좋은가 (해야 하는 것은 없다)
>
> 1. **OData 백엔드를 세워 왕복 실증** — ⑴을 닫는다. `install-sap-assets.md` Step 4.
> 2. **재생 대조 18종** — ⑸의 살아 있는 갈래. SAP 접속 + 반나절.
> 3. **구 오브젝트 정리** — ⑶. ⚠ **⑴과 같은 작업의 앞뒤다.**
>    옛 `ZCL_ZMCP_ADT_DPC_EXT`가 내부에서 `ZMCP_ADT_DISPATCH`를 부르므로
>    **지금 도는 OData 경로가 여전히 옛 이름을 탄다.** 순서: ⑴ 완료 → 프로파일
>    URL 교체 → 왕복 확인 → **그 다음에만** 옛 OData 계열 삭제. ALV OOP 핸들러 7종은
>    즉시 가능(SE24 Where-Used 선행) · ECC DDIC 8종은 애초에 없을 가능성이 높다.
>
