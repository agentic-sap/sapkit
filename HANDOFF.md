...
> - **attended 3단계 미수행** — spec §4의 완료 판정 중 **③ 재생 대조**와 **⑥ 실기
>   확인**은 SAP 접속이 필요해 소유자 자리에서만 한다. 순서: **C1 녹화**(전용 DEV
>   연습 패키지 · 실데이터는 데모 테이블만 · 마스킹 통과분만 커밋) → **C2 재생 판정**
>   → **C3 실기**(프로그램 생성→활성→반영 확인). 지금까지의 증거는 **전부 오프라인**
>   이며, 재생 대조는 픽스처가 0건이라 **아직 무증거**다.
