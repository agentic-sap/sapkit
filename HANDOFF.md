...
>   **설계 입구 3형태(spec-import)** §6 말미 등재(canary 1회 관찰 후 D-결정).
...
**플러그인 백로그 — 설계 입구 3형태 (spec-import) (2026-08-06 대화 세션 발원, D-결정 후보)**:
정식공정(create-program)의 앞문이 인터뷰뿐 — **설계서 지참자(도메인 전문가)는 입구가
없어** 공정 전체(독립 리뷰·기계 검증·위임)가 통째로 우회된다. 실증: ZUNIWTH 6월 런 2건이
"1a/1b 인터뷰 → 설계서 도출로 대체(사용자 결정)"를 수동 편법으로 수행(`ZUNIWR2020/2030
state.json`) — deep-interview 산출물의 자동 소비자 0건(D-052 기실측)과 동근. 후보 처방:
ⓐ deep-interview 산출물을 Phase 1 선입력으로 소비(이미 답한 차원 재질문 금지 — knowledge
preflight 패턴 재사용) ⓑ **spec-import 문 신설**(사용자 설계서 → 결손 차원만 확인 →
Phase 3 해시 승인부터 진입, Phase 0 플랫폼 preflight는 항상 수행) — 그래야 설계서
지참자도 뒤 장치 3개를 탄다. 재심사 트리거 = **canary 1회 관찰 후 D-결정**.
