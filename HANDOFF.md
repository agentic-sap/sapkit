...
> (**「✅ 또는 이유와 함께 못 섰다」**)로 닫혔고 그 30종은 후자다. ⚠ **접속이 돌아오면
> 가장 먼저 시험할 갈래**: `attended-only/` 픽스처를 `--fixture`로 직접 재생한다
> (코드 변경 0 · 픽스처는 엔진 1.0.0에 찍혔고 지금은 1.3.0이라 **자기 대조가 아니라
> 버전 간 회귀 대조**다 · 18종 전부가 픽스처 6편 안에 있다).
>
> **⑹ 그 밖** — D-118 채록본 부재 · 훅 출력 대소문자 미통일 · 체크박스 규범항 재저작
> 보류 · `atomicWriteFileSync`의 0600이 Windows에서 무효 · 하네스 중립 스크립트가
> Claude 어댑터 모듈을 import · `track-a-rebase-v2`의 죽은 `file:line`.
>
> ## 🧭 다음에 뭘 하면 좋은가 (해야 하는 것은 없다)
>
> 1. **OData 백엔드를 세워 왕복 실증** — ⑴을 닫는다. `install-sap-assets.md` Step 4.
> 2. **재생 대조 18종** — ⑸의 살아 있는 갈래. SAP 접속 + 반나절.
> 3. **구 오브젝트 정리** — ⑶. SAP GUI에서 소유자가 판단.
>
