...
>
> ## ✅ 직전 판 — **판3.3: ⑶-b 3차 배치 — 1-B 산업 15 + 1-C 국가 17 재저작** (2026-08-17)
>
> **1-B(산업)·1-C(국가) 두 갈래가 한 판에 소진됐다 — 코드 0줄, 지식 `.md`만.** D-087 ⑧의
> **소비 실증** 기준으로 **32편을 전량 재저작**했다. 실증한 소비 지점은 절차 4곳
> (`create-program.md:176` industry만 · `ask-consultant.md:44` 둘 다 ·
> `package-to-process.md:202-203` 둘 다 · `compare-programs.md:370` **country만**)과
> **페르소나 18편의 트리거 로드**다. **1-B 15/15 · 1-C 17/17** · **잔량 152 → 120**(명부
> `docs/reference/copy-baseline.md`). **새 D-결정은 없다** — 전 선택이 D-087 적용이거나
> 판3.2 선례 승계, 또는 위임 채택이었다(배치 선정과 강화 조항 3건은 **상시 위임 하 산출자
> 권고 채택**으로 기록 — 독립 점검 반영).
>
> - **재저작 32편**: 산업 15(`README` · `automotive` · `banking` · `chemical` ·
>   `construction` · `cosmetics` · `electronics` · `fashion` · `food-beverage` ·
>   `pharmaceutical` · `public-sector` · `retail` · `steel` · `tire` · `utilities`) +
>   국가 17(`README` · `au` · `br` · `cn` · `de` · `es` · `eu-common` · `fr` · `gb` ·
>   `in` · `it` · `jp` · `kr` · `mx` · `nl` · `sg` · `us`). 커밋 해시·리뷰 근거는 명부
>   §6 1-B·1-C가 든다.
> - **검증**: 독립 새-컨텍스트 리뷰 **5건**(전수 대조 4그룹 + 통합 리뷰) 전부 통과 —
>   **사실 대조 32/32 PASS**(누락 0 · 추가 0 · 헤딩 115종 · 표 48+30행 · 코드스팬·볼드·
>   숫자·한자/한글/가나 다중집합 기계 대조) · **회수 2회**(`05643eda` 10곳 · `41110412`
>   2곳) 후 재확인 정상 · **구성 독립성 32/32** · **허브 인용·외부 로드 지점 13종 전부
>   유효**(새로 깨진 것 0). 제품 게이트 9종 + doctor 전부 exit 0 · 지식 `.md` **148 유지** ·
>   버전 무변.
> - **⚠ push 전 확인 의무 (편승 — 판2·판3.1·판3.2에서 승계, 이 판도 동일)**: 판3.3은 **버전을
...
>   한다**).
>
> ## 🔻 다음 판 — **판3.4: ⑶-b 4차 배치 (반복 판 계속)**
>
> **결정은 이미 서 있다 — 판3.4도 「무엇을 다음에 다시 쓸까」만 고르면 된다.** 계수
> 방식·체크 규칙·선정 기준은 **D-087**이 확정했고 명부(`docs/reference/copy-baseline.md`)가
> 그 집행 자리다. **1-D·1-B·1-C 세 갈래가 소진됐으므로**(판3.2 · 판3.3) 판3.4는 **남은 세
> 갈래**에서 D-087 ⑧의 **소비 실증** 기준을 그대로 적용해 다음 배치를 고르고, 재저작 →
> 독립 리뷰 → 명부 체크 → 잔량 이력 append의 같은 루프를 돈다. **잔량 120/170.**
>
> - **남은 갈래 3 — 권고는 1-A**: **1-A 모듈 91**(한 판에 담기엔 커서 **모듈 묶음 분할을
>   제안한다** — 예: `common` 7 + 대형 모듈부터) · **1-E 견본 5**(**후순위** — 코드 파일이라
>   재저작이 곧 생성 코드 골격 변경이 되고 판4급 SAP 확인(판6 이연)이 필요해져 **산문 재저작
>   계약이 그대로 적용되지 않는다**) · **2부 24**(갚는 방법 **미정** — 그 판이 정한다).
> - **이월 목록**(판3.3도 손대지 않고 그대로 넘긴 것): 소비자 오인용 4건 · 사실 의심 2건 ·
>   제품명 표기 갈림(`SAPKIT` vs `sc4sap`) · **이식 전 참조 보존 목록 20곳**(⚠ 그중 1곳은
>   소비자 오인용 ④와 같은 건 — **이중 계수 금지**) · **소비자 서술 불일치 4건** · 헤딩
>   재작성 기록 2건 · **판3.3이 새로 등재한 사실 의심**(`fr.md` `PDF-A/3` · `in.md` 소득세·
>   GSTR 주석 · `kr.md` KERIS·Zengin · `chemical.md` IS-Chem · `food-beverage.md` catch
>   weight · `es.md` Cuaderno) · **배치 내 기존 불일치 4건**.
>   **상세는 `docs/RUN-PLAN.md` 판3.3 브리프의 「판3.4 재료」.**
> - **⚠ 재저작의 함정 — 판3.1·판3.2·판3.3 셋 다 밟았다.** 「사실을 잃지 않았는가」만 재면
>   부족하다. 판3.1은 저자가 설명을 돕겠다고 넣은 문장이 **원문에 없던 SAP 단언·지시**가
>   돼 5편 전부에서 **세 차례 회수**했고, 판3.2는 **강도·범위 미세 이탈 9곳**(`bcbe36f6`),
>   판3.3은 **강도·프레이밍·표기 12곳**을 **두 차례** 회수했다(`05643eda` 10곳 ·
>   `41110412` 2곳). 재저작은 표현·구성만 바꾸는 일이므로 **더하는 것도 범위 밖**이다 —
>   리뷰 근거는 **누락 0 + 추가 0** 양방향으로 잰다. 명부 §2에 명문화돼 있다.
>
...
> 든다). 시작 명령: `/dryforge:ready docs/RUN-PLAN.md 판3.4`.
