...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **BTP service key 확인 → 판M2-b (⑴ M2 attended 실접속)**
>
...
> 「Prepare an Account for ABAP Trial」 → ABAP 인스턴스 Service Keys에서 **JSON 발급**),
> 진행은 사용자 페이스다. **다음 세션은 「service key 받으셨어요?」부터 묻는다** —
> 받았으면 판M2-b 착수, 아직이면 아래 별건으로.
> ⚠ key JSON은 **비밀정보**다 — 채팅에 붙여넣게 하지 말고 레포 밖(`~/.sapkit` 아래
> 등)에 두게 한 뒤 **경로만** 받는다. trial 키의 그랜트가 `authorization_code`로
> 나오면 키에 `"granttype": "client_credentials"` 선언부터 안내한다(D-114의 기본값
> 설계가 정확히 이 자리다).
>
> **③ 그 밖에 혼자 갈 수 있는 판은 소진됐다.** 사다리 ⑵⑶⑷ 완료 · ⑴은 M2 실접속 한
> 칸. service key가 없으면 다음 세션이 할 수 있는 것은 아래 **별건 5 + 이월 2**뿐이다
> (판이 아니다 — 브리프도 `/ready`도 없이 도는 잡무 묶음).
>
> **판M2-b 착수 시 순서**(브리프는 `docs/RUN-PLAN.md`): ① 착수 기준선 네 묶음 ②
> service key 경로 확인(레포 밖) ③ `--mcp` 기동 — 진단이 그랜트·다음 걸음을 말한다
> ④ 실접속이 서면 **P1 read부터**(`GetSystemInfo`류) · destination tier=UNKNOWN이라
> write는 애초에 거부된다(fail-closed) ⑤ D15 해소 판정 + M-표 M2 승격은 **실왕복
> 증거**가 선 뒤에만.
>
> ## ✅ 직전 판 — **판M2-a: ⑴ M2 오프라인 배선 (D-114)** (2026-08-23 · 완료)
>
...
> | 항목 | 성격 | 막고 있는 것 |
> |---|---|---|
> | **판M2-b** | 판(attended) | **BTP 계정 — 사용자 결정.** 모델이 앞당길 수 없다 |
...
> | 이월 ⓐ destination 재적재 시험 | 이월 | 다음 판 리뷰가 본다 |
> | 이월 ⓑ 결정 append를 첫 커밋으로 | 이월 | 규칙 — 다음 판이 지킨다 |
>
> ---
>
