...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판8` (⑷ 잔존 차용 36건을 어떻게 할 것인가 · 오프라인)**
>
> **판7.5가 끝났다** (2026-08-22 · **D-101** + 리뷰 회수 **D-102** · **push 완료**).
> **구 포크 `engine/`이 레포를 떠났다** — 추적 파일 **1,604개 삭제 + 1건 이관**
> (482,714줄). 루트 `LICENSE`의 상류 고지 표가 **2행 → 1행**이 됐고, 남는 1건이 바로
> 판8이 다룰 `interactive/LICENSE`다.
>
> | 커밋 | 무엇 |
> |---|---|
> | `843a6aba` | 은퇴 본체 (⚠ 메시지 본문의 「1,605개」는 **오프바이원** — 인용은 D-102·RUN-PLAN을 따를 것) |
> | `ac26030b` | 재핀 (소스 주석이 바뀌어 provenance가 걸렸다 · **번들 바이트는 불변**) |
> | `6c6a72de` | 독립 리뷰 회수 — 차단 0 · 권고 3 전부 채택 (D-102) |
>
> 검증 **34/34 exit 0** (제품 게이트 9 + `doctor` · 음성시험 9 · `sapkit-engine` 10 ·
> `sapkit-cli` 4 · 재현 빌드). ⚠ **`--rebuild`는 커밋 뒤에야 도달한다** — 작업 트리가
> clean일 때만 그 갈래로 간다(D-095 정직 유보 ⓑ). 판 중간에 exit 1이 나오면 그것부터 볼 것.
>
> **⚠ 이 판이 남긴 유보 셋** (판8이 물려받는다):
> ⓐ **롤백 두 걸음을 실제로 해 본 적이 없다** — 아래 🔙 절. 그것을 재던 CI 잡을 이 판이
> 없앴으므로 **자동으로 해소되지 않는다.**
...
> 기계 편집 위험 대비 이득이 적어 **명시 이월**. ⚠ D-101은 이 이월을 밝히면서 **같은
> 범주의 `BUILD-PLAN.md`를 빠뜨렸고** 리뷰가 그것을 잡았다(D-102 ⓐ) — **유보 목록을 짤
> 때 「내가 안 본 같은 범주가 또 있는가」를 물을 것.**
>
> **⚠ 판8은 「고지를 마저 지우는 판」이 아니다.** `interactive/server/` 아래 차용
> **36파일**(sap-assets 30 + tool-catalog 4 + 유틸 2)이 남아 있어, 그 고지를 지우면
> **상류 저작물을 계속 배포하면서 고지만 지우는 꼴**이 된다(D-095 ⓔ). 판8은 셋 중
> 하나를 **고르는 판**이다 — ① 36건 재저작·재생성(단 `sap-assets/` 30은 D-079 ⑥의
> SAP 오브젝트 존치 결정과 정면 충돌) ② 고지를 은퇴시키지 않고 범위를 좁혀 다시 적는다
> ③ ⑷의 완료 정의를 「고지 0건」에서 「지식 차용분 0」으로 고치고 **포크 0 미달을
> 끝그림에 명시**한다. 브리프는 `docs/RUN-PLAN.md` 판8.
>
...
>    (판7-b가 번들러를 넣은 뒤 `npm ci`를 안 돌린 머신에서 실제로 났다 · **판7.5 착수
>    실측**). `cd sapkit-engine && npm ci`로 해소. ⓑ `sapkit-cli/node_modules`도
>    없으면 첫 실행이 실패한다(판6.2 실측). **둘 다 레포 회귀가 아니다.**
>    ⚠ `doctor`는 판7.5 착수 실측에서 **exit 0**이었다 — 판6.3까지 exit 1이던 원인
>    (이 머신 Antigravity 설치본 1.1.1 ≠ 고정 1.1.4)이 해소됐다.
> 3. 판8을 잡고 실행한다.
>
...
>
> **큐 전체**(2026-08-22 갱신): ~~판7.5~~ **완료** → **판8**(다음) → 판9 · 그리고
> 판M2-a(새 D-결정 필요) · 판M2-b(BTP 계정 — 사용자 결정). **판9의 전제도 판6.3이
> 채웠다**(실사용 분포 갱신). 브리프는 전부 `docs/RUN-PLAN.md`에 있다.
>
...
>
> **되돌리는 법**: **교체 커밋을 revert**한다 — 번들 바이트·VERSION·`integrity.json`·
> 게이트 기대값·문서가 한 커밋에 묶여 있어 함께 돌아간다. CI `engine-tests` 잡이
...
> 배선 → attended 실접속) · **판7-b**(교체 **만**) · **판7.5**(`engine/` 은퇴 — 고지 2→1) ·
