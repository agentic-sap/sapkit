...
> ═══════════════════════════════════════════════════════════════════
>
> ## ▶ 다음 세션은 이것으로 시작한다 — **`판11` (⑷ ㉮ 고지 범위 정합 재시도 · 오프라인)**
>
...
>
> **판11이 열렸다 — ⑷ ㉮ 고지 범위 정합 재시도.** 이제 상류 유래로 남은 것은
> ① `server/sap-assets/` 30(영구 존치 · D-079 ⑥) ② §3.4가 「계약행」으로 보존한 잔존
> (펜스 템플릿 · print-verbatim 블록 · 표 · 사용자 노출 인용문 — 파일 단위가 아니라
> 행 단위로 산재) ③ `[SC4SAP …]` 훅 태그 3건(브랜드 별건)이다. ⚠ **㉮는 D-103→D-104에서
> 한 번 데인 자리다** — 「파일 수로 좁히는」 서술이 두 번 틀렸으므로, 재시도는 고지가
> **파일 목록이 아니라 잔존의 성격**(설치 자산 30 + 산재 계약행)을 적는 쪽이어야 한다.
> 착수 시 **역방향 스캔 1회**(D-106 ⓒ)가 선행이다.
>
> **별건 대기 3** (판11 또는 그 뒤): `atomic-write.mjs` 소비자 0(배선 확인) ·
> `data-access-approval` 파일명 3자 불일치(안전 정본 통일) · executor 깨진 문장 수리.
>
...
> 재저작 = **판3.7**. **새로 생긴 일이 아니라 아무도 세지 않던 일이다.**
>
...
> **단정하지 않고** 판3.7이 표본을 떠서 판정한다(첫 표본 `SD/enhancements.md` 64/75).
>
...
>
> **판3.7은 문장 판이다** — 페르소나 25 · 절차 16 · 어댑터 5 · 지식 5 · 정책 2 · 도구 3 =
> **56파일**을 우리 문장으로 다시 쓴다. ⚠ **지식 파일과 다르다** — 페르소나·절차는
> **행동 지시**라 문장을 다시 쓰면서 지시의 강도·순서가 미묘하게 바뀌면 제품 동작이 바뀐다.
> **배치를 작게 끊고 회귀를 함께 볼 것.** ⚠ **착수 시 역방향 스캔을 1회 돌린다**(D-106 ⓒ) —
> 명부가 갱신된 뒤 레포가 움직였을 수 있다. ⚠ **§3.5 이월분 표본 판정도 이 판 몫**이다
> (첫 표본 `SD/enhancements.md` 64/75). 브리프는 `docs/RUN-PLAN.md` 판3.7.
>
> 1. `git fetch` + `main..origin/main` 확인(재개 전 원격 대조 — 안전 규칙)
> 2. **착수 기준선을 네 묶음 전부** 뜬다(제품 게이트 9 + 음성시험 9 + `sapkit-engine`
...
> ~~판9~~ **완료** → ~~판10~~ **완료**(⑷ ㉯ 활성 차용 6 소진 · D-107) → **판3.7**(**다음** ·
> ⑶-b 상류 잔존 **56** 재저작 — D-104 신설 · **D-106이 42 → 56으로 고쳤다** · 배치 반복형) ·
> 판M2-a(**병행 가능한 별도 후보** — M2 오프라인 배선 · **새 D-결정 필요**) ·
> 판M2-b(**BTP 계정 — 사용자만 답을 가진 것**). 브리프는 전부 `docs/RUN-PLAN.md`.
>
> ⚠ **판이 늘어난 것으로 읽지 말 것** — 판9는 D-079 ⑧이 2026-08-10에 예약한 「1회」가
> M6 소멸로 옮겨 온 것이고(D-097), 판3.7은 **명부가 처음부터 세지 않던 범주**가 드러난
> 것이다. 새로 만든 일은 판10 하나였고 그것도 판8이 미룬 몫이었으며, **그 판은 끝났다.**
>
