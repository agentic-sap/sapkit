...
>
> 오프라인 증거 — **판5 실측(2026-08-18)**: jest **4,139 통과 / 1 skip**(242 스위트) ·
> 신 엔진 자체 게이트 전종 green · `render-ledger --check` 통과 · **제품 게이트 19종
> 전부 exit 0**(9종 + doctor + 음성시험 9종 — 아래 옛 절의 「8종」류 서술을 현행으로
> 읽지 말 것. doctor는 로컬 설치 상태를 읽으므로 CI에는 없다).
...
>
> ## ✅ 직전 판 — **판5: ⑴ 엔진 잔여 코드 — M2 인증 오프라인 절반 + D18 + keyring·러너 CI (D-091 신설)** (2026-08-18)
>
