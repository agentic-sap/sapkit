 ...
 *     같은 **이연**.
 *   - **D141~D146·D148** — 판A2(D-147) 실사용 교훈 승격 4차. 응답에 키가 **늘어난**
 *     성공 갈래(D141·D142·D143)는 「늘어난 키뿐인가」를 재고, 구가 거부·오류로
 *     답하던 갈래(D142·D144·D146·D148)는 신이 등재된 모양으로 답했는지를 잰다.
 *     D145는 FUGR 항목의 전개를, D147은 새 항목이 아니라 D39·D40의 등재 키에
 *     `tool_list_republished`를 더한 것으로 든다. D105의 검사는 신이 **성공**하면
 *     D141의 두 키만 늘었는지로 넘어간다(그 전에는 그 갈래가 정확 일치였다).
 *
...
/**
 * D38·D39·D40의 등재 키 셋 + **D147**의 `tool_list_republished`. D147은 새 항목이 아니라
...
// ── D141~D148 (판A2 · D-147 · 실사용 교훈 승격 4차) ──────────────────────────
...
/** D143 — 구가 「오류 없는 실패」(`success:false · errors:[]`)로 답한 채록분인가. */
...
/** D144 — 구가 거짓 FIXPT precheck로 쓰기를 막은 채록분인가. */
...
/** D145 — 집계 응답에서 FUGR 계열이 **아닌** 결과·건너뜀만 남긴다(구성원은 `function_group`을 단다). */
...
/** D146 — 구가 「다음 후보」 계열 4xx로 죽은 채록분인가. 상태 코드를 돌려준다. */
...
/** D148 — 픽스처의 인클루드 이름이 `=`로 채운 클래스 인클루드인가. */
...
export const M1_DIVERGENCES: readonly DivergenceEntry[] = [
  ...
  },
  {
    ...
    // D141 확장 — 신이 **성공**하면(활성화가 실제로 섰다) 구와 갈린 것은 D141의 두 키뿐이어야
  ...
  // ── D141~D148 (판A2 · D-147 · 실사용 교훈 승격 4차 — 분담 E2) ───────────────
  //
  // 전부 **실기 미검증**이다 — 대체 기대 시험은 오프라인 계약 시험이고, 재생 픽스처에
  // 이 갈래가 실제로 있는지도 확인하지 않았다(있으면 여기가 판정한다). D147은 새 항목이
...

  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d141 · sapkit-engine/src/tools/write/createServiceBinding.ts · ' +
      'sapkit-engine/src/tools/write/internal/serviceBinding.ts · 덧말표(harness/old-surface의 amendments 표)',
    substituteTest:
      'sapkit-engine/src/tools/write/__tests__/createServiceBinding.test.ts — ' +
      '「D141 — 계약(category)을 고를 수 있고, 응답이 무엇을 만들었는지 말한다」 2건',
    resolvesIn: null,
    // D105가 `activated:true` 성공 갈래를 들고(그 검사가 D141의 두 키를 함께 잰다), 여기는
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d142 · sapkit-engine/src/tools/write/updateServiceBinding.ts · ' +
    ...
    substituteTest:
      ...
      '「D142 — srvb:allowedAction이 없으면 거부하지 않고 요청을 보낸다」 4건 · ' +
      'sapkit-engine/src/tools/write/__tests__/deleteServiceBinding.test.ts — 「D142」 1건',
    resolvesIn: null,
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d143 · sapkit-engine/src/tools/read/checkSyntax.ts · ' +
      '덧말표(harness/old-surface의 amendments 표)',
    substituteTest:
      'sapkit-engine/src/tools/read/__tests__/checkSyntax.test.ts — 「D143 — include의 판정불능과 main_program」 5건',
    resolvesIn: null,
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d144 · sapkit-engine/src/tools/write/updateProgram.ts',
    substituteTest:
      'sapkit-engine/src/tools/write/__tests__/updateProgram.test.ts — ' +
      '「D144 — 인라인 precheck가 FIXPT 계열로 실패하고 저장된 판이 깨끗하면 쓰기를 진행한다」 6건',
    resolvesIn: null,
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d145 · sapkit-engine/src/tools/read/grepObjects.ts · ' +
      'sapkit-engine/src/tools/read/internal/objectSource.ts',
    substituteTest:
      'sapkit-engine/src/tools/read/__tests__/grepObjects.test.ts — 「D145 — FUGR는 함수모듈·인클루드로 전개해 훑는다」 4건',
    resolvesIn: null,
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d146 · sapkit-engine/src/tools/read/getTypeInfo.ts',
    substituteTest: 'sapkit-engine/src/tools/read/__tests__/getTypeInfo.test.ts — 「D146」 2건',
    resolvesIn: null,
  ...
  },
  {
    ...
    status: 'active',
    evidence:
      'sapkit-engine/harness/DIVERGENCES.md#d148 · sapkit-engine/src/tools/read/getInclude.ts · ' +
      '덧말표(harness/old-surface의 amendments 표)',
    substituteTest:
      'sapkit-engine/src/tools/read/__tests__/getInclude.test.ts — 「D148 — 클래스 인클루드(CCIMP 등)는 이 경로가 아니다」 3건',
    resolvesIn: null,
