 ...
 * ## 요구 증거 급과 상태 3칸
 *
 * 도구마다 **주 급이 정확히 하나**다. 사다리의 정본은 `harness/ledger/grade.ts`의
 * `gradeOf()`이고, 여기 베껴 두지 않는다 — 판6.3이 인하 갈래를 더하면서
 * (실사용 호출은 있으나 얼린 관측 목록에 없으면 **contract로 인하**) 이 자리에
 * 남아 있던 옛 서술이 **인하 이전 규칙 그대로**가 됐고, 그 문장이 바로 그 판이
 * 막으려던 오독이었다(판6.3 독립 리뷰 V1). 사다리를 알고 싶으면 그 파일을 봐라.
 *
 * 어느 도구가 어느 칸인지를 **여기서 정하지 않는다** — 배정은 부르는 쪽의 몫이고,
 * 이 계산기는 그것을 입력으로 받아 셈만 한다. 입력이 없으면 계약 시험이다.
 *
