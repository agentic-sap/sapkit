 ...
 *  ⓒ 등록점에 없는 이름은 **막지 않는다** — 지금처럼 사후 백스톱에만 걸린다.
 *
 * ⓒ가 없으면 이 판은 반대 방향으로 무너진다: 선언이 없다고 fail-closed로 막으면
 * 표에 없는 이름 한 건이 시퀀스 전체의 녹화를 막는다.
 */
...
const NOT_IN_REGISTRY = [
  // **지어낸 이름이 아니라 구 트리에 실재하는 이름이다.** 구 핸들러 348파일이 내놓는
  // `TOOL_DEFINITION` 이름은 339종인데 이 표면은 186종뿐이라, `low/` 갈래의 `*Low`와
  // `compact/` 갈래의 `Handler*`는 **이 판이 끝나도 지어지지 않는다**(제작 계획이
  // `compact` 묶음을 아예 세우지 않은 근거가 그 교집합 0이다). 판7-b 교체로 녹화 대상은
  // 제품 번들 = 신 엔진이 됐지만, 시나리오는 사람이 손으로 적는 파일이라 표에 없는
  // 이름은 여전히 들어올 수 있다 — 이 갈래가 겨누는 것이 그 입력이다.
...
describe('선언이 없는 도구는 사전 검사에서 막히지 않는다', () => {
  /**
   * 등록점 표에 없는 이름들. 여기서 fail-closed로 바꾸면 그런 이름 한 건이
   * 시퀀스 전체의 녹화를 막는다 — 계획서가 못 박은 자리다.
   */
