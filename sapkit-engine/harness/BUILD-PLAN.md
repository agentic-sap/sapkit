...
**재귀로** 훑은 수(최상위 16 ∪ `attended-only/` 72)이고, 대장의 16은 `ledger/evidence.ts`의
`toolsInFixtures`가 **최상위만** 세는 수 — 곧 **구 엔진 채록 기준선의 폭**이다.
**의도된 차이이지 오차가 아니다.** 대장의 두 행도 서로를 가리키도록 적어 두었다
(판6.3 독립 리뷰 V2).

⚠ **처음 이 문단은 「재생이 `attended-only/`를 수집하지 않기 때문이다」라고 적었고, 그것이
결함으로 드러났다.** 배정(재귀)과 수집(최상위)이 갈려 **「요구 급은 재생 대조인데 재생 대상이
될 수 없는」 도구 18종**이 생겼다(D-122 ⑵). 배정을 낮추는 길은 D-122 ⓑ가 「요구를 낮춰 증거를
만드는 모양」이라며 기각했으므로 **수집을 재귀로 넓혀 맞췄다** — 되돌리지 않는 시퀀스와 P4만
빼고(`harness/replay/batch.ts`), 뺀 것은 이유와 함께 화면에 적는다. 위의 16은 그 뒤로도
**기준선의 폭**을 재는 수라 그대로다.
