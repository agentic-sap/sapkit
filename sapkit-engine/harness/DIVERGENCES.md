...
### D1 활성화 — 실데이터 도구 작업이 대체 기대 시험을 물린다

사전 등재 3건 중 마지막 이연이었다. 장부는 D1의 대체 기대 시험을 "실데이터 도구
작업이 소유한다"고 적어 두었고, 그 작업이 판6.1에서 왔다.

- **활성화 시점**: `GetSqlQuery`의 재생 증거를 세우는 판(판6.1). 도구 자체는 이미
  등록점에 있었으나 판정 자리가 비어 있어, 재생 대조가 이 도구의 갈림을 통과가
  아니라 **무증거**(`allowlisted-deferred`)로 세고 있었다.
- **구 동작(실측)**: 질의문은 구 경로 어디에서도 바뀌지 않는다 — 잃는 쪽은 ADT
  Data Preview freestyle 엔드포인트이고, 실측 경계는 SELECT 목록 18~28컬럼 사이다
  (`HANDOFF.md` §6 13-9 · 2026-08-04 실측). 구 엔진의 결함은 WHERE를 잃는 것이
  아니라 **돌아온 답을 자기 질문에 대조하지 않는 것**이다:
  `engine/src/handlers/system/readonly/handleGetSqlQuery.ts`가 질의문을 응답
  필드로 되울리기만 하고 행과 견주지 않은 채 `isError: false`를 찍는다. 술어를
  무시한 표가 침묵 실패가 아니라 **그럴듯한 오답**으로 나가고, 그대로 P2 판단과
  쓰기 승인 문서로 흘러든다.
- **신 동작**: `sapkit-engine/src/tools/row-data/wherePredicate.ts`가 돌아온 표를
  자기 질의의 WHERE에 대조하고, 어긴 표는 **행을 싣지 않고 거부한다**
  (`ERR_SQLQUERY_PREDICATE_IGNORED` — 필터를 무시한 결과는 부분적으로 맞는 답이
  아니라 통째로 틀린 답이라, 진단한다며 그 값을 되싣으면 거부가 무의미해진다).
  검증기는 **한쪽으로만 틀린다** — 모델링한 술어(`=` · `<>` · `IN`)와 최상위 `AND`
  사슬만 보고, `OR`·`NOT`·하위 질의가 섞이거나 컬럼이 표에 없으면 아무 말도 하지
  않는다. 옳은 결과를 거부하는 것은 틀린 결과를 통과시키는 것만큼 나쁘기 때문이다.
- **대체 기대 시험**:
  `sapkit-engine/src/tools/row-data/__tests__/getSqlQuery.test.ts`의
  「13-9 — WHERE가 결과에 실제로 반영되는가」 절 8건(발송 본문에 WHERE가 실리는가 ·
  무시한 표를 성공으로 내주지 않는가 · 거부에 행 값이 실리지 않는가 · **과수리
  역검증** · 판정할 수 없는 술어에 거짓 경보를 내지 않는가 · 서버 코어를 지나서도
  같은 판정인가) +
  `sapkit-engine/harness/replay/__tests__/divergences.test.ts`의
  「D1 — 술어를 무시한 표를 성공으로 내주지 않는다」 절 6건.
- **기계 장부 반영**: 했다. `harness/replay/divergences.ts`의 D1이 `check`를 들고,
  그 검사는 **도구가 쓰는 검증기를 그대로 부른다**(자를 두 벌 만들면 "도구는
  거부했는데 등재는 옳다고 못 하는" 어긋남이 생기고, 그 어긋남은 신 엔진 결함처럼
  보인다). 덮는 갈림은 하나뿐이다 — 구가 술어를 어긴 표를 성공으로 냈고 신이 그
  표를 거부한 자리. 신이 옳은 표를 거부했거나, 거부 문구가 다른 오류이거나, 구도
  오류였으면 `allowlisted-fail`로 떨어진다.
- **재생 급은 따로 선다.** 등재는 **차이가 났을 때만** 발동하므로, 구·신이 같은
  답을 낸 단계는 그냥 `match`이고 `GetSqlQuery`의 재생 칸을 채운다. 그래서 이
  도구의 요구 급 「재생 + 대체」가 둘 다 설 수 있다.
...
- **분류**: 축소 — **해소 마일스톤 = [M6 (교체 직전 관문)](../../docs/BLUEPRINT.md#마일스톤-체계-m1-잔여부터-m6까지)** (판정은 마일스톤이 아니라 아래 관찰 조건이 연다)
...
- **관측 (2026-08-20 · 판6.3 · D-098 ⓒ)** — **승격 없음. 「관찰되지 않았다」이지 「없다」가
  아니다.** attended 실기 9편·107단계에서 `406`·`415`·`redirect`·`skipSessionType` 문자열
  출현 **0건**이고, 406이 실제로 드러나는 코드 경로(`src/tools/read/internal/classIncludes.ts`가
  내는 `"… read not supported on this system (HTTP 406)."`)를 **로컬 인클루드 되읽기 4회로
  밟았으나 그 문구는 나오지 않았다**.
  ⚠ **그러나 위 판정 조건(「C1 녹화에서 발동이 관찰되는가」)은 셋 중 하나에만 적용된다.**
  ⑴ `406`/`415` — 신 접속 계층에 **자동 재협상이 애초에 없으므로** 「재협상이 발동했는가」는
  원리상 관측 불가이고, 관측 가능한 것은 **「406/415가 도래하는가」**뿐이다(위가 그 관측이다).
  ⑵ discovery 외 폴백·리다이렉트 추종 · ⑶ `skipSessionType` — **현재 채록 설비로는 관측되지
  않는다.** 픽스처는 응답 본문만 담고, 자식 프로세스 stderr에는 기동 배너 2줄뿐이라 HTTP
  수준 로그가 없다. **즉 이 조건을 그대로 두면 판을 몇 번 더 돌려도 ⑵·⑶의 판정은 나지
  않는다 — 설비를 먼저 고쳐야 한다.** 이 사실을 여기 적어 두는 이유는 다음 사람이 실제로
  가는 자리가 결정 기록이 아니라 이 장부이기 때문이다(판6.3 최종 독립 리뷰 A3).
...
- **분류**: 축소 — **해소 마일스톤 = [M2 (접속 방식 확장)](../../docs/BLUEPRINT.md#마일스톤-체계-m1-잔여부터-m6까지)**
...
## 상태 갱신 (append) — D15: 세 통로가 서로 다른 지점까지 왔다 (2026-08-14)

**위 D15 본문은 고치지 않는다.** 이 장부는 append로만 자란다 — D3·D2도 M1 사전 등재
표를 고치지 않고 「활성화」 절을 새로 달아 상태를 바꿨고, 「기계 장부 반영 기록 …
(append)」·「제작 중 발견분 (append)」도 같은 규칙이다. D15의 상태 갱신도 같은 자리다.

**D15는 지워지지 않는다.** 오프라인 대량 제작 판이 도달한 것은 `짓기 완료`뿐이고
**실접속 성공(`증거 완료`)은 미뤘으므로** 항목은 장부에 남는다(D-082 ① — `증거 완료`가
되기 전에는 닫혔다고 말하지 않는다).

| 통로 | D15 등재 시점 (M1) | 지금 (2026-08-14 실측) | 남은 것 |
|---|---|---|---|
| `--env=<name>` | 미구현 — 접속을 만들지 않음 | **접속까지 완성.** 세션 디렉터리의 `<name>.env`를 찾아 **기존 Basic 해석기에 그대로 태운다** (`src/server/startup.ts` ⓑ 갈래 → `resolveProfileDetailed`) — 자격증명·키체인·tier·안전 노브가 전부 한 경로를 지난다 | attended 실접속 확인 |
| `--mcp=<destination>` | 미구현 — 진단 `MCP_DESTINATION_UNSUPPORTED` | **설정 조립까지.** service key를 읽어 OAuth2 설정을 조립하지만 **UAA에서 토큰을 받지 않으므로 접속이 서지 않는다.** 진단 코드가 **`MCP_DESTINATION_TOKEN_PENDING`**으로 바뀌었다 (`src/server/startup.ts:226`) | UAA 토큰 취득 — 실접속이라 오프라인 판의 범위 밖 |
| 브로커 (`--auth-broker` · `MCP_USE_AUTH_BROKER`) | D15 밖 | **저장소 재료 조립까지** — 별도 등재 **D32** | D32 참조 |

- **분류와 해소 마일스톤은 그대로다** — `축소` · 인증 확장(Basic 외) 마일스톤.
- **등재 시점의 「교체 판 전에 반드시 해소해야 한다」도 그대로 유효하다.** destination
  인증으로 등록된 곳은 지금 교체하면 **여전히 접속을 잃는다** — 설정이 조립돼 있다는
  것과 토큰을 받아 붙는다는 것은 다른 일이다.
- **대체 기대 시험**: `src/server/__tests__/startup.test.ts` —
  「destination 통로 — `--mcp=<destination>` (service key)」 절(service key를 읽어 설정을
  조립하되 **접속은 아직 아니라고 말하는지** · `MCP_DESTINATION_TOKEN_PENDING` 2건) ·
  「destination 통로 — `--env=<name>` (세션 env 파일)」 절(세션 파일로 접속을 조립하는지 ·
  못 찾으면 **다른 시스템으로 폴백하지 않는지**).
- **`--env=<경로처럼 생긴 값>`은 이 갱신과 무관하다** — 그 갈래는 D17이 소유하며
  `ENV_DESTINATION_INVALID`로 거절하는 동작 그대로다(이름이지 경로가 아니다).

## 상태 갱신 (append) — M2 인증 계층이 생겼다 (2026-08-18)

**위 본문들은 고치지 않는다.** 이 장부는 append로만 자란다. 이 판(판5 E1)이 지은
것은 UAA 토큰 취득과 그 위의 JWT 접속 계층이고, **실접속 확인은 판6 몫**이다 —
그러므로 아래 어느 항목도 `증거 완료`를 주장하지 않는다(D-082 ①).

### D116 — 인증 계층은 **조용한 회복을 하지 않는다** (신설)

- **분류**: 강화(안전 바닥선을 올림) — 영구 차이다. 해소 마일스톤 없음.
- **구 동작(읽기 참조)**: 구 부품의 토큰 통로는 브라우저 OAuth2 로그인으로
  토큰을 받고(`engine/node_modules/@babamba2/mcp-abap-adt-auth-broker/dist/AuthBroker.js:373-464`
  → `…/mcp-abap-adt-auth-providers/dist/providers/AuthorizationCodeProvider.js:76-90`,
  `src/profile/destination.ts` 머리말에 이미 적힌 실측 읽기), 세션 저장소를 두어
  `--unsafe`에서 실제로 파일을 쓴다(`brokerFactory.ts:407-425` · 기본값은 메모리
  `SafeAbapSessionStore`). 저장소 디렉터리는 없으면 만든다
  (`…/stores/abap/AbapSessionStore.js:71-74`).
- **신 동작 — 셋을 승계하지 않았다**:
  - ⓐ **갱신 실패는 거기서 끝난다.** `UAA_REFRESH_FAILED`를 올리고 처음 취득
    갈래(브라우저 로그인 · `client_credentials`)로 **흘러내리지 않는다**
    (`src/auth/tokenSource.ts`의 `renew` ⓐ 갈래). 흘러내리면 도구 호출 한복판에서
    사람 상호작용이 시작되거나 **다른 권한 주체**의 토큰이 조용히 대신 실린다 —
    사람이 자기가 무엇으로 인증됐는지 모른 채 SAP에 요청을 보내게 된다.
  - ⓑ **토큰은 메모리에만 산다.** 세션 파일도 키체인도 쓰지 않고 디렉터리를
    만들지도 않는다. 이 엔진의 자격증명 취급은 이미 한 방향이다 — 비밀번호는
    프로파일이 `keychain:` 참조로 갖고 서버는 **읽기만** 한다(D-080 ①·
    `src/profile/secrets.ts`). 서버가 토큰을 **쓰기** 시작하면 그 원칙이 뒤집히고
    지워야 할 비밀이 하나 더 생긴다.
  - ⓒ **엔진이 브라우저를 열지 않는다.** 인가 URL은 호출부에 건네고 끝이다
    (`acquireByAuthorizationCode`의 `openAuthorizeUrl`). 사람이 무엇을 승인하는지
    모른 채 창이 뜨는 것은 attended 명시성에 어긋난다.
- **대체 기대 시험**:
  - ⓐ `src/auth/__tests__/tokenSource.test.ts` 「갱신 실패는 재로그인으로 넘어가지
    않는다」 절 — 특히 **acquire를 부르지 않고 왕복이 갱신 한 번뿐**임을 센다.
  - ⓑ 같은 파일 「인증 계층 — 디스크로 나가는 경로가 없다」 절 — 계층의 모든
    모듈이 `node:fs`·키체인 모듈을 import 하지 않는다는 것을 원천에서 본다.
    (쓰지 않는 것을 왕복 시험으로 보이기 어려워 원천 검사로 대신한다.)
  - ⓒ `src/auth/__tests__/callback.test.ts` 「acquireByAuthorizationCode — 왕복
    전체」 절 — 호출부가 URL을 **건네받는 것이 전부**임을 센다.
- **기계 장부 반영**: **안 했다** — 인증 계층이고 도구 응답 시퀀스에 나타나지
  않는다. D91·D112와 같은 자리다.

### D15 상태 갱신 2차 — `--mcp` 통로에 남은 것은 「토큰 미취득」 하나다

**위 「상태 갱신 (append) — D15」 표도 고치지 않는다.** 그 표의 `--mcp` 행이 적은
「남은 것 = UAA 토큰 취득」이 **코드로는 해소됐고**, 남은 것은 실접속 증거다.

| 통로 | 2026-08-14 상태 | 지금 (2026-08-18) | 남은 것 |
|---|---|---|---|
| `--env=<name>` | 접속까지 완성 | 변화 없음 | attended 실접속 확인 (판6) |
| `--mcp=<destination>` | 설정 조립까지 — 토큰을 받지 못함 | **토큰 취득 계층이 생겼다**(`src/auth/` — authorization_code · refresh_token · client_credentials). 다만 **기동은 그것을 돌리지 않는다** — 토큰 취득은 UAA로 나가는 실왕복이고 authorization_code는 사람이 브라우저 앞에 있어야 끝나므로, 기동 한복판에서 말없이 시작하지 않는다. 진단 코드는 `MCP_DESTINATION_TOKEN_PENDING` 그대로이나 **뜻이 좁아졌다**: 「엔진이 못 받는다」 → 「아직 안 받았다」 | 실접속 성공 (판6 · M2 `증거 완료`) |
| 브로커 | 저장소 재료 조립까지 (D32) | 변화 없음 | D32 참조 |

- **분류와 해소 마일스톤은 그대로다** — `축소` · M2.
- **「교체 판 전에 반드시 해소해야 한다」도 그대로다.** 토큰을 받는 코드가 있다는
  것과 그 통로로 실제 시스템에 붙어 봤다는 것은 다른 일이다.
- **대체 기대 시험**: `src/server/__tests__/startup.test.ts`의
  「destination 통로 — `--mcp=<destination>` (service key)」 절(그대로 유효) ·
  `src/profile/__tests__/destination.test.ts`
  「`planServiceKeyConnection` — 재료를 인증 계층 모양으로 옮긴다」 절(service URL이
  없으면 **토큰을 받아도 접속이 서지 않는다**고 미리 가른다).

### D91·D112 전제 변화 — `authType === 'jwt'`가 이제 **가능한 값이다**

두 항목의 판정은 **그대로 선다.** 다만 근거 두 다리 중 하나가 바뀌었으므로 여기
적어 둔다 — 적지 않으면 다음 판이 낡은 근거를 그대로 인용한다.

- **바뀐 다리**: D91·D112 본문은 "신 엔진의 인증은 Basic 하나뿐이라
  `authType === 'jwt'`가 될 수 있는 값이 아예 없다"고 적었고 `src/contracts.ts:42-48`을
  가리켰다. M2가 `AuthType`과 `ConnectionConfig.authType`을 더했으므로 **그 문장은
  이제 사실이 아니다.**
- **선 채로 남는 다리**: 두 묶음의 도구 20종(D91의 16 + D112의 4)은 전부
  `available_in: ['onprem','legacy']`이다. `SAP_SYSTEM_TYPE=cloud`에서는 애초에
  `tools/list`에 뜨지 않으므로, 구가 `isCloudConnection()`으로 거절하던 자리를
  **노출 선언이 계속 막는다**. 대체 기대 시험도 그 축 그대로다.
- **그래서 무엇이 달라지는가**: 이 두 항목은 이제 "있을 수 없는 조건이라 안 지었다"가
  아니라 **"조건은 생겼지만 그 도구들이 그 축에 노출되지 않아 여전히 죽은 코드다"**로
  읽어야 한다. 판정 결과는 같고, 되짚을 자리는 **JWT 접속에서 그 20종이 정말
  안 뜨는가**이며 그것은 판6 실접속에서 확인할 몫이다.
- **기계 장부 반영**: 없음(위 두 항목과 같은 자리).

## 판6.2 실측 (2026-08-19 · attended · D-093)

이 절은 **append**다. 위 항목들의 본문은 고치지 않는다 — 이 장부의 규칙이 그렇다
(정정도 새 항목). 아래 넷은 전부 **판6.2가 실제로 잰 것**이고, 잰 방법과 **무엇을
증명하지 않는지**를 함께 적는다.

### D15 `--env=<name>` — 통로는 **살아 있으나 다른 저장소를 본다**. 접속 자체는 성립.

D15는 `--mcp=<destination>`과 `--env=<name>` 둘을 묶어 「M1에서 미구현」으로 등재했다.
그중 **`--env` 쪽만** 이 판이 쟀다(`--mcp`는 BTP가 필요해 여전히 봉쇄 — D-092 ⓑ).

- **실측 ①(구조)**: `--env=<name>`은 **`~/.sapkit/profiles/`를 보지 않는다.** 플랫폼
  세션 저장소 `<home>/Documents/mcp-abap-adt/sessions/<name>.env`를 본다
  (`sapkit-engine/src/profile/destination.ts:76-124,136-141,196-213`). 뿌리는
  `--auth-broker-path` → `AUTH_BROKER_PATH` → 플랫폼 기본 순이고, cwd는 **세션
  저장소로 쓰지 않는다**(같은 파일 `sessionsDir` 주석 — 「프로젝트 로컬 파일 하나가
  이름만으로 접속이 되어 버린다」).
- **실측 ②(접속)**: 이 머신에는 `sessions/`가 아예 없어 `--env=KR-DEV`는
  `ENV_DESTINATION_NOT_FOUND` + `connection=none`(도구 65종)으로 끝난다. **폴백은
  없다** — D16·D17이 그렇게 잠가 둔 자리와 정합한다.
- **실측 ③(해석기)**: `--env`가 태우는 것과 **같은 해석기**(`src/server/startup.ts:283`
  vs `:309`)를 `--env-path`로 태워 **실접속이 선다**는 것을 확인했다 —
  `connection=yes · tier=DEV · systemType=onprem` · 도구 74종 노출 · 도구 호출 1건
  성공(`GetAdtTypes` — 인자 없는 P1 읽기).
- **그래서 무엇이 남았나**: 「세션 파일이 그 자리에 있을 때 `--env=<name>`이 그것을
  찾아 붙는가」는 **미측정**이다. 자격증명 파일을 다른 자리에 복제하는 일이라
  **사용자가 하지 않기로 정했다**(2026-08-19). 우회하지 않았다.
- **판정**: D15의 `--env` 갈래는 **「미구현」이 아니라 「구현돼 있고 저장소 위치가
  구와 다르다」**로 읽어야 한다. 다만 그 위치 차이 자체가 구 대비 차이인지는
  이 판이 구 셰임을 실행해 대조하지 않았으므로 **미확정**으로 남긴다.

### D21 · D22 · `gateway` — RFC 3경로의 해소 자리를 **교체 판정 시점으로 모은다** (D-093 ⓑ)

**장부의 현행 서술을 먼저 정확히 옮긴다** — D21의 해소 자리는 「SAP NW RFC SDK가
설치된 머신에서의 실행 확인. **교체(swap) 판정 전까지**」이고, **D22는 「attended
세션(실 SAP 접속)」이지 교체 관문이 아니다.** 그리고 **`gateway`는 D21·D22 어느
항목의 소관도 아니다** — 이 장부에 gateway 전용 등재가 없다. 그러므로 D-093 ⓑ는
「장부가 이미 적어 둔 대로」가 아니라 **세 경로의 해소 자리를 한자리로 새로 모으는
것**이다.

- **`native`** (D21) — 유료 SAP NW RFC SDK 7.50+ + 컴파일된 네이티브 애드온이 있는
  머신이 있어야 밟힌다.
- **`zrfc`** (D22) — SAP 측에 핸들러 클래스 + SICF 노드 + 대리자 FM이 설치돼 있어야
  대답한다. **설치 자체가 P3/P4 구간**이다.
- **`gateway`** (**신규 등재**) — 별도 RFC 중계 미들웨어 호스트가 서 있어야 밟힌다.
  분류는 **축소**이고, 대체 기대 시험은 다른 두 경로와 같은 자리(통로 생성 계약 ·
  인자 조립 · 오류 종류의 구 대조)다. 지금까지 등재가 없던 것은 누락이며, 없는
  등재는 「차이가 없다」가 아니라 **「아무도 세지 않았다」**를 뜻했다.

**셋을 M3에 묶어 두면 M3가 가까운 어느 판에서도 닫히지 않는다** — 셋 다 장비·비용·
SAP 측 설치가 선행돼야 하고 그 셋 중 무엇도 지금 이 프로젝트가 조달할 수 있는 것이
아니다. 그래서 M3의 완료를 **전송 3종**으로 좁히고(D-093 ⓑ) 이 세 경로는 교체 판정
때 함께 재평가한다.

⚠ **이것이 교체를 막는 새 조건이 아니다.** D-093 ⓐ가 교체 관문을 BLUEPRINT
「검증 기준 1~4」로 확정했고 그 넷은 RFC 경로를 요구하지 않는다. 세 경로는
**교체 판정 자리에서 함께 판단할 이월 항목**이지 통과 조건이 아니다 — 이 구별을
흐리면 ①과 ②가 서로를 무력화한다.

### D91 · D112 — 20종 미노출을 **배포 축에서** 실측했다. 「JWT 접속에서」는 여전히 미확인.

바로 위 「D91·D112 전제 변화」 절이 「되짚을 자리는 **JWT 접속에서 그 20종이 정말
안 뜨는가**이며 그것은 판6 실접속에서 확인할 몫」이라고 적어 두었다. 이 판이 잰 것은
**그 갈래가 아니다.** 그렇게 적지 않으면 다음 사람이 닫힌 줄 안다.

- **잰 것**: `SAP_SYSTEM_TYPE=cloud`에서 D91의 16종 + D112의 4종 = **20종이
  `tools/list`에 0/20 노출**. 양성 대조로 `onprem`(186종)·`legacy`(96종)에서
  **20/20 복귀**. cloud↔onprem 차이 31종이 소스의 비-cloud 도구 31종과 정확히 일치.
- **왜 접속 없이 쟀는가 — 그것이 곧 한계의 증거다**: 노출 축은 `SAP_SYSTEM_TYPE`
  **하나만** 본다. `src/safety/exposition.ts`의 `ExposureQuery`는 `sets`와
  `systemType`뿐이고(`:72-75`), `selectExposedTools`(`:177-187`)·`isAvailableOn`
  (`:190-195`)도 그 축만 쓴다 — **파일 전체에 `authType` 출현 0회**. 호출부는
  `src/server/core.ts:233-236`, `systemType`의 출처는 `src/profile/resolve.ts:313-318`.
- **증명하는 것**: 배포 축이 `cloud`인 어떤 기동에서도 그 20종은 표면에 없다.
  구가 `isCloudConnection()`으로 거절하던 자리를 노출 선언이 막는다는 판정은 선다.
- **증명하지 않는 것**: **「JWT로 접속했을 때」**의 갈래. 노출이 `authType`을 안 보므로
  JWT 접속이더라도 `SAP_SYSTEM_TYPE`이 `onprem`이면 20종은 **뜬다**. 그 조합에서
  구가 거절하고 신이 거절하지 않는 차이가 실제로 나는지는 **BTP 실접속이 있어야
  잴 수 있고, 이 판은 재지 않았다.** 미확인으로 남는다.
- **기계 장부 반영**: 없음(위 두 항목과 같은 자리).

---

## 판7-b 회수 — D6의 방향을 가른다 (2026-08-19)

**등재 항목의 본문은 고치지 않는다**(이 장부의 규칙). 위 **D6**「blocklist 노브를
프로세스 env로도 받는다」의 신 동작이 **한 방향으로 좁혀졌다**는 사실을 여기 적는다.

- **바뀐 것**: D6 본문의 「두 통로를 모두 받고 **프로파일이 이긴다**」는 이제 부정확하다.
  정확한 현행 동작은 **「프로세스 env는 조일 수는 있어도 풀 수는 없다」**이고, 노브마다
  다르다:

  | 노브 | 방향 | 통로 |
  |---|---|---|
  | `MCP_ALLOW_TABLE` | 푼다 | **활성 프로파일 파일만** |
  | `MCP_BLOCKLIST_PROFILE` | 양방향 | 프로파일이 바닥을 정하고 프로세스 env는 **올릴 때만** 먹는다 |
  | `MCP_BLOCKLIST_EXTEND` | 조인다 | 두 통로의 **합집합** |

- **왜 좁혔는가 — 실측이 먼저다.** 「프로파일이 이긴다」는 **프로파일이 그 키를 실제로
  적었을 때만** 성립하는데, 제품 마법사(`interactive/scripts/setup-state.mjs`의
  `ENV_KEYS`)는 셋 중 `MCP_BLOCKLIST_PROFILE` **하나만** 쓴다. 그래서 마법사가 만든
  프로파일은 `MCP_ALLOW_TABLE`에 대해 **아무것도 막지 않았다.** 죽은 루프백 대상 실측:
  blocklist 키가 없는 프로파일 + 프로세스 env `MCP_ALLOW_TABLE=BNKA` → `BNKA`
  (`minimal`층 deny · 은행 계좌) **REACHED_SAP**. 같은 모양에서 `MCP_BLOCKLIST_PROFILE=off`도
  통했다. 노브가 하나도 없을 때는 정상 차단(BLOCKLIST_DENY).
- **무게**: D-043(소유자 머신 예외)이 실데이터 호출별 사람 승인을 **이 바닥선으로 대체**했다.
  즉 바닥선을 낮출 수 있는 주체가 곧 그 예외의 값어치다.
- **기계 장부 반영**: `harness/replay/divergences.ts` **없음** — 재생 대조에 나타나는
  차이가 아니다(재생은 ADT 왕복을 재고 이 규칙은 기동 시 env 합성이다).
- **대체 기대 시험**: 안전 계층 단위시험 8종(`src/safety/__tests__/blocklist.test.ts`
  `resolveSafetyEnv` describe) + 기동 계층 3종(`src/server/__tests__/startup.test.ts`
  「안전 노브」) + 제품 게이트 `conformance-server-gates.mjs`의 B2p·B2p2·B2p3·B2p4.
- **결정 기록**: D-096.

**보론 (2026-08-19 · 2차 리뷰 회수)** — 위 D6 본문이 인용하는 줄번호
`interactive/scripts/conformance-server-gates.mjs:53-59`·`:37-41`은 판7-b의 그 파일
재작성으로 **반 칸 밀렸다**(GAP-2 서술은 남아 있고 「개선을 회귀로 취급하지 않는다」
규칙 블록은 사라졌다 — 남은 격차가 0이 되어 기구 자체를 은퇴시켰기 때문이다).
본문은 규칙대로 고치지 않는다. 현행 자리는 같은 파일의 「옛 격차 둘은 둘 다 닫혔다」
머리주석이다.

**한계 (같은 회수에서 명시)** — 위 방향 규칙은 프로파일 **파일의 내용**에 대한 것이고
**어느 파일인가**에 대한 것이 아니다. `MCP_ENV_PATH`·`SAPKIT_HOME_DIR`은 여전히
프로세스 env가 정하므로, 그 둘을 적을 수 있는 주체는 자기가 쓴 `sap.env`를 고르게
...
## 상태 갱신 (append) — D15 8차: 이월 리뷰가 본 것 (2026-08-23 · D-121)

**위 D15 본문도, 1차~7차 갱신 표도 고치지 않는다.** 7차까지가 판M2-c 계열의 집행이고,
이 8차는 **그것을 뒤늦게 판정한 독립 리뷰(판R)의 회수분**이다. 판M2-c의 리뷰가 돌던 중에
D-118·D-119 커밋이 착지해 **대상이 두 번 움직였고**, 그 부채를 판R이 갚았다.

| 자리 | 7차까지의 기록 | 판R 판정 (D-121) |
|---|---|---|
| **`::1` 표기** | **이 장부 7차는 정확했다** — 「회수 초안에 넣었다가 뺐다 · IPv6는 대괄호를 요구한다」 | **틀린 것은 `DECISIONS.md`의 D-119 ⓐ였다** — 허용 집합에 `::1`을 넣었다고 적었는데 코드·시험·번들은 거부한다. **정정 append는 D-121 ⓐ**. 코드·번들 **무수정**(지금 상태가 옳다) |
| **돌연변이 수** | 「호스트 검사를 무력화하면 **4건 red**」 — **어떤 변이인지 안 적었다** | 변이 형태 없는 돌연변이 수치는 **재현 불가라 증거가 아니다.** 리뷰어 판독: **허용 판정을 통과 처리**(`callbackHost = rawHost`)하면 `startup.test.ts` **5건** red(비-loopback 거부 · `::1` 거부 · 대소문자 정규형 · 플래그 없는 침묵의 값 `toEqual` · 진단 개수 대조) · **진단만 지우고 폴백은 유지**하면 **3건**. ⚠ **둘 다 코드 판독이고 실행이 아니다**(엔진 게이트 동시 실행 금지 때문) — 7차의 「4건」도 같은 지위인지 불명이라 **어느 수도 실행으로 확정되지 않았다.** 회귀 포착 자체는 튼튼하다는 판정은 유지된다(시험이 진단 문구가 아니라 **실린 값**을 `toEqual`로 잰다) |
| **D-118 재현 수단** | 6차가 「`harness/probe-destination.mjs`와 채록본 `docs/reference/audits/2026-08-23-btp-abap-trial-grant-probe.md`」로 적었다 | **그 둘은 D-118의 실왕복을 재현하지 못한다.** 프로브가 재는 것은 「어느 그랜트의 토큰이 ADT를 여는가」(원시 HTTP)이고, 채록본은 스스로 **「엔진 `AdtClient`가 사용자 토큰으로 도구를 실제로 돌리는지 — 그 확인은 판M2-c 몫이다」**를 「확인하지 못한 것」에 적어 두었다. **정확한 재현 경로는 하나뿐이다** — 제품 번들을 `--mcp=<destination> --auth-interactive`로 다시 띄워 `GetSystemInfo`를 부르는 것. ⚠ **채록본은 없다** — D-118의 수치는 산문 4곳에만 있고 넷이 서로를 인용한다(**부채로 남는다** · D-121 정직 유보 ⓑ) |
| **마감 검증 집계** | `9f69479c` 커밋 메시지의 **「4,251 → 4,260 예상」**뿐 — 문서 어디에도 없다 | **실측했다**(2026-08-23 · `npm test`): **Test Suites 245**(244 통과 · 1 skipped) · **Tests 4,259**(4,258 통과 · 1 skipped) · 167.9s · **exit 0**. ⇒ **「4,260」은 예상이었고 실측은 4,259다.** 제품 게이트 7종도 판R이 재현해 전부 exit 0(`verify-engine` · `check-engine-provenance` · `smoke-mcp` 155/65 · `conformance-server-gates` 26 PASS · `gen-plugin-manifests --check` · `check-links` 깨짐 0 · `verify-checker`) |

- **판정**: **R-FAIL · 차단 1 · 권고 7.** 차단은 **코드가 아니라 결정문 한 줄**이었다. 축별로는
  실측 주장 일관성 PASS(**부풀림 없음**) · **회수 완전성 FAIL**(`::1` 표기) · 문서 계약 PASS
  (6차 `40 0` · 7차 `30 0` · D-119 `15 0` — 전부 추가 0삭제) · M2 승격 정직성 PASS.
- **권고 3건은 판Z로 이월했다**(D-121 ⓒ) — `AUTH_NO_REFRESH_MATERIAL`의 시험 공백 ·
  `DestinationConnectOptions.callbackHost` 무검증 이음매 · `--auth-interactive=true` 형태의
  침묵. 셋 다 **엔진 소스 변경**이라 재번들·재핀을 부르므로 **한 번으로 모은다**.
- **기계 장부 반영**: **안 했다** — 3차~7차와 같은 자리다.
- **결정 기록**: D-121.

## 도구 결함 등재 (append) — 재생 러너의 keyring 미배선 · 수집/배정 단위 불일치 (2026-08-23 · D-122)

**이것은 구·신 엔진의 차이가 아니라 우리 하네스 도구의 결함**이라 위 D-번호 체계에 넣지
않는다. `harness/replay/divergences.ts`에도 넣지 않는다 — **재생 대조에 나타나는 차이가
아니기 때문**이다. 판E1이 착수 실측에서 발견했다.

**⑴ `replay-attended.mjs`에 keyring `NODE_PATH`가 없다.**
짝이 되는 `record-attended.mjs`는 `--node-path`(기본
`interactive/server/runtime-deps/keyring/node_modules`)를 물리고, 그 머리주석이 이유까지
적어 두었다 — **「이 경로를 안 물리면 해석이 실패해 녹화가 인증 오류로 무너진다 — 원인이
드러나지 않는 자리라 기본으로 문다」**. **`replay-attended.mjs`에는 `node-path`·`NODE_PATH`
출현이 0회다.** ⇒ `SAP_PASSWORD`가 `keychain:<service>/<account>` 참조인 프로파일은
**재생이 도달해도 인증 오류로 무너진다.** 녹화와 재생은 같은 짝인데 한쪽에만 있는 배선이고,
재생이 실기로 돌아간 적이 판6.1 한 번뿐이라 지금까지 드러나지 않았다.
⚠ **고치지 않았다**(D-122 ⓒ) — 한 줄이면 되나 **고쳤는지 확인할 수단이 지금 없다**
(확인 = keychain 참조 프로파일로 실접속, 그것이 막혀 있다). **검증 없는 수리를 제품에
싣지 않는다.** ⚠ **이것이 실패의 원인이었는지도 재현하지 못했다** — 그 프로파일(KR-DEV)은
**네트워크에서 먼저 끊겼다.** 두 원인이 겹쳐 있고 순서를 못 가렸다.

**⑵ 재생의 수집 단위와 요구 급의 배정 단위가 다르다.**

| | 무엇을 보는가 | 결과 |
|---|---|---|
| **재생기** (`replay-attended.mjs:138`) | `fs.readdirSync(FIXTURE_DIR)` — **`fixtures/*.json` 최상위만**(재귀 없음) | 8건 · 16종을 태운다 |
| **요구 급 산식** (`build-plan.mjs` 사다리 ②) | 얼린 관측 `phase6-exercised.json` — **`fixtures/` 아래를 재귀로** 훑은 것 | 78종 · `attended-only/`에서만 태워진 도구까지 `replay` 급을 받는다 |

⇒ **「요구 급은 재생 대조인데 재생 대상이 될 수 없는」 도구 18종이 생겼다**(전부 얼린 관측
78종 안에 있음을 대조로 확인). 대장 머리말은 두 수의 **단위 차이**를 이미 적어 두었으나
(「서로 안 맞는 것이 정상이다」) **그 불일치가 요구 급 배정까지 흘러든다는 것은 적혀 있지
않았다.** ⚠ **산식을 고치지 않았다**(D-122 ⓑ) — 고치면 18종이 그 자리에서 `증거 있음`으로
넘어가는데, 그 모양이 **D-120 ⓐ가 경계한 「요구를 낮춰 증거를 만드는」 것과 구별되지
않는다.** 잣대를 정한 지 하루 만에 잣대를 움직이지 않는다.

**⑶ 새 재생 기준선을 만들 수단이 없다.** `attended-guard.mjs`가 `<engine>/fixtures/`
저장을 **무조건 거부**하고(자기 대조), 우회할 정당한 길인 **구 엔진 녹화**는 `engine/`이
판7.5(D-101)에서 레포를 떠나 사라졌다. **되뜨려면 `2264f89d`에서 트리 복원 + 빌드**이고
그것은 은퇴 결정을 뒤집는 일이다.

- **살아 있는 갈래**(D-122 대안 b — 기각했으나 죽이지 않았다): **`attended-only/` 픽스처를
  `--fixture`로 직접 재생한다.** 코드 변경이 0이고, 그 픽스처는 **엔진 1.0.0에 찍혔는데
  지금은 1.2.0**이라 자기 대조가 아니라 **버전 간 회귀 대조**다. 18종 전부가 픽스처 **6편**
  안에 있음을 확인했다(`cds-view-mde-test` 5 · `rap-bdef-bimp-service` 7 ·
  `class-locals-unittest` 4 · `domain-dataelement` 2 · `function` 1 · `interface` 1).
...
## 도구 결함 **수리** (append) — 재생 러너의 keyring 배선 · 수집/배정 단위 (2026-08-24)

위 「도구 결함 등재 (2026-08-23 · D-122)」의 ⑴⑵를 고쳤다. **등재를 지우지 않는다** — 이 장부는
append-only이고, 무엇이 왜 있었는지가 다음 사람의 판독 자료다. 아래는 그 두 건이 지금 어떤
모양인지의 후속 기록이다.

**⑴ keyring 배선 — 물렸다.** `harness/replay-attended.mjs`가 `--node-path`(기본
`interactive/server/runtime-deps/keyring/node_modules` — **녹화와 같은 경로**)를 물고,
`process.env.NODE_PATH`를 세운 뒤 **`Module._initPaths()`**로 해석 경로를 다시 읽는다.
그 두 번째 걸음이 녹화 쪽에는 없는 이유는 **녹화는 번들을 자식 프로세스로 띄우고**
(`childProcessTransport`가 env에 실어 보내면 그만이다) **재생은 신 엔진을 같은 프로세스에
세우기** 때문이다(`InProcessTarget`) — 이미 뜬 프로세스에서는 `NODE_PATH`를 넣는 것만으로
해석이 바뀌지 않는다.

⚠ **여전히 실 인증으로 확인되지 않았다.** 새 음성시험 `gates/test-replay-keyring.mjs`가 재는
것은 **⑴ 배선이 있고 ⑵ 그 두 걸음이 실제로 모듈 해석을 바꾼다**까지다(8건). 「keychain 참조
프로파일로 인증이 선다」는 SAP 접속이 필요하고 여전히 막혀 있다. **그 둘을 같은 것으로 읽지
말 것** — D-122 ⓒ가 수리를 미룬 이유가 정확히 이 구별이었다.

⚠ **이 배선이 언제 실제로 먹는지**도 적어 둔다. `@napi-rs/keyring`이 `sapkit-engine/node_modules`에
설치돼 있으면(optional 설치 성공) 일반 해석이 먼저 이기므로 `NODE_PATH`는 **아무 일도 하지 않는다.**
먹는 자리는 그 선택 의존성이 **없는 트리**(헤드리스·CI·`--omit=optional`)이고, 거기서는 이 배선이
없으면 `KEYCHAIN_UNAVAILABLE`로 접속이 서지 않는다. 즉 이 수리는 **없을 때를 위한 것**이다.

**⑵ 수집 단위 — 배정에 맞췄다(수집을 넓혔다).** `replay-attended.mjs`가 `fixtures/` 아래를
**재귀로** 모은다(`harness/replay/batch.ts`의 `collectFixtureFiles`). 이로써 요구 급 산식의
배정 단위(얼린 관측 = 재귀)와 같아졌고, 「요구 급은 재생 대조인데 재생 대상이 될 수 없는」
도구 **18종**이 사라졌다.

**배정을 낮추지 않았다** — D-122 ⓑ의 기각을 그대로 둔다. `harness/build-plan.mjs`의 산식도
`harness/phase6-exercised.json`의 목록도 손대지 않았고 `--check` 둘 다 그대로 통과한다.
움직인 것은 수집뿐이다.

**최상위만 모으던 옛 수집이 무엇을 지키고 있었는지**는 살렸다. 그것은 「지우지 않는 생성
시퀀스를 전량 재생에서 뺀다」를 **디렉터리로 대신한 것**이었고, 판6.3이 스스로 지우는
생애주기 시퀀스를 같은 디렉터리에 넣으면서 그 대리 지표가 무너졌다(그 판의 픽스처
`description`이 「재실행 가능하다」를 명시한다). 그래서 디렉터리 대신 **성질**을 본다 —
`batchRefusal()`이 둘만 뺀다:

| 빼는 이유 | 지금 걸리는 것 |
|---|---|
| P4(이송·패키지)를 태운다 — 되돌릴 수 없고 건별 사람 승인이 필요하다 | `zsapkit63-p4-transport` |
| 생성만 하고 지우지 않는다 — 두 번째 실행이 「이미 있다」로 실패하고, `coverage.ts`의 `merge()`가 그 실패를 끈끈하게 물어 **다른 시퀀스에서 통과한 도구의 증거까지** 앗아간다 | `zsapkit-m1-include-create` · `zsapkit-m1-program-create` |

뺀 것은 **이유와 함께 화면에 적히고**, `--fixture=<경로>`로 한 건을 명시하면 그 판정을
지나간다(실데이터 관문이 열어 둔 문과 같은 모양).

⚠ **둘째 판정은 「지우는 단계가 있는가」만 본다.** 무엇을 지우는지는 SAP만 안다 —
`zsapkit63-cds-view-mde-test`는 `CreateClass`의 숙주를 `DeleteCdsUnitTest`로 치우므로 이름만
맞대면 갈라 낼 수 없다. 그러므로 이 관문은 **명백히 되돌리지 않는 시퀀스를 거르는 것**이지
재실행 가능성을 보증하지 않는다. 보증은 실기가 한다.

⚠ **D-122 대안 (b)의 정직 유보는 그대로 살아 있다.** 「엔진 1.0.0에 찍힌 픽스처를 지금 엔진으로
재생하는 것이 **버전 간 회귀 대조**로 성립할 것」은 **설계 판단이지 실측이 아니다** — 정규화가
시스템 차이를 어디까지 흡수하는지 재 본 적이 없다. **이 수리는 그 시험을 가능하게 했을 뿐
치르지 않았다.**

**시험**: `harness/replay/__tests__/batch.test.ts`(23건 — 수집 재귀 · 거부 판정 · **레포의 실제
...
### D147 — `CreateServiceBinding`이 계약(category)을 고른다 (수리 · 백로그 13-5)
...
**대체 기대 시험**: `src/tools/write/__tests__/createServiceBinding.test.ts` — 「D147 —
계약(category)을 고를 수 있고, 응답이 무엇을 만들었는지 말한다」 2건 + 「발행 계약」의
덧인자 1건.

**기계 장부 반영**: 했다. 구의 성공 응답과 갈리는 것은 두 키뿐이므로, 그 두 키만 늘었는지를
재는 검사를 D147로 두었다. ⚠ **D105와의 겹침을 여기서 푼다** — D105는 `activated:true`를
주장하는 구 성공 갈래를 통째로 들고 신이 활성화 실패를 되돌리는지만 본다. 신이 **성공**하면
이제 두 키가 늘어 D105의 검사가 「등재된 차이가 아니다」로 떨어진다. 그래서 D105의 검사를
「신이 성공이면 D147의 두-키 검사로 넘긴다」로 **확장**했다(사람용 D105 본문은 그대로다 —
이 문단이 그 확장의 기록이다). D147 자신의 `applies`는 `activated:false` 성공 갈래만 든다.
...
**대체 기대 시험**: `src/tools/write/__tests__/updateServiceBinding.test.ts` — 「D148 —
srvb:allowedAction이 없으면 거부하지 않고 요청을 보낸다」 4건(발행 · 발행취소 · 서버
거절이 그대로 올라옴 · 있고 어긋나면 여전히 거부) ·
`src/tools/write/__tests__/deleteServiceBinding.test.ts` — 「D148 — 발행 중인데 allowedAction
속성이 없으면 발행취소를 시도한다」.

**기계 장부 반영**: 했다(`UpdateServiceBinding`만 — `DeleteServiceBinding` 쪽은 응답이
같고 **와이어에만** 남는 차이라 D104·D124와 같은 가름선으로 옮기지 않는다). 구가
`allowedAction=UNKNOWN`으로 거부한 오류 갈래와, 두 키가 늘어난 성공 갈래를 D148 하나가
든다.

**실기 미검증.**

### D149 — `CheckSyntax`가 판정불능을 실패로 말하지 않는다 · `main_program` (수리)
...
**대체 기대 시험**: `src/tools/read/__tests__/checkSyntax.test.ts` — 「D149 — include의
...
자리는 신이 `success:null`인지를 D149이 잰다.

**실기 미검증** — include 단독 검사에서 SAP이 실제로 어떤 `status`를 내는지는 채록되지
않았다(`notProcessed`로 짐작했다).

### D150 — `UpdateProgram`이 거짓 FIXPT precheck를 통째로 믿지 않는다 (수리)
...
**대체 기대 시험**: `src/tools/write/__tests__/updateProgram.test.ts` — 「D150 — 인라인
...
**대체 기대 시험**: `src/tools/read/__tests__/grepObjects.test.ts` — 「D151 — FUGR는
...
**대체 기대 시험**: `src/tools/read/__tests__/getTypeInfo.test.ts` — 「D152」 2건(422가
...
### D154 — `GetInclude`가 클래스 인클루드(CCIMP 등)를 요청 전에 거절하고 읽는 도구를 이름으로 말한다 (수리 · 백로그 13-8 ⓑ)
...
**대체 기대 시험**: `src/tools/read/__tests__/getInclude.test.ts` — 「D154 — 클래스
...
- **FUGR 노드 구조의 모양은 짐작이다**(D151). **`list_changed`를 클라이언트가 먹는지도
  짐작이다**(D153). 둘 다 첫 실접속 세션의 확인 대상이다.
- **D150는 안전 바닥선을 낮춘 것이 아니다** — 쓰기 전 검사가 빠지는 갈래가 생겼지만, 그
