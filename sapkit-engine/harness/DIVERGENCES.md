...
### D21 — HTTP 전송의 DNS 리바인딩 보호가 **기본으로 켜진다** (구: 노브가 배선되지 않아 항상 꺼짐)
- **분류**: 수리 + 강화
...
### D22 — 비루프백 바인딩은 **명시 옵트인**을 요구한다 (구: 조건 없이 허용)
- **분류**: 강화
...
### D23 — `--http-json-response` 노브가 **실제로 끌 수 있다** (구: 실효값 참에 갇힘)
- **분류**: 수리
...
### D25 — HTTP 요청 헤더로 SAP 접속을 고르는 통로가 없다
