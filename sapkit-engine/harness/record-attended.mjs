 ...
 * 일어난다. 배치·서브에이전트 무인 실행 금지 — spec §2.6.
 *
 ...
 *  ⓑ **SAP을 바꾸는** 도구 — P3는 전용 DEV 연습 패키지 안에서만이다(spec §2.6).
 *     여기 있으면 **실 SAP 호출 전에** 판정된다. 사후에 걸러 봐야 write는 이미
 *     일어난 뒤다(4차 리뷰 지적).
 *
...
if (standardSource.length) {
  die(
    '대상이 고객 객체(Z·Y)가 아니다 — 소스 채록은 커밋 대상이고 write는 연습 패키지 밖으로 나가면 안 된다.',
    ...standardSource,
    '전용 연습 패키지의 Z 객체를 가리켜라. 표준 객체 소스는 이 레포가 재배포할 권리가 없다.',
