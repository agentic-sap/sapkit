...
곧 거짓 성공이다. 미구현 판정은 통로별 설정 검사보다 **먼저** 한다 —
`gateway`를 골랐는데 "`SAP_RFC_GATEWAY_URL`이 없다"고 답하면 채우면 될 것처럼
보여 원인을 바꿔치기하게 된다.
...
| # | 지점 | 구 | 신 | 왜 |
|---|---|---|---|---|
| ① | `zrfc` 값 | 알 수 없는 값으로 **거부** (`rfcBackend.ts:43`) | 유효한 이름으로 인정 · M1에서는 `backend-unsupported` | 결정 D-079 ②가 5경로 전량 승계를 정했고, 문서 3곳이 이미 유효하다고 말한다(§3). 둘 다 오류라 동작 변화는 문구뿐이다 |
