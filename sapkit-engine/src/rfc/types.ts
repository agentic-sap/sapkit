 ...
 * 한 가지 일의 구현이다. 대리자는 기설치된 `ZSAPKIT_ADT_DISPATCH` /
 * `ZSAPKIT_ADT_TEXTPOOL` 두 함수모듈이고(결정 D-079 ⑥ — SAP 측 무접촉), 통로는
...
/** `ZSAPKIT_ADT_TEXTPOOL`이 받는 세 동작. */
...
/** `ZSAPKIT_ADT_DDIC_TABL_READ`가 받는 두 인자. */
...
export interface RfcChannel {
  ...
  /** `ZSAPKIT_ADT_DISPATCH` 호출. `action`은 `CUA_FETCH`·`DYNPRO_READ` 같은 동작 이름. */
 ...
 * 설계로 갈린다: 브리지 함수모듈 `ZSAPKIT_ADT_DDIC_TABL_READ`는 OData 서비스
 * `ZSAPKIT_ADT_SRV`의 FunctionImport로만 노출돼 있어, 나머지 네 통로는 그것을
...
export interface DdicReadChannel {
  /**
   * `ZSAPKIT_ADT_DDIC_TABL_READ` 호출 — 투명 테이블과 구조를 **같은 FM 하나**가
   * TABCLASS로 갈라 처리한다.
   */
