...
describe('오류 갈래', () => {
  it('5xx는 다음 후보로 넘어가지 않고 즉시 실패한다 (구 그대로)', async () => {
    // 구의 tryLookup은 404만 「없음」으로 접고 나머지는 던진다(:161-166). D152이
  ...
  it('D152 — 구조체 이름이 첫 후보에서 HTTP 422를 받아도 구조 폴백까지 간다 (구는 422에서 죽었다)', async () => {
  ...
  it('D152 — 401·403은 여전히 즉시 실패다 (인증은 후보를 바꿔도 같다)', async () => {
