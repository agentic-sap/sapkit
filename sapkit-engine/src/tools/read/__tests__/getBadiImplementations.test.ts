 ...
 * `ZMCP_ADT_DDIC_BADI`)이고 신 엔진의 OData 통로에 그 FunctionImport가 없다
...
describe('발행 계약', () => {
  it('tools/list 선언이 구 번들 채록본과 같다', async () => {
    ...
    try {
      ...
      }).toEqual(publishedDeclaration('GetBadiImplementations'));
    } finally {
      await harness.close();
    }
  });
...
describe('ECC 갈래 — 브리지가 이 판에 없다 (차이 장부 D132)', () => {
  it.each(['ECC', 'ecc', 'Ecc'])(
    ...
    async (sapVersion) => {
      ...
      expect(textOf(result)).toContain('ZMCP_ADT_DDIC_BADI');
