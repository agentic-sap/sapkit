/**
 * `ReadMetadataExtension` — 메타데이터 확장(DDLX)의 소스와 오브젝트 메타데이터.
 *
 * 선언은 구 번들의 발행 계약 그대로다(`harness/old-surface/m1-tools.json`의
 * `ReadMetadataExtension` · 구 소스
 * `engine/src/handlers/metadata_extension/readonly/handleReadMetadataExtension.ts:9-30`).
 * 몸통의 대조 원본은 같은 파일 `:32-100`.
 *
 * 짝인 `GetMetadataExtension`과 **같은 오브젝트를 읽지만 같은 도구가 아니다.**
 ...
 * `./internal/metadataExtensionRead`의 머리주석에 파일·줄로 있다.
 *
 * ## 접속 획득은 두 try 밖이다
 *
 * 구도 그렇다(`handleReadMetadataExtension.ts:45`). 안쪽에 넣으면 **접속이 아예
 * 없는 기동에서도 `success:true`가 나간다**.
 */
...
export const readMetadataExtension = defineTool(
  {
    ...
    inputSchema: {
      metadata_extension_name: z
        .string()
        .describe('Metadata extension name (e.g., Z_MY_DDLX).'),
      version: z
    ...
    // 구 경로는 `handlers/metadata_extension/readonly/`이고, 채록본 `exposures`의
    // 네 조건 전부에 뜬다.
    sets: ['readonly'],
    kind: 'read',
  ...
  async (context: ToolContext, args) => {
    try {
      ...
      try {
        const response = await readMetadataExtensionMetadata(client, name);
