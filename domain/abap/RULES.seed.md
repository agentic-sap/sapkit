...
원천 2곳(DESIGN §10): ① **vsp embedded standards** — `vsp.exe`를 `--offline` 플래그로
...
## Offline lint Error 4종은 별도 문서에서 관리

`vsp lint`의 Error-severity 규칙 키(`line_length` 255자 초과 시·`empty_statement`·
`max_one_statement`·`preferred_compare_operator`·`hardcoded_credentials`·
`commit_in_loop`)는 오프라인 게이트의 실효 차단력 그 자체이며 규칙 후보라기보다
**offline 초안 셀프체크 항목**이라 `CHECKLIST.md`에 실측 목록으로 옮겼다 — 이 파일에는
중복 등재하지 않는다.
