...
3. 과거 결정의 '왜'는 `docs/reference/DECISIONS.md` (append-only 결정 로그, D-001~). **D-번호를
   인용·재해석하기 전 원문을 확인한다.**
   **주제별 합성 지도는 `docs/reference/wiki/INDEX.md`**(콜드 층 색인 — 결정 로그·보관소·
   감사 기록을 주제로 묶은 **재생성 가능한 파생물**이다. 정본은 여전히 로그이고 위키는 입구다).
...
| `docs/reference/wiki/` | **판이 끝날 때, 그리고 판 밖 세션이 D-항목·콜드 문서를 추가할 때** 새 결정·완료 판을 페이지·색인에 부기 — 인용·고아는 게이트가 잰다 (결정 정본 D-137) |
...
**공방 문서 게이트도 같은 층에 있다**(제품에 동봉되지 않으므로 위 제품 게이트 목록에
넣지 않는다): `node scripts/check-wiki.mjs`가 **콜드 층(`docs/reference/`)의 위키 구역**을
잰다 — 링크 무결성 · **콜드 전수 고아 검출**(아카이브 디렉터리 제외) · D-번호 인용 실재 ·
파생물 배너 · **부재 = 위반** · 발견 배선 앵커. 그 음성시험은
`node scripts/test-check-wiki.mjs`이고, 둘 다 CI(`node-gates` 잡)에서 돈다. 위키를 세운
결정의 정본은 **D-137**이고, 무엇을 왜 재는지의 정본은 그 스크립트 머리 주석이다.
