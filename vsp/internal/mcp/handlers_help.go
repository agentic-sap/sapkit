...
Install tools:
  ...
  SAP(action="system", params={"type": "install_alv_handlers"})
...
File operations:
  ...
  SAP(action="system", params={"type": "rename", "objType": "CLAS/OC", "oldName": "ZCL_OLD", "newName": "ZCL_NEW", "packageName": "$TMP"})`)
	...
	case "standards":
		return mcp.NewToolResultText(`SAP(action="standards") - Embedded ABAP development standards

List all documents (rules / templates / quality):
  SAP(action="standards")

Fetch one document by path:
  SAP(action="standards", target="rules/naming-conventions.md")
  SAP(action="standards", target="templates/procedural/main-program.abap")
  SAP(action="standards", params={"path": "quality/phase6-review.md"})

Categories:
  rules/     - Coding rules: naming conventions, Clean ABAP (core/OOP/procedural),
               field typing (data-element first), include structure, OK-code pattern,
               ALV rules, constants/text elements/function modules, release-gated
               syntax (abap-release-reference + examples), ECC/Cloud constraints,
               data extraction policy
  templates/ - Program skeletons to mirror: OOP two-class sample (templates/oop/),
               procedural main program (templates/procedural/), ALV field catalog
               guide (templates/alv/), ECC DDIC creation helpers (templates/ecc/)
  quality/   - Review checklists: 12-section code review (phase6-review.md),
               14-dimension analysis rubric (analysis-dimensions.md)

Workflow: consult the relevant rules and template BEFORE generating ABAP code,
mirror the template structure, then verify against quality/phase6-review.md.

The same documents are exposed as MCP resources under vsp://standards/*.`)

	case "tips", "best_practices", "workflows", "best":
		return mcp.NewToolResultText(`SAP Best Practices & Workflows

=== DEVELOPMENT STANDARDS (consult before writing ABAP) ===
1. List embedded standards:      SAP(action="standards")
2. Fetch a rule or template:     SAP(action="standards", target="rules/naming-conventions.md")
   → Coding rules (naming, Clean ABAP, field typing), program skeletons (OOP/procedural/ALV),
     and review checklists (quality/phase6-review.md). Mirror the templates; follow the rules.
...
Actions:
  ...
  standards - Embedded ABAP development standards (coding rules, templates, review checklists)
...
func getUnhandledErrorMessage(action, objectType, objectName string) string {
	...
	default:
		sb.WriteString("Valid actions: read, edit, create, delete, search, query, grep, test, analyze, debug, system, standards, help\n")
