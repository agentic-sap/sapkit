...
func (s *Server) routeDevToolsAction(ctx context.Context, action, objectType, objectName string, params map[string]any) (*mcp.CallToolResult, bool, error) {
	...
	if action == "edit" {
		...
		case "ACTIVATE_GROUP":
			return s.callHandler(ctx, s.handleActivateGroup, params)
		}
	}
...
func (s *Server) handleSyntaxCheck(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// content is optional: empty checks the server's persisted source
	// (group-wide when object_url is a function group).
...
func (s *Server) handleActivateGroup(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	group, ok := request.GetArguments()["function_group"].(string)
	if !ok || group == "" {
		return newToolResultError("function_group is required"), nil
	}

	refs, err := s.adtClient.GroupActivationRefs(ctx, group)
	if err != nil {
		return newToolResultError(fmt.Sprintf("Failed to enumerate function group objects: %v", err)), nil
	}
	...
	output, _ := json.MarshalIndent(struct {
		ActivatedRefs []adt.ActivationRef  `json:"activatedRefs"`
		Result        *adt.ActivationResult `json:"result"`
	}{refs, result}, "", "  ")
	return mcp.NewToolResultText(string(output)), nil
}
