...
func (s *Server) registerTools(mode string, disabledGroups string, toolsConfig map[string]bool) {
	// Offline mode: only local-only tools, no SAP connection needed. Takes
	// priority over mode/hyperfocused — offline never registers SAP tools.
	if s.config != nil && s.config.Offline {
		s.registerOfflineTools()
		return
	}
	...
	s.registerStandardsTools(shouldRegister)
...
// registerOfflineTools registers only the local-only tools that need no SAP
// connection: ListStandards, GetStandard, CheckBoundaries (source mode), and
// AnalyzeABAPCode (source mode). Used when the server is started with
// --offline, for air-gapped environments doing abapGit-based local ABAP
// development. Descriptions are kept identical to their online-mode blocks
// (registerStandardsTools / registerAnalysisTools / registerTestingQualityTools).
func (s *Server) registerOfflineTools() {
	s.mcpServer.AddTool(mcp.NewTool("ListStandards",
		mcp.WithDescription("List embedded ABAP development standards: coding rules (naming conventions, Clean ABAP, field typing, include structure, OK-code pattern), program skeleton templates (OOP/procedural/ALV), and quality checklists (code review, QA gate). Consult these BEFORE creating or modifying ABAP objects, then fetch the relevant documents with GetStandard."),
	), s.handleListStandards)

	s.mcpServer.AddTool(mcp.NewTool("GetStandard",
		mcp.WithDescription("Get the full content of an embedded ABAP development standard by path (e.g. 'rules/naming-conventions.md', 'templates/procedural/main-program.abap'). Follow these standards when generating, reviewing, or refactoring ABAP code. Use ListStandards to discover available documents."),
		mcp.WithString("path",
			mcp.Required(),
			mcp.Description("Document path from ListStandards (category/file). Bare file names are accepted when unambiguous."),
		),
	), s.handleGetStandard)
...
func (s *Server) registerReadTools(shouldRegister func(string) bool) {
	...
	if shouldRegister("GetTableContents") {
		s.mcpServer.AddTool(mcp.NewTool("GetTableContents",
			mcp.WithDescription("Retrieve contents of an ABAP table. For simple queries use table_name + max_rows. For filtered queries use sql_query parameter with ABAP SQL syntax (use ASCENDING/DESCENDING, not ASC/DESC). Results are capped by max_rows: check Truncated/TotalRows in the response - if Truncated=true you do NOT have the full result set. Complex SQL (many-way joins, long IN lists) can fail with HTTP 400: shorten aliases, replace long IN lists with BETWEEN ranges; sporadic 400s under concurrent load usually succeed on retry."),
			mcp.WithString("table_name",
				mcp.Required(),
				mcp.Description("Name of the ABAP table"),
			),
	...
	if shouldRegister("RunQuery") {
		s.mcpServer.AddTool(mcp.NewTool("RunQuery",
			mcp.WithDescription("Execute a freestyle SQL query against the SAP database. IMPORTANT: Uses ABAP SQL syntax, NOT standard SQL. Use ASCENDING/DESCENDING instead of ASC/DESC. Use max_rows parameter instead of LIMIT. GROUP BY and WHERE work normally. Results are capped by max_rows: check Truncated/TotalRows in the response before treating the result as complete. Complex SQL (4+ table joins, long IN lists) can fail with HTTP 400: shorten table aliases, replace long IN lists with BETWEEN ranges; sporadic 400s under concurrent load usually succeed on retry."),
			mcp.WithString("sql_query",
				mcp.Required(),
...
func (s *Server) registerDevTools(shouldRegister func(string) bool) {
	if shouldRegister("SyntaxCheck") {
		s.mcpServer.AddTool(mcp.NewTool("SyntaxCheck",
			mcp.WithDescription("Check ABAP source code for syntax errors. Two modes: (1) with content - checks the provided source in the object's context before saving; (2) WITHOUT content - checks the source already persisted on the server. Mode 2 with a function group URL (/sap/bc/adt/functions/groups/ZFG) returns errors across the ENTIRE group (all FMs + includes) with line info in one read-only call - the fastest way to diagnose a broken FUGR. Note: function module checks always compile the whole group, so errors may come from sibling objects (check each result's uri field)."),
			mcp.WithString("object_url",
				mcp.Required(),
				mcp.Description("ADT URL of the object (e.g., /sap/bc/adt/programs/programs/ZTEST, /sap/bc/adt/functions/groups/ZFG)"),
			),
			mcp.WithString("content",
				mcp.Description("ABAP source code to check. Omit to check the source persisted on the server (e.g., group-wide FUGR diagnosis)."),
			),
		), s.handleSyntaxCheck)
	}
	...
	if shouldRegister("Activate") {
		s.mcpServer.AddTool(mcp.NewTool("Activate",
			mcp.WithDescription("Activate an ABAP object. READ THE RESPONSE: success=false with messages means activation FAILED even though the call returned normally. Function group caveat: activating a single FM can fail with 'FUNCTION ... FUNCTION-POOL' errors when the group's main program/TOP include/sibling FMs are inconsistent - use ActivateGroup to activate the whole group in one run. A successful source read is NOT proof an object is active: never-activated objects also return source; trust only a clean SyntaxCheck plus success=true here."),
			mcp.WithString("object_url",
				mcp.Required(),
	...
	if shouldRegister("ActivateGroup") {
		s.mcpServer.AddTool(mcp.NewTool("ActivateGroup",
			mcp.WithDescription("Activate a function group as ONE activation run: main program SAPL<group> + all includes + all function modules + the group itself, in a single POST. Use this when activating an FM alone fails with 'FUNCTION ... FUNCTION-POOL' errors - FMs cannot activate while the rest of the group is inconsistent. Response lists the bundled references and the activation result; success=false means the run failed - read the messages."),
			mcp.WithString("function_group",
				mcp.Required(),
				mcp.Description("Function group name (e.g., ZFG)"),
			),
		), s.handleActivateGroup)
	}

	if shouldRegister("ActivatePackage") {
		s.mcpServer.AddTool(mcp.NewTool("ActivatePackage",
			mcp.WithDescription("Activate all inactive objects. Objects are sorted by dependency order (interfaces before classes) but activated ONE BY ONE in separate activation runs, not as a single batch. For function groups this matters: an FM can fail with FUNCTION-POOL errors until the group's main program/includes are active - prefer ActivateGroup for a whole function group, or rerun if the first pass leaves failures. If no package specified, activates ALL inactive objects for current user."),
			mcp.WithString("package",
...
func (s *Server) registerEditTools(shouldRegister func(string) bool) {
	if shouldRegister("EditSource") {
		s.mcpServer.AddTool(mcp.NewTool("EditSource",
			mcp.WithDescription("Surgical string replacement on ABAP source code. Matches the Edit tool pattern for local files. Workflow: GetSource → FindReplace → SyntaxCheck → Lock → Update → Unlock → Activate. Example: EditSource(object_url=\"/sap/bc/adt/programs/programs/ZTEST\", old_string=\"METHOD foo.\\n  ENDMETHOD.\", new_string=\"METHOD foo.\\n  rv_result = 42.\\n  ENDMETHOD.\", replace_all=false, syntax_check=true). Requires unique match if replace_all=false. Use this for incremental edits between syntax checks - no need to download/upload full source! Function module caveat: the pre-save syntax check compiles the WHOLE function group, so pre-existing errors in sibling FMs can block your edit (such errors are annotated with their object URI in the output; your change is NOT saved in that case). success=true requires clean activation; if activation fails, the response says explicitly that the source was persisted (inactive)."),
			mcp.WithString("object_url",
				mcp.Required(),
...
func (s *Server) registerInstallTools(shouldRegister func(string) bool) {
	...
	if shouldRegister("InstallALVHandlers") {
		s.mcpServer.AddTool(mcp.NewTool("InstallALVHandlers",
			mcp.WithDescription("Deploy reusable ALV/Tree OOP handler classes (ZCL_S4SAP_CM_* — 1 interface + 1 exception + 5 classes) to SAP. These are the reuse classes referenced by the embedded standards templates (see GetStandard rules/oop-pattern.md). Target must be a local package starting with $."),
			mcp.WithString("package",
				mcp.Description("Target package name (default: $ZS4SAP_CM). Must be local package starting with $."),
			),
			mcp.WithBoolean("check_only",
				mcp.Description("Only check prerequisites without deploying (default: false)"),
			),
		), s.handleInstallALVHandlers)
	}
...
// registerStandardsTools registers embedded ABAP development standards tools.
func (s *Server) registerStandardsTools(shouldRegister func(string) bool) {
	if shouldRegister("ListStandards") {
		s.mcpServer.AddTool(mcp.NewTool("ListStandards",
			mcp.WithDescription("List embedded ABAP development standards: coding rules (naming conventions, Clean ABAP, field typing, include structure, OK-code pattern), program skeleton templates (OOP/procedural/ALV), and quality checklists (code review, QA gate). Consult these BEFORE creating or modifying ABAP objects, then fetch the relevant documents with GetStandard."),
		), s.handleListStandards)
	}

	if shouldRegister("GetStandard") {
		s.mcpServer.AddTool(mcp.NewTool("GetStandard",
			mcp.WithDescription("Get the full content of an embedded ABAP development standard by path (e.g. 'rules/naming-conventions.md', 'templates/procedural/main-program.abap'). Follow these standards when generating, reviewing, or refactoring ABAP code. Use ListStandards to discover available documents."),
			mcp.WithString("path",
				mcp.Required(),
