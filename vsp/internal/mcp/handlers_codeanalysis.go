...
func (s *Server) handleAnalyzeABAPCode(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Offline / no SAP connection: adt.Client.AnalyzeABAPCode dereferences the
	// client's config for the safety check, so it cannot be called on a nil
	// client. Without a connection, source is required; run the pure local
	// analyzer directly instead of going through the client.
	if s.adtClient == nil {
		if source == "" {
			return newToolResultError("no SAP connection: 'source' is required to analyze ABAP code offline"), nil
		}
		result := adt.AnalyzeABAPSource(source)
