...
func (s *Server) registerGetSource() {
	s.mcpServer.AddTool(mcp.NewTool("GetSource",
		mcp.WithDescription("Unified tool for reading ABAP source code across different object types. Replaces GetProgram, GetClass, GetInterface, GetFunction, GetInclude, GetFunctionGroup, GetClassInclude. Caveats: a successful read is NOT proof the object is active/compilable - never-activated objects also return source (verify with SyntaxCheck + Activate). If a previous session may have left unactivated edits, check GetInactiveObjects before editing on top, and re-read after writes to confirm the edit survived."),
