...
actions: read, edit, create, delete, search, query, grep, test, analyze, debug, system, standards, help
some actions (analyze, test, debug, system, standards, help) use params only — no target needed.
...
SAP(action="standards") — list embedded ABAP dev standards (naming, Clean ABAP, templates, review checklists); consult BEFORE writing ABAP
SAP(action="standards", target="rules/naming-conventions.md") — fetch one standard
SAP(action="help") — full docs; SAP(action="help", target="tips") — best practices`),
		mcp.WithString("action",
			mcp.Required(),
			mcp.Description("Action to perform: read, edit, create, delete, search, query, grep, test, analyze, debug, system, standards, help"),
		),
...
func (s *Server) handleUniversalTool(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Standards action: embedded ABAP development standards (rules/templates/quality)
	if action == "standards" {
		path := target
		if path == "" {
		...
		if path == "" {
			return s.handleListStandards(ctx, request)
		}
		return s.handleGetStandard(ctx, newRequest(map[string]any{"path": path}))
	}
