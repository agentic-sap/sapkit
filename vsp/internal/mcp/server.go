...
type Config struct {
	...
	// Offline disables all SAP connectivity and registers only local-only
	// tools (ListStandards, GetStandard, CheckBoundaries, AnalyzeABAPCode).
	// For air-gapped environments doing abapGit-based local ABAP development.
	Offline bool
}
...
func NewServer(cfg *Config) *Server {
	if cfg.Offline {
		// Offline mode: no ADT client, no feature prober, no keep-alive.
		// Only local-only tools are registered (see registerOfflineTools).
	...
	// Expose embedded ABAP development standards as MCP resources
	s.registerStandardsResources()
