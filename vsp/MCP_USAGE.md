...
```mermaid
flowchart TD
    subgraph Focused["Focused Mode (representative subset)"]
        U[Unified Tools]
        U --> GS[GetSource]
        U --> WS[WriteSource]
    ...
    subgraph Expert["Expert Mode (all tools)"]
        direction TB
        F[All Focused Tools]
        A[+ Atomic Operations]
        A --> LO[LockObject]
...
`vsp --offline` starts the MCP server with no SAP connection at all — it takes priority over `--mode` and skips authentication entirely. Only 4 local-only tools are registered: `ListStandards`, `GetStandard`, `CheckBoundaries` (with the `source` parameter), and `AnalyzeABAPCode` (with the `source` parameter), plus the embedded standards documents as MCP resources. Use it in security-isolated (air-gapped) environments to develop against a local ABAP project checked out via abapGit, using the embedded coding standards and the local parser/lint/boundary analysis without ever reaching a SAP system.
...
## ABAP Development Standards (Embedded)

The server ships with embedded ABAP development standards — coding rules,
program skeleton templates, and quality checklists. **Consult them before
generating or modifying ABAP code** instead of relying on generic ABAP
knowledge.

| Tool | Purpose |
|------|---------|
| `ListStandards` | Catalog of all embedded documents (rules / templates / quality) |
| `GetStandard` | Fetch one document by path, e.g. `rules/naming-conventions.md` |

The same documents are also exposed as MCP resources under `vsp://standards/*`.

**Recommended flow when creating a new ABAP program:**

```
Step 1: ListStandards
        → Discover available rules, templates, checklists

Step 2: GetStandard(path="rules/naming-conventions.md")
        GetStandard(path="rules/include-structure.md")
        GetStandard(path="templates/procedural/main-program.abap")  ← or templates/oop/*
        → Load the rules and the skeleton that apply to the task

Step 3: Generate code mirroring the template structure and rules
        (naming, includes, field typing, OK-code pattern, text elements)

Step 4: After activation, verify against the checklist:
        GetStandard(path="quality/phase6-review.md")
```

Key rule documents: `naming-conventions.md` (object naming), `clean-code.md`
(+ `-oop` / `-procedural` variants), `field-typing-rule.md` (data element
first typing), `include-structure.md`, `ok-code-pattern.md`, `alv-rules.md`,
`abap-release-reference.md` (release-gated syntax).
