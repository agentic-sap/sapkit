...
func runCopy(cmd *cobra.Command, args []string) error {
	...
	// Create ADT client
	client, err := getWriteClient(params)
	if err != nil {
		return err
	}
	...
	if wsAvailable {
		fmt.Println("Mode: ADT native (WebSocket planned — ZADT_VSP detected but not yet used for deployment)")
	} else {
...
// checkWriteSourceResult normalizes a WriteSource outcome into a single
// error. WriteSource signals a rejected write (e.g. syntax error) via a
// nil error + result.Success == false, not via err — callers that check
// only err silently count that failure as a success.
func checkWriteSourceResult(result *adt.WriteSourceResult, err error) error {
	if err != nil {
		return err
	}
	if !result.Success {
...
func deployClass(ctx context.Context, client *adt.Client, obj deps.DeploymentObject, packageName string) error {
	...
	// Deploy main class
	result, err := client.WriteSource(ctx, "CLAS", obj.Name, obj.MainSource, &adt.WriteSourceOptions{
		Package:     packageName,
		Description: desc,
	})
	if err := checkWriteSourceResult(result, err); err != nil {
		return err
	}
...
func deployInterface(ctx context.Context, client *adt.Client, obj deps.DeploymentObject, packageName string) error {
	...
	return checkWriteSourceResult(result, err)
}
