...
func getClient(params *systemParams) (*adt.Client, error) {
	...
	// CLI safety opt-ins via documented env vars (getClient otherwise ignores them).
	if os.Getenv("SAP_ALLOW_TRANSPORTABLE_EDITS") == "true" {
		opts = append(opts, adt.WithAllowTransportableEdits())
	}
	if os.Getenv("SAP_ENABLE_TRANSPORTS") == "true" {
		opts = append(opts, adt.WithEnableTransports())
	}
...
// enforceWriteProfile rejects write operations client-side — before any
// network call — when the active safety profile forbids them. Only
// write-capable subcommands consult it (via getWriteClient); read-only
// commands call getClient directly and are never gated here, so reads are
// unaffected by either variable.
//
// Two environment variables are honored:
//
//   - SAP_READ_ONLY: interpreted with the same truthy rule the server's
//     resolveConfig uses (viper.GetBool). When true, every write is blocked.
//   - SAP_TIER: the CLI has no per-tier semantics beyond a single gate —
//     "dev" (case-insensitive) is the only tier that permits writes; any
//     other non-empty tier (qa, prd, ...) blocks them. An unset SAP_TIER
//     imposes no tier constraint.
//
// The messages follow the existing transportable-edit gate style: they
// name the offending variable and how to lift the block.
func enforceWriteProfile() error {
	if viper.GetBool("READ_ONLY") {
		return fmt.Errorf("blocked: SAP_READ_ONLY=true — write operations are disabled for this profile (unset SAP_READ_ONLY or set it to false to enable writes)")
	}
	if tier := strings.TrimSpace(os.Getenv("SAP_TIER")); tier != "" && !strings.EqualFold(tier, "dev") {
		return fmt.Errorf("blocked: SAP_TIER=%s — write operations are only allowed on the dev tier (unset SAP_TIER or set SAP_TIER=dev to enable writes)", tier)
	}
	return nil
}

// getWriteClient is getClient for write-capable subcommands. It enforces
// the write profile first, so a forbidden profile is rejected before any
// network call, then constructs the client. Read-only subcommands use
// getClient directly and are never gated here.
func getWriteClient(params *systemParams) (*adt.Client, error) {
	if err := enforceWriteProfile(); err != nil {
		return nil, err
	}
