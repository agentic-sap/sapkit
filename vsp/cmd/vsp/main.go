...
func init() {
	...
	// Offline mode: local-only tools, no SAP connection needed
	rootCmd.Flags().BoolVar(&cfg.Offline, "offline", false, "Run with local-only tools, no SAP connection needed (ListStandards, GetStandard, CheckBoundaries, AnalyzeABAPCode) — for abapGit-based offline ABAP development")
	...
	viper.BindPFlag("offline", rootCmd.Flags().Lookup("offline"))
...
func runServer(cmd *cobra.Command, args []string) error {
	...
	// Offline mode needs no authentication at all — skip browser/SAML/cookie
	// auth entirely, even if related flags/env vars happen to be set
	// (offline takes priority over any SAP connection info).
	if !cfg.Offline {
...
func validateConfig() error {
	if cfg.BaseURL == "" && !cfg.Offline {
		return fmt.Errorf("SAP URL is required. Use --url flag or SAP_URL environment variable (or start with --offline for local-only tools)")
	}
