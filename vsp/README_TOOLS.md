...
| Tool | Description | Mode |
|------|-------------|------|
| `SyntaxCheck` | Check source code for syntax errors (omit content to check persisted source, e.g. whole-FUGR diagnosis) | Focused |
| `Activate` | Activate an ABAP object | Expert |
| `ActivateGroup` | Activate a whole function group (main program + includes + FMs) in one activation run | Focused |
