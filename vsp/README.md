# Vibing Steampunk (vsp) — Custom Fork

> **Fork notice**: This is a customized fork of [oisee/vibing-steampunk](https://github.com/oisee/vibing-steampunk) (MIT).
> It adds embedded ABAP development standards (coding rules, program skeleton
> templates, quality checklists) served via the `ListStandards`/`GetStandard`
> MCP tools and `vsp://standards/*` resources, an `InstallALVHandlers` deploy
> tool, and an `--offline` mode for air-gapped development.
> **Guide: [MANUAL.ko.md](MANUAL.ko.md)** · Change log: [CUSTOMIZATIONS.md](CUSTOMIZATIONS.md)
...
vsp --offline                                    # MCP server with local-only tools, no SAP needed
```
