...
Agents MUST determine the SAP version (via `GetSystemInfo` / `GetInstalledComponents`, or the configured `SAP_VERSION` in the project's SAP connection config) **before** recommending any TCode, table, BAPI, or pattern. Agents MUST also check the ABAP release before generating ABAP code — see `abap-release-reference.md` for release-by-release syntax.

If the SAP version cannot be determined, fail safe: query `GetSystemInfo` or ask the user to confirm the target version before proceeding.
...
- Agents MUST determine `SAP_VERSION` (via `GetSystemInfo` or config) before recommending TCodes, tables, BAPIs, or development patterns.
- Agents MUST check the ABAP release before generating ABAP code — using unsupported syntax causes activation errors.
- If `SAP_VERSION` cannot be determined, fail safe: query `GetSystemInfo` or ask the user before proceeding.
- When migrating ECC→S/4HANA scripts, never assume compatibility views alone are enough — verify with `GetSource` (type `VIEW`) before SELECT.
- For S/4HANA ABAP Cloud tier (`SAP_SYSTEM_TYPE=cloud`), only released APIs (C1 tier) are callable — consult SAP Note guidance before recommending classic FMs/BAPIs.
