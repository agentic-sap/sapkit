...
Agents MUST NOT emit features newer than the target system's ABAP release (check with `GetSystemInfo`) — doing so causes activation failures on the target system.
...
### 2.9 ABAP Cloud Restriction (≥ 756 on ABAP Cloud systems)
...
Agents MUST check the target system type — on ABAP Cloud, only released (C1) APIs, CDS views, and BAdIs are permitted. On on-premise systems, classic DDIC tables are allowed but still discouraged when a released view exists.
