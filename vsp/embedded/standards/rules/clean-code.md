...
> **Gate rule**: every rule below is subordinate to `abap-release-reference.md` — never use syntax newer than the project's configured `ABAP_RELEASE`.

> **Paradigm-specific rules live in companion files — load the right one based on the program's paradigm (decided in the requirements interview)**:
...
- **Use CDS views** for any reusable read logic (ABAP 750+). See the module-specific standards for module-standard views.
...
- **Large transactional tables — mandatory pre-count**: before a SELECT on a transactional table (VBRK, VBAP, BKPF, BSEG, EKKO, EKPO, ACDOCA, MATDOC, LIPS, MKPF, MSEG, WBCROSSGT and equivalents), first run `SELECT COUNT(*) FROM <table> WHERE <same predicate>` to size the result set. If the count exceeds **1,000,000 rows**, do NOT run the main SELECT as-is — apply at least one of the following tuning measures before proceeding:
  ...
  Record the count and the chosen tuning measure in `spec.md` / `plan.md` when planning detects the risk, and in the review notes when confirmed.
- **Blocked tables**: before any `GetTableContents` / `RunQuery`, consult [`data-extraction-policy.md`](data-extraction-policy.md).
...
- **Each commit's tests pass** — `RunUnitTests` via MCP before release.
...
- **Measure, don't guess**. Use the MCP ABAP profiler tools (`ListTraces` / `GetTrace`).
...
- **Data extraction rules are hard rules** — see [`data-extraction-policy.md`](data-extraction-policy.md). Risk acknowledgement (`acknowledge_risk`) requires explicit user affirmative (`yes` / `authorize` / `approve` / `proceed` / `confirmed`). Never auto-set.
...
6. `RunUnitTests` and `SyntaxCheck` both green; `GetInactiveObjects` empty.
7. Syntax matches the configured `ABAP_RELEASE`.
8. Blocklist / `acknowledge_risk` rules respected in any `GetTableContents` / `RunQuery` invocation.
