...
**Scope.** Every Table / Structure / Table Type field-type decision made by AI agents or automated workflows (standard `CreateTable` / structure-creation flow **and** the ECC helper-program fallback under `ecc-ddic-fallback.md`).
...
| **2 — CBO DE** | Existing customer Z/Y data element from the project CBO inventory | No standard DE fits, but a previously-created project DE does (from the project CBO inventory, e.g. `inventory.json`). Reuse over create. |
| **3 — New CBO DE** | Freshly-created Z data element (triggers data-element creation — MCP DDIC creation flow or ECC DTEL helper) | Semantics are genuinely new AND the field will appear in ≥ 2 places OR carries domain-specific meaning that deserves a label + F1 help. Must follow `naming-conventions.md` (`ZFIE00010`, …). |
...
1. **Standard DE search** — call `SearchObject` with the field-semantic guess as the search pattern (data elements / DTEL), or match the field name against the **Common Standard DE Reference** below.
   - Hit → priority 1. Use that DE as `rollname`. Stop.
   - Miss → continue.
2. **CBO DE search** — if the project CBO inventory (`inventory.json`) exists, grep it for a DE whose role / domain / length / label matches. Also call `SearchObject` with the project Z-prefix (e.g., `ZFIE*`).
   - Hit → priority 2. Stop.
   - Miss → continue.
3. **New CBO DE decision** — does the field justify a new DE? Apply the priority-3 gate (reuse ≥ 2 OR domain-specific label). If yes → priority 3; create the data element (S/4: MCP DDIC creation flow; ECC: DTEL helper program per `ecc-ddic-fallback.md`). Then reference it as `rollname`.
...
Agents facing an unlisted field MUST call `SearchObject` against data elements with 2–3 synonym queries (e.g., `vendor`, `supplier`, `creditor`) before concluding "no standard DE exists." Document negative searches in the plan so the next run can verify.
...
- Object-creation workflows — both the standard flow and the ECC helper-program generation route field-type decisions through this rule.
- Program-generation workflows that create DDIC artifacts (parallel `CreateTable` / structure creation) apply this rule per field before emitting `rollname`.
- Plan review fails if any field drops to priority 4 without an inline justification, or if any priority-1 miss is visible (a standard DE existed and the plan still used a primitive).
- The CBO inventory (`inventory.json`) is the source for priority 2 (existing CBO DE). Keep it fresh; stale inventory forces needless priority-3 creations.

## Enforcement Checklist (per field, before `CreateTable` / table-update / ECC helper emission)

1. Ran `SearchObject` for standard DE OR matched against the quick-lookup table → decision recorded.
2. If no standard hit, checked the project CBO inventory for a CBO DE → decision recorded.
