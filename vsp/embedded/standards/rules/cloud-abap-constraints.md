...
Preflight reference for program generation when the target system is **S/4HANA Cloud Public**. Classic ABAP features on the left are **prohibited**; use the Cloud ABAP equivalent on the right.
...
## Impact on Program Generation

On Cloud Public, the default Main+Include + Dynpro + CL_GUI_ALV_GRID pipeline **cannot execute**. Program generation must redirect:
