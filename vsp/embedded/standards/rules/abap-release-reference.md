...
Syntax feature inventory by ABAP release. Agents MUST NOT emit features newer than the target system's ABAP release (check `GetSystemInfo`, or the configured `ABAP_RELEASE` in the project's SAP connection config) — doing so causes activation failures on the target system.

If the release cannot be determined, fail safe: query `GetSystemInfo` or ask the user to confirm the target release before generating code.
...
1. Did I determine the target ABAP release (via `GetSystemInfo` or config)? If no → stop and ask the user.
2. Is any feature I'm about to emit newer than the target release? If yes → rewrite to the older idiom in [abap-release-examples.md](abap-release-examples.md).
3. Is the target system ABAP Cloud (`SAP_SYSTEM_TYPE=cloud`)? If yes → every `SELECT`/`CALL FUNCTION`/`CALL METHOD` must target a **released** API (check `GetPackage` or released annotations).
