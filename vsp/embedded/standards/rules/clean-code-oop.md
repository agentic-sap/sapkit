...
Clean ABAP rules specific to the **OOP paradigm** — local classes (`LCL_DATA` / `LCL_ALV`) inside a REPORT, global classes, interfaces, exception classes, ABAP Unit with test doubles. Load this file when the requirements interview picks `paradigm = OOP` (and NOT when it picks `Procedural`). Companion to [`clean-code.md`](clean-code.md) (paradigm-neutral shared baseline). Everything here is gated by `abap-release-reference.md`.

> Paired with the OOP pattern file [`oop-pattern.md`](oop-pattern.md) — that file defines the two-class split (LCL_DATA + LCL_ALV / LCL_SCREEN); this file defines the coding style rules that fill the class bodies.

## Mandatory Main Program Template (MUST match)

When generating an OOP program's main `REPORT` source, the generated code MUST start from and conform to the canonical sample:

**Source of truth**: [`templates/oop/zrsc4sap_oop_ex.prog.abap`](templates/oop/zrsc4sap_oop_ex.prog.abap) (companion includes: `zrsc4sap_oop_exa/exc/exe/exf/exi/exo/exs/ext.prog.abap`, screens `zrsc4sap_oop_ex.prog.screen_0100.abap` / `_0200.abap`).
...
- **Deviation requires written justification in `spec.md`** before implementation. Undocumented structural drift from the template is a MAJOR review finding.
...
- **OOP two-class split** (project convention) — `LCL_DATA` for business logic / BAPI I/O, `LCL_ALV` or `LCL_SCREEN` for presentation. See [`oop-pattern.md`](oop-pattern.md).
