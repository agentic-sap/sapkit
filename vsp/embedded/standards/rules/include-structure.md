...
Shared convention for ABAP programs that wrap logic in a Main Program + conditional Includes. **Source of truth for Procedural programs: [`templates/procedural/main-program.abap`](templates/procedural/main-program.abap)**. OOP programs follow [`templates/oop/zrsc4sap_oop_ex.prog.abap`](templates/oop/zrsc4sap_oop_ex.prog.abap).

## Include Set

| Include | Suffix | Procedural | OOP | Content |
|---------|--------|------------|-----|---------|
| TOP | `{PROG}t` | Always | Always | `TYPES`, `DATA`, `CONSTANTS` declarations. **MUST declare `DATA: gv_okcode TYPE sy-ucomm.`** when a screen is present — full 3-step binding contract (TOP decl + screen OK_CODE NAME + PAI FORM routing) in [`ok-code-pattern.md`](ok-code-pattern.md); `CASE sy-ucomm` in PAI is a MAJOR review finding. |
...
## Activation Protocol — MANDATORY (applies to every workflow that creates includes)

**Writing the main program with activation (`WriteSource` / `WriteProgram`) activates ONLY the main program. It does NOT cascade to sub-includes.** Workflows that generate a main program + N includes MUST:

1. Create each include (`CreateObject`, type include) and upload its body (`UpdateSource`).
2. After every include is uploaded, either:
   - Call `Activate` for each include **in dependency order** (typically `t → s → a → o → i → f` because later includes reference earlier declarations), OR
   - Call `ActivatePackage` once to batch-activate all inactive objects in the package (covering main + every include + screen + gui_status).
3. **Verify with `GetInactiveObjects`** — if any program-scoped include appears in the result, the workflow has FAILED regardless of what individual tool responses reported.

Agents that report "5/5 프로그램 활성화 OK" without a `GetInactiveObjects=0` check are producing **false positives** and are a MAJOR review finding.

## Anti-patterns (each is a MAJOR review finding)
