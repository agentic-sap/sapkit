...
Shared naming rules for ABAP programs and related objects.
...
- **Must not collide with an existing SAP object.** Before proposing or creating any `Z{MODULE}{TYPE}{NN}`, verify via MCP (`SearchObject` — name-pattern search) that the candidate name does NOT already exist in the target system. Never assume an unused number.
...
Per-type pattern details for other object types (tables, structures, data elements, domains, programs, DDIC views, search helps, RAP/OData artifacts, IDoc, enhancements, packages) live in `naming-conventions-objects.md` (per-object naming reference).
