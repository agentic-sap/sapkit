...
Authoritative wiring rule for screen user commands in a Procedural ABAP program that uses classical Dynpro (`CALL SCREEN {num}`). Non-negotiable — this pattern is enforced by code review.
...
**MCP screen-update path**: the screen's `fields_to_containers[]` entry with `TYPE=OKCODE` MUST have `NAME=GV_OKCODE`. A screen whose OKCODE field has the default placeholder (`TEXT=____________________`) and no NAME causes the code to land in `sy-ucomm` only — defeating the whole pattern.

Example screen-update payload fragment:
```json
{
...
- **Screen OK_CODE field has no NAME** — the screen-update payload's OKCODE field has only TYPE and placeholder TEXT, no NAME attribute. Same failure mode as the missing TOP declaration.
...
- `include-structure.md` TOP include row — mandates the `DATA: gv_okcode TYPE sy-ucomm.` declaration when a screen is present; this file is the "why + full contract + anti-pattern" companion.
- `clean-code-procedural.md` § PBO / PAI Module — references this pattern as the source of truth for PAI user-command routing.
- Screen generation — the screen-update payload for any screen with an OKCODE field MUST set `NAME=GV_OKCODE`. An OKCODE field with no NAME is a MAJOR review finding.
- Code review — the reviewer verifies the 3-step contract: TOP declaration exists, screen OKCODE field NAME=`GV_OKCODE`, PAI FORM uses `gv_okcode` not `sy-ucomm`.
