...
func (m *methodPathMock) Do(req *http.Request) (*http.Response, error) {
	...
	for _, r := range m.routes {
		...
		if r.queryContains != "" && !strings.Contains(req.URL.RawQuery, r.queryContains) {
			continue
		}
		if r.err != nil {
			return nil, r.err
...
// TestLockObject_WarnsButProceedsOnNoModification covers the on-premise
// false-positive proven by a live probe (2026-07-12, S/4HANA 2021): a
// successful LOCK can return MODIFICATION_SUPPORT=NoModification even
// though the subsequent source/main PUT + activation succeeds under the
// very same lock. LockObject used to fail fast here (issue #91's original
// fix), but that turned a false alarm into a hard blocker. The corrected
// behaviour is to warn (stderr) and still return the lock result so the
// caller can proceed — and, since the caller is expected to use the lock,
// LockObject must NOT release it: only the LOCK POST should go out, no
// UNLOCK.
func TestLockObject_WarnsButProceedsOnNoModification(t *testing.T) {
...
<asx:abap xmlns:asx="http://www.sap.com/abapxml" version="1.0">
  <asx:values>
	...
	if err != nil {
		t.Fatalf("LockObject should warn and proceed on NoModification, not fail: %v", err)
	}
	...
	if result.ModificationSupport != "NoModification" {
		t.Errorf("ModificationSupport = %q, want it preserved on the returned result", result.ModificationSupport)
	}

	// Only the LOCK POST should have gone out — the lock must NOT be
	// released, since the caller is expected to use it to proceed.
	postCalls := 0
	for _, c := range mock.calls {
	...
	if postCalls != 1 {
		t.Errorf("expected 1 POST call (LOCK only, no UNLOCK), got %d", postCalls)
	}
}
