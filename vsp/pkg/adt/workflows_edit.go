...
func (c *Client) EditSourceWithOptions(ctx context.Context, objectURL, oldString, newString string, opts *EditSourceOptions) (*EditSourceResult, error) {
	...
	if opts.SyntaxCheck {
		...
		if len(syntaxResults) > 0 {
			...
			for _, e := range syntaxResults {
				...
				// FM/include checks compile the whole function group / class:
				// findings whose URI lies outside the edited object are
				// pre-existing defects in sibling objects, not this edit.
				if e.URI != "" && !strings.HasPrefix(e.URI, objectURL) {
					msg = fmt.Sprintf("Line %d [in %s - outside the edited object, likely a pre-existing defect]: %s", e.Line, e.URI, e.Text)
					if e.Severity == "E" {
						foreignErrors++
					}
				}
			...
			if len(errors) > 0 {
				...
				if foreignErrors == len(errors) {
					result.Message = fmt.Sprintf("Syntax check found %d error(s), all in OTHER objects of the same group/class (pre-existing defects, not introduced by this edit). Changes NOT saved. Fix the sibling objects first, or use syntax_check=false to bypass.", len(errors))
				} else {
	...
	if !activation.Success {
		result.Message = fmt.Sprintf("Source updated (persisted, inactive), but activation failed: %s. Fix the reported errors and re-activate.", summarizeActivationErrors(activation))
		return result, nil
	}
