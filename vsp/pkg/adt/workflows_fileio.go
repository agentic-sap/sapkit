...
func (c *Client) RenameObject(ctx context.Context, objType CreatableObjectType, oldName, newName, packageName, transport string) (*RenameObjectResult, error) {
	...
	if err != nil {
		// The LOCK above already succeeded, so SAP is holding an ENQUEUE
		// lock for the old object. Release it before returning —
		// otherwise the lock leaks and the object stays untouchable
		// until the session expires (same pattern as 00b4b21).
		if unlockErr := c.UnlockObject(ctx, oldURL, oldLockResult.LockHandle); unlockErr != nil {
			result.Message = fmt.Sprintf("New object %s created successfully, but failed to delete old object %s: %v (additionally failed to release the lock acquired above: %v). Please delete manually.", newName, oldName, err, unlockErr)
		} else {
