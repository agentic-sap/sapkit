...
func (c *Client) CreateFromFile(ctx context.Context, filePath, packageName, transport string) (*DeployResult, error) {
	...
	if !activation.Success {
		return &DeployResult{
			...
			ObjectType: string(info.ObjectType),
			Success:    false,
			Errors:     []string{fmt.Sprintf("activation failed: %s", summarizeActivationErrors(activation))},
			Message:    fmt.Sprintf("Object created and source written (persisted, inactive), but activation failed: %s", summarizeActivationErrors(activation)),
		}, nil
	}
...
func (c *Client) UpdateFromFile(ctx context.Context, filePath, transport string) (*DeployResult, error) {
	...
	if !activation.Success {
		return &DeployResult{
			...
			Success:    false,
			Errors:     []string{fmt.Sprintf("activation failed: %s", summarizeActivationErrors(activation))},
			Message:    fmt.Sprintf("Source written (persisted, inactive), but activation failed: %s", summarizeActivationErrors(activation)),
		}, nil
	}
