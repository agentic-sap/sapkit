...
// content is the source code to check. Empty content checks the source
// already persisted on the server instead - with a function group URL this
// returns errors across the whole group in one call.
func (c *Client) SyntaxCheck(ctx context.Context, objectURL string, content string) ([]SyntaxCheckResult, error) {
	...
	var body string
	if content == "" {
		// No artifact: the server checks its own persisted source.
		body = fmt.Sprintf(`<?xml version="1.0" encoding="UTF-8"?>
<chkrun:checkObjectList xmlns:chkrun="http://www.sap.com/adt/checkrun" xmlns:adtcore="http://www.sap.com/adt/core">
  <chkrun:checkObject adtcore:uri="%s" chkrun:version="active"/>
</chkrun:checkObjectList>`, checkObjectURI)
	} else {
...
<chkrun:checkObjectList xmlns:chkrun="http://www.sap.com/adt/checkrun" xmlns:adtcore="http://www.sap.com/adt/core">
  <chkrun:checkObject adtcore:uri="%s" chkrun:version="active">
	...
	resp, err := c.transport.Request(ctx, "/sap/bc/adt/checkruns?reporters=abapCheckRun", &RequestOptions{
		...
		Stateful:    true, // Must not tear down a lock session held across lock→check→write (issue #88)
	})
	if err != nil {
...
// ActivationRef identifies one object in a multi-object activation run.
type ActivationRef struct {
	URI  string `json:"uri"`
	Name string `json:"name"`
}

// ActivateMultiple activates several objects in ONE activation run (a single
// POST with multiple object references). Required for function groups: the
// group's main program, includes and FMs must activate together or single-FM
// activation fails with "FUNCTION ... FUNCTION-POOL" errors.
func (c *Client) ActivateMultiple(ctx context.Context, refs []ActivationRef) (*ActivationResult, error) {
	if err := c.checkSafety(OpActivate, "ActivateMultiple"); err != nil {
		return nil, err
	}
	if len(refs) == 0 {
		return nil, fmt.Errorf("no objects to activate")
	}
...
// GroupActivationRefs assembles the reference set for activating a function
// group in one run: main program SAPL<group> + every include + every function
// module (enumerated from the group's objectstructure) + the group itself.
func (c *Client) GroupActivationRefs(ctx context.Context, groupName string) ([]ActivationRef, error) {
	groupName = strings.ToUpper(groupName)
	lower := strings.ToLower(groupName)

	structPath := fmt.Sprintf("/sap/bc/adt/functions/groups/%s/objectstructure", url.PathEscape(lower))
	...
	var refs []ActivationRef
	add := func(uri, name string) {
		if uri == "" || seen[uri] {
			return
		}
		seen[uri] = true
		refs = append(refs, ActivationRef{URI: uri, Name: name})
	}

	// Main program first: the FUNCTION-POOL skeleton must be part of the run.
	add(fmt.Sprintf("/sap/bc/adt/programs/programs/sapl%s", url.PathEscape(lower)), "SAPL"+groupName)

	// The system-generated UXX include (L<group>UXX) is editor-locked by SAP*
	// and must NOT be part of the activation run - including it fails the
	// whole run with 403 ExceptionResourceNoAccess (verified live).
	uxxName := "L" + groupName + "UXX"

	var walk func(e element)
	walk = func(e element) {
		if !strings.EqualFold(e.Name, uxxName) {
			for _, l := range e.Links {
				if strings.Contains(l.Href, "/source/main") {
					add(strings.TrimSuffix(l.Href, "/source/main"), e.Name)
					break
				}
			}
		}
	...
	// The group object itself, in case objectstructure carried no link for it.
	add(fmt.Sprintf("/sap/bc/adt/functions/groups/%s", url.PathEscape(lower)), groupName)
...
// summarizeActivationErrors returns a one-line summary of why an activation
// with Success=false failed, for embedding in error/result messages.
func summarizeActivationErrors(a *ActivationResult) string {
	if a == nil {
	...
	if len(a.Inactive) > 0 {
		return fmt.Sprintf("%d object(s) still inactive", len(a.Inactive))
	}
	return "activation reported failure - check activation messages"
}
