...
func (c *Client) LockObject(ctx context.Context, objectURL string, accessMode string) (*LockResult, error) {
	...
	// object is flagged read-only via ADT (typical for SAP-delivered
	// objects in hyperfocused mode, or systems where the user lacks the
	// edit role). This used to be treated as a hard error (issue #91),
	// but a live probe (2026-07-12, on-premise S/4HANA 2021) proved that
	// signal is a false positive on at least some on-premise systems:
	// CLAS MODIFY locks there report NoModification unconditionally
	// (regardless of Accept format — v1 lock.result and v2 lock.result2
	// both observed), yet the subsequent source/main PUT + activation
	// succeeds under the very same lock (verified with another client).
	// Fail-fast here would block a legitimate write, so we now only warn
	// and let the caller proceed with the acquired lock; if the write
	// does fail afterwards with 423/permission errors, this message is
	// the likely cause.
	if accessMode == "MODIFY" && strings.EqualFold(result.ModificationSupport, "NoModification") {
		fmt.Fprintf(os.Stderr,
			"warning: lock reports modificationSupport=NoModification — proceeding anyway "+
				"(on-premise systems are known to report this spuriously); if the subsequent "+
				"write fails with 423/permission errors, this is the likely cause (object %s)\n",
			objectURL)
	}
...
<blue:blueSource xmlns:blue="http://www.sap.com/wbobj/blue"
  ...
  <adtcore:packageRef adtcore:name="%s"/>
	...
	if !activation.Success {
		return fmt.Errorf("table %s created and source written, but activation failed: %s", opts.Name, summarizeActivationErrors(activation))
	}
