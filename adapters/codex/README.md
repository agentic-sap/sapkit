...
```
codex plugin marketplace add "D:\claude for SAP\sc4sap-lite"
codex plugin add sc4sap-lite@sc4sap-lite
```

캐시(`~/.codex/plugins/cache/sc4sap-lite/...`)에 core/·server/ 포함 전체가 패키징됨을 확인.

## MCP 서버 등록 (전역 — 프로젝트 config는 trust 게이트가 있어 비권장)

```
codex mcp add sap --env NODE_PATH="D:\claude for SAP\sc4sap-lite\server\runtime-deps\keyring\node_modules" -- node "D:\claude for SAP\sc4sap-lite\server\server.bundle.cjs" --exposition=readonly
```
