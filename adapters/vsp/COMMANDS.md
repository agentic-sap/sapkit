...
6. **`vsp deploy`의 CLAS 경로는 IDES에서 불가 + 고아 잠금 생산** (2026-07-12
   ...
   EU510이 아닌 동일 NoModification = 잔존 잠금 없음. **NoModification 자체(CLAS
   deploy 불가)는 미수리** — 근본 원인 조사 대기. 수리 중 동일 패턴 누수 후보 1건
   추가 발견(미수정): `pkg/adt/workflows_fileio.go` RenameObject의 구객체 삭제 실패
   경로가 lock 획득 후 unlock 없이 리턴 — 후속 수리 후보.
7. **`vsp copy`의 기존재 CLAS 경로는 3중 거짓 성공** (동일 세션 실측): ZADT_VSP
   ...
   조회로 확인), 그 과정에서 잠금까지 누수. **CLAS 서버 반영은 vsp 수리 전까지
   경로 없음(GUI 수동)** — deploy든 copy든 CLAS는 성공 보고를 믿지 말고 반드시
   `vsp source read`로 반영을 확인할 것 (.harness R-006).
